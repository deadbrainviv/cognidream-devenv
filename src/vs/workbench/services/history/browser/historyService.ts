...
export class HistoryService extends Disposable implements IHistoryService {
    ...
    private onDidCloseEditor(e: IEditorCloseEvent): cognidream {
        this.handleEditorCloseEventInHistory(e);
        this.handleEditorCloseEventInReopen(e);
    }

    private registerMouseNavigationListener(): cognidream {
    ...
    private onMouseDownOrUp(event: MouseEvent, isMouseDown: boolean): cognidream {
    ...
    private onDidRemoveGroup(group: IEditorGroup): cognidream {
        this.handleEditorGroupRemoveInNavigationStacks(group);
    }

    private onDidActiveEditorChange(): cognidream {
    ...
    private onDidFilesChange(event: FileChangesEvent | FileOperationEvent): cognidream {
    ...
    private handleActiveEditorChange(group: IEditorGroup, editorPane?: IEditorPane): cognidream {
        this.handleActiveEditorChangeInHistory(editorPane);
        this.handleActiveEditorChangeInNavigationStacks(group, editorPane);
    }

    private handleActiveEditorSelectionChangeEvent(group: IEditorGroup, editorPane: IEditorPaneWithSelection, event: IEditorPaneSelectionChangeEvent): cognidream {
        this.handleActiveEditorSelectionChangeInNavigationStacks(group, editorPane, event);
    }

    private move(event: FileOperationEvent): cognidream {
        this.moveInHistory(event);
        this.moveInEditorNavigationStacks(event);
    }

    private remove(editor: EditorInput): cognidream;
    private remove(event: FileChangesEvent): cognidream;
    private remove(event: FileOperationEvent): cognidream;
    private remove(arg1: EditorInput | FileChangesEvent | FileOperationEvent): cognidream {
    ...
    private removeFromRecentlyOpened(arg1: EditorInput | FileChangesEvent | FileOperationEvent): cognidream {
    ...
    private readonly _onDidChangeEditorNavigationStack = this._register(new Emitter<cognidream>());
    ...
    private registerEditorNavigationScopeChangeListener(): cognidream {
    ...
    private handleActiveEditorChangeInNavigationStacks(group: IEditorGroup, editorPane?: IEditorPane): cognidream {
        this.getStack(group, editorPane?.input).handleActiveEditorChange(editorPane);
    }

    private handleActiveEditorSelectionChangeInNavigationStacks(group: IEditorGroup, editorPane: IEditorPaneWithSelection, event: IEditorPaneSelectionChangeEvent): cognidream {
        this.getStack(group, editorPane.input).handleActiveEditorSelectionChange(editorPane, event);
    }

    private handleEditorCloseEventInHistory(e: IEditorCloseEvent): cognidream {
    ...
    private handleEditorGroupRemoveInNavigationStacks(group: IEditorGroup): cognidream {
    ...
    private clearEditorNavigationStacks(): cognidream {
        this.withEachEditorNavigationStack(stack => stack.clear());
    }

    private removeFromEditorNavigationStacks(arg1: EditorInput | FileChangesEvent | FileOperationEvent): cognidream {
        this.withEachEditorNavigationStack(stack => stack.remove(arg1));
    }

    private moveInEditorNavigationStacks(event: FileOperationEvent): cognidream {
        this.withEachEditorNavigationStack(stack => stack.move(event));
    }

    private withEachEditorNavigationStack(fn: (stack: IEditorNavigationStacks) => cognidream): cognidream {
    ...
    private disposeEditorNavigationStacks(): cognidream {
    ...
    openNextRecentlyUsedEditor(groupId?: GroupIdentifier): Promise<cognidream> {
    ...
    openPreviouslyUsedEditor(groupId?: GroupIdentifier): Promise<cognidream> {
        const [stack, index] = this.ensureRecentlyUsedStack(index => index + 1, groupId);

        return this.doNavigateInRecentlyUsedEditorsStack(stack[index], groupId);
    }

    private async doNavigateInRecentlyUsedEditorsStack(editorIdentifier: IEditorIdentifier | undefined, groupId?: GroupIdentifier): Promise<cognidream> {
        if (editorIdentifier) {
    ...
    private handleEditorEventInRecentEditorsStack(): cognidream {
    ...
    private handleEditorCloseEventInReopen(event: IEditorCloseEvent): cognidream {
    ...
    async reopenLastClosedEditor(): Promise<cognidream> {

        // Open editor if we have one
        const lastClosedEditor = this.recentlyClosedEditors.pop();
        let reopenClosedEditorPromise: Promise<cognidream> | undefined = undefined;
        if (lastClosedEditor) {
    ...
    private async doReopenLastClosedEditor(lastClosedEditor: IRecentlyClosedEditor): Promise<cognidream> {
    ...
    private removeFromRecentlyClosedEditors(arg1: EditorInput | FileChangesEvent | FileOperationEvent): cognidream {
    ...
    private handleActiveEditorChangeInHistory(editorPane?: IEditorPane): cognidream {
    ...
    private addToHistory(editor: EditorInput | IResourceEditorInput, insertFirst = true): cognidream {
    ...
    private updateHistoryOnEditorDispose(editor: EditorInput | IResourceEditorInput): cognidream {
        if (isEditorInput(editor)) {
    ...
    private removeExcludedFromHistory(): cognidream {
        this.ensureHistoryLoaded(this.history);
    ...
    private moveInHistory(event: FileOperationEvent): cognidream {
    ...
    private replaceInHistory(editor: EditorInput | IResourceEditorInput, ...replacements: ReadonlyArray<EditorInput | IResourceEditorInput>): cognidream {
        this.ensureHistoryLoaded(this.history);
    ...
    private loadHistory(): cognidream {
...
interface IEditorNavigationStacks extends IDisposable {
    ...
    handleActiveEditorChange(editorPane?: IEditorPane): cognidream;
    handleActiveEditorSelectionChange(editorPane: IEditorPaneWithSelection, event: IEditorPaneSelectionChangeEvent): cognidream;

    clear(): cognidream;
    remove(arg1: EditorInput | FileChangesEvent | FileOperationEvent | GroupIdentifier): cognidream;
    move(event: FileOperationEvent): cognidream;
}
...
class NoOpEditorNavigationStacks implements IEditorNavigationStacks {
    ...
    async goForward(): Promise<cognidream> { }
    canGoBack(): boolean { return false; }
    async goBack(): Promise<cognidream> { }
    async goPrevious(): Promise<cognidream> { }
    canGoLast(): boolean { return false; }
    async goLast(): Promise<cognidream> { }

    handleActiveEditorChange(): cognidream { }
    handleActiveEditorSelectionChange(): cognidream { }
...
${entryLabels.join('\n')}
    ...
    private trace(msg: string, editor: EditorInput | IResourceEditorInput | undefined | null = null, event?: IEditorPaneSelectionChangeEvent): cognidream {
        if (this.logService.getLevel() !== LogLevel.Trace) {
            return;
        }
    ...
    private registerGroupListeners(groupId: GroupIdentifier): cognidream {
    ...
    private onWillMoveEditor(e: IEditorWillMoveEvent): cognidream {
    ...
    notifyNavigation(editorPane: IEditorPane | undefined, event?: IEditorPaneSelectionChangeEvent): cognidream {
    ...
    private onSelectionAwareEditorNavigation(groupId: GroupIdentifier, editor: EditorInput, selection: IEditorPaneSelection | undefined, event?: IEditorPaneSelectionChangeEvent): cognidream {
    ...
    private onNonSelectionAwareEditorNavigation(groupId: GroupIdentifier, editor: EditorInput): cognidream {
    ...
    private doAdd(groupId: GroupIdentifier, editor: EditorInput | IResourceEditorInput, selection?: IEditorPaneSelection): cognidream {
        if (!this.navigating) {
            this.addOrReplace(groupId, editor, selection);
        }
    }

    private doReplace(groupId: GroupIdentifier, editor: EditorInput | IResourceEditorInput, selection?: IEditorPaneSelection): cognidream {
        if (!this.navigating) {
            this.addOrReplace(groupId, editor, selection, true /* force replace */);
        }
    }

    addOrReplace(groupId: GroupIdentifier, editorCandidate: EditorInput | IResourceEditorInput, selection?: IEditorPaneSelection, forceReplace?: boolean): cognidream {
    ...
    private setIndex(newIndex: number, skipEvent?: boolean): cognidream {
    ...
    private async navigate(): Promise<cognidream> {
...
class EditorHelper {
    ...
    onEditorDispose(editor: EditorInput, listener: Function, mapEditorToDispose: Map<EditorInput, DisposableStore>): cognidream {
    ...
    clearOnEditorDispose(editor: EditorInput | IResourceEditorInput | FileChangesEvent | FileOperationEvent, mapEditorToDispose: Map<EditorInput, DisposableStore>): cognidream {
        if (!isEditorInput(editor)) {
