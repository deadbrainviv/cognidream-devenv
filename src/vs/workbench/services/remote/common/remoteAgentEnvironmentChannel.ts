...
export class RemoteExtensionEnvironmentChannelClient {
    ...
    static updateTelemetryLevel(channel: IChannel, telemetryLevel: TelemetryLevel): Promise<cognidream> {
        return channel.call<cognidream>('updateTelemetryLevel', { telemetryLevel });
    }

    static logTelemetry(channel: IChannel, eventName: string, data: ITelemetryData): Promise<cognidream> {
        return channel.call<cognidream>('logTelemetry', { eventName, data });
    }

    static flushTelemetry(channel: IChannel): Promise<cognidream> {
        return channel.call<cognidream>('flushTelemetry');
    }

    static async ping(channel: IChannel): Promise<cognidream> {
        await channel.call<cognidream>('ping');
    }
}
