...
export abstract class AbstractRemoteAgentService extends Disposable implements IRemoteAgentService {
    ...
    updateTelemetryLevel(telemetryLevel: TelemetryLevel): Promise<cognidream> {
    ...
    logTelemetry(eventName: string, data: ITelemetryData): Promise<cognidream> {
        return this._withTelemetryChannel(
    ...
    flushTelemetry(): Promise<cognidream> {
        return this._withTelemetryChannel(
    ...
    async endConnection(): Promise<cognidream> {
        if (this._connection) {
...
class RemoteAgentConnection extends Disposable implements IRemoteAgentConnection {
    ...
    private readonly _onReconnecting = this._register(new Emitter<cognidream>());
    ...
    end: () => Promise<cognidream> = () => Promise.resolve();
    ...
    registerChannel<T extends IServerChannel<RemoteAgentConnectionContext>>(channelName: string, channel: T): cognidream {
