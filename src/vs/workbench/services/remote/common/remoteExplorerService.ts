...
export interface IRemoteExplorerService {
    ...
    setEditable(tunnelItem: ITunnelItem | undefined, editId: TunnelEditId, data: IEditableData | null): cognidream;
    ...
    close(remote: { host: string; port: number }, reason: TunnelCloseReason): Promise<cognidream>;
    setTunnelInformation(tunnelInformation: TunnelInformation | undefined): cognidream;
    setCandidateFilter(filter: ((candidates: CandidatePort[]) => Promise<CandidatePort[]>) | undefined): IDisposable;
    onFoundNewCandidates(candidates: CandidatePort[]): cognidream;
    restore(): Promise<cognidream>;
    enablePortsFeatures(viewOnly: boolean): cognidream;
    onEnabledPortsFeatures: Event<cognidream>;
...
class RemoteExplorerService implements IRemoteExplorerService {
    ...
    private readonly _onEnabledPortsFeatures: Emitter<cognidream> = new Emitter();
    public readonly onEnabledPortsFeatures: Event<cognidream> = this._onEnabledPortsFeatures.event;
