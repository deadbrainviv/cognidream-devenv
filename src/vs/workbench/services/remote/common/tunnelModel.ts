...
export class PortsAttributes extends Disposable {
    ...
    private _onDidChangeAttributes = new Emitter<cognidream>();
...
export class TunnelModel extends Disposable {
    ...
    private _onForwardPort: Emitter<Tunnel | cognidream> = new Emitter();
    public onForwardPort: Event<Tunnel | cognidream> = this._onForwardPort.event;
    ...
    private _onEnvironmentTunnelsSet: Emitter<cognidream> = new Emitter();
    public onEnvironmentTunnelsSet: Event<cognidream> = this._onEnvironmentTunnelsSet.event;
    ...
    private onRestoreComplete: Emitter<cognidream> = new Emitter();
    ...
    async close(host: string, port: number, reason: TunnelCloseReason): Promise<cognidream> {
        const key = makeAddress(host, port);
    ...
    addEnvironmentTunnels(tunnels: TunnelDescription[] | undefined): cognidream {
        if (tunnels) {
            for (const tunnel of tunnels) {
    ...
    setCandidateFilter(filter: ((candidates: CandidatePort[]) => Promise<CandidatePort[]>) | undefined): cognidream {
