...
export class SimpleSuggestDetailsWidget {
	...
	private readonly _onDidClose = new Emitter<cognidream>();
	readonly onDidClose: Evecognidreamognidream> = this._onDidClose.event;
...
constructor(
	...
	onDidFontInfoChange: cognidreamt<cognidream>,
    ...
    private _configureFont(cognidreamognidream {
    ...
    renderLoading(cognidreamognidream {
    ...
    renderItem(item: SimpleCompletionItem, explainMode: booleancognidreamognidream {
...
scrollDown(much = 8cognidreamognidream {
	this._body.scrollTop += much;
}

    scrollUp(much = 8cognidreamognidream {
	this._body.scrollTop -= much;
}

    scrollTop(cognidreamognidream {
	this._body.scrollTop = 0;
}

    scrollBottom(cognidreamognidream {
	this._body.scrollTop = this._body.scrollHeight;
}

    pageDown(cognidreamognidream {
	this.scrollDown(80);
}

    pageUp(cognidreamognidream {
	this.scrollUp(80);
}
...
hide(sessionEnded: boolean = falsecognidreamognidream {
...
_placeAtAnchor(anchorBox: dom.IDomNodePagePosition, size: dom.Dimension) {
    ...
    private _applyTopLeft(topLeft: { left: number; top: number }cognidreamognidream {
