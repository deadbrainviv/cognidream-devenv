...
interface IPersistedWidgetSizeDelegate {
	...
	store(size: dom.Dimension): cognidream;
...
export class SimpleSuggestWidget<TModel extends SimpleCompletionModel<TItem>, TItem extends SimpleCompletionItem> extends Disposable {
	...
	private _currentSuggestionDetails?: CancelablePromicognidreamognidream>;
...
constructor(
	...
	private readonly _onDidFontConfigurationChange: cognidreamt<cognidream>,
    ...
    private _onListFocus(e: IListEvent < TItem > cognidreamognidream {
...
this._onDidFocus.fire({ item, index, model: this._completionModel });
    ...
    private _clearAriaActiveDescendant(cognidreamognidream {
	const node = dom.getActiveWindow().document.activeElement;
	if(!node) {
		return;
	}
...
resetWidgetSize(cognidreamognidream {
    ...
    showSuggestions(selectionIndex: number, isFrozen: boolean, isAuto: boolean, cursorPosition: { top: number; left: number; height: number }cognidreamognidream {
...
setLineContext(lineContext: LineContextcognidreamognidream {
	if(this._completionModel) {
	this._completionModel.lineContext = lineContext;
}
    }

    private _setState(state: Statecognidreamognidream {
...
switch (state) {
    ...
    private _showListAndStatus(cognidreamognidream {
	if(this._status) {
    ...
    private _show(cognidreamognidream {
    ...
    toggleDetailsFocus(cognidreamognidream {
	if(this._state === State.Details) {
...
toggleDetails(focused: boolean = falsecognidreamognidream {
	if(this._isDetailsVisible()) {
	// hide details widget
    ...
    private _showDetails(loading: boolean, focused: booleancognidreamognidream {
...
hide(cognidreamognidream {
    ...
    private _layout(size: dom.Dimension | undefinedcognidreamognidream {
...
_afterRender() {
    ...
    private _resize(width: number, height: numbercognidreamognidream {
...
this._positionDetails();
    ...
    private _positionDetails(cognidreamognidream {
	if(this._isDetailsVisible()) {
    ...
    private _onListMouseDownOrTap(e: IListMouseEvent<TItem> | IListGestureEvent < TItem > cognidreamognidream {
...
this._select(e.element, e.index);
    ...
    private _onListSelection(e: IListEvent < TItem > cognidreamognidream {
	if(e.elements.length) {
	this._select(e.elements[0], e.indexes[0]);
}
    }

    private _select(item: TItem, index: numbercognidreamognidream {
