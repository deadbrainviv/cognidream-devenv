...
export class AuthenticationExtensionsService extends Disposable implements IAuthenticationExtensionsService {
    ...
    private async updateNewSessionRequests(providerId: string, addedSessions: readonly AuthenticationSession[]): Promise<cognidream> {
    ...
    private updateBadgeCount(): cognidream {
    ...
    private removeAccessRequest(providerId: string, extensionId: string): cognidream {
        const providerRequests = this._sessionAccessRequestItems.get(providerId) || {};
    ...
    updateAccountPreference(extensionId: string, providerId: string, account: AuthenticationSessionAccount): cognidream {
    ...
    removeAccountPreference(extensionId: string, providerId: string): cognidream {
        const realExtensionId = ExtensionIdentifier.toKey(extensionId);
        const key = this._getKey(this._inheritAuthAccountPreferenceChildToParent[realExtensionId] ?? realExtensionId, providerId);
    ...
    updateSessionPreference(providerId: string, extensionId: string, session: AuthenticationSession): cognidream {
        const realExtensionId = ExtensionIdentifier.toKey(extensionId);
    ...
    removeSessionPreference(providerId: string, extensionId: string, scopes: string[]): cognidream {
        const realExtensionId = ExtensionIdentifier.toKey(extensionId);
        // The 3 parts of this key are important:
        // * Extension id: The extension that has a preference
        // * Provider id: The provider that the preference is for
    ...
    private _updateAccountAndSessionPreferences(providerId: string, extensionId: string, session: AuthenticationSession): cognidream {
    ...
    private async completeSessionAccessRequest(provider: IAuthenticationProvider, extensionId: string, extensionName: string, scopes: string[]): Promise<cognidream> {
    ...
    requestSessionAccess(providerId: string, extensionId: string, extensionName: string, scopes: string[], possibleSessions: AuthenticationSession[]): cognidream {
        const providerRequests = this._sessionAccessRequestItems.get(providerId) || {};
    ...
    async requestNewSession(providerId: string, scopes: string[], extensionId: string, extensionName: string): Promise<cognidream> {
        if (!this._authenticationService.isAuthenticationProviderRegistered(providerId)) {
            ...
            await new Promise<cognidream>((resolve, _) => {
