...
export interface IEditorGroupsContainer {
     ...
     * resolve.
     */
    readonly whenRestored: Promise<cognidream>;
     ...
     */
    setSize(group: IEditorGroup | GroupIdentifier, size: { width: number; height: number }): cognidream;

    /**
     * Arrange all groups in the container according to the provided arrangement.
     */
    arrangeGroups(arrangement: GroupsArrangement, target?: IEditorGroup | GroupIdentifier): cognidream;

    /**
     * Toggles the target goup size to maximize/unmaximize.
     */
    toggleMaximizeGroup(group?: IEditorGroup | GroupIdentifier): cognidream;

    /**
     * Toggles the target goup size to expand/distribute even.
     */
    toggleExpandGroup(group?: IEditorGroup | GroupIdentifier): cognidream;

    /**
     * Applies the provided layout by either moving existing groups or creating new groups.
     */
    applyLayout(layout: EditorGroupLayout): cognidream;
     ...
     */
    setGroupOrientation(orientation: GroupOrientation): cognidream;
     ...
     */
    removeGroup(group: IEditorGroup | GroupIdentifier): cognidream;
...
export interface IEditorPart extends IEditorGroupsContainer {
     ...
     */
    centerLayout(active: boolean): cognidream;
...
export interface IEditorGroupsService extends IEditorGroupsContainer {
     ...
     */
    deleteWorkingSet(workingSet: IEditorWorkingSet): cognidream;
...
export interface IActiveEditorActions {
    ...
    readonly onDidChange: Event<IMenuChangeEvent | cognidream>;
}
...
export interface IEditorGroup {
     ...
     */
    setSelection(activeSelectedEditor: EditorInput, inactiveSelectedEditors: EditorInput[]): Promise<cognidream>;
     ...
     */
    copyEditor(editor: EditorInput, target: IEditorGroup, options?: IEditorOptions): cognidream;
     ...
     * Note: It is currently not supported to show the same editor more than once in the same group.
     */
    copyEditors(editors: EditorInputWithOptions[], target: IEditorGroup): cognidream;
     ...
     */
    replaceEditors(editors: IEditorReplacement[]): Promise<cognidream>;
     ...
     * if unspecified.
     */
    pinEditor(editor?: EditorInput): cognidream;
     ...
     * if unspecified.
     */
    stickEditor(editor?: EditorInput): cognidream;
     ...
     * if unspecified.
     */
    unstickEditor(editor?: EditorInput): cognidream;
     ...
     */
    lock(locked: boolean): cognidream;
