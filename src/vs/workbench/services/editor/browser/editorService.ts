...
export class EditorService extends Disposable implements EditorServiceImpl {
    ...
    private readonly _onDidActiveEditorChange = this._register(new Emitter<cognidream>());
    readonly onDidActiveEditorChange = this._onDidActiveEditorChange.event;

    private readonly _onDidVisibleEditorsChange = this._register(new Emitter<cognidream>());
    ...
    private readonly _onDidMostRecentlyActiveEditorsChange = this._register(new Emitter<cognidream>());
    ...
    private onEditorGroupsReady(): cognidream {
    ...
    private handleActiveEditorChange(group: IEditorGroup): cognidream {
    ...
    private doHandleActiveEditorChangeEvent(): cognidream {
    ...
    private registerGroupListeners(group: IEditorGroupView): cognidream {
    ...
    private handleVisibleEditorsChange(): cognidream {
    ...
    private async onDidRunFileOperation(e: FileOperationEvent): Promise<cognidream> {
    ...
    private onDidFilesChange(e: FileChangesEvent): cognidream {
        if (e.gotDeleted()) {
            this.handleDeletedFile(e, true);
        }
    }

    private async handleMovedFile(source: URI, target: URI): Promise<cognidream> {
        for (const group of this.editorGroupsContainer.groups) {
    ...
    private onConfigurationUpdated(e?: IConfigurationChangeEvent): cognidream {
    ...
    private handleDeletedFile(arg1: URI | FileChangesEvent, isExternal: boolean, movedTo?: URI): cognidream {
    ...
    private getAllNonDirtyEditors(options: { includeUntitled: boolean; supportSideBySide: boolean }): EditorInput[] {
        ...
        function conditionallyAddEditor(editor: EditorInput): cognidream {
    ...
    async closeEditor({ editor, groupId }: IEditorIdentifier, options?: ICloseEditorOptions): Promise<cognidream> {
    ...
    async closeEditors(editors: IEditorIdentifier[], options?: ICloseEditorOptions): Promise<cognidream> {
    ...
    async replaceEditors(replacements: IUntypedEditorReplacement[], group: IEditorGroup | GroupIdentifier): Promise<cognidream>;
    async replaceEditors(replacements: IEditorReplacement[], group: IEditorGroup | GroupIdentifier): Promise<cognidream>;
    async replaceEditors(replacements: Array<IEditorReplacement | IUntypedEditorReplacement>, group: IEditorGroup | GroupIdentifier): Promise<cognidream> {
