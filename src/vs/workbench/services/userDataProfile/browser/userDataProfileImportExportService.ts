...
export class UserDataProfileImportExportService extends Disposable implements IUserDataProfileImportExportService {
    ...
    unregisterProfileContentHandler(id: string): cognidream {
    ...
    async createFromProfile(from: IUserDataProfile, options: IUserDataProfileCreateOptions, token: CancellationToken): Promise<IUserDataProfile | undefined> {
        ...
        let creationPromise: CancelablePromise<cognidream>;
    ...
    private async applyProfileTemplate(profileTemplate: IUserDataProfileTemplate, profile: IUserDataProfile, options: IUserDataProfileCreateOptions, reportProgress: (message: string) => cognidream, token: CancellationToken): Promise<cognidream> {
    ...
    async exportProfile(profile: IUserDataProfile, exportFlags?: ProfileResourceTypeFlags): Promise<cognidream> {
        const disposables = new DisposableStore();
        try {
    ...
    async createTroubleshootProfile(): Promise<cognidream> {
    ...
    private async doExportProfile(userDataProfilesExportState: UserDataProfileExportState, location: ProgressLocation | string): Promise<cognidream> {
        ...
        try {
            await this.progressService.withProgress({
                ...
                if (profileContentHandler.extensionId) {
                    const buttons: IPromptButton<cognidream>[] = [];
    ...
    private async doCreateProfile(profileTemplate: IUserDataProfileTemplate, temporaryProfile: boolean, extensions: boolean, options: IUserDataProfileOptions | undefined, progress: (message: string) => cognidream): Promise<IUserDataProfile | undefined> {
...
abstract class UserDataProfileImportExportState extends Disposable implements ITreeViewDataProvider {
    ...
    private readonly _onDidChangeRoots = this._register(new Emitter<cognidream>());
