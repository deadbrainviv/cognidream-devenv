...
class RemoteUserDataProfilesService extends Disposable implements IRemoteUserDataProfilesService {
    ...
    private readonly initPromise: Promise<cognidream>;
    ...
    private async init(): Promise<cognidream> {
    ...
    private async onDidChangeLocalProfiles(e: DidChangeProfilesEvent): Promise<cognidream> {
    ...
    private setAssociatedRemoteProfiles(profiles: string[]): cognidream {
        if (this.environmentService.remoteAuthority) {
            const remotes = this.parseAssociatedRemoteProfiles();
    ...
    private async cleanUp(): Promise<cognidream> {
