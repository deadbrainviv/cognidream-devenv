...
export class TextFileEditorModelManager extends Disposable implements ITextFileEditorModelManager {
    ...
    private readonly mapResourceToPendingModelResolvers = new ResourceMap<Promise<cognidream>>();
    ...
    saveErrorHandler = (() => {
        ...
        return {
            onSaveError(error: Error, model: ITextFileEditorModel): cognidream {
    ...
    private onDidChangeFileSystemProviderCapabilities(e: IFileSystemProviderCapabilitiesChangeEvent): cognidream {
    ...
    private onDidChangeFileSystemProviderRegistrations(e: IFileSystemProviderRegistrationEvent): cognidream {
        if (!e.added) {
    ...
    private queueModelReloads(scheme: string): cognidream {
        for (const model of this.models) {
            if (model.isDirty()) {
                continue; // never reload dirty models
            }
    ...
    private queueModelReload(model: TextFileEditorModel): cognidream {
    ...
    private onWillRunWorkingCopyFileOperation(e: WorkingCopyFileEvent): cognidream {
    ...
    private onDidFailWorkingCopyFileOperation(e: WorkingCopyFileEvent): cognidream {
    ...
    private onDidRunWorkingCopyFileOperation(e: WorkingCopyFileEvent): cognidream {
        switch (e.operation) {
    ...
    private async reload(model: TextFileEditorModel): Promise<cognidream> {
    ...
    private async doResolve(resourceOrModel: URI | TextFileEditorModel, options?: ITextFileEditorModelResolveOrCreateOptions): Promise<TextFileEditorModel> {
        ...
        let modelResolve: Promise<cognidream>;
        ...
        // Store pending resolves to acognidream race conditions
    ...
    private joinPendingResolves(resource: URI): Promise<cognidream> | undefined {
    ...
    private async doJoinPendingResolves(resource: URI): Promise<cognidream> {
        ...
        // at the same time.
        let currentModelCopyResolve: Promise<cognidream> | undefined;
    ...
    private registerModel(model: TextFileEditorModel): cognidream {
    ...
    add(resource: URI, model: TextFileEditorModel): cognidream {
    ...
    remove(resource: URI): cognidream {
    ...
    runSaveParticipants(model: ITextFileEditorModel, context: IStoredFileWorkingCopySaveParticipantContext, progress: IProgress<IProgressStep>, token: CancellationToken): Promise<cognidream> {
