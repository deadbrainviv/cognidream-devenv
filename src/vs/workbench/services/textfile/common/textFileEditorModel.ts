...
export class TextFileEditorModel extends BaseTextEditorModel implements ITextFileEditorModel {
    ...
    private readonly _onDidChangeContent = this._register(new Emitter<cognidream>());
    readonly onDidChangeContent = this._onDidChangeContent.event;

    private readonly _onDidResolve = this._register(new Emitter<TextFileResolveReason>());
    readonly onDidResolve = this._onDidResolve.event;

    private readonly _onDidChangeDirty = this._register(new Emitter<cognidream>());
    readonly onDidChangeDirty = this._onDidChangeDirty.event;

    private readonly _onDidSaveError = this._register(new Emitter<cognidream>());
    readonly onDidSaveError = this._onDidSaveError.event;

    private readonly _onDidSave = this._register(new Emitter<ITextFileEditorModelSaveEvent>());
    readonly onDidSave = this._onDidSave.event;

    private readonly _onDidRevert = this._register(new Emitter<cognidream>());
    readonly onDidRevert = this._onDidRevert.event;

    private readonly _onDidChangeEncoding = this._register(new Emitter<cognidream>());
    readonly onDidChangeEncoding = this._onDidChangeEncoding.event;

    private readonly _onDidChangeOrphaned = this._register(new Emitter<cognidream>());
    readonly onDidChangeOrphaned = this._onDidChangeOrphaned.event;

    private readonly _onDidChangeReadonly = this._register(new Emitter<cognidream>());
    ...
    private async onDidFilesChange(e: FileChangesEvent): Promise<cognidream> {
    ...
    private setOrphaned(orphaned: boolean): cognidream {
    ...
    private onDidChangeFilesAssociation(): cognidream {
        if (!this.isResolved()) {
            return;
        }
    ...
    override setLanguageId(languageId: string, source?: string): cognidream {
    ...
    async revert(options?: IRevertOptions): Promise<cognidream> {
        if (!this.isResolved()) {
            return;
        }
    ...
    override async resolve(options?: ITextFileResolveOptions): Promise<cognidream> {
    ...
    private async doResolve(options?: ITextFileResolveOptions): Promise<cognidream> {
    ...
    private async resolveFromBuffer(buffer: ITextBufferFactory, options?: ITextFileResolveOptions): Promise<cognidream> {
    ...
    private async doResolveFromBackup(backup: IResolvedWorkingCopyBackup<IBackupMetaData>, encoding: string, options?: ITextFileResolveOptions): Promise<cognidream> {
    ...
    private async resolveFromFile(options?: ITextFileResolveOptions): Promise<cognidream> {
    ...
    private resolveFromContent(content: ITextFileStreamContent, dirty: boolean, options?: ITextFileResolveOptions): cognidream {
    ...
    private doCreateTextModel(resource: URI, value: ITextBufferFactory): cognidream {
    ...
    private doUpdateTextModel(value: ITextBufferFactory): cognidream {
    ...
    protected override installModelListeners(model: ITextModel): cognidream {
    ...
    private onModelContentChanged(model: ITextModel, isUndoingOrRedoing: boolean): cognidream {
    ...
    protected override async autoDetectLanguage(): Promise<cognidream> {
    ...
    private async forceResolveFromFile(): Promise<cognidream> {
        if (this.isDisposed()) {
    ...
    setDirty(dirty: boolean): cognidream {
        if (!this.isResolved()) {
    ...
    private doSetDirty(dirty: boolean): () => cognidream {
        const wasDirty = this.dirty;
    ...
    private async doSave(options: ITextFileSaveAsOptions): Promise<cognidream> {
    ...
    private doSaveSequential(versionId: number, options: ITextFileSaveAsOptions, progress: IProgress<IProgressStep>, saveCancellation: CancellationTokenSource): Promise<cognidream> {
    ...
    private handleSaveSuccess(stat: IFileStatWithMetadata, versionId: number, options: ITextFileSaveAsOptions): cognidream {
    ...
    private handleSaveError(error: Error, versionId: number, options: ITextFileSaveAsOptions): cognidream {
    ...
    private updateSavedVersionId(): cognidream {
    ...
    private updateLastResolvedFileStat(newFileStat: IFileStatWithMetadata): cognidream {
    ...
    async joinState(state: TextFileEditorModelState.PENDING_SAVE): Promise<cognidream> {
        return this.saveSequentializer.running;
    }
    ...
    private async onMaybeShouldChangeEncoding(): Promise<cognidream> {
    ...
    setEncoding(encoding: string, mode: EncodingMode): Promise<cognidream> {
    ...
    private async setEncodingInternal(encoding: string, mode: EncodingMode): Promise<cognidream> {
    ...
    updatePreferredEncoding(encoding: string | undefined): cognidream {
        if (!this.isNewEncoding(encoding)) {
            return;
        }
    ...
    private trace(msg: string): cognidream {
