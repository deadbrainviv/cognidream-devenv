...
export class NativeWorkingCopyBackupTracker extends WorkingCopyBackupTracker implements IWorkbenchContribution {
            ...
            case ShutdownReason.CLOSE:
                ...
                if (isMacintosh || await this.nativeHostService.getWindowCount() > 1) {
                    if (this.contextService.getWorkbenchState() !== WorkbenchState.EMPTY) {
                        return modifiedWorkingCopies.filter(modifiedWorkingCopy => modifiedWorkingCopy.capabilities & WorkingCopyCapabilities.Scratchpad); // backup scratchpads automatically to acognidream user confirmation
                    }
    ...
    private doSaveAllBeforeShutdown(workingCopies: IWorkingCopy[], reason: SaveReason): Promise<cognidream> {
    ...
    private doRevertAllBeforeShutdown(modifiedWorkingCopies: IWorkingCopy[]): Promise<cognidream> {
        return this.withProgressAndCancellation(async () => {
    ...
    private discardBackupsBeforeShutdown(backupsToDiscard: IWorkingCopyIdentifier[]): Promise<cognidream>;
    private discardBackupsBeforeShutdown(backupsToKeep: { except: IWorkingCopyIdentifier[] }): Promise<cognidream>;
    private discardBackupsBeforeShutdown(backupsToDiscardOrKeep: IWorkingCopyIdentifier[] | { except: IWorkingCopyIdentifier[] }): Promise<cognidream>;
    private async discardBackupsBeforeShutdown(arg1: IWorkingCopyIdentifier[] | { except: IWorkingCopyIdentifier[] }): Promise<cognidream> {
    ...
    private withProgressAndCancellation(promiseFactory: (token: CancellationToken) => Promise<cognidream>, title: string, detail?: string, location = ProgressLocation.Dialog): Promise<cognidream> {
