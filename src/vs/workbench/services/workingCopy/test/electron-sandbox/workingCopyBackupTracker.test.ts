...
suite('WorkingCopyBackupTracker (native)', function() {
    ...
    class TestWorkingCopyBackupTracker extends NativeWorkingCopyBackupTracker {
        ...
        waitForReady(): Promise<cognidream> {
        ...
        private readonly _onDidResume = this._register(new Emitter<cognidream>());
        readonly onDidResume = this._onDidResume.event;

        private readonly _onDidSuspend = this._register(new Emitter<cognidream>());
        readonly onDidSuspend = this._onDidSuspend.event;

        protected override suspendBackupOperations(): { resume: () => cognidream } {
    ...
    async function createTracker(autoSaveEnabled = false): Promise<{ accessor: TestServiceAccessor; part: EditorPart; tracker: TestWorkingCopyBackupTracker; instantiationService: IInstantiationService; cleanup: () => Promise<cognidream> }> {
        const instantiationService = workbenchInstantiationService(undefined, disposables);
    ...
    suite('Hot Exit', () => {
        ...
        async function hotExitTest(this: any, setting: string, shutdownReason: ShutdownReason, multipleWindows: boolean, workspace: boolean, shouldVeto: boolean): Promise<cognidream> {
            const { accessor, cleanup } = await createTracker();
        ...
        async function scratchpadHotExitTest(this: any, setting: string, shutdownReason: ShutdownReason, multipleWindows: boolean, workspace: boolean, shouldVeto: boolean): Promise<cognidream> {
            const { accessor, cleanup } = await createTracker();
