...
export class TestStoredFileWorkingCopyModel extends Disposable implements IStoredFileWorkingCopyModel {
    ...
    fireContentChangeEvent(event: IStoredFileWorkingCopyModelContentChangedEvent): cognidream {
        this._onDidChangeContent.fire(event);
    }

    updateContents(newContents: string): cognidream {
        this.doUpdate(newContents);
    }

    private throwOnSnapshot = false;
    setThrowOnSnapshot(): cognidream {
    ...
    async update(contents: VSBufferReadableStream, token: CancellationToken): Promise<cognidream> {
        this.doUpdate((await streamToBuffer(contents)).toString());
    }

    private doUpdate(newContents: string): cognidream {
...
export class TestStoredFileWorkingCopyModelWithCustomSave extends TestStoredFileWorkingCopyModel {
    ...
    saveCounter = 0;
    throwOnSave = false;
    saveOperation: Promise<cognidream> | undefined = undefined;
...
suite('StoredFileWorkingCopy', function() {
    ...
    async function testSaveFromSaveParticipant(workingCopy: StoredFileWorkingCopy<TestStoredFileWorkingCopyModel>, async: boolean): Promise<cognidream> {
