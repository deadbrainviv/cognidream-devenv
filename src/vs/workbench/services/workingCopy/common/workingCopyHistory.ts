...
export interface IWorkingCopyHistoryService {
     ...
     */
    onDidMoveEntries: Event<cognidream>;

    /**
     * An event when all entries are removed from the history.
     */
    onDidRemoveEntries: Event<cognidream>;
     ...
     */
    updateEntry(entry: IWorkingCopyHistoryEntry, properties: { source: SaveSource }, token: CancellationToken): Promise<cognidream>;
     ...
     */
    removeAll(token: CancellationToken): Promise<cognidream>;
}
