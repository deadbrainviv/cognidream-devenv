...
export class WorkingCopyBackupsModel {
    ...
    private async resolve(): Promise<cognidream> {
        try {
    ...
    add(resource: URI, versionId = 0, meta?: IWorkingCopyBackupMeta): cognidream {
    ...
    update(resource: URI, meta?: IWorkingCopyBackupMeta): cognidream {
...
export abstract class WorkingCopyBackupService extends Disposable implements IWorkingCopyBackupService {
    ...
    reinitialize(backupWorkspaceHome: URI | undefined): cognidream {
    ...
    backup(identifier: IWorkingCopyIdentifier, content?: VSBufferReadableStream | VSBufferReadable, versionId?: number, meta?: IWorkingCopyBackupMeta, token?: CancellationToken): Promise<cognidream> {
        return this.impl.backup(identifier, content, versionId, meta, token);
    }

    discardBackup(identifier: IWorkingCopyIdentifier, token?: CancellationToken): Promise<cognidream> {
        return this.impl.discardBackup(identifier, token);
    }

    discardBackups(filter?: { except: IWorkingCopyIdentifier[] }): Promise<cognidream> {
    ...
    joinBackups(): Promise<cognidream> {
...
class WorkingCopyBackupServiceImpl extends Disposable implements IWorkingCopyBackupService {
    ...
    initialize(backupWorkspaceResource: URI): cognidream {
    ...
    async backup(identifier: IWorkingCopyIdentifier, content?: VSBufferReadable | VSBufferReadableStream, versionId?: number, meta?: IWorkingCopyBackupMeta, token?: CancellationToken): Promise<cognidream> {
        const model = await this.ready;
    ...
    async discardBackups(filter?: { except: IWorkingCopyIdentifier[] }): Promise<cognidream> {
        const model = await this.ready;
    ...
    private async doDiscardBackup(backupResource: URI, token?: CancellationToken): Promise<cognidream> {
        const model = await this.ready;
        if (token?.isCancellationRequested) {
            return;
        }
    ...
    private async deleteIgnoreFileNotFound(backupResource: URI): Promise<cognidream> {
        try {
...
export class InMemoryWorkingCopyBackupService extends Disposable implements IWorkingCopyBackupService {
    ...
    async discardBackup(identifier: IWorkingCopyIdentifier): Promise<cognidream> {
    ...
    async joinBackups(): Promise<cognidream> {
        return;
    }
}
