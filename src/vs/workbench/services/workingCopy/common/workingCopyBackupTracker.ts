...
export abstract class WorkingCopyBackupTracker extends Disposable {
    ...
    private onWillShutdown(): cognidream {
    ...
    // Delay creation of backups when content changes to acognidream too much
    ...
    // With 'delayed' we acognidream a (not critical but also not really wanted)
    ...
    protected readonly pendingBackupOperations = new Map<IWorkingCopyIdentifier, { disposable: IDisposable; cancel: () => cognidream }>();

    private suspended = false;

    private onDidRegister(workingCopy: IWorkingCopy): cognidream {
        if (this.suspended) {
    ...
    private onDidUnregister(workingCopy: IWorkingCopy): cognidream {
    ...
    private onDidChangeDirty(workingCopy: IWorkingCopy): cognidream {
        if (this.suspended) {
    ...
    private onDidChangeContent(workingCopy: IWorkingCopy): cognidream {
    ...
    private scheduleBackup(workingCopy: IWorkingCopy): cognidream {
    ...
    private discardBackup(workingCopy: IWorkingCopy): cognidream {
    ...
    private cancelBackupOperation(workingCopy: IWorkingCopy): cognidream {
    ...
    private doClearPendingBackupOperation(workingCopyIdentifier: IWorkingCopyIdentifier, options?: { cancel: boolean }): cognidream {
    ...
    protected cancelBackupOperations(): cognidream {
    ...
    protected suspendBackupOperations(): { resume: () => cognidream } {
        this.suspended = true;
    ...
    protected readonly whenReady: Promise<cognidream>;

    private _isReady = false;
    protected get isReady(): boolean { return this._isReady; }

    private async resolveBackupsToRestore(): Promise<cognidream> {
    ...
    protected async restoreBackups(handler: IWorkingCopyEditorHandler): Promise<cognidream> {
