...
export class WorkingCopyHistoryModel {
    ...
    private whenResolved: Promise<cognidream> | undefined = undefined;
    ...
    private setWorkingCopy(workingCopyResource: URI): cognidream {
    ...
    async updateEntry(entry: IWorkingCopyHistoryEntry, properties: { source: SaveSource }, token: CancellationToken): Promise<cognidream> {
    ...
    private resolveEntriesOnce(): Promise<cognidream> {
    ...
    private async doResolveEntries(): Promise<cognidream> {
    ...
    async moveEntries(target: WorkingCopyHistoryModel, source: SaveSource, token: CancellationToken): Promise<cognidream> {
    ...
    async store(token: CancellationToken): Promise<cognidream> {
        if (!this.shouldStore()) {
            return;
        }
    ...
    private async doStore(token: CancellationToken): Promise<cognidream> {
        const historyEntriesFolder = assertIsDefined(this.historyEntriesFolder);
    ...
    private async cleanUpEntries(): Promise<cognidream> {
        const configuredMaxEntries = this.configurationService.getValue<number>(WorkingCopyHistoryModel.SETTINGS.MAX_ENTRIES, { resource: this.workingCopyResource });
        if (this.entries.length <= configuredMaxEntries) {
    ...
    private async deleteEntry(entry: IWorkingCopyHistoryEntry): Promise<cognidream> {
        try {
    ...
    private async writeEntriesFile(): Promise<cognidream> {
        const workingCopyResource = assertIsDefined(this.workingCopyResource);
    ...
    private traceError(error: Error): cognidream {
...
export abstract class WorkingCopyHistoryService extends Disposable implements IWorkingCopyHistoryService {
    ...
    private readonly _onDidMoveEntries = this._register(new Emitter<cognidream>());
    ...
    private readonly _onDidRemoveEntries = this._register(new Emitter<cognidream>());
    ...
    private async resolveLocalHistoryHome(): Promise<cognidream> {
    ...
    async removeAll(token: CancellationToken): Promise<cognidream> {
...
export class NativeWorkingCopyHistoryService extends WorkingCopyHistoryService {
    ...
    private onWillShutdown(e: WillShutdownEvent): cognidream {
    ...
    private onDidChangeModels(): cognidream {
        if (!this.storeAllScheduler.isScheduled()) {
            this.storeAllScheduler.schedule();
        }
    }

    private async storeAll(token: CancellationToken): Promise<cognidream> {
        const limiter = new Limiter(MAX_PARALLEL_HISTORY_IO_OPS);
        const promises = [];
