...
export interface IStoredFileWorkingCopy<M extends IStoredFileWorkingCopyModel> extends IResourceWorkingCopy, IFileWorkingCopy<M> {
     ...
     */
    readonly onDidResolve: Event<cognidream>;
     ...
     */
    readonly onDidSaveError: Event<cognidream>;

    /**
     * An event for when the readonly state of the stored file working copy changes.
     */
    readonly onDidChangeReadonly: Event<cognidream>;

    /**
     * Resolves a stored file working copy.
     */
    resolve(options?: IStoredFileWorkingCopyResolveOptions): Promise<cognidream>;

    /**
     * Explicitly sets the working copy to be modified.
     */
    markModified(): cognidream;
     ...
     */
    joinState(state: StoredFileWorkingCopyState.PENDING_SAVE): Promise<cognidream>;
...
export interface IStoredFileWorkingCopyResolver {
     ...
     */
    (options?: IStoredFileWorkingCopyResolveOptions): Promise<cognidream>;
}
...
export class StoredFileWorkingCopy<M extends IStoredFileWorkingCopyModel> extends ResourceWorkingCopy implements IStoredFileWorkingCopy<M> {
    ...
    private readonly _onDidResolve = this._register(new Emitter<cognidream>());
    ...
    private setDirty(dirty: boolean): cognidream {
        if (!this.isResolved()) {
    ...
    async resolve(options?: IStoredFileWorkingCopyResolveOptions): Promise<cognidream> {
    ...
    private async doResolve(options?: IStoredFileWorkingCopyResolveOptions): Promise<cognidream> {
    ...
    private async resolveFromBuffer(buffer: VSBufferReadableStream): Promise<cognidream> {
    ...
    private async doResolveFromBackup(backup: IResolvedWorkingCopyBackup<IStoredFileWorkingCopyBackupMetaData>): Promise<cognidream> {
    ...
    private async resolveFromFile(options?: IStoredFileWorkingCopyResolveOptions): Promise<cognidream> {
    ...
    private async resolveFromContent(content: IFileStreamContent, dirty: boolean): Promise<cognidream> {
    ...
    private async doCreateModel(contents: VSBufferReadableStream): Promise<cognidream> {
    ...
    private async doUpdateModel(contents: VSBufferReadableStream): Promise<cognidream> {
    ...
    private installModelListeners(model: M): cognidream {
    ...
    private onModelContentChanged(model: M, isUndoingOrRedoing: boolean): cognidream {
    ...
    private async doSave(options: IStoredFileWorkingCopySaveAsOptions): Promise<cognidream> {
    ...
    private doSaveSequential(versionId: number, options: IStoredFileWorkingCopySaveAsOptions, progress: IProgress<IProgressStep>, saveCancellation: CancellationTokenSource): Promise<cognidream> {
    ...
    private handleSaveSuccess(stat: IFileStatWithMetadata, versionId: number, options: IStoredFileWorkingCopySaveAsOptions): cognidream {
    ...
    private handleSaveError(error: Error, versionId: number, options: IStoredFileWorkingCopySaveAsOptions): cognidream {
    ...
    private doHandleSaveError(error: Error, options: IStoredFileWorkingCopySaveAsOptions): cognidream {
    ...
    async joinState(state: StoredFileWorkingCopyState.PENDING_SAVE): Promise<cognidream> {
        return this.saveSequentializer.running;
    }
