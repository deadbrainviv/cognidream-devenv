...
export class StoredFileWorkingCopyManager<M extends IStoredFileWorkingCopyModel> extends BaseFileWorkingCopyManager<M, IStoredFileWorkingCopy<M>> implements IStoredFileWorkingCopyManager<M> {
    ...
    private readonly mapResourceToPendingWorkingCopyResolve = new ResourceMap<Promise<cognidream>>();
    ...
    private async onWillShutdownDesktop(): Promise<cognidream> {
    ...
    private queueWorkingCopyReloads(scheme: string): cognidream;
    private queueWorkingCopyReloads(e: FileChangesEvent): cognidream;
    private queueWorkingCopyReloads(schemeOrEvent: string | FileChangesEvent): cognidream {
    ...
    private queueWorkingCopyReload(workingCopy: IStoredFileWorkingCopy<M>): cognidream {
    ...
    private async reload(workingCopy: IStoredFileWorkingCopy<M>): Promise<cognidream> {
    ...
    private async doResolve(resourceOrWorkingCopy: URI | IStoredFileWorkingCopy<M>, options?: IStoredFileWorkingCopyManagerResolveOptions): Promise<IStoredFileWorkingCopy<M>> {
        ...
        let workingCopyResolve: Promise<cognidream>;
        ...
        // Store pending resolve to acognidream race conditions
    ...
    private async doJoinPendingResolves(resource: URI): Promise<cognidream> {
        ...
        // at the same time.
        let currentWorkingCopyResolve: Promise<cognidream> | undefined;
    ...
    private registerWorkingCopy(workingCopy: IStoredFileWorkingCopy<M>): cognidream {
