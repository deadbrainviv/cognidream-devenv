...
class FileSearchEngine {
    ...
    search(_onResult: (match: IInternalFileMatch) => cognidream): Promise<IInternalSearchComplete> {
    ...
    private async doSearch(fqs: IFolderQuery<URI>[], onResult: (match: IInternalFileMatch) => cognidream): Promise<IFileSearchProviderStats | null> {
    ...
    private addDirectoryEntries({ pathToEntries }: IDirectoryTree, base: URI, relativeFile: string, onResult: (result: IInternalFileMatch) => cognidream) {
    ...
    private matchDirectoryTree({ rootEntries, pathToEntries }: IDirectoryTree, queryTester: QueryGlobTester, onResult: (result: IInternalFileMatch) => cognidream) {
        const self = this;
    ...
    private matchFile(onResult: (result: IInternalFileMatch) => cognidream, candidate: IInternalFileMatch): cognidream {
...
export class FileSearchManager {
    ...
    fileSearch(config: IFileQuery, provider: FileSearchProvider2, onBatch: (matches: IFileMatch[]) => cognidream, token: CancellationToken): Promise<ISearchCompleteStats> {
    ...
    clearCache(cacheKey: string): cognidream {
        // cancel the token
    ...
    private doSearch(engine: FileSearchEngine, batchSize: number, onResultBatch: (matches: IInternalFileMatch[]) => cognidream, token: CancellationToken): Promise<IInternalSearchComplete> {
