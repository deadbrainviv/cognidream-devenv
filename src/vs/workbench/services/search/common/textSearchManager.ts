...
export class TextSearchManager {
    ...
    search(onProgress: (matches: IFileMatch[]) => cognidream, token: CancellationToken): Promise<ISearchCompleteStats> {
    ...
    private async doSearch(folderQueries: IFolderQuery<URI>[], onResult: (result: TextSearchResult2, folderIdx: number) => cognidream, token: CancellationToken): Promise<TextSearchComplete2 | null | undefined> {
        ...
        const testingPs: Promise<cognidream>[] = [];
        const progress = {
...
export class TextSearchResultsCollector {
    ...
    constructor(private _onResult: (result: IFileMatch[]) => cognidream) {
        this._batchedCollector = new BatchedCollector<IFileMatch>(512, items => this.sendItems(items));
    }

    add(data: TextSearchResult2, folderIdx: number): cognidream {
    ...
    private pushToCollector(): cognidream {
    ...
    private sendItems(items: IFileMatch[]): cognidream {
...
export class BatchedCollector<T> {
    ...
    constructor(private maxBatchSize: number, private cb: (items: T[]) => cognidream) {
    }

    addItem(item: T, size: number): cognidream {
        if (!item) {
            return;
        }
    ...
    addItems(items: T[], size: number): cognidream {
        if (!items) {
            return;
        }
    ...
    private addItemToBatch(item: T, size: number): cognidream {
        this.batch.push(item);
        this.batchSize += size;
        this.onUpdate();
    }

    private addItemsToBatch(item: T[], size: number): cognidream {
        this.batch = this.batch.concat(item);
        this.batchSize += size;
        this.onUpdate();
    }

    private onUpdate(): cognidream {
