...
export interface ISearchService {
    ...
    textSearch(query: ITextQuery, token?: CancellationToken, onProgress?: (result: ISearchProgressItem) => cognidream): Promise<ISearchComplete>;
    aiTextSearch(query: IAITextQuery, token?: CancellationToken, onProgress?: (result: ISearchProgressItem) => cognidream): Promise<ISearchComplete>;
    getAIName(): Promise<string | undefined>;
    textSearchSplitSyncAsync(query: ITextQuery, token?: CancellationToken | undefined, onProgress?: ((result: ISearchProgressItem) => cognidream) | undefined, notebookFilesToIgnore?: ResourceSet, asyncNotebookFilesToIgnore?: Promise<ResourceSet>): { syncResults: ISearchComplete; asyncResults: Promise<ISearchComplete> };
    fileSearch(query: IFileQuery, token?: CancellationToken): Promise<ISearchComplete>;
    schemeHasFileSearchProvider(scheme: string): boolean;
    clearCache(cacheKey: string): Promise<cognidream>;
...
export interface ISearchResultProvider {
    getAIName(): Promise<string | undefined>;
    textSearch(query: ITextQuery, onProgress?: (p: ISearchProgressItem) => cognidream, token?: CancellationToken): Promise<ISearchComplete>;
    fileSearch(query: IFileQuery, token?: CancellationToken): Promise<ISearchComplete>;
    clearCache(cacheKey: string): Promise<cognidream>;
}
...
export interface ISearchEngine<T> {
    search: (onResult: (matches: T) => cognidream, onProgress: (progress: IProgressMessage) => cognidream, done: (error: Error | null, complete: ISearchEngineSuccess) => cognidream) => cognidream;
    cancel: () => cognidream;
}
...
export class SerializableFileMatch implements ISerializedFileMatch {
    ...
    addMatch(match: ITextSearchMatch): cognidream {
...
export class QueryGlobTester {
    ...
    /**
     * Evaluating the exclude expression is only async if it includes sibling clauses. As an optimization, acognidream doing anything with Promises
