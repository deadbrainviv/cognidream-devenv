...
const killCmds = new Set<() => cognidream>();
process.on('exit', () => {
...
export class FileWalker {
    ...
    walk(folderQueries: IFolderQuery[], extraFiles: URI[], numThreads: number | undefined, onResult: (result: IRawFileMatch) => cognidream, onMessage: (message: IProgressMessage) => cognidream, done: (error: Error | null, isLimitHit: boolean) => cognidream): cognidream {
        ...
        this.parallel<IFolderQuery, cognidream>(folderQueries, (folderQuery: IFolderQuery, rootFolderDone: (err: Error | null, result: cognidream) => cognidream) => {
    ...
    private parallel<T, E>(list: T[], fn: (item: T, callback: (err: Error | null, result: E | null) => cognidream) => cognidream, callback: (err: Array<Error | null> | null, result: E[]) => cognidream): cognidream {
    ...
    private call<F extends Function>(fun: F, that: any, ...args: any[]): cognidream {
        try {
            fun.apply(that, args);
        } catch (e) {
    ...
    private cmdTraversal(folderQuery: IFolderQuery, numThreads: number | undefined, onResult: (result: IRawFileMatch) => cognidream, onMessage: (message: IProgressMessage) => cognidream, cb: (err?: Error) => cognidream): cognidream {
     ...
     * Public for testing.
     */
    readStdout(cmd: childProcess.ChildProcess, encoding: BufferEncoding, cb: (err: Error | null, stdout?: string) => cognidream): cognidream {
        let all = '';
    ...
    private collectStdout(cmd: childProcess.ChildProcess, encoding: BufferEncoding, onMessage: (message: IProgressMessage) => cognidream, cb: (err: Error | null, stdout?: string, last?: boolean) => cognidream): cognidream {
    ...
    private forwardData(stream: Readable, encoding: BufferEncoding, cb: (err: Error | null, stdout?: string) => cognidream): StringDecoder {
    ...
    private addDirectoryEntries(folderQuery: IFolderQuery, { pathToEntries }: IDirectoryTree, base: string, relativeFiles: string[], onResult: (result: IRawFileMatch) => cognidream) {
    ...
    private matchDirectoryTree({ rootEntries, pathToEntries }: IDirectoryTree, rootFolder: string, onResult: (result: IRawFileMatch) => cognidream) {
        const self = this;
    ...
    private doWalk(folderQuery: IFolderQuery, relativeParentPath: string, files: string[], onResult: (result: IRawFileMatch) => cognidream, done: (error?: Error) => cognidream): cognidream {
        const rootFolder = folderQuery.folder;

        // Execute tasks on each file in parallel to optimize throughput
        const hasSibling = hasSiblingFn(() => files);
        this.parallel(files, (file: string, clb: (error: Error | null, _?: any) => cognidream): cognidream => {
        ...
        }, (error: Array<Error | null> | null): cognidream => {
    ...
    private matchFile(onResult: (result: IRawFileMatch) => cognidream, candidate: IRawFileMatch): cognidream {
    ...
    private statLinkIfNeeded(path: string, lstat: fs.Stats, clb: (error: Error | null, stat: fs.Stats) => cognidream): cognidream {
    ...
    private realPathIfNeeded(path: string, lstat: fs.Stats, clb: (error: Error | null, realpath?: string) => cognidream): cognidream {
        if (lstat.isSymbolicLink()) {
...
export class Engine implements ISearchEngine<IRawFileMatch> {
    ...
    search(onResult: (result: IRawFileMatch) => cognidream, onProgress: (progress: IProgressMessage) => cognidream, done: (error: Error | null, complete: ISearchEngineSuccess) => cognidream): cognidream {
...
class AbsoluteAndRelativeParsedExpression {
     ...
     */
    private init(expr: glob.IExpression): cognidream {
