...
export class RipgrepParser extends EventEmitter {
    ...
    override on(event: 'result', listener: (result: TextSearchResult2) => cognidream): this;
    override on(event: 'hitLimit', listener: () => cognidream): this;
    override on(event: string, listener: (...args: any[]) => cognidream): this {
        super.on(event, listener);
        return this;
    }

    handleData(data: Buffer | string): cognidream {
        if (this.isDone) {
            return;
        }
    ...
    private handleDecodedData(decodedData: string): cognidream {
    ...
    private handleLine(outputLine: string): cognidream {
    ...
    private onResult(match: TextSearchResult2): cognidream {
