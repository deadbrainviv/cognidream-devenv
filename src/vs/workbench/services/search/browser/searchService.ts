...
export class LocalFileSearchWorkerClient extends Disposable implements ISearchResultProvider {
    ...
    sendTextSearchMatch(match: IFileMatch<UriComponents>, queryId: number): cognidream {
    ...
    async textSearch(query: ITextQuery, onProgress?: (p: ISearchProgressItem) => cognidream, token?: CancellationToken): Promise<ISearchComplete> {
        try {
