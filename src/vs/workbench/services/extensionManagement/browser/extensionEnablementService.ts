...
export class ExtensionEnablementService extends Disposable implements IWorkbenchExtensionEnablementService {
    ...
    private readonly delayer = this._register(new Delayer<cognidream>(0));
    ...
    private throwErrorIfCannotChangeEnablement(extension: IExtension, donotCheckDependencies?: boolean): cognidream {
    ...
    private throwErrorIfEnablementStateCannotBeChanged(extension: IExtension, enablementStateOfExtension: EnablementState, donotCheckDependencies?: boolean): cognidream {
    ...
    private throwErrorIfCannotChangeWorkspaceEnablement(extension: IExtension): cognidream {
    ...
    private _enableExtensionInWorkspace(identifier: IExtensionIdentifier): cognidream {
        this._removeFromWorkspaceDisabledExtensions(identifier);
        this._addToWorkspaceEnabledExtensions(identifier);
    }

    private _disableExtensionInWorkspace(identifier: IExtensionIdentifier): cognidream {
    ...
    private _setEnabledExtensions(enabledExtensions: IExtensionIdentifier[]): cognidream {
    ...
    private _setDisabledExtensions(disabledExtensions: IExtensionIdentifier[]): cognidream {
    ...
    private _setExtensions(storageId: string, extensions: IExtensionIdentifier[]): cognidream {
        this.storageManager.set(storageId, extensions, StorageScope.WORKSPACE);
    }

    private async _onDidChangeGloballyDisabledExtensions(extensionIdentifiers: ReadonlyArray<IExtensionIdentifier>, source?: string): Promise<cognidream> {
        if (source !== SOURCE) {
            await this.extensionsManager.whenInitialized();
    ...
    private _onDidChangeExtensions(added: ReadonlyArray<IExtension>, removed: ReadonlyArray<IExtension>, isProfileSwitch: boolean): cognidream {
    ...
    public async updateExtensionsEnablementsWhenWorkspaceTrustChanges(): Promise<cognidream> {
        await this.extensionsManager.whenInitialized();
    ...
    private loopCheckForMaliciousExtensions(): cognidream {
        this.checkForMaliciousExtensions()
            .then(() => this.delayer.trigger(() => { }, 1000 * 60 * 5)) // every five minutes
            .then(() => this.loopCheckForMaliciousExtensions());
    }

    private async checkForMaliciousExtensions(): Promise<cognidream> {
        try {
...
class ExtensionsManager extends Disposable {
    ...
    whenInitialized(): Promise<cognidream> {
    ...
    private updateExtensions(added: IExtension[], identifiers: IExtensionIdentifier[], server: IExtensionManagementServer | undefined, isProfileSwitch: boolean): cognidream {
        if (added.length) {
