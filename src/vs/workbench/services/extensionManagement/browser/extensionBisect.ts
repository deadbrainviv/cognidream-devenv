...
export interface IExtensionBisectService {
    ...
    start(extensions: ILocalExtension[]): Promise<cognidream>;
    next(seeingBad: boolean): Promise<{ id: string; bad: boolean } | undefined>;
    reset(): Promise<cognidream>;
}
...
class ExtensionBisectService implements IExtensionBisectService {
    ...
    async start(extensions: ILocalExtension[]): Promise<cognidream> {
        if (this._state) {
    ...
    async reset(): Promise<cognidream> {
...
class ExtensionBisectUi {
    ...
    private _showBisectPrompt(): cognidream {
...
registerAction2(class extends Action2 {
    ...
    async run(accessor: ServicesAccessor, seeingBad: boolean | undefined): Promise<cognidream> {
        const dialogService = accessor.get(IDialogService);
        const hostService = accessor.get(IHostService);
