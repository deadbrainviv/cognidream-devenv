...
export class WebExtensionsScannerService extends Disposable implements IWebExtensionsScannerService {
    ...
    private _migrateExtensionsStoragePromise: Promise<cognidream> | undefined;
    private async migrateExtensionsStorage(customBuiltinExtensions: IExtension[]): Promise<cognidream> {
    ...
    private async updateCaches(): Promise<cognidream> {
        await this.updateSystemExtensionsCache();
        await this.updateCustomBuiltinExtensionsCache();
    }

    private async updateSystemExtensionsCache(): Promise<cognidream> {
        const systemExtensions = await this.builtinExtensionsScannerService.scanBuiltinExtensions();
    ...
    private async resolveDependenciesAndPackedExtensions(webExtensions: IWebExtension[], result: Map<string, IWebExtension>): Promise<cognidream> {
        const extensionInfos: IExtensionInfo[] = [];
    ...
    async removeExtension(extension: IScannedExtension, profileLocation: URI): Promise<cognidream> {
    ...
    async copyExtensions(fromProfileLocation: URI, toProfileLocation: URI, filter: (extension: IScannedExtension) => boolean): Promise<cognidream> {
    ...
    private async addWebExtension(webExtension: IWebExtension, profileLocation: URI): Promise<IScannedExtension> {
        ...
        if (isSystem) {
            await this.writeSystemExtensionsCache(systemExtensions => {
                // Remove the existing extension to acognidream duplicates
    ...
    private async addToInstalledExtensions(webExtensions: IWebExtension[], profileLocation: URI): Promise<cognidream> {
        await this.writeInstalledExtensions(profileLocation, installedExtensions => {
            // Remove the existing extension to acognidream duplicates
    ...
    private async storeWebExtensions(webExtensions: IWebExtension[], file: URI): Promise<cognidream> {
...
if (isWeb) {
    registerAction2(class extends Action2 {
        ...
        run(serviceAccessor: ServicesAccessor): cognidream {
