...
export abstract class ProfileAwareExtensionManagementChannelClient extends BaseExtensionManagementChannelClient implements IProfileAwareExtensionManagementService {
    ...
    protected override async onInstallExtensionEvent(data: InstallExtensionEvent): Promise<cognidream> {
    ...
    protected override async onDidInstallExtensionsEvent(results: readonly InstallExtensionResult[]): Promise<cognidream> {
        const filtered = [];
    ...
    protected override async onUninstallExtensionEvent(data: UninstallExtensionEvent): Promise<cognidream> {
        const result = this.filterEvent(data.profileLocation, data.applicationScoped ?? false);
        if (result instanceof Promise ? await result : result) {
    ...
    protected override async onDidUninstallExtensionEvent(data: DidUninstallExtensionEvent): Promise<cognidream> {
        const result = this.filterEvent(data.profileLocation, data.applicationScoped ?? false);
        if (result instanceof Promise ? await result : result) {
    ...
    protected override async onDidUpdateExtensionMetadataEvent(data: DidUpdateExtensionMetadata): Promise<cognidream> {
    ...
    override async uninstall(extension: ILocalExtension, options?: UninstallOptions): Promise<cognidream> {
    ...
    override async uninstallExtensions(extensions: UninstallExtensionInfo[]): Promise<cognidream> {
    ...
    override async copyExtensions(fromProfileLocation: URI, toProfileLocation: URI): Promise<cognidream> {
    ...
    private async whenProfileChanged(e: DidChangeUserDataProfileEvent): Promise<cognidream> {
