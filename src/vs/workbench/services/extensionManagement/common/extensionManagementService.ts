...
export class ExtensionManagementService extends Disposable implements IWorkbenchExtensionManagementService {
    ...
    uninstall(extension: ILocalExtension, options: UninstallOptions): Promise<cognidream> {
        return this.uninstallExtensions([{ extension, options }]);
    }

    async uninstallExtensions(extensions: UninstallExtensionInfo[]): Promise<cognidream> {
    ...
    private async uninstallInServer(server: IExtensionManagementServer, extensions: UninstallExtensionInfo[]): Promise<cognidream> {
    ...
    async resetPinnedStateForAllUserExtensions(pinned: boolean): Promise<cognidream> {
    ...
    private async uninstallExtensionFromWorkspace(extension: ILocalExtension): Promise<cognidream> {
        if (!extension.isWorkspaceScoped) {
    ...
    async requestPublisherTrust(extensions: InstallExtensionInfo[]): Promise<cognidream> {
        const manifests = await Promise.all(extensions.map(async ({ extension }) => {
            const manifest = await this.extensionGalleryService.getManifest(extension, CancellationToken.None);
            if (!manifest) {
                throw new Error(localize('Manifest is not found', "Installing Extension {0} failed: Manifest is not found.", extension.displayName || extension.name));
    ...
    private async checkForTrustedPublishers(extensions: { extension: IGalleryExtension; manifest: IExtensionManifest; checkForPackAndDependencies: boolean }[]): Promise<cognidream> {
        ...
        const installButton: IPromptButton<cognidream> = {
        ...
        const learnMoreButton: IPromptButton<cognidream> = {
    ...
    private async getDependenciesAndPackedExtensionsRecursively(toGet: string[], result: Map<string, IGalleryExtension>, token: CancellationToken): Promise<cognidream> {
        if (toGet.length === 0) {
            return;
        }
    ...
    private async checkForWorkspaceTrust(manifest: IExtensionManifest, requireTrust: boolean): Promise<cognidream> {
    ...
    private async checkInstallingExtensionOnWeb(extension: IGalleryExtension, manifest: IExtensionManifest): Promise<cognidream> {
        ...
        let message: string;
        let buttons: IPromptButton<cognidream>[] = [];
        let detail: string | undefined;

        const installAnywayButton: IPromptButton<cognidream> = {
            label: localize({ key: 'install anyways', comment: ['&& denotes a mnemonic'] }, "&&Install Anyway"),
            run: () => { }
        };

        const showExtensionsButton: IPromptButton<cognidream> = {
    ...
    async cleanUp(): Promise<cognidream> {
    ...
    copyExtensions(from: URI, to: URI): Promise<cognidream> {
        if (this.extensionManagementServerService.remoteExtensionManagementServer) {
            throw new Error('Not Supported');
        }
        if (this.extensionManagementServerService.localExtensionManagementServer) {
    ...
    trustPublishers(...publishers: IPublisherInfo[]): cognidream {
        const trustedPublishers = this.getTrustedPublishersFromStorage();
    ...
    untrustPublishers(...publishers: string[]): cognidream {
        const trustedPublishers = this.getTrustedPublishersFromStorage();
        for (const publisher of publishers) {
...
class WorkspaceExtensionsManagementService extends Disposable {
    ...
    private readonly initializePromise: Promise<cognidream>;
    ...
    private async initialize(): Promise<cognidream> {
    ...
    private watchInvalidExtensions(): cognidream {
    ...
    private async checkExtensionsValidity(extensions: ILocalExtension[]): Promise<cognidream> {
    ...
    async uninstall(extension: ILocalExtension): Promise<cognidream> {
        await this.initializePromise;
    ...
    private saveWorkspaceExtensions(): cognidream {
