...
class ExtensionFeaturesManagementService extends Disposable implements IExtensionFeaturesManagementService {
    ...
    setEnablement(extension: ExtensionIdentifier, featureId: string, enabled: boolean): cognidream {
        const feature = this.registry.getExtensionFeature(featureId);
        if (!feature) {
    ...
    setStatus(extension: ExtensionIdentifier, featureId: string, status: { readonly severity: Severity; readonly message: string } | undefined): cognidream {
        const feature = this.registry.getExtensionFeature(featureId);
        if (!feature) {
            throw new Error(`No feature with id '${featureId}'`);
        }
    ...
    private onDidStorageChange(e: IStorageChangeEvent): cognidream {
        if (e.external) {
    ...
    private garbageCollectOldRequests(): cognidream {
        const now = new Date();
