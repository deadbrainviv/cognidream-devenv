...
class WorkbenchHostService extends Disposable implements IHostService {
    ...
    private doOpenWindow(toOpen: IWindowOpenable[], options?: IOpenWindowOptions): Promise<cognidream> {
    ...
    private doOpenEmptyWindow(options?: IOpenEmptyWindowOptions): Promise<cognidream> {
        const remoteAuthority = this.environmentService.remoteAuthority;
    ...
    toggleFullScreen(targetWindow: Window): Promise<cognidream> {
    ...
    focus(targetWindow: Window, options?: { force: boolean }): Promise<cognidream> {
