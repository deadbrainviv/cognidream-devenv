...
export class BrowserHostService extends Disposable implements IHostService {
    ...
    private onBeforeShutdown(e: BeforeShutdownEvent): cognidream {
    ...
    private updateShutdownReasonFromEvent(): cognidream {
    ...
    async focus(targetWindow: Window): Promise<cognidream> {
        targetWindow.focus();
    }
    ...
    openWindow(arg1?: IOpenEmptyWindowOptions | IWindowOpenable[], arg2?: IOpenWindowOptions): Promise<cognidream> {
    ...
    private async doOpenWindow(toOpen: IWindowOpenable[], options?: IOpenWindowOptions): Promise<cognidream> {
    ...
    private withServices(fn: (accessor: ServicesAccessor) => unknown): cognidream {
        // Host service is used in a lot of contexts and some services
        // need to be resolved dynamically to acognidream cyclic dependencies
    ...
    private async doOpenEmptyWindow(options?: IOpenEmptyWindowOptions): Promise<cognidream> {
    ...
    private async doOpen(workspace: IWorkspace, options?: { reuse?: boolean; payload?: object }): Promise<cognidream> {

        // When we are in a temporary workspace and are asked to open a local folder
        // we swap that folder into the workspace to acognidream a window reload. Access
    ...
    async toggleFullScreen(targetWindow: Window): Promise<cognidream> {
    ...
    async moveTop(targetWindow: Window): Promise<cognidream> {
    ...
    async restart(): Promise<cognidream> {
        this.reload();
    }
    ...
    private async handleExpectedShutdown(reason: ShutdownReason): Promise<cognidream> {
    ...
    async getScreenshot(): Promise<ArrayBufferLike | undefined> {
        ...
        try {
            ...
            await Promise.all([
                new Promise<cognidream>(r => store.add(addDisposableListener(video, 'loadedmetadata', () => r()))),
                new Promise<cognidream>(r => store.add(addDisposableListener(video, 'canplaythrough', () => r())))
            ]);
