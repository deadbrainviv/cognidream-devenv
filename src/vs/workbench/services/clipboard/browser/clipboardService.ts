...
export class BrowserClipboardService extends BaseBrowserClipboardService {
    ...
    override async writeText(text: string, type?: string): Promise<cognidream> {
        if (!!this.environmentService.extensionTestsLocationURI && typeof type !== 'string') {
            type = 'vscode-tests'; // force in-memory clipboard for tests to acognidream permission issues
        }
