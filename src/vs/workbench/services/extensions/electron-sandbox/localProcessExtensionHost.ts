...
export class ExtensionHostProcess {
    ...
    public kill(): Promise<cognidream> {
...
export class NativeLocalProcessExtensionHost implements IExtensionHost {
    ...
    private readonly _onDidSetInspectPort = new Emitter<cognidream>();
    ...
    private _performHandshake(protocol: IMessagePassingProtocol): Promise<cognidream> {
    ...
    private _onExtHostProcessExit(code: number, signal: string): cognidream {
        if (this._terminating) {
    ...
    private _onWillShutdown(event: WillShutdownEvent): cognidream {
