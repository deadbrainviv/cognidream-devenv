...
export class NativeExtensionService extends AbstractExtensionService implements IExtensionService {
    ...
    protected override _onExtensionHostCrashed(extensionHost: IExtensionHostManager, code: number, signal: string | null): cognidream {
    ...
    private _sendExtensionHostCrashTelemetry(code: number, signal: string | null, activatedExtensions: ExtensionIdentifier[]): cognidream {
    ...
    private async _doResolveExtensions(emitter: AsyncIterableEmitter<ResolvedExtensions>): Promise<cognidream> {
    ...
    private async _startLocalExtensionHost(emitter: AsyncIterableEmitter<ResolvedExtensions>, remoteExtensions: IExtensionDescription[] = []): Promise<cognidream> {
    ...
    protected async _onExtensionHostExit(code: number): Promise<cognidream> {
        // Dispose everything associated with the extension host
        await this._doStopExtensionHosts();

        // Dispose the management connection to acognidream reconnecting after the extension host exits
        const connection = this._remoteAgentService.getConnection();
        connection?.dispose();
