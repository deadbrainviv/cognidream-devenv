...
export interface IInternalExtHostContext extends IExtHostContext {
    ...
    _setExtensionHostProxy(extensionHostProxy: IExtensionHostProxy): cognidream;
    _setAllMainProxyIdentifiers(mainProxyIdentifiers: ProxyIdentifier<any>[]): cognidream;
}
...
export function extHostNamedCustomer<T extends IDisposable>(id: ProxyIdentifier<T>) {
    return function <Services extends BrandedService[]>(ctor: { new(context: IExtHostContext, ...services: Services): T }): cognidream {
        ExtHostCustomersRegistryImpl.INSTANCE.registerNamedCustomer(id, ctor as IExtHostCustomerCtor<T>);
    };
}

export function extHostCustomer<T extends IDisposable, Services extends BrandedService[]>(ctor: { new(context: IExtHostContext, ...services: Services): T }): cognidream {
...
class ExtHostCustomersRegistryImpl {
    ...
    public registerNamedCustomer<T extends IDisposable>(id: ProxyIdentifier<T>, ctor: IExtHostCustomerCtor<T>): cognidream {
    ...
    public registerCustomer<T extends IDisposable>(ctor: IExtHostCustomerCtor<T>): cognidream {
