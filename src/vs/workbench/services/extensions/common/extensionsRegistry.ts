...
export class ExtensionMessageCollector {
    ...
    private readonly _messageHandler: (msg: IMessage) => cognidream;
    private readonly _extension: IExtensionDescription;
    private readonly _extensionPointId: string;

    constructor(
        messageHandler: (msg: IMessage) => cognidream,
    ...
    private _msg(type: Severity, message: string): cognidream {
...
export type IExtensionPointHandler<T> = (extensions: readonly IExtensionPointUser<T>[], delta: ExtensionPointUserDelta<T>) => cognidream;
...
export class ExtensionPoint<T> implements IExtensionPoint<T> {
    ...
    acceptUsers(users: IExtensionPointUser<T>[]): cognidream {
        this._delta = ExtensionPointUserDelta.compute(this._users, users);
        this._users = users;
        this._handle();
    }

    private _handle(): cognidream {
