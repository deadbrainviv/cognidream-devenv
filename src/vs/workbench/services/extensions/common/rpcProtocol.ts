...
export class RPCProtocol extends Disposable implements IRPCProtocol {
    ...
    private readonly _cancelInvokedHandlers: { [req: string]: () => cognidream };
    ...
    private _onWillSendRequest(req: number): cognidream {
    ...
    private _onDidReceiveAcknowledge(req: number): cognidream {
    ...
    private _checkUnresponsive(): cognidream {
        if (this._unacknowledgedCount === 0) {
    ...
    private _setResponsiveState(newResponsiveState: ResponsiveState): cognidream {
    ...
    public assertRegistered(identifiers: ProxyIdentifier<any>[]): cognidream {
    ...
    private _receiveOneMessage(rawmsg: VSBuffer): cognidream {
        if (this._isDisposed) {
            return;
        }
    ...
    private _receiveRequest(msgLength: number, req: number, rpcId: number, method: string, args: any[], usesCancellationToken: boolean): cognidream {
    ...
    private _receiveCancel(msgLength: number, req: number): cognidream {
        this._logger?.logIncoming(msgLength, req, RequestInitiator.OtherSide, `receiveCancel`);
        const callId = String(req);
        this._cancelInvokedHandlers[callId]?.();
    }

    private _receiveReply(msgLength: number, req: number, value: any): cognidream {
    ...
    private _receiveReplyErr(msgLength: number, req: number, value: any): cognidream {
...
class PendingRPCReply {
    ...
    public resolveOk(value: any): cognidream {
        this._promise.resolveOk(value);
        this._disposable.dispose();
    }

    public resolveErr(err: any): cognidream {
...
class MessageBuffer {
    ...
    public writeUInt8(n: number): cognidream {
    ...
    public writeUInt32(n: number): cognidream {
    ...
    public writeShortString(str: VSBuffer): cognidream {
    ...
    public writeLongString(str: VSBuffer): cognidream {
    ...
    public writeBuffer(buff: VSBuffer): cognidream {
    ...
    public writeVSBuffer(buff: VSBuffer): cognidream {
        this._buff.writeUInt32BE(buff.byteLength, this._offset); this._offset += 4;
        this._buff.set(buff, this._offset); this._offset += buff.byteLength;
    }
    ...
    public writeMixedArray(arr: readonly MixedArg[]): cognidream {
