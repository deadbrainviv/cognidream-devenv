...
export class LazyCreateExtensionHostManager extends Disposable implements IExtensionHostManager {
    ...
    public async deltaExtensions(extensionsDelta: IExtensionDescriptionDelta): Promise<cognidream> {
        await this._startCalled.wait();
        if (this._actual) {
    ...
    public async activateByEvent(activationEvent: string, activationKind: ActivationKind): Promise<cognidream> {
