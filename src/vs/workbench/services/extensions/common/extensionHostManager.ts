...
export class ExtensionHostManager extends Disposable implements IExtensionHostManager {
     ...
     */
    private readonly _cachedActivationEvents: Map<string, Promise<cognidream>>;
    ...
    constructor(
        ...
        super();
        this._cachedActivationEvents = new Map<string, Promise<cognidream>>();
    ...
    public async disconnect(): Promise<cognidream> {
    ...
    public async ready(): Promise<cognidream> {
        await this._proxy;
    }
    ...
    private _createExtensionHostCustomers(kind: ExtensionHostKind, protocol: IMessagePassingProtocol): IExtensionHostProxy {
        ...
        const extHostContext: IInternalExtHostContext = {
            ...
            dispose: (): cognidream => this._rpcProtocol!.dispose(),
            assertRegistered: (identifiers: ProxyIdentifier<any>[]): cognidream => this._rpcProtocol!.assertRegistered(identifiers),
            drain: (): Promise<cognidream> => this._rpcProtocol!.drain(),

            //#region internal
            internalExtensionService: this._internalExtensionService,
            _setExtensionHostProxy: (value: IExtensionHostProxy): cognidream => {
                extensionHostProxy = value;
            },
            _setAllMainProxyIdentifiers: (value: ProxyIdentifier<any>[]): cognidream => {
    ...
    public activateByEvent(activationEvent: string, activationKind: ActivationKind): Promise<cognidream> {
    ...
    private async _activateByEvent(activationEvent: string, activationKind: ActivationKind): Promise<cognidream> {
        if (!this._proxy) {
            return;
        }
        const proxy = await this._proxy;
    ...
    public async start(extensionRegistryVersionId: number, allExtensions: IExtensionDescription[], myExtensions: ExtensionIdentifier[]): Promise<cognidream> {
        const proxy = await this._proxy;
        if (!proxy) {
            return;
        }
    ...
    public async deltaExtensions(incomingExtensionsDelta: IExtensionDescriptionDelta): Promise<cognidream> {
        const proxy = await this._proxy;
        if (!proxy) {
            return;
        }
...
class RPCLogger implements IRPCProtocolLogger {
    ...
    private _log(direction: string, totalLength: number, msgLength: number, req: number, initiator: RequestInitiator, str: string, data: any): cognidream {
        data = pretty(data);
    ...
    logIncoming(msgLength: number, req: number, initiator: RequestInitiator, str: string, data?: any): cognidream {
        this._totalIncoming += msgLength;
        this._log('Ext \u2192 Win', this._totalIncoming, msgLength, req, initiator, str, data);
    }

    logOutgoing(msgLength: number, req: number, initiator: RequestInitiator, str: string, data?: any): cognidream {
...
class TelemetryRPCLogger implements IRPCProtocolLogger {
    ...
    logIncoming(msgLength: number, req: number, initiator: RequestInitiator, str: string): cognidream {
    ...
    logOutgoing(msgLength: number, req: number, initiator: RequestInitiator, str: string): cognidream {
