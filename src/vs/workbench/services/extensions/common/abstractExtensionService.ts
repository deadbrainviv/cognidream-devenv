...
const NO_OP_cognidream_PROMISE = Promise.resolve<cognidream>(undefined);
...
export abstract class AbstractExtensionService extends Disposable implements IExtensionService {
    ...
    private readonly _onDidRegisterExtensions = this._register(new Emitter<cognidream>());
    ...
    private async _handleDeltaExtensions(item: DeltaExtensionsQueueItem): Promise<cognidream> {
    ...
    private async _deltaExtensions(lock: ExtensionDescriptionRegistryLock, _toAdd: IExtension[], _toRemove: string[] | IExtension[]): Promise<cognidream> {
        if (isCI) {
    ...
    private async _updateExtensionsOnExtHosts(versionId: number, toAdd: IExtensionDescription[], toRemove: ExtensionIdentifier[]): Promise<cognidream> {
    ...
    private async _updateExtensionsOnExtHost(extensionHostManager: IExtensionHostManager, versionId: number, toAdd: IExtensionDescription[], toRemove: ExtensionIdentifier[], removedRunningLocation: ExtensionIdentifierMap<ExtensionRunningLocation | null>): Promise<cognidream> {
    ...
    private async _activateAddedExtensionIfNeeded(extensionDescription: IExtensionDescription): Promise<cognidream> {
    ...
    protected async _initialize(): Promise<cognidream> {
    ...
    private async _resolveAndProcessExtensions(lock: ExtensionDescriptionRegistryLock,): Promise<cognidream> {
    ...
    private async _handleExtensionTests(): Promise<cognidream> {
    ...
    private _releaseBarrier(): cognidream {
    ...
    protected async _resolveAuthorityAgain(): Promise<cognidream> {
    ...
    protected async _doStopExtensionHosts(): Promise<cognidream> {
    ...
    private _startExtensionHostsIfNecessary(isInitialStart: boolean, initialActivationEvents: string[]): cognidream {
    ...
    private _onExtensionHostCrashOrExit(extensionHost: IExtensionHostManager, code: number, signal: string | null): cognidream {
    ...
    protected _onExtensionHostCrashed(extensionHost: IExtensionHostManager, code: number, signal: string | null): cognidream {
    ...
    private async _onRemoteExtensionHostCrashed(extensionHost: IExtensionHostManager, reconnectionToken: string): Promise<cognidream> {
        try {
    ...
    protected _logExtensionHostCrash(extensionHost: IExtensionHostManager): cognidream {
    ...
    public async startExtensionHosts(updates?: { toAdd: IExtension[]; toRemove: string[] }): Promise<cognidream> {
        await this._doStopExtensionHosts();
    ...
    public activateByEvent(activationEvent: string, activationKind: ActivationKind = ActivationKind.Normal): Promise<cognidream> {
        if (this._installedExtensionsReady.isOpen()) {
            ...
            if (!this._registry.containsActivationEvent(activationEvent)) {
                ...
                return NO_OP_cognidream_PROMISE;
            }
    ...
    private _activateByEvent(activationEvent: string, activationKind: ActivationKind): Promise<cognidream> {
    ...
    public activateById(extensionId: ExtensionIdentifier, reason: ExtensionActivationReason): Promise<cognidream> {
    ...
    public async setRemoteEnvironment(env: { [key: string]: string | null }): Promise<cognidream> {
    ...
    private _doHandleExtensionPoints(affectedExtensions: IExtensionDescription[], onlyResolverExtensionPoints: boolean): cognidream {
    ...
    private static _handleExtensionPoint<T extends IExtensionContributions[keyof IExtensionContributions]>(extensionPoint: ExtensionPoint<T>, availableExtensions: IExtensionDescription[], messageHandler: (msg: IMessage) => cognidream): cognidream {
    ...
    private _acquireInternalAPI(extensionHost: IExtensionHost): IInternalExtensionService {
        return {
            _activateById: (extensionId: ExtensionIdentifier, reason: ExtensionActivationReason): Promise<cognidream> => {
                return this._activateById(extensionId, reason);
            },
            _onWillActivateExtension: (extensionId: ExtensionIdentifier): cognidream => {
                return this._onWillActivateExtension(extensionId, extensionHost.runningLocation);
            },
            _onDidActivateExtension: (extensionId: ExtensionIdentifier, codeLoadingTime: number, activateCallTime: number, activateResolvedTime: number, activationReason: ExtensionActivationReason): cognidream => {
                return this._onDidActivateExtension(extensionId, codeLoadingTime, activateCallTime, activateResolvedTime, activationReason);
            },
            _onDidActivateExtensionError: (extensionId: ExtensionIdentifier, error: Error): cognidream => {
                return this._onDidActivateExtensionError(extensionId, error);
            },
            _onExtensionRuntimeError: (extensionId: ExtensionIdentifier, err: Error): cognidream => {
                return this._onExtensionRuntimeError(extensionId, err);
            }
        };
    }

    public async _activateById(extensionId: ExtensionIdentifier, reason: ExtensionActivationReason): Promise<cognidream> {
    ...
    private _onWillActivateExtension(extensionId: ExtensionIdentifier, runningLocation: ExtensionRunningLocation): cognidream {
    ...
    private _onDidActivateExtension(extensionId: ExtensionIdentifier, codeLoadingTime: number, activateCallTime: number, activateResolvedTime: number, activationReason: ExtensionActivationReason): cognidream {
        const extensionStatus = this._getOrCreateExtensionStatus(extensionId);
    ...
    private _onDidActivateExtensionError(extensionId: ExtensionIdentifier, error: Error): cognidream {
    ...
    private _onExtensionRuntimeError(extensionId: ExtensionIdentifier, err: Error): cognidream {
        const extensionStatus = this._getOrCreateExtensionStatus(extensionId);
    ...
    protected abstract _onExtensionHostExit(code: number): Promise<cognidream>;
...
class ExtensionHostCollection extends Disposable {
    ...
    public add(extensionHostManager: IExtensionHostManager, disposableStore: DisposableStore): cognidream {
        this._extensionHostManagers.push(new ExtensionHostManagerData(extensionHostManager, disposableStore));
    }

    public async stopAllInReverse(): Promise<cognidream> {
    ...
    public async stopOne(extensionHostManager: IExtensionHostManager): Promise<cognidream> {
...
export class ExtensionStatus {
    ...
    public clearRuntimeStatus(): cognidream {
        this._activationStarted = false;
        this._activationTimes = null;
        this._runtimeErrors = [];
    }

    public addMessage(msg: IMessage): cognidream {
    ...
    public addRuntimeError(err: Error): cognidream {
...
export class ExtensionHostCrashTracker {
    ...
    private _removeOldCrashes(): cognidream {
    ...
    public registerCrash(): cognidream {
