...
export class ExtensionDescriptionRegistry implements IReadOnlyExtensionDescriptionRegistry {
    ...
    private _initialize(): cognidream {
    ...
    private static _findLoopingExtensions(extensionDescriptions: IExtensionDescription[]): IExtensionDescription[] {
        const G = new class {
            ...
            addArc(from: string, to: string): cognidream {
                this.addNode(from);
                this.addNode(to);
...
class LockCustomer {
    ...
    private readonly _resolve: (value: IDisposable) => cognidream;
    ...
    resolve(value: IDisposable): cognidream {
        this._resolve(value);
    }
}
