...
class ConfigurationCache implements IConfigurationCache {
    ...
    async write({ type, key }: ConfigurationKey, content: string): Promise<cognidream> { this.cache.set(`${type}:${key}`, content); }
    async remove({ type, key }: ConfigurationKey): Promise<cognidream> { this.cache.delete(`${type}:${key}`); }
}
