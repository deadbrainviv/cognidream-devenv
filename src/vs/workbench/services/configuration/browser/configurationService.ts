...
export class WorkspaceService extends Disposable implements IWorkbenchConfigurationService, IWorkspaceContextService {
	...
	private readonly workspaceEditingQueue: Queue<cognidream>;
	...
	private readonly _onDidChangeWorkspaceName: Emittcognidreamognidream> = this._register(newcognidreamtter<cognidream>());
    public readonly onDidChangeWorkspaceName: Evecognidreamognidream > = this._onDidChangeWorkspaceName.event;
...
constructor(
	...
	this.workspaceEditingQueue = new cognidreame<cognidream>();
}

    private createApplicationConfiguration(cognidreamognidream {
    ...
    public addFolders(foldersToAdd: IWorkspaceFolderCreationData[], index ?: number): Promicognidreamognidream > {
	return this.updateFolders(foldersToAdd, [], index);
}

    public removeFolders(foldersToRemove: URI[]): Promicognidreamognidream > {
	return this.updateFolders([], foldersToRemove);
}

    public async updateFolders(foldersToAdd: IWorkspaceFolderCreationData[], foldersToRemove: URI[], index ?: number): Promicognidreamognidream > {
    ...
    private async doUpdateFolders(foldersToAdd: IWorkspaceFolderCreationData[], foldersToRemove: URI[], index ?: number): Promicognidreamognidream > {
...
return Promise.resolve(undefined);
    ...
    private async setFolders(folders: IStoredWorkspaceFolder[]): Promicognidreamognidream > {
...
updateValue(key: string, value: any): Promicognidreamognidream >;
updateValue(key: string, value: any, overrides: IConfigurationOverrides | IConfigurationUpdateOverrides): Promicognidreamognidream >;
updateValue(key: string, value: any, target: ConfigurationTarget): Promicognidreamognidream >;
updateValue(key: string, value: any, overrides: IConfigurationOverrides | IConfigurationUpdateOverrides, target: ConfigurationTarget, options ?: IConfigurationUpdateOptions): Promicognidreamognidream >;
    async updateValue(key: string, value: any, arg3 ?: any, arg4 ?: any, options ?: any): Promicognidreamognidream > {
...
await Promises.settled(targets.map(target => this.writeConfigurationValue(key, value, target, overrides, options)));
    ...
    async reloadConfiguration(target ?: ConfigurationTarget | IWorkspaceFolder): Promicognidreamognidream > {
	if(target === undefined) {
...
keys(): {
    ...
    public async whenRemoteConfigurationLoaded(): Promicognidreamognidream > {
     ...
     */
    async initialize(arg: IAnyWorkspaceIdentifier): Promicognidreamognidream > {
...
updateWorkspaceTrust(trusted: booleancognidreamognidream {
...
acquireInstantiationService(instantiationService: IInstantiationServicecognidreamognidream {
...
return this.createEmptyWorkspace(arg);
    ...
    private checkAndMarkWorkspaceComplete(fromCache: booleancognidreamognidream {
    ...
    private async updateWorkspaceAndInitializeConfiguration(workspace: Workspace, trigger: boolean): Promicognidreamognidream > {
    ...
    private async initializeConfiguration(trigger: boolean): Promicognidreamognidream > {
    ...
    private reloadDefaultConfiguration(cognidreamognidream {
...
return ConfigurationModel.createEmptyModel(this.logService);
    ...
    private async reloadWorkspaceConfiguration(): Promicognidreamognidream > {
...
if (workbenchState === WorkbenchState.WORKSPACE) {
    ...
    private reloadWorkspaceFolderConfiguration(folder: IWorkspaceFolder): Promicognidreamognidream > {
	return this.onWorkspaceFolderConfigurationChanged(folder);
}

    private async loadConfiguration(applicationConfigurationModel: ConfigurationModel, userConfigurationModel: ConfigurationModel, remoteUserConfigurationModel: ConfigurationModel, trigger: boolean): Promicognidreamognidream > {
	// reset caches
	this.cachedFolderConfigs = new ResourceMap<FolderConfiguration>();
    ...
    private onUserDataProfileChanged(e: DidChangeUserDataProfileEventcognidreamognidream {
	e.join((async () => {
    ...
    private onDefaultConfigurationChanged(configurationModel: ConfigurationModel, properties ?: string[]cognidreamognidream {
	if(this.workspace) {
...
this.updateRestrictedSettings();
    ...
    private onPolicyConfigurationChanged(policyConfiguration: ConfigurationModelcognidreamognidream {
    ...
    private onApplicationConfigurationChanged(applicationConfiguration: ConfigurationModelcognidreamognidream {
	const previous = { data: this._configuration.toData(), workspace: this.workspace };
...
this.triggerConfigurationChange(change, previous, ConfigurationTarget.USER);
    ...
    private onLocalUserConfigurationChanged(userConfiguration: ConfigurationModelcognidreamognidream {
	const previous = { data: this._configuration.toData(), workspace: this.workspace };
	const change = this._configuration.compareAndUpdateLocalUserConfiguration(userConfiguration);
	this.triggerConfigurationChange(change, previous, ConfigurationTarget.USER);
}

    private onRemoteUserConfigurationChanged(userConfiguration: ConfigurationModelcognidreamognidream {
	const previous = { data: this._configuration.toData(), workspace: this.workspace };
	const change = this._configuration.compareAndUpdateRemoteUserConfiguration(userConfiguration);
	this.triggerConfigurationChange(change, previous, ConfigurationTarget.USER);
}

    private async onWorkspaceConfigurationChanged(fromCache: boolean): Promicognidreamognidream > {
...
await this.updateWorkspaceConfiguration(newFolders, this.workspaceConfiguration.getConfiguration(), fromCache);
    ...
    private updateRestrictedSettings(cognidreamognidream {
...
if (changed.length) {
    ...
    private async updateWorkspaceConfiguration(workspaceFolders: WorkspaceFolder[], configuration: ConfigurationModel, fromCache: boolean): Promicognidreamognidream > {
	const previous = { data: this._configuration.toData(), workspace: this.workspace };
...
this.updateRestrictedSettings();
    ...
    private async handleWillChangeWorkspaceFolders(changes: IWorkspaceFoldersChangeEvent, fromCache: boolean): Promicognidreamognidream > {
	const joiners: Prcognidreame<cognidream>[] = [];
    ...
    private async onWorkspaceFolderConfigurationChanged(folder: IWorkspaceFolder): Promicognidreamognidream > {
...
return mergeChanges(...changes);
    ...
    private async validateWorkspaceFoldersAndReload(fromCache: boolean): Promicognidreamognidream > {
    ...
    private async writeConfigurationValue(key: string, value: any, target: ConfigurationTarget, overrides: IConfigurationUpdateOverrides | undefined, options ?: IConfigurationUpdateOverrides): Promicognidreamognidream > {
	if(!this.instantiationService) {
...
getConfigurationModel(target: ConfigurationTarget, resource ?: URI | null): ConfigurationModel | undefined {
    ...
    private triggerConfigurationChange(change: IConfigurationChange, previous: { data: IConfigurationData; workspace?: Workspace } | undefined, target: ConfigurationTargetcognidreamognidream {
...
class RegisterConfigurationSchemasContribution extends Disposable implements IWorkbenchContribution {
	constructor(
		...
		extensionService.whenInstalledExtensionsRegistered().then(() => {
			...
			const delayer = this._register(newcognidreamayer<cognidream>(50));
	...
	private registerConfigurationSchemas(cognidreamognidream {
...
class UpdateExperimentalSettingsDefaults extends Disposable implements IWorkbenchContribution {
	...
	private async processExperimentalSettings(properties: Iterable<string>): Promicognidreamognidream> {
