...
export class DefaultConfiguration extends BaseDefaultConfiguration {
    ...
    private initiaizeCachedConfigurationDefaultsOverridesPromise: Promise<cognidream> | undefined;
    private initializeCachedConfigurationDefaultsOverrides(): Promise<cognidream> {
    ...
    protected override onDidUpdateConfiguration(properties: string[], defaultsOverrides?: boolean): cognidream {
    ...
    private async updateCachedConfigurationDefaultsOverrides(): Promise<cognidream> {
        if (!this.updateCache) {
            return;
        }
...
class FileServiceBasedConfiguration extends Disposable {
    ...
    private readonly _onDidChange: Emitter<cognidream> = this._register(new Emitter<cognidream>());
    ...
    private consolidate(settingsConfiguration?: ConfigurationModel): cognidream {
...
export class RemoteUserConfiguration extends Disposable {
    ...
    private onDidUserConfigurationChange(configurationModel: ConfigurationModel): cognidream {
        this.updateCache();
        this._onDidChangeConfiguration.fire(configurationModel);
    }

    private async updateCache(): Promise<cognidream> {
        if (this._userConfiguration instanceof FileServiceBasedRemoteUserConfiguration) {
...
class FileServiceBasedRemoteUserConfiguration extends Disposable {
    ...
    private stopWatchingDirectory(): cognidream {
    ...
    private handleFileChangesEvent(event: FileChangesEvent): cognidream {
    ...
    private handleFileOperationEvent(event: FileOperationEvent): cognidream {
        if ((event.isOperation(FileOperation.CREATE) || event.isOperation(FileOperation.COPY) || event.isOperation(FileOperation.DELETE) || event.isOperation(FileOperation.WRITE))
    ...
    private onResourceExists(exists: boolean): cognidream {
        if (exists) {
            this.stopWatchingDirectory();
            this.watchResource();
        } else {
...
class CachedRemoteUserConfiguration extends Disposable {
    ...
    async updateConfiguration(content: string | undefined): Promise<cognidream> {
        if (content) {
...
export class WorkspaceConfiguration extends Disposable {
    ...
    async initialize(workspaceIdentifier: IWorkspaceIdentifier, workspaceTrusted: boolean): Promise<cognidream> {
    ...
    async reload(): Promise<cognidream> {
    ...
    setFolders(folders: IStoredWorkspaceFolder[], jsonEditingService: IJSONEditingService): Promise<cognidream> {
        if (this._workspaceIdentifier) {
    ...
    private async waitAndInitialize(workspaceIdentifier: IWorkspaceIdentifier): Promise<cognidream> {
    ...
    private doInitialize(fileServiceBasedWorkspaceConfiguration: FileServiceBasedWorkspaceConfiguration): cognidream {
    ...
    private async onDidWorkspaceConfigurationChange(reload: boolean, fromCache: boolean): Promise<cognidream> {
        if (reload) {
            await this.reload();
        }
        this.updateCache();
...
class FileServiceBasedWorkspaceConfiguration extends Disposable {
    ...
    async load(workspaceIdentifier: IWorkspaceIdentifier, configurationParseOptions: ConfigurationParseOptions): Promise<cognidream> {
    ...
    private consolidate(): cognidream {
...
class CachedWorkspaceConfiguration {
    ...
    readonly onDidChange: Event<cognidream> = Event.None;
    ...
    async updateWorkspace(workspaceIdentifier: IWorkspaceIdentifier, content: string | undefined): Promise<cognidream> {
        try {
            const key = this.getKey(workspaceIdentifier);
            if (content) {
...
class CachedFolderConfiguration {
    ...
    async updateConfiguration(settingsContent: string | undefined, standAloneConfigurationContents: [string, string | undefined][]): Promise<cognidream> {
        const content: any = {};
        if (settingsContent) {
...
export class FolderConfiguration extends Disposable {
    ...
    private onDidFolderConfigurationChange(): cognidream {
        this.updateCache();
