...
export class DiskFileSystemProvider extends AbstractDiskFileSystemProvider implements
    ...
    get onDidChangeCapabilities(): Event<cognidream> { return this.provider.onDidChangeCapabilities; }
    ...
    writeFile(resource: URI, content: Uint8Array, opts: IFileWriteOptions): Promise<cognidream> {
    ...
    close(fd: number): Promise<cognidream> {
    ...
    mkdir(resource: URI): Promise<cognidream> {
        return this.provider.mkdir(resource);
    }

    delete(resource: URI, opts: IFileDeleteOptions): Promise<cognidream> {
        return this.provider.delete(resource, opts);
    }

    rename(from: URI, to: URI, opts: IFileOverwriteOptions): Promise<cognidream> {
        return this.provider.rename(from, to, opts);
    }

    copy(from: URI, to: URI, opts: IFileOverwriteOptions): Promise<cognidream> {
    ...
    cloneFile(from: URI, to: URI): Promise<cognidream> {
    ...
    protected createUniversalWatcher(
        onChange: (changes: IFileChange[]) => cognidream,
        onLogMessage: (msg: ILogMessage) => cognidream,
    ...
    protected override logWatcherMessage(msg: ILogMessage): cognidream {
