...
export interface IWorkspaceExtensionsConfigService {
    ...
    onDidChangeExtensionsConfigs: Event<cognidream>;
    getExtensionsConfigs(): Promise<IExtensionsConfigContent[]>;
    getRecommendations(): Promise<string[]>;
    getUnwantedRecommendations(): Promise<string[]>;

    toggleRecommendation(extensionId: string): Promise<cognidream>;
    toggleUnwantedRecommendation(extensionId: string): Promise<cognidream>;
}
...
export class WorkspaceExtensionsConfigService extends Disposable implements IWorkspaceExtensionsConfigService {
    ...
    private readonly _onDidChangeExtensionsConfigs = this._register(new Emitter<cognidream>());
    ...
    async toggleRecommendation(extensionId: string): Promise<cognidream> {
    ...
    async toggleUnwantedRecommendation(extensionId: string): Promise<cognidream> {
        const workspace = this.workspaceContextService.getWorkspace();
        const workspaceExtensionsConfigContent = workspace.configuration ? await this.resolveWorkspaceExtensionConfig(workspace.configuration) : undefined;
        const workspaceFolderExtensionsConfigContents = new ResourceMap<IExtensionsConfigContent>();
        await Promise.all(workspace.folders.map(async workspaceFolder => {
    ...
    private async addOrRemoveWorkspaceFolderRecommendation(extensionId: string, workspaceFolder: IWorkspaceFolder, extensionsConfigContent: IExtensionsConfigContent, add: boolean): Promise<cognidream> {
    ...
    private async addOrRemoveWorkspaceRecommendation(extensionId: string, workspace: IWorkspace, extensionsConfigContent: IExtensionsConfigContent | undefined, add: boolean): Promise<cognidream> {
        const values: IJSONValue[] = [];
    ...
    private async addOrRemoveWorkspaceFolderUnwantedRecommendation(extensionId: string, workspaceFolder: IWorkspaceFolder, extensionsConfigContent: IExtensionsConfigContent, add: boolean): Promise<cognidream> {
        const values: IJSONValue[] = [];
        if (add) {
    ...
    private async addOrRemoveWorkspaceUnwantedRecommendation(extensionId: string, workspace: IWorkspace, extensionsConfigContent: IExtensionsConfigContent | undefined, add: boolean): Promise<cognidream> {
        const values: IJSONValue[] = [];
        if (extensionsConfigContent) {
            if (add) {
