...
export interface IEmbedderTerminalService {
    ...
    createTerminal(options: IEmbedderTerminalOptions): cognidream;
}
...
class EmbedderTerminalProcess extends Disposable implements ITerminalChildProcess {
    ...
    async processBinary(): Promise<cognidream> {
        // not supported
    }
    resize(): cognidream {
        // no-op
    ...
    acknowledgeDataEvent(): cognidream {
        // no-op, flow control not currently implemented
    }
    async setUnicodeVersion(): Promise<cognidream> {
        // no-op
    }
    ...
    updateProperty(property: ProcessPropertyType, value: any): Promise<cognidream> {
