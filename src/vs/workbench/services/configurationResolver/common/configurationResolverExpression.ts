...
interface IConfigurationResolverExpression<T> {
     ...
     */
    resolve(replacement: Replacement, data: string | IResolvedValue): cognidream;
...
export class ConfigurationResolverExpression<T> implements IConfigurationResolverExpression<T> {
    ...
    private parseObject(obj: any): cognidream {
    ...
    private parseString(object: any, propertyName: string | number, value: string): cognidream {
        let pos = 0;
    ...
    public resolve(replacement: Replacement, data: string | IResolvedValue): cognidream {
