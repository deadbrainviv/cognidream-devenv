...
export class UserDataSyncWorkbenchService extends Disposable implements IUserDataSyncWorkbenchService {
    ...
    private readonly _onDidTurnOnSync = this._register(new Emitter<cognidream>());
    ...
    private async waitAndInitialize(): Promise<cognidream> {
        try {
            /* wait */
    ...
    private async update(reason: string): Promise<cognidream> {
    ...
    private async updateCurrentAccount(): Promise<cognidream> {
    ...
    private async updateToken(current: UserDataSyncAccount | undefined): Promise<cognidream> {
    ...
    private traceOrInfo(msg: string, ...args: any[]): cognidream {
    ...
    private updateAccountStatus(accountStatus: AccountStatus): cognidream {
    ...
    async turnOn(): Promise<cognidream> {
    ...
    async turnoff(everywhere: boolean): Promise<cognidream> {
        if (this.userDataSyncEnablementService.isEnabled()) {
    ...
    async synchroniseUserDataSyncStoreType(): Promise<cognidream> {
    ...
    private async doTurnOnSync(token: CancellationToken): Promise<cognidream> {
    ...
    private async handleConflictsWhileTurningOn(token: CancellationToken): Promise<cognidream> {
    ...
    private async replace(local: boolean): Promise<cognidream> {
    ...
    async accept(resource: IUserDataSyncResource, conflictResource: URI, content: string | null | undefined, apply: boolean | { force: boolean }): Promise<cognidream> {
        return this.userDataSyncService.accept(resource, conflictResource, content, apply);
    }

    async showConflicts(conflictToOpen?: IResourcePreview): Promise<cognidream> {
    ...
    async resetSyncedData(): Promise<cognidream> {
        const { confirmed } = await this.dialogService.confirm({
            type: 'info',
    ...
    async showSyncActivity(): Promise<cognidream> {
    ...
    private async waitForActiveSyncViews(): Promise<cognidream> {
    ...
    async signIn(): Promise<cognidream> {
        const currentAuthenticationProviderId = this.currentAuthenticationProviderId;
    ...
    private async doSignIn(accountOrAuthProvider: UserDataSyncAccount | IAuthenticationProvider): Promise<cognidream> {
    ...
    private async onDidAuthFailure(): Promise<cognidream> {
        this.currentSessionId = undefined;
        await this.update('auth failure');
    }

    private onDidChangeSessions(e: AuthenticationSessionsChangeEvent): cognidream {
    ...
    private onDidChangeStorage(): cognidream {
