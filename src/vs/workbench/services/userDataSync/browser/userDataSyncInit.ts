...
export class UserDataSyncInitializer implements IUserDataInitializer {
    ...
    private async initializeUserDataSyncStore(authenticationSession: AuthenticationSessionInfo): Promise<cognidream> {
        const userDataSyncStore = this.userDataSyncStoreManagementService.userDataSyncStore;
    ...
    private async initializeExtensions(instantiationService: IInstantiationService): Promise<cognidream> {
        try {
    ...
    private initializeInstalledExtensionsPromise: Promise<cognidream> | undefined;
    ...
    private initializeNewExtensionsPromise: Promise<cognidream> | undefined;
    private async initializeNewExtensions(instantiationService: IInstantiationService): Promise<cognidream> {
    ...
    private async initialize(syncResources: SyncResource[]): Promise<cognidream> {
        const userDataSyncStoreClient = await this.createUserDataSyncStoreClient();
        if (!userDataSyncStoreClient) {
            return;
        }
...
class ExtensionsPreviewInitializer extends AbstractExtensionsInitializer {
    ...
    override initialize(): Promise<cognidream> {
        throw new Error('should not be called directly');
    }

    protected override async doInitialize(remoteUserData: IRemoteUserData): Promise<cognidream> {
