...
export class ProgressService extends Disposable implements IProgressService {
    ...
    async withProgress<R = unknown>(options: IProgressOptions, originalTask: (progress: IProgress<IProgressStep>) => Promise<R>, onDidCancel?: (choice?: number) => cognidream): Promise<R> {
    ...
    private withNotificationProgress<P extends Promise<R>, R = unknown>(options: IProgressNotificationOptions, callback: (progress: IProgress<IProgressStep>) => P, onDidCancel?: (choice?: number) => cognidream): P {
        ...
        const progressStateModel = new class extends Disposable {
            ...
            report(step: IProgressStep): cognidream {
                this._step = step;

                this._onDidReport.fire(step);
            }

            cancel(choice?: number): cognidream {
                onDidCancel?.(choice);
        ...
        const updateProgress = (notification: INotificationHandle, increment?: number): cognidream => {
        ...
        const updateNotification = (step?: IProgressStep): cognidream => {
    ...
    private showOnActivityBar<P extends Promise<R>, R = unknown>(viewletId: string, options: IProgressCompositeOptions, promise: P): cognidream {
    ...
    private withDialogProgress<P extends Promise<R>, R = unknown>(options: IProgressDialogOptions, task: (progress: IProgress<IProgressStep>) => P, onDidCancel?: (choice?: number) => cognidream): P {
        ...
        const updateDialog = function(message?: string): cognidream {
