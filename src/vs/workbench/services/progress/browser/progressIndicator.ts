...
export class EditorProgressIndicator extends Disposable implements IProgressIndicator {
    ...
    async showWhile(promise: Promise<unknown>, delay?: number): Promise<cognidream> {
    ...
    private async doShowWhile(promise: Promise<unknown>, delay?: number): Promise<cognidream> {
        try {
            this.progressBar.infinite().show(delay);
...
export interface IProgressScope {
     ...
     */
    readonly onDidChangeActive: Event<cognidream>;
...
export class ScopedProgressIndicator extends Disposable implements IProgressIndicator {
    ...
    private onDidScopeActivate(): cognidream {
    ...
    private onDidScopeDeactivate(): cognidream {
        this.progressBar.stop().hide();
    }
    ...
    private doShowWhile(delay?: number): cognidream {
...
export abstract class AbstractProgressScope extends Disposable implements IProgressScope {
    ...
    private readonly _onDidChangeActive = this._register(new Emitter<cognidream>());
