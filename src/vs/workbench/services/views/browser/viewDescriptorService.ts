...
export class ViewDescriptorService extends Disposable implements IViewDescriptorService {
    ...
    private migrateToViewsCustomizationsStorage(): cognidream {
    ...
    private registerGroupedViews(groupedViews: Map<string, IViewDescriptor[]>): cognidream {
    ...
    private deregisterGroupedViews(groupedViews: Map<string, IViewDescriptor[]>): cognidream {
    ...
    private moveOrphanViewsToDefaultLocation(): cognidream {
    ...
    whenExtensionsRegistered(): cognidream {
    ...
    private onDidRegisterViews(views: { views: IViewDescriptor[]; viewContainer: ViewContainer }[]): cognidream {
    ...
    private onDidDeregisterViews(views: IViewDescriptor[], viewContainer: ViewContainer): cognidream {
        // When views are registered, we need to regroup them based on the customizations
        const regroupedViews = this.regroupViews(viewContainer.id, views);
    ...
    moveViewContainerToLocation(viewContainer: ViewContainer, location: ViewContainerLocation, requestedIndex?: number, reason?: string): cognidream {
    ...
    setViewContainerBadgeEnablementState(id: string, badgesEnabled: boolean): cognidream {
        this.viewContainerBadgeEnablementStates.set(id, badgesEnabled);
        this.saveViewCustomizations();
    }

    moveViewToLocation(view: IViewDescriptor, location: ViewContainerLocation, reason?: string): cognidream {
    ...
    moveViewsToContainer(views: IViewDescriptor[], viewContainer: ViewContainer, visibilityState?: ViewVisibilityState, reason?: string): cognidream {
        if (!views.length) {
            return;
        }
    ...
    private onDidChangeDefaultContainer(views: IViewDescriptor[], from: ViewContainer, to: ViewContainer): cognidream {
    ...
    private reportMovedViews(views: IViewDescriptor[], from: ViewContainer, to: ViewContainer): cognidream {
    ...
    private moveViewsWithoutSaving(views: IViewDescriptor[], from: ViewContainer, to: ViewContainer, visibilityState: ViewVisibilityState = ViewVisibilityState.Expand): cognidream {
    ...
    private moveViewContainerToLocationWithoutSaving(viewContainer: ViewContainer, location: ViewContainerLocation, requestedIndex?: number): cognidream {
    ...
    private cleanUpGeneratedViewContainer(viewContainerId: string): cognidream {
    ...
    private onDidStorageChange(): cognidream {
    ...
    private onDidViewCustomizationsStorageChange(): cognidream {
    ...
    private saveViewCustomizations(): cognidream {
    ...
    private setStoredViewCustomizationsValue(value: string): cognidream {
    ...
    private onDidRegisterViewContainer(viewContainer: ViewContainer): cognidream {
    ...
    private onDidDeregisterViewContainer(viewContainer: ViewContainer): cognidream {
    ...
    private onDidChangeActiveViews({ added, removed }: { added: ReadonlyArray<IViewDescriptor>; removed: ReadonlyArray<IViewDescriptor> }): cognidream {
        this.contextKeyService.bufferChangeEvents(() => {
    ...
    private onDidChangeVisibleViews({ added, removed }: { added: IViewDescriptor[]; removed: IViewDescriptor[] }): cognidream {
        this.contextKeyService.bufferChangeEvents(() => {
    ...
    private registerViewsVisibilityActions(viewContainer: ViewContainer, { viewContainerModel, disposables }: { viewContainerModel: ViewContainerModel; disposables: DisposableStore }): cognidream {
        this.viewsVisibilityActionDisposables.deleteAndDispose(viewContainer);
        ...
        viewContainerModel.activeViewDescriptors.forEach((viewDescriptor, index) => {
            if (!viewDescriptor.remoteAuthority) {
                disposables.add(registerAction2(class extends ViewPaneContainerAction<ViewPaneContainer> {
                    ...
                    async runInViewPaneContainer(serviceAccessor: ServicesAccessor, viewPaneContainer: ViewPaneContainer): Promise<cognidream> {
    ...
    private addViews(container: ViewContainer, views: IViewDescriptor[], visibilityState: ViewVisibilityState = ViewVisibilityState.Default): cognidream {
        this.contextKeyService.bufferChangeEvents(() => {
            views.forEach(view => {
    ...
    private removeViews(container: ViewContainer, views: IViewDescriptor[]): cognidream {
