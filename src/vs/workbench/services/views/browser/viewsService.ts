...
export class ViewsService extends Disposable implements IViewsService {
    ...
    private readonly _onDidChangeFocusedView = this._register(new Emitter<cognidream>());
    ...
    private onDidChangeContainers(added: ReadonlyArray<{ container: ViewContainer; location: ViewContainerLocation }>, removed: ReadonlyArray<{ container: ViewContainer; location: ViewContainerLocation }>): cognidream {
    ...
    private onDidRegisterViewContainer(viewContainer: ViewContainer, viewContainerLocation: ViewContainerLocation): cognidream {
    ...
    private onDidDeregisterViewContainer(viewContainer: ViewContainer, viewContainerLocation: ViewContainerLocation): cognidream {
    ...
    private onDidChangeContainerLocation(viewContainer: ViewContainer, from: ViewContainerLocation, to: ViewContainerLocation): cognidream {
    ...
    private onViewDescriptorsAdded(views: ReadonlyArray<IViewDescriptor>, container: ViewContainer): cognidream {
    ...
    private onViewDescriptorsRemoved(views: ReadonlyArray<IViewDescriptor>): cognidream {
    ...
    private updateViewContainerEnablementContextKey(viewContainer: ViewContainer): cognidream {
    ...
    async closeViewContainer(id: string): Promise<cognidream> {
        const viewContainer = this.viewDescriptorService.getViewContainerById(id);
        if (viewContainer) {
            const viewContainerLocation = this.viewDescriptorService.getViewContainerLocation(viewContainer);
    ...
    closeView(id: string): cognidream {
        const viewContainer = this.viewDescriptorService.getViewContainerByViewId(id);
        if (viewContainer) {
            const activeViewPaneContainer = this.getActiveViewPaneContainer(viewContainer);
            if (activeViewPaneContainer) {
    ...
    private registerFocusViewAction(viewDescriptor: IViewDescriptor, category?: string | ILocalizedString): IDisposable {
        return registerAction2(class FocusViewAction extends Action2 {
            ...
            run(accessor: ServicesAccessor, options?: { preserveFocus?: boolean }): cognidream {
    ...
    private registerPaneComposite(viewContainer: ViewContainer, viewContainerLocation: ViewContainerLocation): cognidream {
        const that = this;
    ...
    private deregisterPaneComposite(viewContainer: ViewContainer, viewContainerLocation: ViewContainerLocation): cognidream {
