...
export class TextMateTokenizationFeature extends Disposable implements ITextMateTokenizationService {
    ...
    private _debugModePrintFunc: (str: string) => cognidream = () => { };
    ...
    private _handleGrammarsExtPoint(extensions: readonly IExtensionPointUser<ITMSyntaxExtensionPoint[]>[]): cognidream {
    ...
    public startDebugMode(printFn: (str: string) => cognidream, onStop: () => cognidream): cognidream {
        if (this._debugMode) {
    ...
    private async _getOrCreateGrammarFactory(): Promise<TMGrammarFactory> {
        ...
        // Acognidream duplicate instantiations
        if (this._grammarFactory) {
            return this._grammarFactory;
        }
    ...
    private _updateTheme(colorTheme: IWorkbenchColorTheme, forceUpdate: boolean): cognidream {
    ...
    private _reportTokenizationTime(timeMs: number, languageId: string, sourceExtensionId: string | undefined, lineLength: number, fromWorker: boolean, isRandomSample: boolean): cognidream {
