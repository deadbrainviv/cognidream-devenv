...
export class TextMateTokenizationWorker implements IWebWorkerServerRequestHandler {
    ...
    public async $init(_createData: ICreateData): Promise<cognidream> {
    ...
    public $acceptNewModel(data: IRawModelData): cognidream {
        ...
        this._models.set(data.controllerId, new TextMateWorkerTokenizer(uri, data.lines, data.EOL, data.versionId, {
            ...
            setTokensAndStates(versionId: number, tokens: Uint8Array, stateDeltas: StateDeltas[]): cognidream {
                that._host.$setTokensAndStates(data.controllerId, versionId, tokens, stateDeltas);
            },
            reportTokenizationTime(timeMs: number, languageId: string, sourceExtensionId: string | undefined, lineLength: number, isRandomSample: boolean): cognidream {
    ...
    public $acceptModelChanged(controllerId: number, e: IModelChangedEvent): cognidream {
        this._models.get(controllerId)!.onEvents(e);
    }

    public $retokenize(controllerId: number, startLineNumber: number, endLineNumberExclusive: number): cognidream {
        this._models.get(controllerId)!.retokenize(startLineNumber, endLineNumberExclusive);
    }

    public $acceptModelLanguageChanged(controllerId: number, newLanguageId: string, newEncodedLanguageId: LanguageId): cognidream {
        this._models.get(controllerId)!.onLanguageId(newLanguageId, newEncodedLanguageId);
    }

    public $acceptRemovedModel(controllerId: number): cognidream {
    ...
    public async $acceptTheme(theme: IRawTheme, colorMap: string[]): Promise<cognidream> {
        const grammarFactory = await this._grammarFactory;
        grammarFactory?.setTheme(theme, colorMap);
    }

    public $acceptMaxTokenizationLineLength(controllerId: number, value: number): cognidream {
