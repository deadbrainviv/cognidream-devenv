...
export abstract class AbstractWorkspaceEditingService extends Disposable implements IWorkspaceEditingService {
    ...
    async updateFolders(index: number, deleteCount?: number, foldersToAddCandidates?: IWorkspaceFolderCreationData[], donotNotifyError?: boolean): Promise<cognidream> {
    ...
    private async doUpdateFolders(foldersToAdd: IWorkspaceFolderCreationData[], foldersToDelete: URI[], index?: number, donotNotifyError: boolean = false): Promise<cognidream> {
        try {
    ...
    addFolders(foldersToAddCandidates: IWorkspaceFolderCreationData[], donotNotifyError: boolean = false): Promise<cognidream> {
    ...
    private async doAddFolders(foldersToAdd: IWorkspaceFolderCreationData[], index?: number, donotNotifyError: boolean = false): Promise<cognidream> {
    ...
    async removeFolders(foldersToRemove: URI[], donotNotifyError: boolean = false): Promise<cognidream> {
    ...
    async createAndEnterWorkspace(folders: IWorkspaceFolderCreationData[], path?: URI): Promise<cognidream> {
    ...
    async saveAndEnterWorkspace(workspaceUri: URI): Promise<cognidream> {
    ...
    protected async saveWorkspaceAs(workspace: IWorkspaceIdentifier, targetConfigPathURI: URI): Promise<cognidream> {
    ...
    protected async saveWorkspace(workspace: IWorkspaceIdentifier): Promise<cognidream> {
        const configPathURI = workspace.configPath;
    ...
    private handleWorkspaceConfigurationEditingError(error: JSONEditingError): cognidream {
        switch (error.code) {
    ...
    private onInvalidWorkspaceConfigurationFileError(): cognidream {
    ...
    private askToOpenWorkspaceConfigurationFile(message: string): cognidream {
    ...
    abstract enterWorkspace(workspaceUri: URI): Promise<cognidream>;
    ...
    private migrateWorkspaceSettings(toWorkspace: IWorkspaceIdentifier): Promise<cognidream> {
        return this.doCopyWorkspaceSettings(toWorkspace, setting => setting.scope === ConfigurationScope.WINDOW);
    }

    copyWorkspaceSettings(toWorkspace: IWorkspaceIdentifier): Promise<cognidream> {
        return this.doCopyWorkspaceSettings(toWorkspace);
    }

    private doCopyWorkspaceSettings(toWorkspace: IWorkspaceIdentifier, filter?: (config: IConfigurationPropertySchema) => boolean): Promise<cognidream> {
    ...
    private async trustWorkspaceConfiguration(configPathURI: URI): Promise<cognidream> {
