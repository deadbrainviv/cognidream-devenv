...
export class WorkspaceTrustManagementService extends Disposable implements IWorkspaceTrustManagementService {
    ...
    private readonly _workspaceResolvedPromise: Promise<cognidream>;
    private readonly _workspaceResolvedPromiseResolve: () => cognidream;
    private readonly _workspaceTrustInitializedPromise: Promise<cognidream>;
    private readonly _workspaceTrustInitializedPromiseResolve: () => cognidream;
    ...
    private readonly _onDidChangeTrustedFolders = this._register(new Emitter<cognidream>());
    ...
    private initializeWorkspaceTrust(): cognidream {
    ...
    private async resolveCanonicalUris(): Promise<cognidream> {
        // Open editors
    ...
    private async saveTrustInfo(): Promise<cognidream> {
    ...
    private async updateWorkspaceTrust(trusted?: boolean): Promise<cognidream> {
        if (!this.workspaceTrustEnablementService.isWorkspaceTrustEnabled()) {
            return;
        }
    ...
    private async doSetUrisTrust(uris: URI[], trusted: boolean): Promise<cognidream> {
        let changed = false;
    ...
    get workspaceResolved(): Promise<cognidream> {
        return this._workspaceResolvedPromise;
    }

    get workspaceTrustInitialized(): Promise<cognidream> {
    ...
    async setUrisTrust(uris: URI[], trusted: boolean): Promise<cognidream> {
...
export class WorkspaceTrustRequestService extends Disposable implements IWorkspaceTrustRequestService {
    ...
    private _openFilesTrustRequestResolver?: (response: WorkspaceTrustUriResponse) => cognidream;

    private _workspaceTrustRequestPromise?: Promise<boolean | undefined>;
    private _workspaceTrustRequestResolver?: (trusted: boolean | undefined) => cognidream;

    private readonly _onDidInitiateOpenFilesTrustRequest = this._register(new Emitter<cognidream>());
    ...
    private readonly _onDidInitiateWorkspaceTrustRequestOnStartup = this._register(new Emitter<cognidream>());
    ...
    async completeOpenFilesTrustRequest(result: WorkspaceTrustUriResponse, saveResponse?: boolean): Promise<cognidream> {
    ...
    private resolveWorkspaceTrustRequest(trusted?: boolean): cognidream {
    ...
    async completeWorkspaceTrustRequest(trusted?: boolean): Promise<cognidream> {
...
class WorkspaceTrustTransitionManager extends Disposable {
    ...
    async participate(trusted: boolean): Promise<cognidream> {
