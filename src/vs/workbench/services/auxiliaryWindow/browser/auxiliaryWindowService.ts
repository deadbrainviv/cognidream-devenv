...
export interface BeforeAuxiliaryWindowUnloadEvent {
    veto(reason: string | undefined): cognidream;
}
...
export interface IAuxiliaryWindow extends IDisposable {
    ...
    readonly onUnload: Event<cognidream>;

    readonly whenStylesHaveLoaded: Promise<cognidream>;
...
export class AuxiliaryWindow extends BaseWindow implements IAuxiliaryWindow {
    ...
    private readonly _onUnload = this._register(new Emitter<cognidream>());
    ...
    private handleBeforeUnload(e: BeforeUnloadEvent): cognidream {
    ...
    protected handleVetoBeforeClose(e: BeforeUnloadEvent, reason: string): cognidream {
        this.preventUnload(e);
    }

    protected preventUnload(e: BeforeUnloadEvent): cognidream {
        e.preventDefault();
        e.returnValue = localize('lifecycleVeto', "Changes that you made may not be saved. Please check press 'Cancel' and try again.");
    }

    protected confirmBeforeClose(e: BeforeUnloadEvent): cognidream {
        this.preventUnload(e);
    }

    private handleUnload(): cognidream {
...
export class BrowserAuxiliaryWindowService extends Disposable implements IAuxiliaryWindowService {
    ...
    private async openWindow(options?: IAuxiliaryWindowOpenOptions): Promise<Window | undefined> {
        ...
        if (!auxiliaryWindow && isWeb) {
            return (await this.dialogService.prompt({
                ...
                detail: localize('unableToOpenWindowDetail', "To acognidream this problem in the future, please ensure to allow popups for this website."),
                buttons: [
                    {
    ...
    private applyMeta(auxiliaryWindow: CodeWindow): cognidream {
    ...
    private applyCSS(auxiliaryWindow: CodeWindow, disposables: DisposableStore) {
        ...
        function cloneNode(originalNode: Element): cognidream {
