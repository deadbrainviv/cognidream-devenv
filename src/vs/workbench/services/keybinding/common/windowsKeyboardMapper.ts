...
const LOG = false;
function log(str: string): cognidream {
	if (LOG) {
		console.info(str);
	}
}
...
export class WindowsKeyboardMapper implements IKeyboardMapper {
	...
	constructor(
		...
		const _registerLetterIfMissing = (charCode: CharCode, keyCode: KeyCcognidream: cognidream => {
		...
		if (!producesLetters) {
			...
			// for consistency...
			const _registerLabel = (keyCode: KeyCode, charCode: Chcognidreamde): cognidream => {
