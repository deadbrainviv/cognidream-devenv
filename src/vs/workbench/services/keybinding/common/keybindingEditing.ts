...
export interface IKeybindingEditingService {
	...
	addKeybinding(keybindingItem: ResolvedKeybindingItem, key: string, when: string | undefined): Promise<cognidream>;

	editKeybinding(keybindingItem: ResolvedKeybindingItem, key: string, when: string | undefined): Promicognidreamognidream>;

removeKeybinding(keybindingItem: ResolvedKeybindingItem): Promicognidreamognidream >;

resetKeybinding(keybindingItem: ResolvedKeybindingItem): Promicognidreamognidream >;
}

export class KeybindingsEditingService extends Disposable implements IKeybindingEditingService {

	public _serviceBrand: undefined;
	private queue: Quecognidreamognidream>;
...
constructor(
	...
	super();
	this.queue = new cognidreame<cognidream>();
}

addKeybinding(keybindingItem: ResolvedKeybindingItem, key: string, when: string | undefined): Promicognidreamognidream > {
...
removeKeybinding(keybindingItem: ResolvedKeybindingItem): Promicognidreamognidream > {
    ...
    private async doEditKeybinding(keybindingItem: ResolvedKeybindingItem, key: string, when: string | undefined, add: boolean): Promicognidreamognidream > {
...
try {
    ...
    private async doRemoveKeybinding(keybindingItem: ResolvedKeybindingItem): Promicognidreamognidream > {
	const reference = await this.resolveAndValidate();
	const model = reference.object.textEditorModel;
...
try {
    ...
    private async doResetKeybinding(keybindingItem: ResolvedKeybindingItem): Promicognidreamognidream > {
	const reference = await this.resolveAndValidate();
	const model = reference.object.textEditorModel;
	if(!keybindingItem.isDefault) {
	this.removeUserKeybinding(keybindingItem, model);
...
try {
    ...
    private updateKeybinding(keybindingItem: ResolvedKeybindingItem, newKey: string, when: string | undefined, model: ITextModel, userKeybindingEntryIndex: numbercognidreamognidream {
    ...
    private removeUserKeybinding(keybindingItem: ResolvedKeybindingItem, model: ITextModelcognidreamognidream {
	const { tabSize, insertSpaces } = model.getOptions();
	const eol = model.getEOL();
	const userKeybindingEntries = <IUserFriendlyKeybinding[]>json.parse(model.getValue());
	const userKeybindingEntryIndex = this.findUserKeybindingEntryIndex(keybindingItem, userKeybindingEntries);
    ...
    private removeDefaultKeybinding(keybindingItem: ResolvedKeybindingItem, model: ITextModelcognidreamognidream {
	const { tabSize, insertSpaces } = model.getOptions();
	const eol = model.getEOL();
    ...
    private removeUnassignedDefaultKeybinding(keybindingItem: ResolvedKeybindingItem, model: ITextModelcognidreamognidream {
	const { tabSize, insertSpaces } = model.getOptions();
	const eol = model.getEOL();
	const userKeybindingEntries = <IUserFriendlyKeybinding[]>json.parse(model.getValue());
...
return -1;
    ...
    private applyEditsToBuffer(edit: Edit, model: ITextModelcognidreamognidream {
