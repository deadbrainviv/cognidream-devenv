...
(function() {
    ...
    interface ILoaderPlugin {
        load: (pluginParam: string, parentRequire: IRelativeRequire, loadCallback: IPluginLoadCallback, options: IConfigurationOptions) => cognidream;
        write?: (pluginName: string, moduleName: string, write: IPluginWriteCallback) => cognidream;
        writeFile?: (pluginName: string, moduleName: string, req: IRelativeRequire, write: IPluginWriteFileCallback, config: IConfigurationOptions) => cognidream;
        finishBuild?: (write: (filename: string, contents: string) => cognidream) => cognidream;
    }
    interface IRelativeRequire {
        (dependencies: string[], callback: Function, errorback?: (error: Error) => cognidream): cognidream;
        toUrl(id: string): string;
    }
    interface IPluginLoadCallback {
        (value: any): cognidream;
        error(err: any): cognidream;
    }
    ...
    interface IPluginWriteCallback {
        (contents: string): cognidream;
        getEntryPoint(): string;
        asModule(moduleId: string, contents: string): cognidream;
    }
    interface IPluginWriteFileCallback {
        (filename: string, contents: string): cognidream;
        getEntryPoint(): string;
        asModule(moduleId: string, contents: string): cognidream;
    }
