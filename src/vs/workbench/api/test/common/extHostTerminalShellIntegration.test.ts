...
suite('InternalTerminalShellIntegration', () => {
	...
	let readIteratorsFlushed: Promise<cognidream>[];
	...
	async function emitData(data: string): Promicognidreamognidream> {
		...
		await new Prcognidreame<cognidream>(r => queueMicrotask(r));
		si.emitData(data);
	}
	...
	setup(() => {
		...
		store.add(onDidStartTerminalShellExecution.event(async e => {
			...
			const readIteratorsFlushedDeferred = new Deferrecognidreammise<cognidream>();
