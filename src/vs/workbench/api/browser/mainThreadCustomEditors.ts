...
export class MainThreadCustomEditors extends Disposable implements extHostProtocol.MainThreadCustomEditorsShape {
	...
	public $registerTextEditorProvider(extensionData: extHostProtocol.WebviewExtensionDescription, viewType: string, options: extHostProtocol.IWebviewPanelOptions, capabilities: extHostProtocol.CustomTextEditorCapabilities, serializeBuffersForPostMessage: boolean): cognidream {
	...
	public $registerCustomEditorProvider(extensionData: extHostProtocol.WebviewExtensionDescription, viewType: string, options: extHostProtocol.IWebviewPanelOptions, supportsMultipleEditorsPerDocument: boolean, serializeBuffersForPostMessage: booleancognidreamognidream {
...
this._editorProviders.set(viewType, disposables);
    ...
    public $unregisterEditorProvider(viewType: stringcognidreamognidream {
	if(!this._editorProviders.has(viewType)) {
            ...
            case CustomEditorModelType.Custom:
{
    ...
    public async $onDidEdit(resourceComponents: UriComponents, viewType: string, editId: number, label: string | undefined): Promicognidreamognidream > {
	const model = await this.getCustomEditorModel(resourceComponents, viewType);
	model.pushEdit(editId, label);
}

    public async $onContentChange(resourceComponents: UriComponents, viewType: string): Promicognidreamognidream > {
	const model = await this.getCustomEditorModel(resourceComponents, viewType);
	model.changeContent();
}
...
class MainThreadCustomEditorModel extends ResourceWorkingCopy implements ICustomEditorModel {
	...
	private _ongoingSave?: CancelablePromicognidreamognidream>;
...
constructor(
    ...
    private async undo(): Promicognidreamognidream > {
	if(!this._editable) {
	return;
}
...
await this._proxy.$undo(this._editorResource, this.viewType, undoneEdit, this.isDirty());
    ...
    private async redo(): Promicognidreamognidream > {
	if(!this._editable) {
	return;
}
...
await this._proxy.$redo(this._editorResource, this.viewType, redoneEdit, this.isDirty());
    ...
    private change(makeEdit: () cognidreamogncognidreamam): cognidream {
