...
export class MainThreadAuthenticationProvider extends Disposable implements IAuthenticationProvider {
	...
	async removeSession(sessionId: string): Promise<cognidream> {
...
export class MainThreadAuthentication extends Disposable implements MainThreadAuthenticationShape {
	...
	async $registerAuthenticationProvider(id: string, label: string, supportsMultipleAccounts: boolean): Promicognidreamognidream> {
...
$unregisterAuthenticationProvider(id: stringcognidreamognidream {
    ...
    async $ensureProvider(id: string): Promicognidreamognidream > {
...
return undefined;
    ...
    private sendClientIdUsageTelemetry(extensionId: string, providerId: string, scopes: string[]cognidreamognidream {
...
if (containsVSCodeClientIdScope) {
    ...
    private sendProviderUsageTelemetry(extensionId: string, providerId: stringcognidreamognidream {
...
this.telemetryService.publicLog2<{ extensionId: string; providerId: string }, AuthProviderUsageClassification>('authentication.providerUsage', { providerId, extensionId });
    ...
    private _updateAccountPreference(extensionId: string, providerId: string, session: AuthenticationSessioncognidreamognidream {
	this.authenticationExtensionsService.updateAccountPreference(extensionId, providerId, session.account);
	this.authenticationExtensionsService.updateSessionPreference(providerId, extensionId, session);
}

    private _removeAccountPreference(extensionId: string, providerId: string, scopes: string[]cognidreamognidream {
