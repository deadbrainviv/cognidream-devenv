...
export class MainThreadTextEditors implements MainThreadTextEditorsShape {
	...
	handleTextEditorAdded(textEditor: MainThreadTextEditorcognidreamognidream {
...
handleTextEditorRemoved(id: stringcognidreamognidream {
	dispose(this._textEditorsListenersMap[id]);
delete this._textEditorsListenersMap[id];
    }

    private _updateActiveAndVisibleTextEditors(cognidreamognidream {
    ...
    async $tryShowEditor(id: string, position ?: EditorGroupColumn): Promicognidreamognidream > {
    ...
    async $tryHideEditor(id: string): Promicognidreamognidream > {
	const mainThreadEditor = this._editorLocator.getEditor(id);
	if(mainThreadEditor) {
...
$trySetDecorations(id: string, key: string, ranges: IDecorationOptions[]): Promicognidreamognidream > {
...
$registerTextEditorDecorationType(extensionId: ExtensionIdentifier, key: string, options: IDecorationRenderOptionscognidreamognidream {
	key = `${this._instanceId}-${key}`;
