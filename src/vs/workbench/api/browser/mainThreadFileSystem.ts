...
export class MainThreadFileSystem implements MainThreadFileSystemShape {
	...
	async $registerFileSystemProvider(handle: number, scheme: string, capabilities: FileSystemProviderCapabilities, readonlyMessage?: IMarkdownString): Promicognidreamognidream> {
...
$unregisterProvider(handle: numbercognidreamognidream {
    ...
    $onFileSystemChange(handle: number, changes: IFileChangeDto[]cognidreamognidream {
    ...
    async $writeFile(uri: UriComponents, content: VSBuffer): Promicognidreamognidream > {
	try {
    ...
    async $rename(source: UriComponents, target: UriComponents, opts: IFileOverwriteOptions): Promicognidreamognidream > {
	try {
    ...
    async $copy(source: UriComponents, target: UriComponents, opts: IFileOverwriteOptions): Promicognidreamognidream > {
	try {
    ...
    async $mkdir(uri: UriComponents): Promicognidreamognidream > {
	try {
    ...
    async $delete(uri: UriComponents, opts: IFileDeleteOptions): Promicognidreamognidream > {
	try {
...
class RemoteFileSystemProvider implements IFileSystemProviderWithFileReadWriteCapability, IFileSystemProviderWithOpenReadWriteCloseCapability, IFileSystemProviderWithFileFolderCopyCapability {
	...
	readonly onDidChangeCapabilities: Evecognidreamognidream> = Event.None;
...
dispose(cognidreamognidream {
    ...
    $onFileSystemChange(changes: IFileChangeDto[]cognidreamognidream {
    ...
    writeFile(resource: URI, content: Uint8Array, opts: IFileWriteOptions): Promicognidreamognidream > {
	return this._proxy.$writeFile(this._handle, resource, VSBuffer.wrap(content), opts);
}

    delete (resource: URI, opts: IFileDeleteOptions): Promicognidreamognidream > {
	return this._proxy.$delete(this._handle, resource, opts);
}

    mkdir(resource: URI): Promicognidreamognidream > {
    ...
    rename(resource: URI, target: URI, opts: IFileOverwriteOptions): Promicognidreamognidream > {
	return this._proxy.$rename(this._handle, resource, target, opts);
}

    copy(resource: URI, target: URI, opts: IFileOverwriteOptions): Promicognidreamognidream > {
    ...
    close(fd: number): Promicognidreamognidream > {
