...
export class MainThreadQuickDiff implements MainThreadQuickDiffShape {
	...
	async $registerQuickDiffProvider(handle: number, selector: IDocumentFilterDto[], label: string, rootUri: UriComponents | undefined, visible: boolean): Promise<cognidream> {
	...
	async $unregisterQuickDiffProvider(handle: number): Promicognidreamognidream> {
