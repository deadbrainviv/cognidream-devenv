...
this._providerRegistrations.set(handle, dipsosables);
    ...
    async $reportResponsePart(requestId: number, chunk: IChatResponseFragment): Promicognidreamognidream > {
    ...
    async $reportResponseDone(requestId: number, err: SerializedError | undefined): Promicognidreamognidream > {
	const data = this._pendingProgress.get(requestId);
...
$unregisterProvider(handle: numbercognidreamognidream {
    ...
    $whenLanguageModelChatRequestMade(identifier: string, extensionId: ExtensionIdentifier, participant ?: string | undefined, tokenCount ?: number | undefinedcognidreamognidream {
...
$registerFileIgnoreProvider(handle: numbercognidreamognidream {
	...
	class LanguageModelAccessAuthProvider implements IAuthenticationProvider {
		...
		removeSession(sessionId: string): Promicognidreamognidream> {
		if(this._session) {
