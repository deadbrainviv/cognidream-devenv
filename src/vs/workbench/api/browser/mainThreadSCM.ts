...
class MainThreadSCMResourceGroup implements ISCMResourceGroup {
    ...
    private readonly _onDidChangeResources = new Emittcognidreamognidream > ();
...
$updateGroup(features: SCMGroupFeaturescognidreamognidream {
	this.features = { ...this.features, ...features };
	this._onDidChange.fire();
}

    $updateGroupLabel(label: stringcognidreamognidream {
	this.label = label;
	this._onDidChange.fire();
}
}
	...
	class MainThreadSCMResource implements ISCMResource {
		...
		open(preserveFocus: boolean): Promicognidreamognidream> {
	...
	class MainThreadSCMHistoryProvider implements ISCMHistoryProvider {
		...
		$onDidChangeCurrentHistoryItemRefs(historyItemRef?: SCMHistoryItemRefDto, historyItemRemoteRef?: SCMHistoryItemRefDto, historyItemBaseRef?: SCMHistoryItemRefDtocognidreamognidream {
			transaction(tx => {
...
$onDidChangeHistoryItemRefs(historyItemRefs: SCMHistoryItemRefsChangeEventDtocognidreamognidream {
	...
	class MainThreadSCMProvider implements ISCMProvider, QuickDiffProvider {
		...
		private readonly _onDidChangeResourceGroups = new Emittcognidreamognidream > ();
		...
		$updateSourceControl(features: SCMProviderFeaturescognidreamognidream {
			this.features = { ...this.features, ...features };
...
$registerGroups(_groups: [number /*handle*/, string /*id*/, string /*label*/, SCMGroupFeatures, /* multiDiffEditorEnableViewChanges */ boolean][]cognidreamognidream {
    ...
    $updateGroup(handle: number, features: SCMGroupFeaturescognidreamognidream {
    ...
    $updateGroupLabel(handle: number, label: stringcognidreamognidream {
	const group = this._groupsByHandle[handle];
    ...
    $spliceGroupResourceStates(splices: SCMRawResourceSplices[]cognidreamognidream {
    ...
    $unregisterGroup(handle: numbercognidreamognidream {
	const group = this._groupsByHandle[handle];
...
$onDidChangeHistoryProviderCurrentHistoryItemRefs(historyItemRef ?: SCMHistoryItemRefDto, historyItemRemoteRef ?: SCMHistoryItemRefDto, historyItemBaseRef ?: SCMHistoryItemRefDtocognidreamognidream {
...
$onDidChangeHistoryProviderHistoryItemRefs(historyItemRefs: SCMHistoryItemRefsChangeEventDtocognidreamognidream {
	if(!this.historyProvider.get()) {
	return;
}
...
this._disposables.dispose();
    ...
    async $registerSourceControl(handle: number, id: string, label: string, rootUri: UriComponents | undefined, inputBoxDocumentUri: UriComponents): Promicognidreamognidream > {
...
this._repositoryBarriers.get(handle)?.open();
    ...
    async $updateSourceControl(handle: number, features: SCMProviderFeatures): Promicognidreamognidream > {
    ...
    async $unregisterSourceControl(handle: number): Promicognidreamognidream > {
	await this._repositoryBarriers.get(handle)?.wait();
	const repository = this._repositories.get(handle);
    ...
    async $registerGroups(sourceControlHandle: number, groups: [number /*handle*/, string /*id*/, string /*label*/, SCMGroupFeatures, /* multiDiffEditorEnableViewChanges */ boolean][], splices: SCMRawResourceSplices[]): Promicognidreamognidream > {
    ...
    async $updateGroup(sourceControlHandle: number, groupHandle: number, features: SCMGroupFeatures): Promicognidreamognidream > {
	await this._repositoryBarriers.get(sourceControlHandle)?.wait();
	const repository = this._repositories.get(sourceControlHandle);
    ...
    async $updateGroupLabel(sourceControlHandle: number, groupHandle: number, label: string): Promicognidreamognidream > {
	await this._repositoryBarriers.get(sourceControlHandle)?.wait();
	const repository = this._repositories.get(sourceControlHandle);
    ...
    async $spliceResourceStates(sourceControlHandle: number, splices: SCMRawResourceSplices[]): Promicognidreamognidream > {
	await this._repositoryBarriers.get(sourceControlHandle)?.wait();
	const repository = this._repositories.get(sourceControlHandle);
    ...
    async $unregisterGroup(sourceControlHandle: number, handle: number): Promicognidreamognidream > {
	await this._repositoryBarriers.get(sourceControlHandle)?.wait();
	const repository = this._repositories.get(sourceControlHandle);
    ...
    async $setInputBoxValue(sourceControlHandle: number, value: string): Promicognidreamognidream > {
	await this._repositoryBarriers.get(sourceControlHandle)?.wait();
	const repository = this._repositories.get(sourceControlHandle);
    ...
    async $setInputBoxPlaceholder(sourceControlHandle: number, placeholder: string): Promicognidreamognidream > {
	await this._repositoryBarriers.get(sourceControlHandle)?.wait();
	const repository = this._repositories.get(sourceControlHandle);
    ...
    async $setInputBoxEnablement(sourceControlHandle: number, enabled: boolean): Promicognidreamognidream > {
	await this._repositoryBarriers.get(sourceControlHandle)?.wait();
	const repository = this._repositories.get(sourceControlHandle);
    ...
    async $setInputBoxVisibility(sourceControlHandle: number, visible: boolean): Promicognidreamognidream > {
	await this._repositoryBarriers.get(sourceControlHandle)?.wait();
	const repository = this._repositories.get(sourceControlHandle);
    ...
    async $showValidationMessage(sourceControlHandle: number, message: string | IMarkdownString, type: InputValidationType): Promicognidreamognidream > {
	await this._repositoryBarriers.get(sourceControlHandle)?.wait();
	const repository = this._repositories.get(sourceControlHandle);
	if(!repository) {
		return;
    ...
    async $setValidationProviderIsEnabled(sourceControlHandle: number, enabled: boolean): Promicognidreamognidream > {
	await this._repositoryBarriers.get(sourceControlHandle)?.wait();
	const repository = this._repositories.get(sourceControlHandle);
    ...
    async $onDidChangeHistoryProviderCurrentHistoryItemRefs(sourceControlHandle: number, historyItemRef ?: SCMHistoryItemRefDto, historyItemRemoteRef ?: SCMHistoryItemRefDto, historyItemBaseRef ?: SCMHistoryItemRefDto): Promicognidreamognidream > {
	await this._repositoryBarriers.get(sourceControlHandle)?.wait();
	const repository = this._repositories.get(sourceControlHandle);
    ...
    async $onDidChangeHistoryProviderHistoryItemRefs(sourceControlHandle: number, historyItemRefs: SCMHistoryItemRefsChangeEventDto): Promicognidreamognidream > {
	await this._repositoryBarriers.get(sourceControlHandle)?.wait();
	const repository = this._repositories.get(sourceControlHandle);
