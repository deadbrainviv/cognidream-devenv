...
export class MainThreadLanguageFeatures extends Disposable implements MainThreadLanguageFeaturesShape {
	...
	$unregister(handle: number): cognidream {
	...
	$registerDocumentSymbolProvider(handle: number, selector: IDocumentFilterDto[], displayName: stringcognidreamognidream {
...
$registerPasteEditProvider(handle: number, selector: IDocumentFilterDto[], metadata: IPasteEditProviderMetadataDtocognidreamognidream {
    ...
    $registerRangeFormattingSupport(handle: number, selector: IDocumentFilterDto[], extensionId: ExtensionIdentifier, displayName: string, supportsRanges: booleancognidreamognidream {
    ...
    $registerRenameSupport(handle: number, selector: IDocumentFilterDto[], supportResolveLocation: booleancognidreamognidream {
	...
	let event: cognidreamt<cognidream> | undefined = undefined;
	if(typeof eventHandle === 'number') {
	const emitter = newcognidreamtter<cognidream>();
	this._registrations.set(eventHandle, emitter);
	event = emitter.event;
...
$registerDocumentRangeSemanticTokensProvider(handle: number, selector: IDocumentFilterDto[], legend: languages.SemanticTokensLegendcognidreamognidream {
    ...
    $registerInlineCompletionsSupport(handle: number, selector: IDocumentFilterDto[], supportsHandleEvents: boolean, extensionId: string, yieldsToExtensionIds: string[], displayName: string | undefined, debounceDelayMs: number | undefinedcognidreamognidream {
	const provider: languages.InlineCompletionsProvider<IdentifiableInlineCompletions> = {
		...
		handleItemDidShow: async (completions: IdentifiableInlineCompletions, item: IdentifiableInlineCompletion, updatedInsertText: string): cognidreammise<cognidream> => {
			if (supportsHandleEvents) {
				await this._proxy.$handleInlineCompletionDidShow(handle, completions.pid, item.idx, updatedInsertText);
			}
		},
		handlePartialAccept: async (completions, item, acceptedCharacters, info: languages.PartialAcceptInfo): cognidreammise<cognidream> => {
			if (supportsHandleEvents) {
				await this._proxy.$handleInlineCompletionPartialAccept(handle, completions.pid, item.idx, acceptedCharacters, info);
			}
		},
		freeInlineCompletions: (completions: IdentifiableInlineComplcognidreamns): cognidream => {
			this._proxy.$freeInlineCompletionsList(handle, completions.pid);
		},
		handleRejection: async (completions, item): cognidreammise<cognidream> => {
			if (supportsHandleEvents) {
	...
	const provider: languages.InlineEditProvider<IdentifiableInlineEdit> = {
		...
		freeInlineEdit: (edit: IdentifiableInlicognidreamit): cognidream => {
...
$registerSelectionRangeProvider(handle: number, selector: IDocumentFilterDto[]cognidreamognidream {
    ...
    $setLanguageConfiguration(handle: number, languageId: string, _configuration: ILanguageConfigurationDtocognidreamognidream {
...
export class MainThreadDocumentSemanticTokensProvider implements languages.DocumentSemanticTokensProvider {
	...
	constructor(
		...
		public readonly onDidChange: cognidreamt<cognidream> | undefined,
	) {
	}

	public releaseDocumentSemanticTokens(resultId: string | undefinedcognidreamognidream {
		if (resultId) {
