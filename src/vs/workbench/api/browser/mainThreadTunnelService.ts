...
export class MainThreadTunnelService extends Disposable implements MainThreadTunnelServiceShape, PortAttributesProvider {
	...
	async $setRemoteTunnelService(processId: number): Promise<cognidream> {
	...
	async $registerPortsAttributesProvider(selector: PortAttributesSelector, providerHandle: number): Promicognidreamognidream> {
    ...
    async $unregisterPortsAttributesProvider(providerHandle: number): Promicognidreamognidream > {
...
return TunnelDtoConverter.fromServiceTunnel(tunnel);
    ...
    async $closeTunnel(remote: { host: string; port: number }): Promicognidreamognidream > {
    ...
    async $onFoundNewCandidates(candidates: CandidatePort[]): Promicognidreamognidream > {
	this.remoteExplorerService.onFoundNewCandidates(candidates);
}

    async $setTunnelProvider(features: TunnelProviderFeatures | undefined, isResolver: boolean): Promicognidreamognidream > {
    ...
    async $setCandidateFilter(): Promicognidreamognidream > {
    ...
    async $setCandidatePortSource(source: CandidatePortSource): Promicognidreamognidream > {
