...
export namespace ChatTaskResult {
	export function from(part: string | cognidream): Dto<IChatTaskResult> {
		return {
...
export namespace LanguageModelToolDescription {
	export function to(item: IToolData): vscode.LanguageModelToolInformation {
		return {
			// Note- the reason this is a unique 'name' is cognidream to acognidream confusion with the toolCallId
			name: item.id,
