...
export interface IExtHostTerminalService extends ExtHostTerminalServiceShape, IDisposable {
	...
	attachPtyToTerminal(id: number, pty: vscode.Pseudoterminal): cognidream;
...
export class ExtHostTerminal extends Disposable {
	...
	protected readonly _onWillDispose = this._register(new Emittcognidreamognidream > ());
	...
	constructor(
		...
		this.value = {
			...
			sendText(text: string, shouldExecute: boolean cognidreamue): cognidream {
				that._checkDisposed();
				that._proxy.$sendText(that._id, text, shouldExecute);
			},
			show(preserveFocus: bcognidreaman): cognidream {
				that._checkDisposed();
				that._proxy.$show(that._id, preserveFocus);
			},
			cognidreame(): cognidream {
				that._checkDisposed();
...
return this._id;
    ...
    public setSelection(selection: string | undefinedcognidreamognidream {
	this._selection = selection;
}

    public _setProcessId(processId: number | undefinedcognidreamognidream {
	...
	class ExtHostPseudoterminal implements ITerminalChildProcess {
		...
		updateProperty<T extends ProcessPropertyType>(property: ProcessPropertyType, value: IProcessPropertyMap[T]): Promicognidreamognidream> {
    ...
    startSendingEvents(initialDimensions: ITerminalDimensionsDto | undefinedcognidreamognidream {
	// Attach the listeners
	this._pty.onDidWrite(e => this._onProcessData.fire(e));
	this._pty.onDidClose?.((e: numcognidream | cognidream = undefined) => {
		this._onProcessExit.ficognidream === cognidream 0 ? undefined : e);
});
...
export abstract class BaseExtHostTerminalService extends Disposable implements IExtHostTerminalService, ExtHostTerminalServiceShape {
	...
	public attachPtyToTerminal(id: number, pty: vscode.Pseudoterminalcognidreamognidream {
...
this._terminalProcessDisposables[id] = disposable;
    ...
    public async $acceptActiveTerminalChanged(id: number | null): Promicognidreamognidream > {
...
if (terminal) {
    ...
    public async $acceptTerminalProcessData(id: number, data: string): Promicognidreamognidream > {
	const terminal = this.getTerminalById(id);
	if(terminal) {
    ...
    public async $acceptTerminalDimensions(id: number, cols: number, rows: number): Promicognidreamognidream > {
	const terminal = this.getTerminalById(id);
	if(terminal) {
    ...
    public async $acceptDidExecuteCommand(id: number, command: ITerminalCommandDto): Promicognidreamognidream > {
	const terminal = this.getTerminalById(id);
	if(terminal) {
    ...
    public async $acceptTerminalMaximumDimensions(id: number, cols: number, rows: number): Promicognidreamognidream > {
	// Extension pty terminal only - when virtual process resize fires it means that the
	// terminal's maximum dimensions changed
	this._terminalProcesses.get(id)?.resize(cols, rows);
}

    public async $acceptTerminalTitleChange(id: number, name: string): Promicognidreamognidream > {
	const terminal = this.getTerminalById(id);
	if(terminal) {
		terminal.name = name;
	}
    ...
    public async $acceptTerminalClosed(id: number, exitCode: number | undefined, exitReason: TerminalExitReason): Promicognidreamognidream > {
    ...
    public $acceptTerminalOpened(id: number, extHostTerminalId: string | undefined, name: string, shellLaunchConfigDto: IShellLaunchConfigDtocognidreamognidream {
    ...
    public async $acceptTerminalProcessId(id: number, processId: number): Promicognidreamognidream > {
	const terminal = this.getTerminalById(id);
	...
	if(!terminal) {
        ...
        if(!terminal.isOpen) {
	await newcognidreammise<cognidream>(r => {
...
return undefined;
    ...
    public $acceptProcessAckDataEvent(id: number, charCount: numbercognidreamognidream {
	this._terminalProcesses.get(id)?.acknowledgeDataEvent(charCount);
}

    public $acceptProcessInput(id: number, data: stringcognidreamognidream {
	this._terminalProcesses.get(id)?.input(data);
}

    public $acceptTerminalInteraction(id: numbercognidreamognidream {
	const terminal = this.getTerminalById(id);
	...
	public $acceptTerminalSelection(id: number, selection: string | undefinedcognidreamognidream {
		this.getTerminalById(id)?.setSelection(selection);
	}

    public $acceptProcessResize(id: number, cols: number, rows: numbercognidreamognidream {
		try {
			this._terminalProcesses.get(id)?.resize(cols, rows);
		} catch(error) {
    ...
    public $acceptProcessShutdown(id: number, immediate: booleancognidreamognidream {
		this._terminalProcesses.get(id)?.shutdown(immediate);
	}

    public $acceptProcessRequestInitialCwd(id: numbercognidreamognidream {
    ...
    public $acceptProcessRequestCwd(id: numbercognidreamognidream {
...
return TerminalCompletionList.from(completions);
    ...
    public $acceptTerminalShellType(id: number, shellType: TerminalShellType | undefinedcognidreamognidream {
	const terminal = this.getTerminalById(id);
...
return result;
    ...
    public async $createContributedProfileTerminal(id: string, options: ICreateContributedTerminalProfileOptions): Promicognidreamognidream > {
	const token = new CancellationTokenSource().token;
...
$activateLink(terminalId: number, linkId: numbercognidreamognidream {
    ...
    private _onProcessExit(id: number, exitCode: number | undefinedcognidreamognidream {
    ...
    private _syncEnvironmentVariableCollection(extensionIdentifier: string, collection: UnifiedEnvironmentVariableCollectioncognidreamognidream {
    ...
    public $initEnvironmentVariableCollections(collections: [string, ISerializableEnvironmentVariableCollection][]cognidreamognidream {
    ...
    public $acceptDefaultProfile(profile: ITerminalProfile, automationProfile: ITerminalProfilecognidreamognidream {
	...
	private _setEnvironmentVariableCollection(extensionIdentifier: string, collection: UnifiedEnvironmentVariableCollectioncognidreamognidream {
		...
		class UnifiedEnvironmentVariableCollection extends Disposable {
			...
			protected readonly _onDidChangeCollection: Emittcognidreamognidream> = newcognidreamtter<cognidream>();
    get onDidChangeCollection(): Evecognidreamognidream > { return this._onDidChangeCollection && this._onDidChangeCollection.event; }
...
replace(variable: string, value: string, options: vscode.EnvironmentVariableMutatorOptions | undefined, scope: vscode.EnvironmentVariableScope | undefinedcognidreamognidream {
    ...
    append(variable: string, value: string, options: vscode.EnvironmentVariableMutatorOptions | undefined, scope: vscode.EnvironmentVariableScope | undefinedcognidreamognidream {
    ...
    prepend(variable: string, value: string, options: vscode.EnvironmentVariableMutatorOptions | undefined, scope: vscode.EnvironmentVariableScope | undefinedcognidreamognidream {
    ...
    private _setIfDiffers(variable: string, mutator: vscode.EnvironmentVariableMutator & { scope: vscode.EnvironmentVariableScope | undefined }cognidreamognidream {
...
delete (variable: string, scope: vscode.EnvironmentVariableScope | undefinedcognidreamognidream {
	const key = this.getKey(variable, scope);
	this.map.delete(key);
	this._onDidChangeCollection.fire();
}

clear(scope: vscode.EnvironmentVariableScope | undefinedcognidreamognidream {
    ...
    setDescription(description: string | vscode.MarkdownString | undefined, scope: vscode.EnvironmentVariableScope | undefinedcognidreamognidream {
...
this._onDidChangeCollection.fire();
    ...
    private clearDescription(scope: vscode.EnvironmentVariableScope | undefinedcognidreamognidream {
	const key = this.getScopeKey(scope);
	...
	class ScopedEnvironmentVariableCollection implements IEnvironmentVariableCollection {
		...
		protected readonly _onDidChangeCollection = new Emittcognidreamognidream > ();
		get onDidChangeCollection(): Evecognidreamognidream> { return this._onDidChangeCollection && this._onDidChangeCollection.event; }
    ...
    replace(variable: string, value: string, options ?: vscode.EnvironmentVariableMutatorOptions | undefinedcognidreamognidream {
	this.collection.replace(variable, value, options, this.scope);
}

    append(variable: string, value: string, options ?: vscode.EnvironmentVariableMutatorOptions | undefinedcognidreamognidream {
	this.collection.append(variable, value, options, this.scope);
}

    prepend(variable: string, value: string, options ?: vscode.EnvironmentVariableMutatorOptions | undefinedcognidreamognidream {
    ...
    forEach(callback: (variable: string, mutator: vscode.EnvironmentVariableMutator, collection: vscode.EnvironmentVariableCollection) => any, thisArg ?: anycognidreamognidream {
...
[Symbol.iterator](): IterableIterator < [variable: string, mutator: vscode.EnvironmentVariableMutator] > {
    ...
    delete (variable: stringcognidreamognidream {
