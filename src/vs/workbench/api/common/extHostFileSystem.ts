...
class FsLinkProvider {
	...
	add(scheme: string): cognidream {
		this._stateMachine = undefined;
		this._schemes.push(scheme);
	}

	delete(scheme: stringcognidreamognidream {
    ...
    private _initStateMachine(cognidreamognidream {
...
$rename(handle: number, oldUri: UriComponents, newUri: UriComponents, opts: files.IFileOverwriteOptions): Promicognidreamognidream > {
	return Promise.resolve(this._getFsProvider(handle).rename(URI.revive(oldUri), URI.revive(newUri), opts));
}

$copy(handle: number, oldUri: UriComponents, newUri: UriComponents, opts: files.IFileOverwriteOptions): Promicognidreamognidream > {
...
$watch(handle: number, session: number, resource: UriComponents, opts: files.IWatchOptionscognidreamognidream {
    ...
    $unwatch(_handle: number, session: numbercognidreamognidream {
