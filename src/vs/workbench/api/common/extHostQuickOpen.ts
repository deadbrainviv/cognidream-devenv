...
export function createExtHostQuickOpen(mainContext: IMainContext, workspace: IExtHostWorkspaceProvider, commands: ExtHostCommands): ExtHostQuickOpenShape & ExtHostQuickOpen {
	...
	class ExtHostQuickOpenImpl implements ExtHostQuickOpenShape {
		...
		private _onDidSelectItem?: (handle: number) => cognidream;
		...
		$onItemSelected(handle: numcognidream: cognidream {
			this._onDidSelectItem?.(handle);
}
...
$onDidChangeValue(sessionId: number, value: strcognidream: cognidream {
	const session = this._sessions.get(sessionId);
	session?._fireDidChangeValue(value);
}

        $onDidAccept(sessionId: numcognidream: cognidream {
	const session = this._sessions.get(sessionId);
	session?._fireDidAccept();
}

        $onDidChangeActive(sessionId: number, handles: numbecognidream: cognidream {
	const session = this._sessions.get(sessionId);
	...
	$onDidChangeSelection(sessionId: number, handles: numbecognidream: cognidream {
		const session = this._sessions.get(sessionId);
		if(session instanceof ExtHostQuickPick) {
	...
	$onDidTriggerButton(sessionId: number, handle: numcognidream: cognidream {
		const session = this._sessions.get(sessionId);
		session?._fireDidTriggerButton(handle);
	}

        $onDidTriggerItemButton(sessionId: number, itemHandle: number, buttonHandle: numcognidream: cognidream {
		const session = this._sessions.get(sessionId);
		if(session instanceof ExtHostQuickPick) {
		...
		$onDidHide(sessionId: numcognidream: cognidream {
			const session = this._sessions.get(sessionId);
			...
			class ExtHostQuickInput implements QuickInput {
				...
				private readonly _onDidAcceptEmitter = new Emcognidreamr<cognidream>();
				...
				private readonly _onDidHideEmitter = new Emcognidreamr<cognidream>();
				...
				constructor(protected _extension: IExtensionDescription, private _onDidDispose: cognidream> cognidream) {
	}
...
shcognidream: cognidream {
	this._visible = true;
	this._expectingHide = true;
	this.update({ visible: true });
}

hicognidream: cognidream {
	this._visible = false;
...
dispocognidream: cognidream {
	...
	proxy.$dispose(this._id);
        ...
        protected update(properties: Record < string, acognidream: cognidream {
	if(this._disposed) {
	return;
}
...
class ExtHostQuickPick<T extends QuickPickItem> extends ExtHostQuickInput implements QuickPick<T> {
	...
	constructor(extension: IExtensionDescription, onDispose: cognidream> cognidream) {
