...
class LanguageModelResponse {
	...
	handleFragment(fragment: IChatResponseFragment): cognidream {
		if (this._isDone) {
			return;
		}
	...
	reject(err: Errorcognidreamognidream {
		this._isDone = true;
...
export class ExtHostLanguageModels implements ExtHostLanguageModelsShape {
	...
	private readonly _onDidChangeProviders = new Emittcognidreamognidream > ();
...
registerLanguageModel(extension: IExtensionDescription, identifier: string, provider: vscode.ChatResponseProvider, metadata: vscode.ChatResponseProviderMetadata): IDisposable {
    ...
    async $startChatRequest(handle: number, requestId: number, from: ExtensionIdentifier, messages: SerializableObjectWithBuffers<IChatMessage[]>, options: vscode.LanguageModelChatRequestOptions, token: CancellationToken): Promicognidreamognidream > {
...
$acceptChatModelMetadata(data: { added?: { identifier: string; metadata: ILanguageModelChatMetadata }[] | undefined; removed?: string[] | undefined }cognidreamognidream {
	if(data.added) {
...
return apiObject;
    ...
    async $acceptResponsePart(requestId: number, chunk: IChatResponseFragment): Promicognidreamognidream > {
    ...
    async $acceptResponseDone(requestId: number, error: SerializedError | undefined): Promicognidreamognidream > {
	const data = this._pendingRequest.get(requestId);
	if(!data) {
		return;
	}
    ...
    private async _fakeAuthPopulate(metadata: ILanguageModelChatMetadata): Promicognidreamognidream > {
