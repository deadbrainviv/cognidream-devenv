...
interface ITestRunner {
	...
	run(testsRoot: string, clb: (error: Error, failures?: number) => cognidream): cognidream;
}

interface INewTestRunner {
	/** New test runner API, as explained in the extension test doc */
	run(): Promicognidreamognidream>;
}
...
export interface IHostUtils {
	...
	exit(code: numbercognidreamognidream;
...
export abstract class AbstractExtHostExtensionService extends Disposable implements ExtHostExtensionServiceShape {
	...
	private readonly _onDidChangeRemoteConnectionData = this._register(new Emittcognidreamognidream > ());
	...
	constructor(
		...
		this._activator = this._register(new ExtensionsActivator(
			...
			{
				onExtensionActivationError: (extensionId: ExtensionIdentifier, error: Error, missingExtensionDependency: MissingExtensionDependencognidream null): cognidream => {
	...
	public async initialize(): Promicognidreamognidream> {
		try {
    ...
    private async _deactivateAll(): Promicognidreamognidream > {
	this._storagePath.onWillDeactivateAll();

	let allPromises: Prcognidreame < cognidream > [] =[];
	try {
    ...
    public terminate(reason: string, code: number = 0cognidreamognidream {
...
Promise.race([timeout(5000), extensionsDeactivated]).finally(() => {
    ...
    private _activateByEvent(activationEvent: string, startup: boolean): Promicognidreamognidream > {
	return this._activator.activateByEvent(activationEvent, startup);
}

    private _activateById(extensionId: ExtensionIdentifier, reason: ExtensionActivationReason): Promicognidreamognidream > {
	return this._activator.activateById(extensionId, reason);
}

    public activateByIdWithErrors(extensionId: ExtensionIdentifier, reason: ExtensionActivationReason): Promicognidreamognidream > {
...
return this._extensionPathIndex;
    ...
    private _deactivate(extensionId: ExtensionIdentifier): Promicognidreamognidream > {
...
return this._doActivateExtension(extensionDescription, reason).then((activatedExtension) => {
    ...
    private _activateOneStartupFinished(desc: IExtensionDescription, activationEvent: stringcognidreamognidream {
    ...
    private _activateAllStartupFinishedDeferred(extensions: IExtensionDescription[], start: number = 0cognidreamognidream {
    ...
    private _activateAllStartupFinished(cognidreamognidream {
    ...
    private _handleEagerExtensions(): Promicognidreamognidream > {
    ...
    private _handleWorkspaceContainsEagerExtensions(folders: ReadonlyArray<vscode.WorkspaceFolder>): Promicognidreamognidream > {
	if(folders.length === 0) {
	return Promise.resolve(undefined);
}
...
return Promise.all(
    ...
    private async _handleWorkspaceContainsEagerExtension(folders: ReadonlyArray<vscode.WorkspaceFolder>, desc: IExtensionDescription): Promicognidreamognidream > {
...
return (
    ...
    private async _handleRemoteResolverEagerExtensions(): Promicognidreamognidream > {
...
return new Promise<number>((resolve, reject) => {
	...
	// Using the new API `run():cognidreammise<cognidream>`
    ...
    private _startExtensionHost(): Promicognidreamognidream > {
	if(this._started) {
...
return { authorityPrefix, resolver: this._resolvers[authorityPrefix] };
    ...
    public async $startExtensionHost(extensionsDelta: IExtensionDescriptionDelta): Promicognidreamognidream> {
    ...
    public $activateByEvent(activationEvent: string, activationKind: ActivationKind): Promicognidreamognidream> {
    ...
    public async $deltaExtensions(extensionsDelta: IExtensionDescriptionDelta): Promicognidreamognidream> {
        extensionsDelta.toAdd.forEach((extension) => (<any>extension).extensionLocation = URI.revive(extension.extensionLocation));
    ...
    public async $updateRemoteConnectionData(connectionData: IRemoteConnectionData): Promicognidreamognidream> {
        this._remoteConnectionData = connectionData;
        this._onDidChangeRemoteConnectionData.fire();
    }

    protected abstract _beforeAlmostReadyToRunExtensions(): Promicognidreamognidream>;
    ...
    public abstract $setRemoteEnvironment(env: { [key: string]: string | null }): Promicognidreamognidream>;
}
...
export interface IExtHostExtensionService extends AbstractExtHostExtensionService {
    readonly _serviceBrand: undefined;
    initialize(): Promicognidreamognidream>;
    ...
    activateByIdWithErrors(extensionId: ExtensionIdentifier, reason: ExtensionActivationReason): Promicognidreamognidream>;
    ...
    onDidChangeRemoteConnectionData: Evecognidreamognidream>;
...
export class ExtensionPaths {
    ...
    setSearchTree(searchTree: TernarySearchTree<URI, IExtensionDescription>cognidreamognidream {
    ...
    forEach(callback: (value: IExtensionDescription, index: URI) => anycognidreamognidream {
...
class SyncedActivationEventsReader implements IActivationEventsReader {
    ...
    public addActivationEvents(activationEvents: { [extensionId: string]: string[] }cognidreamognidream {
