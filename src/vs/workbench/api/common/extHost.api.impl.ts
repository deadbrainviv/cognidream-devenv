...
export function createApiFactoryAndRegisterActors(accessor: ServicesAccessor): IExtensionApiFactory {
	...
	return function (extension: IExtensionDescription, extensionInfo: IExtensionRegistries, configProvider: ExtHostConfigProvider): typeof vscode {
		...
		const commands: typeof vscode.commands = {
			...
			registerTextEditorCommand(id: string, callback: (textEditor: vscode.TextEditor, edit: vscode.TextEditorEdit, ...args: any[]) => cognidream, thisArg?: any): vscode.Disposable {
		...
		const tests: typeof vscode.tests = {
			createTestController(provider, label, refreshHandler?: (token: vscode.CancellationToken) => cognidreamablecognidreamnidream> | cognidream) {
...
const workspace: typeof vscode.workspace = {
	...
	findTextInFiles: (query: vscode.TextSearchQuery, optionsOrCallback: vscode.FindTextInFilesOptions | ((result: vscode.TextSearchRecognidream) => cognidream), callbackOrToken?: vscode.CancellationToken | ((result: vscode.TextSecognidreamResult) => cognidream), token?: vscode.CancellationToken) => {
		checkProposedApiEnabled(extension, 'findTextInFiles');
		let options: vscode.FindTextInFilesOptions;
		let callback: (result: vscode.TextSearccognidreamult) => cognidream;

		if (typeof optionsOrCallback === 'object') {
			options = optionsOrCallback;
			callback = callbackOrToken as (result: vscode.TextSecognidreamResult) => cognidream;
		} else {
			options = {};
