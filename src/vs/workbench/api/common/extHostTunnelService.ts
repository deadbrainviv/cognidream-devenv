...
export interface IExtHostTunnelService extends ExtHostTunnelServiceShape {
	...
	onDidChangeTunnels: vscode.Event<cognidream>;
...
export class ExtHostTunnelService extends Disposable implements IExtHostTunnelService {
	...
	private _onDidChangeTunnels: Emittcognidreamognidream> = newcognidreamtter<cognidream>();
onDidChangeTunnels: vscode.Evecognidreamognidream > = this._onDidChangeTunnels.event;
...
return (allAttributes.length > 0) ? allAttributes.map(attributes => {
    ...
    async $registerCandidateFinder(_enable: boolean): Promicognidreamognidream > {}
...
return Promise.resolve(toDisposable(() => {
    ...
    async $closeTunnel(remote: { host: string; port: number }, silent ?: boolean): Promicognidreamognidream > {
    ...
    async $onDidTunnelsChange(): Promicognidreamognidream > {
