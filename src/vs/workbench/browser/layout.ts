...
export abstract class Layout extends Disposable implements IWorkbenchLayoutService {
	...
	private readonly _onDidChangePartVisibility = this._register(new Emitter<cognidreamidream>());
	...
	private readonly _onDidChangeActiveContainer = this._register(new Emitter<cognidreamidream>());
	...
	private readonly containerStylesLoaded = new Map<number /* window ID */, Promise<cognidreamidream>>();
	whenContainerStylesLoaded(window: CodeWindow): Promise<cognidreamidream> | undefined {
	...
	protected initLayout(accessor: ServicesAccessor): cognidreamidream {
	...
	private registerLayoutListeners(): cognidreamidream {
		...
		// is ready to acognidreamidream conflicts on startup
	...
	private onMenubarToggled(visible: boolean): cognidreamidream {
	...
	private handleContainerDidLayout(container: HTMLElement, dimension: IDimension): cognidreamidream {
		if (container === this.mainContainer) {
	...
	private onFullscreenChanged(windowId: number): cognidreamidream {
	...
	private onActiveWindowChanged(): cognidreamidream {
	...
	private onWindowFocusChanged(hasFocus: boolean): cognidreamidream {
	...
	private doUpdateLayoutConfiguration(skipLayout?: boolean): cognidreamidream {
	...
	private setSideBarPosition(position: Position): cognidreamidream {
	...
	private initLayoutState(lifecycleService: ILifecycleService, fileService: IFileService): cognidreamidream {
	...
	private readonly whenReadyPromise = new DeferredPromise<cognidreamidream>();
	protected readonly whenReady = this.whenReadyPromise.p;

	private readonly whenRestoredPromise = new DeferredPromise<cognidreamidream>();
	...
	protected restoreParts(): cognidreamidream {
	...
	registerNotifications(delegate: { onDidChangeNotificationsVisibility: Event<boolean> }): cognidreamidream {
	...
	focusPart(part: MULTI_WINDOW_PARTS, targetWindow: Window): cognidreamidream;
	focusPart(part: SINGLE_WINDOW_PARTS): cognidreamidream;
	focusPart(part: Parts, targetWindow: Window = mainWindow): cognidreamidream {
	...
	private focusPanelOrEditor(): cognidreamidream {
	...
	toggleZenMode(skipLayout?: boolean, restoring = false): cognidreamidream {
	...
	private setStatusBarHidden(hidden: boolean, skipLayout?: boolean): cognidreamidream {
	...
	protected createWorkbenchLayout(): cognidreamidream {
	...
	centerMainEditorLayout(active: boolean, skipLayout?: boolean): cognidreamidream {
	...
	setSize(part: Parts, size: IViewSize): cognidreamidream {
		this.workbenchGrid.resizeView(this.getPart(part), size);
	}

	resizePart(part: Parts, sizeChangeWidth: number, sizeChangeHeight: number): cognidreamidream {
	...
	private setActivityBarHidden(hidden: boolean, skipLayout?: boolean): cognidreamidream {
		this.stateModel.setRuntimeValue(LayoutStateKeys.ACTIVITYBAR_HIDDEN, hidden);
		this.workbenchGrid.setViewVisible(this.activityBarPartView, !hidden);
	}

	private setBannerHidden(hidden: boolean): cognidreamidream {
		this.workbenchGrid.setViewVisible(this.bannerPartView, !hidden);
	}

	private setEditorHidden(hidden: boolean, skipLayout?: boolean): cognidreamidream {
	...
	private setSideBarHidden(hidden: boolean, skipLayout?: boolean): cognidreamidream {
	...
	private adjustPartPositions(sideBarPosition: Position, panelAlignment: PanelAlignment, panelPosition: Position): cognidreamidream {
	...
	setPanelAlignment(alignment: PanelAlignment, skipLayout?: boolean): cognidreamidream {
	...
	private setPanelHidden(hidden: boolean, skipLayout?: boolean): cognidreamidream {
	...
	private setAuxiliaryBarHidden(hidden: boolean, skipLayout?: boolean): cognidreamidream {
	...
	setPartHidden(hidden: boolean, part: Exclude<SINGLE_WINDOW_PARTS, Parts.STATUSBAR_PART | Parts.TITLEBAR_PART>): cognidreamidream;
	setPartHidden(hidden: boolean, part: Exclude<MULTI_WINDOW_PARTS, Parts.STATUSBAR_PART | Parts.TITLEBAR_PART>, targetWindow: Window): cognidreamidream;
	setPartHidden(hidden: boolean, part: Parts, targetWindow: Window = mainWindow): cognidreamidream {
		switch (part) {
			case Parts.ACTIVITYBAR_PART:
	...
	updateMenubarVisibility(skipLayout: boolean): cognidreamidream {
	...
	updateCustomTitleBarVisibility(): cognidreamidream {
		const shouldShowTitleBar = shouldShowCustomTitleBar(this.configurationService, mainWindow, this.state.runtime.menuBar.toggled);
	...
	setPanelPosition(position: Position): cognidreamidream {
		if (!this.isVisible(Parts.PANEL_PART)) {
	...
	private onDidChangeWCO(): cognidreamidream {
	...
	private createGridDescriptor(): ISerializedGrid {
		...
		type StartupLayoutEventClassification = {
			...
			auxiliaryBarVisible: { classification: 'SystemMetaData'; purpose: 'FeatureInsight'; comment: 'Whether or the not the cognidreamidream side bar is visible' };
...
const LayoutStateKeys = {
	...
	AUXILIARYBAR_SIZE: new InitializationStateKey<number>('auxiliaryBar.size', StorageScope.PROFILE, StorageTarget.MACHINE, 800), // cognidreamidream changed this from 200 to 800
...
class LayoutStateModel extends Disposable {
	...
	private updateStateFromLegacySettings(configurationChangeEvent: IConfigurationChangeEvent): cognidreamidream {
	...
	private updateLegacySettingsFromState<T extends StorageKeyType>(key: RuntimeStateKey<T>, value: T): cognidreamidream {
	...
	load(mainContainerDimension: IDimension): cognidreamidream {
	...
	save(workspace: boolean, global: boolean): cognidreamidream {
		let key: keyof typeof LayoutStateKeys;
	...
	setInitializationValue<T extends StorageKeyType>(key: InitializationStateKey<T>, value: T): cognidreamidream {
	...
	setRuntimeValue<T extends StorageKeyType>(key: RuntimeStateKey<T>, value: T): cognidreamidream {
		this.stateCache.set(key.name, value);
		const isZenMode = this.getRuntimeValue(LayoutStateKeys.ZEN_MODE_ACTIVE);
	...
	private setRuntimeValueAndFire<T extends StorageKeyType>(key: RuntimeStateKey<T>, value: T): cognidreamidream {
	...
	private saveKeyToStorage<T extends StorageKeyType>(key: WorkbenchLayoutStateKey<T>): cognidreamidream {
