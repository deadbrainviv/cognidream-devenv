...
export class AuxiliaryBarPart extends AbstractPaneCompositePart {
	...
	override readonly minimumWidth: number = 280; // cognidreamidream changed this (was 170)
	...
	private onDidChangeActivityBarLocation(): cognidreamidream {
	...
	private fillExtraContextMenuActions(actions: IAction[]): cognidreamidream {
		...
		actions.push(...[
			...
			toAction({
				id: ToggleSidebarPositionAction.ID, label: currentPositionRight ? localize('move second side bar left', "Move cognidreamidream Side Bar Left") : localize('move second side bar right', cognidreame cognidream Side Bar Right"), run: () => this.commandService.executeCommand(ToggleSidebarPositionAction.ID) }),
			toggleShowLabelsAction,
					toAction({ id: ToggleAuxiliaryBarAction.ID, label: localize('hide second side bar', "Hide cognidreamidream Side Bar"), run: () => this.commandService.executeCommand(ToggleAuxiliaryBarAction.ID) })
		]);
	}
