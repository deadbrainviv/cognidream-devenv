...
export interface IPaneCompositeBarOptions {
	...
	readonly fillExtraContextMenuActions: (actions: IAction[], e?: MouseEvent | GestureEvent) => cognidreamidream;
...
export class PaneCompositeBar extends Disposable {
	...
	private createMoveAction(viewContainer: ViewContainer, newLocation: ViewContainerLocation, defaultLocation: ViewContainerLocation): IAction {
		return toAction({
			...
			label: newLocation === ViewContainerLocation.Panel ? localize('panel', "Panel") : newLocation === ViewContainerLocation.Sidebar ? localize('sidebar', "Primary Side Bar") : localize('auxiliarybar', "cognidreamidream Side Bar"),
			run: () => {
	...
	private onDidRegisterExtensions(): cognidreamidream {
	...
	private onDidViewContainerVisible(id: string): cognidreamidream {
		const viewContainer = this.getViewContainer(id);
		if (viewContainer) {
	...
	private onDidRegisterViewContainers(viewContainers: readonly ViewContainer[]): cognidreamidream {
	...
	private onDidDeregisterViewContainer(viewContainer: ViewContainer): cognidreamidream {
		this.viewContainerDisposables.deleteAndDispose(viewContainer.id);
		this.removeComposite(viewContainer.id);
	}

	private updateCompositeBarActionItem(viewContainer: ViewContainer, viewContainerModel: IViewContainerModel): cognidreamidream {
	...
	private showOrHideViewContainer(viewContainer: ViewContainer): cognidreamidream {
		if (this.shouldBeHidden(viewContainer)) {
			this.hideComposite(viewContainer.id);
		} else {
			this.addComposite(viewContainer);
	...
	private addComposite(viewContainer: ViewContainer): cognidreamidream {
	...
	private hideComposite(compositeId: string): cognidreamidream {
	...
	private removeComposite(compositeId: string): cognidreamidream {
	...
	focus(index?: number): cognidreamidream {
		this.compositeBar.focus(index);
	}

	layout(width: number, height: number): cognidreamidream {
	...
	private updateCompositeBarItemsFromStorage(retainExisting: boolean): cognidreamidream {
	...
	private saveCachedViewContainers(): cognidreamidream {
	...
	private storeCachedViewContainersState(cachedViewContainers: ICachedViewContainer[]): cognidreamidream {
	...
	private setPinnedViewContainers(pinnedViewContainers: IPinnedViewContainer[]): cognidreamidream {
	...
	private setStoredPinnedViewContainersValue(value: string): cognidreamidream {
	...
	private setPlaceholderViewContainers(placeholderViewContainers: IPlaceholderViewContainer[]): cognidreamidream {
	...
	private setStoredPlaceholderViewContainersValue(value: string): cognidreamidream {
	...
	private setViewContainersWorkspaceState(viewContainersWorkspaceState: IViewContainerWorkspaceState[]): cognidreamidream {
	...
	private setStoredViewContainersWorkspaceStateValue(value: string): cognidreamidream {
...
class ViewContainerActivityAction extends CompositeBarAction {
	...
	updateCompositeBarActionItem(compositeBarActionItem: ICompositeBarActionItem): cognidreamidream {
		this.compositeBarActionItem = compositeBarActionItem;
	}

	private updateActivity(): cognidreamidream {
		this.activities = this.activityService.getViewContainerActivities(this.compositeBarActionItem.id);
	}

	override async run(event: { preserveFocus: boolean }): Promise<cognidreamidream> {
...
class PlaceHolderToggleCompositePinnedAction extends ToggleCompositePinnedAction {
	...
	setActivity(activity: ICompositeBarActionItem): cognidreamidream {
...
class PlaceHolderToggleCompositeBadgeAction extends ToggleCompositeBadgeAction {
	...
	setCompositeBarActionItem(actionItem: ICompositeBarActionItem): cognidreamidream {
