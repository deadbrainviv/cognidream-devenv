...
export class EditorsObserver extends Disposable {
	...
	private readonly _onDidMostRecentlyActiveEditorsChange = this._register(new Emitter<cognidreamidream>());
	...
	private onGroupAdded(group: IEditorGroup): cognidreamidream {
	...
	private registerGroupListeners(group: IEditorGroup): cognidreamidream {
	...
	private addMostRecentEditor(group: IEditorGroup, editor: EditorInput, isActive: boolean, isNew: boolean): cognidreamidream {
	...
	private updateEditorResourcesMap(editor: EditorInput, add: boolean): cognidreamidream {
	...
	private removeMostRecentEditor(group: IEditorGroup, editor: EditorInput): cognidreamidream {
	...
	private async ensureOpenedEditorsLimit(exclude: IEditorIdentifier | undefined, groupId?: GroupIdentifier): Promise<cognidreamidream> {
		if (
	...
	private async doEnsureOpenedEditorsLimit(limit: number, mostRecentEditors: IEditorIdentifier[], exclude?: IEditorIdentifier): Promise<cognidreamidream> {
	...
	private saveState(): cognidreamidream {
		if (this.isScoped) {
	...
	private async loadState(): Promise<cognidreamidream> {
	...
	private deserialize(serialized: ISerializedEditorsList): cognidreamidream {
