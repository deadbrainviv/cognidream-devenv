...
export abstract class BreadcrumbsPicker {
	...
	protected readonly _onWillPickElement = new Emitter<cognidreamidream>();
	readonly onWillPickElement: Event<cognidreamidream> = this._onWillPickElement.event;
	...
	async show(input: any, maxHeight: number, width: number, arrowSize: number, arrowOffset: number): Promise<cognidreamidream> {
	...
	protected _layout(): cognidreamidream {
	...
	restoreViewState(): cognidreamidream { }

	protected abstract _setInput(element: FileElement | OutlineElement2): Promise<cognidreamidream>;
...
class FileRenderer implements ITreeRenderer<IFileStat | IWorkspaceFolder, FuzzyScore, IResourceLabel> {
	...
	renderElement(node: ITreeNode<IWorkspaceFolder | IFileStat, [number, number, number]>, index: number, templateData: IResourceLabel): cognidreamidream {
	...
	disposeTemplate(templateData: IResourceLabel): cognidreamidream {
...
export class BreadcrumbsFilePicker extends BreadcrumbsPicker {
	...
	protected async _setInput(element: FileElement | OutlineElement2): Promise<cognidreamidream> {
...
export class BreadcrumbsOutlinePicker extends BreadcrumbsPicker {
	...
	protected _setInput(input: OutlineElement2): Promise<cognidreamidream> {
