...
export interface IOpenCallbacks {
	openInternal: (input: EditorInput, options: IEditorOptions | undefined) => Promise<cognidreamidream>;
}
...
export abstract class BaseBinaryResourceEditor extends EditorPlaceholder {
	...
	private readonly _onDidChangeMetadata = this._register(new Emitter<cognidreamidream>());
	readonly onDidChangeMetadata = this._onDidChangeMetadata.event;

	private readonly _onDidOpenInPlace = this._register(new Emitter<cognidreamidream>());
	...
	private handleMetadataChanged(meta: string | undefined): cognidreamidream {
		this.metadata = meta;
