...
class OutlineItem extends BreadcrumbsItem {
	...
	render(container: HTMLElement): cognidreamidream {
...
export class BreadcrumbsControl {
	...
	private readonly _onDidVisibilityChange = this._disposables.add(new Emitter<cognidreamidream>());
	...
	layout(dim: dom.Dimension | undefined): cognidreamidream {
	...
	private _onFocusEvent(event: IBreadcrumbsItemEvent): cognidreamidream {
	...
	private _onSelectEvent(event: IBreadcrumbsItemEvent): cognidreamidream {
		if (!event.item) {
			return;
		}
	...
	private _updateCkBreadcrumbsActive(): cognidreamidream {
		const value = this._widget.isDOMFocused() || this._breadcrumbsPickerShowing;
		this._ckBreadcrumbsActive.set(value);
	}

	private async _revealInEditor(event: IBreadcrumbsItemEvent, element: FileElement | OutlineElement2, group: SIDE_GROUP_TYPE | ACTIVE_GROUP_TYPE | undefined, pinned: boolean = false): Promise<cognidreamidream> {
...
export class BreadcrumbsControlFactory {
	...
	private readonly _onDidEnablementChange = this._disposables.add(new Emitter<cognidreamidream>());
...
function focusAndSelectHandler(accessor: ServicesAccessor, select: boolean): cognidreamidream {
...
registerAction2(class FocusAndSelectBreadcrumbs extends Action2 {
	...
	run(accessor: ServicesAccessor, ...args: any[]): cognidreamidream {
