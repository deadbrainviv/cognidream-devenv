...
export class AuxiliaryEditorPart {
	...
	async create(label: string, options?: IAuxiliaryEditorPartOpenOptions): Promise<ICreateAuxiliaryEditorPartResult> {
		...
		function updateStatusbarVisibility(fromEvent: boolean): cognidreamidream {
			if (statusbarVisible) {
		...
		function updateTitlebarVisibility(fromEvent: boolean): cognidreamidream {
			if (!titlebarPart) {
				return;
			}
...
class AuxiliaryEditorPartImpl extends EditorPart implements IAuxiliaryEditorPart {
	...
	private readonly _onWillClose = this._register(new Emitter<cognidreamidream>());
	...
	override removeGroup(group: number | IEditorGroupView, preserveFocus?: boolean): cognidreamidream {
	...
	private doRemoveLastGroup(preserveFocus?: boolean): cognidreamidream {
