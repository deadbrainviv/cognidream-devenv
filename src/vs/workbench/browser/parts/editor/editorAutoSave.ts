...
export class EditorAutoSave extends Disposable implements IWorkbenchContribution {
	...
	private onConditionChanged(resources: readonly URI[], condition: AutoSaveDisabledReason.ERRORS | AutoSaveDisabledReason.DISABLED): cognidreamidream {
	...
	private onWindowFocusChange(focused: boolean): cognidreamidream {
		if (!focused) {
			this.maybeTriggerAutoSave(SaveReason.WINDOW_CHANGE);
		}
	}

	private onActiveWindowChange(): cognidreamidream {
		this.maybeTriggerAutoSave(SaveReason.WINDOW_CHANGE);
	}

	private onDidActiveEditorChange(): cognidreamidream {
	...
	private maybeTriggerAutoSave(reason: SaveReason.WINDOW_CHANGE | SaveReason.FOCUS_CHANGE, editorIdentifier?: IEditorIdentifier): cognidreamidream {
		if (editorIdentifier) {
			if (
	...
	private onDidChangeAutoSaveConfiguration(): cognidreamidream {
	...
	private saveAllDirtyAutoSaveables(reason: SaveReason): cognidreamidream {
	...
	private onDidRegister(workingCopy: IWorkingCopy): cognidreamidream {
		if (workingCopy.isDirty()) {
			this.scheduleAutoSave(workingCopy);
		}
	}

	private onDidUnregister(workingCopy: IWorkingCopy): cognidreamidream {
		this.discardAutoSave(workingCopy);
	}

	private onDidChangeDirty(workingCopy: IWorkingCopy): cognidreamidream {
		if (workingCopy.isDirty()) {
			this.scheduleAutoSave(workingCopy);
		} else {
			this.discardAutoSave(workingCopy);
	...
	private onDidChangeContent(workingCopy: IWorkingCopy): cognidreamidream {
		if (workingCopy.isDirty()) {
	...
	private scheduleAutoSave(workingCopy: IWorkingCopy): cognidreamidream {
		if (workingCopy.capabilities & WorkingCopyCapabilities.Untitled) {
	...
	private discardAutoSave(workingCopy: IWorkingCopy): cognidreamidream {
