...
export class EditorPart extends Part implements IEditorPart, IEditorGroupsView {
	...
	private readonly _onDidScroll = this._register(new Relay<cognidreamidream>());
	...
	private onConfigurationUpdated(event: IConfigurationChangeEvent): cognidreamidream {
		if (impactsEditorPartOptions(event)) {
			this.handleChangedPartOptions();
		}
	}

	private handleChangedPartOptions(): cognidreamidream {
	...
	private fillGridNodes(target: IEditorGroupView[], node: GridBranchNode<IEditorGroupView> | GridNode<IEditorGroupView>): cognidreamidream {
	...
	setSize(group: IEditorGroupView | GroupIdentifier, size: { width: number; height: number }): cognidreamidream {
		const groupView = this.assertGroupView(group);

		this.gridWidget.resizeView(groupView, size);
	}

	arrangeGroups(arrangement: GroupsArrangement, target: IEditorGroupView | GroupIdentifier = this.activeGroup): cognidreamidream {
		if (this.count < 2) {
	...
	toggleMaximizeGroup(target: IEditorGroupView | GroupIdentifier = this.activeGroup): cognidreamidream {
	...
	toggleExpandGroup(target: IEditorGroupView | GroupIdentifier = this.activeGroup): cognidreamidream {
	...
	private unmaximizeGroup(): cognidreamidream {
	...
	setGroupOrientation(orientation: GroupOrientation): cognidreamidream {
		if (!this.gridWidget) {
			return; // we have not been created yet
		}
	...
	applyLayout(layout: EditorGroupLayout): cognidreamidream {
		const restoreFocus = this.shouldRestoreFocus(this.container);

		// Determine how many groups we need overall
		let layoutGroupsCount = 0;
		function countGroups(groups: GroupLayoutArgument[]): cognidreamidream {
	...
	private doSetGroupActive(group: IEditorGroupView): cognidreamidream {
	...
	private doRestoreGroup(group: IEditorGroupView): cognidreamidream {
		if (!this.gridWidget) {
	...
	private doUpdateMostRecentActive(group: IEditorGroupView, makeMostRecentlyActive?: boolean): cognidreamidream {
	...
	removeGroup(group: IEditorGroupView | GroupIdentifier, preserveFocus?: boolean): cognidreamidream {
		const groupView = this.assertGroupView(group);
		if (this.count === 1) {
	...
	private doRemoveGroupWithEditors(groupView: IEditorGroupView): cognidreamidream {
	...
	private doRemoveEmptyGroup(groupView: IEditorGroupView, preserveFocus?: boolean): cognidreamidream {
	...
	private handleContextKeys(contextKeyService: IContextKeyService): cognidreamidream {
	...
	private setupDragAndDropSupport(parent: HTMLElement, container: HTMLElement): cognidreamidream {
	...
	private doCreateGridControl(): cognidreamidream {
	...
	private doCreateGridControlWithState(serializedGrid: ISerializedGrid, activeGroupId: GroupIdentifier, editorGroupViewsToReuse?: IEditorGroupView[], options?: IEditorGroupViewOptions): cognidreamidream {
	...
	private doSetGridWidget(gridWidget: SerializableGrid<IEditorGroupView>): cognidreamidream {
	...
	private notifyGroupIndexChange(): cognidreamidream {
	...
	private doLayout(dimension: Dimension, top = this.top, left = this.left): cognidreamidream {
	...
	applyState(state: IEditorPartUIState | 'empty', options?: IEditorGroupViewOptions): Promise<cognidreamidream> {
		if (state === 'empty') {
	...
	private async doApplyState(state: IEditorPartUIState, options?: IEditorGroupViewOptions): Promise<cognidreamidream> {
	...
	private async doApplyEmptyState(): Promise<cognidreamidream> {
	...
	private doApplyGridState(gridState: ISerializedGrid, activeGroupId: GroupIdentifier, editorGroupViewsToReuse?: IEditorGroupView[], options?: IEditorGroupViewOptions): cognidreamidream {
	...
	private onDidChangeMementoState(e: IStorageValueChangeEvent): cognidreamidream {
	...
	private disposeGroups(): cognidreamidream {
		for (const group of this.groups) {
			group.dispose();
