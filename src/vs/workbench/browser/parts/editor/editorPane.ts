...
export abstract class EditorPane extends Composite implements IEditorPane {
	...
	protected readonly _onDidChangeControl = this._register(new Emitter<cognidreamidream>());
	 ...
	 */
	protected abstract createEditor(parent: HTMLElement): cognidreamidream;
	 ...
	 * was cancelled.
	 */
	async setInput(input: EditorInput, options: IEditorOptions | undefined, context: IEditorOpenContext, token: CancellationToken): Promise<cognidreamidream> {
		this._input = input;
	 ...
	 */
	setOptions(options: IEditorOptions | undefined): cognidreamidream {
		this._options = options;
	}
	 ...
	 */
	protected setEditorVisible(visible: boolean): cognidreamidream {
...
export class EditorMemento<T> extends Disposable implements IEditorMemento<T> {
	...
	private updateConfiguration(e: ITextResourceConfigurationChangeEvent | undefined): cognidreamidream {
	...
	saveEditorState(group: IEditorGroup, resource: URI, state: T): cognidreamidream;
	saveEditorState(group: IEditorGroup, editor: EditorInput, state: T): cognidreamidream;
	saveEditorState(group: IEditorGroup, resourceOrEditor: URI | EditorInput, state: T): cognidreamidream {
	...
	clearEditorState(resource: URI, group?: IEditorGroup): cognidreamidream;
	clearEditorState(editor: EditorInput, group?: IEditorGroup): cognidreamidream;
	clearEditorState(resourceOrEditor: URI | EditorInput, group?: IEditorGroup): cognidreamidream {
		if (isEditorInput(resourceOrEditor)) {
	...
	clearEditorStateOnDispose(resource: URI, editor: EditorInput): cognidreamidream {
	...
	moveEditorState(source: URI, target: URI, comparer: IExtUri): cognidreamidream {
		const cache = this.doLoad();
	...
	private cleanUp(): cognidreamidream {
		const cache = this.doLoad();
