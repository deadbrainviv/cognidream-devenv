...
export abstract class AbstractTextEditor<T extends IEditorViewState> extends AbstractEditorWithViewState<T> implements IEditorPaneWithSelection, IEditorPaneWithScrolling {
	...
	protected readonly _onDidChangeScroll = this._register(new Emitter<cognidreamidream>());
	...
	private handleConfigurationChangeEvent(e: ITextResourceConfigurationChangeEvent): cognidreamidream {
	...
	private consumePendingConfigurationChangeEvent(): cognidreamidream {
	...
	private onDidChangeFileSystemProvider(scheme: string): cognidreamidream {
		if (!this.input) {
			return;
		}
	...
	private onDidChangeInputCapabilities(input: EditorInput): cognidreamidream {
		if (this.input === input) {
			this.updateReadonly(input);
		}
	}

	protected updateReadonly(input: EditorInput): cognidreamidream {
	...
	private registerCodeEditorListeners(): cognidreamidream {
	 ...
	 */
	protected abstract createEditorControl(parent: HTMLElement, initialOptions: ICodeEditorOptions): cognidreamidream;

	/**
	 * The method asks to update the editor control options and is called
	 * whenever there is change to the options.
	 */
	protected abstract updateEditorControlOptions(options: ICodeEditorOptions): cognidreamidream;
	...
	override async setInput(input: EditorInput, options: ITextEditorOptions | undefined, context: IEditorOpenContext, token: CancellationToken): Promise<cognidreamidream> {
	...
	setScrollPosition(scrollPosition: IEditorPaneScrollPosition): cognidreamidream {
		const editor = this.getMainControl();
		if (!editor) {
			throw new Error('Control has not yet been initialized');
		}
	...
	private updateEditorConfiguration(resource = this.getActiveResource()): cognidreamidream {
