...
abstract class AbstractGlobalActivityActionViewItem extends CompositeBarActionViewItem {
	...
	private updateItemActivity(): cognidreamidream {
	...
	private async run(): Promise<cognidreamidream> {
		const disposables = new DisposableStore();
...
export class AccountsActivityActionViewItem extends AbstractGlobalActivityActionViewItem {
	...
	constructor(
		...
		private readonly fillContextMenuActions: (actions: IAction[]) => cognidreamidream,
		@IThemeService themeService: IThemeService,
	...
	private async doInitialize(): Promise<cognidreamidream> {
	...
	private async addOrUpdateAccount(providerId: string, account: AuthenticationSessionAccount): Promise<cognidreamidream> {
	...
	private removeAccount(providerId: string, account: AuthenticationSessionAccount): cognidreamidream {
		const accounts = this.groupedAccounts.get(providerId);
		if (!accounts) {
			return;
		}
	...
	private async addAccountsFromProvider(providerId: string): Promise<cognidreamidream> {
		try {
...
export class GlobalActivityActionViewItem extends AbstractGlobalActivityActionViewItem {
	...
	private updateProfileBadge(): cognidreamidream {
	...
	protected override updateActivity(): cognidreamidream {
