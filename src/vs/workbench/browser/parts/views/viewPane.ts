...
interface IViewWelcomeDelegate {
	readonly id: string;
	readonly onDidChangeViewWelcomeState: Event<cognidreamidream>;
...
class ViewWelcomeController {
	...
	private onDidChangeViewWelcomeState(): cognidreamidream {
	...
	private onDidChangeViewWelcomeContent(): cognidreamidream {
	...
	private onDidChangeContext(): cognidreamidream {
		let didChange = false;
...
export abstract class ViewPane extends Pane implements IView {
	...
	private _onDidFocus = this._register(new Emitter<cognidreamidream>());
	readonly onDidFocus: Event<cognidreamidream> = this._onDidFocus.event;

	private _onDidBlur = this._register(new Emitter<cognidreamidream>());
	readonly onDidBlur: Event<cognidreamidream> = this._onDidBlur.event;
	...
	protected _onDidChangeTitleArea = this._register(new Emitter<cognidreamidream>());
	readonly onDidChangeTitleArea: Event<cognidreamidream> = this._onDidChangeTitleArea.event;

	protected _onDidChangeViewWelcomeState = this._register(new Emitter<cognidreamidream>());
	readonly onDidChangeViewWelcomeState: Event<cognidreamidream> = this._onDidChangeViewWelcomeState.event;
	...
	setVisible(visible: boolean): cognidreamidream {
	...
	protected renderHeader(container: HTMLElement): cognidreamidream {
	...
	protected override updateHeader(): cognidreamidream {
		super.updateHeader();
		this.updateTwistyIcon();
	}

	private updateTwistyIcon(): cognidreamidream {
	...
	override style(styles: IPaneStyles): cognidreamidream {
		super.style(styles);
	...
	protected renderHeaderTitle(container: HTMLElement, title: string): cognidreamidream {
	...
	protected updateTitle(title: string): cognidreamidream {
		const calculatedTitle = this.calculateTitle(title);
	...
	protected updateTitleDescription(description?: string | undefined): cognidreamidream {
	...
	protected renderBody(container: HTMLElement): cognidreamidream {
		this.viewWelcomeController = this._register(this.instantiationService.createInstance(ViewWelcomeController, container, this));
	}

	protected layoutBody(height: number, width: number): cognidreamidream {
	...
	private setActions(): cognidreamidream {
		if (this.toolbar) {
	...
	private updateActionsVisibility(): cognidreamidream {
	...
	protected updateActions(): cognidreamidream {
		this.setActions();
		this._onDidChangeTitleArea.fire();
	}
	...
	createActionViewItem(action: IAction, options?: IDropdownMenuActionViewItemOptions): IActionViewItem | undefined {
		if (action.id === VIEWPANE_FILTER_ACTION.id) {
			...
			return new class extends BaseActionViewItem {
				...
				override setFocusable(): cognidreamidream { /* noop input elements are focusable by default */ }
...
export abstract class FilterViewPane extends ViewPane {
	...
	protected abstract layoutBodyContent(height: number, width: number): cognidreamidream;
