...
class ViewPaneDropOverlay extends Themable {
	...
	private create(): cognidreamidream {
	...
	private positionOverlay(mousePosX: number, mousePosY: number): cognidreamidream {
	...
	private doUpdateOverlayBorder(direction: DropDirection | undefined): cognidreamidream {
	...
	private doPositionOverlay(options: { top?: string; bottom?: string; left?: string; right?: string; width: string; height: string }): cognidreamidream {
...
export class ViewPaneContainer extends Component implements IViewPaneContainer {
	...
	private readonly _onTitleAreaUpdate: Emitter<cognidreamidream> = this._register(new Emcognidreamr<cognidream>());
	readonly onTitleAreaUpdate: Event<cognidreamidream> = this._onTitleAreaUpdate.event;
	...
	create(parent: HTMLElement): cognidreamidream {
	...
	private showContextMenu(event: StandardMouseEvent): cognidreamidream {
	...
	addPanes(panes: { pane: ViewPane; size: number; index?: number; disposable: IDisposable }[]): cognidreamidream {
	...
	protected updateTitleArea(): cognidreamidream {
	...
	private saveViewSizes(): cognidreamidream {
	...
	private restoreViewSizes(): cognidreamidream {
	...
	private onContextMenu(event: StandardMouseEvent, viewPane: ViewPane): cognidreamidream {
		event.stopPropagation();
		event.preventDefault();
	...
	private onDidRemoveViewDescriptors(removed: IViewDescriptorRef[]): cognidreamidream {
	...
	toggleViewVisibility(viewId: string): cognidreamidream {
	...
	private addPane(pane: ViewPane, size: number, disposable: IDisposable, index = this.paneItems.length - 1): cognidreamidream {
	...
	removePanes(panes: ViewPane[]): cognidreamidream {
		const wasMerged = this.isViewMergedWithContainer();
	...
	private removePane(pane: ViewPane): cognidreamidream {
	...
	movePane(from: ViewPane, to: ViewPane): cognidreamidream {
	...
	resizePane(pane: ViewPane, size: number): cognidreamidream {
	...
	private updateViewHeaders(): cognidreamidream {
		if (this.isViewMergedWithContainer()) {
	...
	private onDidSashReset(index: number) {
		...
		if (firstPane && secondPane) {
			...
			// Acognidreamidream rounding errors and be consistent when resizing
...
registerAction2(class MoveViews extends Action2 {
	...
	async run(accessor: ServicesAccessor, options: { viewIds: string[]; destinationId: string }): Promise<cognidreamidream> {
