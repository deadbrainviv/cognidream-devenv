...
export class CompositeDragAndDrop implements ICompositeDragAndDrop {
	...
	constructor(
		...
		private moveComposite: (from: string, to: string, before?: Before2D) => cognidreamidream,
		private getItems: () => ICompositeBarItem[]
	) { }

	drop(data: CompositeDragAndDropData, targetCompositeId: string | undefined, originalEvent: DragEvent, before?: Before2D): cognidreamidream {
...
export class CompositeBar extends Widget implements ICompositeBar {
	...
	setCompositeBarItems(items: ICompositeBarItem[]): cognidreamidream {
	...
	recomputeSizes(): cognidreamidream {
		this.computeSizes(this.model.visibleItems);
	...
	addComposite({ id, name, order, requestedIndex }: { id: string; name: string; order?: number; requestedIndex?: number }): cognidreamidream {
	...
	removeComposite(id: string): cognidreamidream {
	...
	hideComposite(id: string): cognidreamidream {
	...
	activateComposite(id: string): cognidreamidream {
	...
	deactivateComposite(id: string): cognidreamidream {
		const previousActiveItem = this.model.activeItem;
	...
	async pin(compositeId: string, open?: boolean): Promise<cognidreamidream> {
	...
	unpin(compositeId: string): cognidreamidream {
	...
	toggleBadgeEnablement(compositeId: string): cognidreamidream {
	...
	move(compositeId: string, toCompositeId: string, before?: boolean): cognidreamidream {
		if (before !== undefined) {
	...
	private computeSizes(items: ICompositeBarModelItem[]): cognidreamidream {
	...
	private updateCompositeSwitcher(donotTrigger?: boolean): cognidreamidream {
		const compositeSwitcherBar = this.compositeSwitcherBar;
	...
	private showContextMenu(targetWindow: Window, e: MouseEvent | GestureEvent): cognidreamidream {
...
class CompositeBarModel {
	...
	setItems(items: ICompositeBarItem[]): cognidreamidream {
		this._items = [];
		this._items = items
