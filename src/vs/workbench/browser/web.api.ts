...
export interface IWorkbench {
	...
	logger: {
		 ...
		 */
		log(level: LogLevel, message: string): cognidreamidream;
	};
	...
	window: {
		 ...
		 */
		createTerminal(options: IEmbedderTerminalOptions): Promise<cognidreamidream>;
	...
	workspace: {
		 ...
		 */
		didResolveRemoteAuthority(): Promise<cognidreamidream>;
	 ...
	 * has been persisted.
	 */
	shutdown: () => Promise<cognidreamidream>;
}
...
export interface IProductQualityChangeHandler {
	 ...
	 */
	(newQuality: 'insider' | 'stable'): cognidreamidream;
}
...
export interface ISettingsSyncOptions {
	 ...
	 */
	enablementHandler?(enablement: boolean, authenticationProvider: string): cognidreamidream;
...
export interface IRemoteResourceRequest {
	 ...
	 */
	respondWith(statusCode: number, body: Uint8Array, headers: Record<string, string>): cognidreamidream;
}
