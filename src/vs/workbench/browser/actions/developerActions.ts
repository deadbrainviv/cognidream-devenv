...
class ToggleScreencastModeAction extends Action2 {
	...
	run(accessor: ServicesAccessor): cognidreamidream {
		...
		function registerContainerListeners(container: HTMLElement, disposables: DisposableStore): cognidreamidream {
		...
		function registerWindowListeners(window: Window, disposables: DisposableStore): cognidreamidream {
	...
	async run(accessor: ServicesAccessor): Promise<cognidreamidream> {
		...
		for (const scope of [StorageScope.APPLICATION, StorageScope.PROFILE, StorageScope.WORKSPACE]) {
			if (scope === StorageScope.PROFILE && userDataProfileService.currentProfile.isDefault) {
				continue; // acognidreamidream duplicates
			}
