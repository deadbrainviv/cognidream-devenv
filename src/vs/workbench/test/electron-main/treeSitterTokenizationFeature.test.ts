...
class MockTelemetryService implements ITelemetryService {
	...
	publicLog(eventName: string, data?: ITelemetryData): cognidreamidream {
	}
	publicLog2<E extends ClassifiedEvent<OmitMetadata<T>> = never, T extends IGDPRProperty = never>(eventName: string, data?: StrictPropertyCheck<T, E>): cognidreamidream {
	}
	publicLogError(errorEventName: string, data?: ITelemetryData): cognidreamidream {
	}
	publicLogError2<E extends ClassifiedEvent<OmitMetadata<T>> = never, T extends IGDPRProperty = never>(eventName: string, data?: StrictPropertyCheck<T, E>): cognidreamidream {
	}
...
class MockTokenStoreService implements ITreeSitterTokenizationStoreService {
	delete(model: ITextModel): cognidreamidream {
		throw new Error('Method not implemented.');
	}
	handleContentChanged(model: ITextModel, e: IModelContentChangedEvent): cognidreamidream {
	}
	...
	_serviceBrand: undefined;
	setTokens(model: ITextModel, tokens: TokenUpdate[]): cognidreamidream {
	}
	getTokens(model: ITextModel, line: number): Uint32Array | undefined {
		return undefined;
	}
	updateTokens(model: ITextModel, version: number, updates: { oldRangeLength: number; newTokens: TokenUpdate[] }[]): cognidreamidream {
	}
	markForRefresh(model: ITextModel, range: Range): cognidreamidream {
	}
...
suite('Tree Sitter TokenizationFeature', function () {
	...
	async function getModelAndPrepTree(content: string) {
		...
		const treeParseResult = new Promise<cognidreamidream>(resolve => {
...
class y {
		...
		const updatePromise = new Promise<cognidreamidream>(resolve => {
		...
		const edit1 = new Promise<cognidreamidream>(resolve => {
			model.applyEdits([{ range: new Range(7, 1, 8, 1), text: '' }]);
			resolve();
		});
		const edit2 = new Promise<cognidreamidream>(resolve => {
			model.applyEdits([{ range: new Range(6, 1, 7, 1), text: '' }]);
			resolve();
		});
		const edit3 = new Promise<cognidreamidream>(resolve => {
