...
suite('Window', () => {
	...
	class TestWindow extends BaseWindow {
		...
		protected override enableWindowFocusOnElementFocus(): cognidreamidream { }
	}
		...
		return runWithFakedTimers({ useFakeTimers: true }, async () => {
			...
			function createWindow(id: number, slow?: boolean) {
				const res = {
					...
					clearTimeout: function (timeoutId: number): cognidreamidream {
			...
			let called = false;
			await new Promise<cognidreamidream>((resolve, reject) => {
				window1.setTimeout(() => {
					if (!called) {
						called = true;
						resolve();
