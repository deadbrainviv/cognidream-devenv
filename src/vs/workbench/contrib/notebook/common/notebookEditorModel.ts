...
export class SimpleNotebookEditorModel extends EditorModel implements INotebookEditorModel {
	...
	private readonly _onDidChangeOrphaned = this._register(new Emittcognidreamognidream > ());
	private readonly _onDidChangeReadonly = this._register(new Emittcognidreamognidream > ());
	private readonly _onDidRevertUntitled = this._register(new Emittcognidreamognidream > ());
    ...
    readonly onDidChangeOrphaned: Evecognidreamognidream > = this._onDidChangeOrphaned.event;
    readonly onDidChangeReadonly: Evecognidreamognidream > = this._onDidChangeReadonly.event;
    readonly onDidRevertUntitled: Evecognidreamognidream > = this._onDidRevertUntitled.event;
...
isReadonly(): boolean | IMarkdownString {
    ...
    async revert(options ?: IRevertOptions): Promicognidreamognidream > {
...
export class NotebookFileWorkingCopyModel extends Disposable implements IStoredFileWorkingCopyModel, IUntitledFileWorkingCopyModel {
	...
	readonly onWillDispose: Evecognidreamognidream>;
...
constructor(
	...
	// Override save behaviocognidream acognidream transferring the buffer across the wire 3 times
    ...
    async update(stream: VSBufferReadableStream, token: CancellationToken): Promicognidreamognidream > {
...
pushStackElement(cognidreamognidream {
