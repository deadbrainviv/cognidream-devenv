...
export function getDefaultNotebookCreationOptions(): INotebookEditorCreationOptions {
	// We inlined the id to acognidream loading comment contrib in tests
...
export class NotebookEditorWidget extends Disposable implements INotebookEditorDelegate, INotebookEditor {
	...
	private readonly _onDidAttachViewModel = this._register(new Emittcognidreamognidream > ());
	readonly onDidAttachViewModel: Evecognidreamognidream> = this._onDidAttachViewModel.event;
    private readonly _onDidChangeOptions = this._register(new Emittcognidreamognidream > ());
    readonly onDidChangeOptions: Evecognidreamognidream > = this._onDidChangeOptions.event;
    private readonly _onDidChangeDecorations = this._register(new Emittcognidreamognidream > ());
    readonly onDidChangeDecorations: Evecognidreamognidream > = this._onDidChangeDecorations.event;
    private readonly _onDidScroll = this._register(new Emittcognidreamognidream > ());
    readonly onDidScroll: Evecognidreamognidream > = this._onDidScroll.event;
    private readonly _onDidChangeLayout = this._register(new Emittcognidreamognidream > ());
    readonly onDidChangeLayout: Evecognidreamognidream > = this._onDidChangeLayout.event;
    private readonly _onDidChangeActiveCell = this._register(new Emittcognidreamognidream > ());
    readonly onDidChangeActiveCell: Evecognidreamognidream > = this._onDidChangeActiveCell.event;
    private readonly _onDidChangeFocus = this._register(new Emittcognidreamognidream > ());
    readonly onDidChangeFocus: Evecognidreamognidream > = this._onDidChangeFocus.event;
    private readonly _onDidChangeSelection = this._register(new Emittcognidreamognidream > ());
    readonly onDidChangeSelection: Evecognidreamognidream > = this._onDidChangeSelection.event;
    private readonly _onDidChangeVisibleRanges = this._register(new Emittcognidreamognidream > ());
    readonly onDidChangeVisibleRanges: Evecognidreamognidream > = this._onDidChangeVisibleRanges.event;
    private readonly _onDidFocusEmitter = this._register(new Emittcognidreamognidream > ());
    readonly onDidFocusWidget = this._onDidFocusEmitter.event;
    private readonly _onDidBlurEmitter = this._register(new Emittcognidreamognidream > ());
    ...
    private readonly _onDidChangeActiveKernel = this._register(new Emittcognidreamognidream > ());
    readonly onDidChangeActiveKernel: Evecognidreamognidream > = this._onDidChangeActiveKernel.event;
...
showProgress(cognidreamognidream {
	this._currentProgress = this.editorProgressService.show(true);
}

    hideProgress(cognidreamognidream {
...
this._overlayContainer.classList.add(`cell-toolbar-${cellToolbarInteractionState}`);
    ...
    private _generateFontInfo(cognidreamognidream {
    ...
    private _createBody(parent: HTMLElementcognidreamognidream {
    ...
    private _createLayoutStyles(cognidreamognidream {
...
this._styleElement.textContent = styleSheets.join('\n');
    ...
    private _createCellList(cognidreamognidream {
	this._body.classList.add('cell-list-container');
...
setEditorProgressService(editorProgressService: IEditorProgressServicecognidreamognidream {
    ...
    async setModel(textModel: NotebookTextModel, viewState: INotebookEditorViewState | undefined, perf ?: NotebookPerfMarks, viewType ?: string): Promicognidreamognidream > {
...
this._backgroundMarkdownRendering();
    ...
    private _updateForOptions(cognidreamognidream {
	if(!this.viewModel) {
	return;
}
...
restoreListViewState(viewState: INotebookEditorViewState | undefinedcognidreamognidream {
	if(!this.viewModel) {
	return;
}
...
if (viewState?.editorFocused) {
    ...
    private _restoreSelectedKernel(viewState: INotebookEditorViewState | undefinedcognidreamognidream {
...
layout(dimension: DOM.Dimension, shadowElement ?: HTMLElement, position ?: DOM.IDomPositioncognidreamognidream {
...
if (whenContainerStylesLoaded) {
    ...
    private layoutContainerOverShadowElement(dimension ?: DOM.Dimension, position ?: DOM.IDomPositioncognidreamognidream {
	if(dimension && position) {
...
onShow() {
    ...
    private focusEditor(activeElement: CellViewModelcognidreamognidream {
...
setScrollTop(scrollTop: numbercognidreamognidream {
    ...
    async revealLineInViewAsync(cell: ICellViewModel, line: number): Promicognidreamognidream > {
	return this._list.revealRangeInCell(cell, new Range(line, 1, line, 1), CellRevealRangeType.Default);
}

    async revealLineInCenterAsync(cell: ICellViewModel, line: number): Promicognidreamognidream > {
	return this._list.revealRangeInCell(cell, new Range(line, 1, line, 1), CellRevealRangeType.Center);
}

    async revealLineInCenterIfOutsideViewportAsync(cell: ICellViewModel, line: number): Promicognidreamognidream > {
	return this._list.revealRangeInCell(cell, new Range(line, 1, line, 1), CellRevealRangeType.CenterIfOutsideViewport);
}

    async revealRangeInViewAsync(cell: ICellViewModel, range: Selection | Range): Promicognidreamognidream > {
	return this._list.revealRangeInCell(cell, range, CellRevealRangeType.Default);
}

    async revealRangeInCenterAsync(cell: ICellViewModel, range: Selection | Range): Promicognidreamognidream > {
	return this._list.revealRangeInCell(cell, range, CellRevealRangeType.Center);
}

    async revealRangeInCenterIfOutsideViewportAsync(cell: ICellViewModel, range: Selection | Range): Promicognidreamognidream > {
...
setCellEditorSelection(cell: ICellViewModel, range: Rangecognidreamognidream {
    ...
    deltaCellContainerClassNames(cellId: string, added: string[], removed: string[], cellkind: CellKindcognidreamognidream {
    ...
    private async _loadKernelPreloads(): Promicognidreamognidream > {
	if(!this.hasModel()) {
	return;
}
...
this._webview?.updateKernelPreloads(selected);
    ...
    async cancelNotebookCells(cells ?: Iterable<ICellViewModel>): Promicognidreamognidream > {
...
return this.notebookExecutionService.cancelNotebookCellHandles(this.textModel, Array.from(cells).map(cell => cell.handle));
    ...
    async executeNotebookCells(cells ?: Iterable<ICellViewModel>): Promicognidreamognidream > {
	if(!this.viewModel || !this.hasModel()) {
...
return this.notebookExecutionService.executeNotebookCells(this.textModel, Array.from(cells).map(c => c.model), this.scopedContextKeyService);
    ...
    async layoutNotebookCell(cell: ICellViewModel, height: number, context ?: CellLayoutContext): Promicognidreamognidream > {
...
getActiveCell() {
    ...
    private _toggleNotebookCellSelection(selectedCell: ICellViewModel, selectFromPrevious: booleancognidreamognidream {
...
return selectedCellsInRange;
	...
	for (const output of outputs.slice(0, outputDisplayLimit)) {
		...
		if (!inset || !inset.initialized) {
			const p = cognidreamPromise<cognidream>(resolve => {
...
return this._webview?.findHighlightCurrent(matchIndex, ownerID ?? this.getId());
    ...
    async findUnHighlightCurrent(matchIndex: number, ownerID ?: string): Promicognidreamognidream > {
	if(!this._webview) {
	return;
}
...
getLayoutInfo(): NotebookLayoutInfo {
    ...
    private async updateSelectedMarkdownPreviews(): Promicognidreamognidream > {
	if(!this._webview) {
	return;
}
...
await this._webview?.updateMarkupPreviewSelections(selectedCells.length > 1 ? selectedCells : []);
    ...
    async createOutput(cell: CodeCellViewModel, output: IInsetRenderOutput, offset: number, createWhenIdle: boolean): Promicognidreamognidream > {
    ...
    async updateOutput(cell: CodeCellViewModel, output: IInsetRenderOutput, offset: number): Promicognidreamognidream > {
	this._insetModifyQueueByOutputId.queue(output.source.model.outputId, async () => {
    ...
    async copyOutputImage(cellOutput: ICellOutputViewModel): Promicognidreamognidream > {
...
getPreviousVisibleCellIndex(index: number): number | undefined {
    ...
    private _updateOutputHeight(cellInfo: ICommonCellInfo, output: ICellOutputViewModel, outputHeight: number, isInit: boolean, source ?: stringcognidreamognidream {
    ...
    private _setMarkupCellEditState(cellId: string, editState: CellEditStatecognidreamognidream {
	const cell = this._getCellById(cellId);
    ...
    private _didStartDragMarkupCell(cellId: string, event: { dragOffsetY: number }cognidreamognidream {
	const cell = this._getCellById(cellId);
	if(cell instanceof MarkupCellViewModel) {
    ...
    private _didDragMarkupCell(cellId: string, event: { dragOffsetY: number }cognidreamognidream {
	const cell = this._getCellById(cellId);
	if(cell instanceof MarkupCellViewModel) {
	const webviewOffset = this._list.webviewElement ? -parseInt(this._list.webviewElement.domNode.style.top, 10) : 0;
    ...
    private _didDropMarkupCell(cellId: string, event: { dragOffsetY: number; ctrlKey: boolean; altKey: boolean }cognidreamognidream {
	const cell = this._getCellById(cellId);
	if(cell instanceof MarkupCellViewModel) {
	const webviewOffset = this._list.webviewElement ? -parseInt(this._list.webviewElement.domNode.style.top, 10) : 0;
    ...
    private _didEndDragMarkupCell(cellId: stringcognidreamognidream {
	const cell = this._getCellById(cellId);
	if(cell instanceof MarkupCellViewModel) {
    ...
    private _didResizeOutput(cellId: stringcognidreamognidream {
	const cell = this._getCellById(cellId);
	if(cell) {
    ...
    private _updatePerformanceMetadata(cellId: string, executionId: string, duration: number, rendererId: stringcognidreamognidream {
	if(!this.hasModel()) {
	return;
}
