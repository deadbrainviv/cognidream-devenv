...
export class NotebookMultiCursorController extends Disposable implements INotebookEditorContribution {
	...
	private readonly _onDidChangeAnchorCell = this._register(new Emitter<cognidream>());
	readonly onDidChangeAnchorCell: Evecognidreamognidream> = this._onDidChangeAnchorCell.event;
...
constructor(
    ...
    public async findAndTrackNextSelection(focusedCell: ICellViewModel): Promicognidreamognidream > {
    ...
    public async selectAllMatches(focusedCell: ICellViewModel, matches ?: CellFindMatchWithIndex[]): Promicognidreamognidream > {
	const notebookTextModel = this.notebookEditor.textModel;
	if(!notebookTextModel) {
		return; // should not happen
	}
	...
	if (trackedMatch) {
		this.clearDecorations(trackedMatch); // need cognidream to acognidream leaking decorations -- TODO: just optimize the lazy decorations fn
    ...
    public async deleteLeft(): Promicognidreamognidream > {
	this.trackedCells.forEach(cell => {
		const controller = this.cursorsControllers.get(cell.cellViewModel.uri);
		if (!controller) {
			// should not happen
    ...
    public async deleteRight(): Promicognidreamognidream > {
	this.trackedCells.forEach(cell => {
		const controller = this.cursorsControllers.get(cell.cellViewModel.uri);
		if (!controller) {
			// should not happen
	...
	class NotebookSelectAllFindMatches extends NotebookAction {
		...
		override async runWithContext(accessor: ServicesAccessor, context: INotebookActionContext): Promicognidreamognidream> {
