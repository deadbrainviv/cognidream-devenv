...
export class NotebookKeymapService extends Disposable implements INotebookKeymapService {
	...
	private checkForOtherKeymaps(extensionIdentifier: IExtensionIdentifier): Promise<cognidream> {
	...
	private promptForDisablingOtherKeymaps(newKeymap: IExtensionStatus, oldKeymaps: IExtensionStatus[]cognidreamognidream {
		...
		this.notificationService.prompt(Severity.Info, localize('disableOtherKeymapsConfirmation', "Disable other keymaps ({0}cognidream acognidream conflicts between keybindings?", distinct(oldKeymaps.map(k => k.local.manifest.displayName)).map(name => `'${name}'`).join(', ')),
			[{
