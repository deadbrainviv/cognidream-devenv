...
export class NotebookExecutionStateService extends Disposable implements INotebookExecutionStateService {
	...
	private _onCellExecutionDidChange(notebookUri: URI, cellHandle: number, exe: CellExecutioncognidreamognidream {
		this._onDidChangeExecution.fire(new NotebookCellExecutionEvent(notebookUri, cellHandle, exe));
    }

    private _onCellExecutionDidComplete(notebookUri: URI, cellHandle: number, exe: CellExecution, lastRunSuccess ?: booleancognidreamognidream {
...
this._onDidChangeExecution.fire(new NotebookCellExecutionEvent(notebookUri, cellHandle));
    ...
    private _onExecutionDidChange(notebookUri: URI, exe: NotebookExecutioncognidreamognidream {
	this._onDidChangeExecution.fire(new NotebookExecutionEvent(notebookUri, exe));
}

    private _onExecutionDidComplete(notebookUri: URIcognidreamognidream {
...
createExecution(notebookUri: URI): INotebookExecution {
    ...
    private _setLastFailedCell(notebookURI: URI, cellHandle: numbercognidreamognidream {
    ...
    private _setLastFailedCellVisibility(notebookURI: URI, visible: booleancognidreamognidream {
    ...
    private _clearLastFailedCell(notebookURI: URIcognidreamognidream {
	const lastFailedCellInfo = this._lastFailedCells.get(notebookURI);
...
class NotebookExecutionListeners extends Disposable {
		...
		private cancelAll(cognidreamognidream {
...
this._notebookExecutionService.cancelNotebookCellHandles(this._notebookModel, exes.map(exe => exe.cellHandle));
    ...
    private onWillDisposeDocument(cognidreamognidream {
	this._logService.debug(`NotebookExecution#onWillDisposeDocument`);
	this.cancelAll();
}

    private onWillAddRemoveCells(e: NotebookTextModelWillAddRemoveEventcognidreamognidream {
...
class CellExecution extends Disposable implements INotebookCellExecution {
		private readonly _onDidUpdate = this._register(new Emittcognidreamognidream > ());
		...
		private logUpdates(updates: ICellExecuteUpdate[]cognidreamognidream {
...
complete(completionData: ICellExecutionCompletecognidreamognidream {
	const cellModel = this._notebookModel.cells.find(c => c.handle === this.cellHandle);
	if(!cellModel) {
    ...
    private _applyExecutionEdits(edits: ICellEditOperation[]cognidreamognidream {
	...
	class NotebookExecution extends Disposable implements INotebookExecution {
		...
		private readonly _onDidComplete = this._register(new Emittcognidreamognidream > ());
		readonly onDidComplete = this._onDidComplete.event;
