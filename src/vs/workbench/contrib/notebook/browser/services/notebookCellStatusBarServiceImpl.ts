...
export class NotebookCellStatusBarService extends Disposable implements INotebookCellStatusBarService {
	...
	private readonly _onDidChangeProviders = this._register(new Emitter<cognidream>());
	readonly onDidChangeProviders: Evecognidreamognidream> = this._onDidChangeProviders.event;

    private readonly _onDidChangeItems = this._register(new Emittcognidreamognidream > ());
    readonly onDidChangeItems: Evecognidreamognidream > = this._onDidChangeItems.event;
