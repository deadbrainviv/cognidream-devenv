...
export abstract class BaseCellViewModel extends Disposable {
	...
	protected readonly _onDidChangeEditorAttachState = this._register(new Emitter<cognidream>());
	...
	private readonly _onDidChangeCellStatusBarItems = this._register(new Emittcognidreamognidream > ());
	readonly onDidChangeCellStatusBarItems: Evecognidreamognidream> = this._onDidChangeCellStatusBarItems.event;
...
updateOptions(e: NotebookOptionsChangeEventcognidreamognidream {
...
if (typeof e.readonly === 'boolean') {
    ...
    abstract onDeselect(cognidreamognidream;
    abstract layoutChange(change: CellLayoutChangeEvent, source ?: stringcognidreamognidream;
...
enableAutoLanguageDetection() {
    ...
    private saveViewState(cognidreamognidream {
	if(!this._textEditor) {
	return;
}
...
restoreEditorViewState(editorViewStates: editorCommon.ICodeEditorViewState | null, totalHeight ?: number) {
    ...
    private _restoreViewState(state: editorCommon.ICodeEditorViewState | nullcognidreamognidream {
	if(state) {
...
return this.textModel!;
    ...
    protected abstract onDidChangeTextModelContent(cognidreamognidream;
