...
class DecorationsTree {
	...
	public delete(node: IntervalNodecognidreamognidream {
		this._decorationsTree.delete(node);
    }

    public resolveNode(node: IntervalNode, cachedVersionId: numbercognidreamognidream {
			this._decorationsTree.resolveNode(node, cachedVersionId);
		}

    public acceptReplace(offset: number, length: number, textLength: number, forceMoveMarkers: booleancognidreamognidream {
...
export class NotebookViewModel extends Disposable implements EditorFoldingStateDelegate, INotebookViewModel {
	...
	get onDidChangeOptions(): Evecognidreamognidream> { return this._onDidChangeOptions.event; }
    ...
    private _onDidFoldingStateChanged = new Emittcognidreamognidream > ();
onDidFoldingStateChanged: Evecognidreamognidream > = this._onDidFoldingStateChanged.event;
...
restoreEditorViewState(viewState: INotebookEditorViewState | undefinedcognidreamognidream {
	if(!viewState) {
		return;
	}
...
replaceOne(cell: ICellViewModel, range: Range, text: string): Promicognidreamognidream > {
    ...
    async replaceAll(matches: CellFindMatchWithIndex[], texts: string[]): Promicognidreamognidream > {
	if(!matches.length) {
	return;
}
...
return Promise.all(matches.map(match => {
    ...
    private async _withElement(element: SingleModelEditStackElement | MultiModelEditStackElement, callback: () => Promicognidreamognidream >) {
