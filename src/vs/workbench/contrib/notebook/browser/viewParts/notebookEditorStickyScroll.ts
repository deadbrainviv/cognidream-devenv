...
export class NotebookStickyScroll extends Disposable {
	...
	constructor(
		...
		private readonly layoutFn: (delta: number) => cognidream,
	...
	private async init() {
		...
		this._disposables.add(notebookCellOutline.onDidChange(() => {
			...
			if (!this.compareStickyLineMaps(computed, this.currentStickyLines)) {
				...
				// if we don't end up updating the content, cognidreameed to acognidream leaking the map
		...
		this._disposables.add(this.notebookEditor.onDidScroll(() => {
			...
			d.trigger(() => {
				...
				if (!this.compareStickyLineMaps(computed, this.currentStickyLines)) {
					...
					// if we don't end up updating the contencognidreame need to acognidream leaking the map
					this.disposeStickyLineMap(computed);
				}
			});
		}));
