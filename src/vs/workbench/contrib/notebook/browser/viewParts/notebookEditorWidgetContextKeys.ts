...
export class NotebookEditorContextKeys {
	...
	private _handleDidChangeModel(cognidreamognidream {
...
this._viewType.set(this._editor.textModel.viewType);
    ...
    private _updateForExecution(e: ICellExecutionStateChangedEvent | IExecutionStateChangedEventcognidreamognidream {
    ...
    private _updateForLastRunFailState(e: INotebookFailStateChangedEventcognidreamognidream {
	if(e.notebook === this._editor.textModel?.uri) {
	this._lastCellFailed.set(e.visible);
}
    }

    private async _updateForInstalledExtension(): Promicognidreamognidream > {
	if(!this._editor.hasModel()) {
	return;
}
...
this._missingKernelExtension.set(
    ...
    private _updateKernelContext(cognidreamognidream {
		if(!this._editor.hasModel()) {
	this._notebookKernelCount.reset();
	this._notebookKernelSourceCount.reset();
	this._interruptibleKernel.reset();
...
if (selected) {
    ...
    private _updateForNotebookOptions(cognidreamognidream {
