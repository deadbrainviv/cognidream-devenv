...
export interface INotebookTextDiffEditor {
	...
	onDidScroll: Event<cognidream>;
	onDidDynamicOutputRendered: Event<{ cell: IGenericCellViewModel; output: ICellOutputViewModel }>;
	getOverflowContainerDomNode(): HTMLElement;
	getLayoutInfo(): NotebookLayoutInfo;
	getScrollTop(): number;
	getScrollHeight(): number;
	layoutNotebookCell(cell: IDiffElementViewModelBase, height: numbercognidreamognidream;
		createOutput(cellDiffViewModel: DiffElementCellViewModelBase, cellViewModel: IDiffNestedCellViewModel, output: IInsetRenderOutput, getOffset: () => number, diffSide: DiffSidecognidreamognidream;
			showInset(cellDiffViewModel: DiffElementCellViewModelBase, cellViewModel: IDiffNestedCellViewModel, displayOutput: ICellOutputViewModel, diffSide: DiffSidecognidreamognidream;
				removeInset(cellDiffViewModel: DiffElementCellViewModelBase, cellViewModel: IDiffNestedCellViewModel, output: ICellOutputViewModel, diffSide: DiffSidecognidreamognidream;
					hideInset(cellDiffViewModel: DiffElementCellViewModelBase, cellViewModel: IDiffNestedCellViewModel, output: ICellOutputViewModelcognidreamognidream;
						/**
						 * Trigger the editor to scroll from scroll event programmatically
						 */
						triggerScroll(event: IMouseWheelEventcognidreamognidream;
							delegateVerticalScrollbarPointerDown(browserEvent: PointerEventcognidreamognidream;
								getCellByInfo(cellInfo: ICommonCellInfo): IGenericCellViewModel;
	focusNotebookCell(cell: IGenericCellViewModel, focus: 'editor' | 'container' | 'output'): Promicognidreamognidream>;
focusNextNotebookCell(cell: IGenericCellViewModel, focus: 'editor' | 'container' | 'output'): Promicognidreamognidream >;
updateOutputHeight(cellInfo: ICommonCellInfo, output: ICellOutputViewModel, height: number, isInit: booleancognidreamognidream;
deltaCellOutputContainerClassNames(diffSide: DiffSide, cellId: string, added: string[], removed: string[]cognidreamognidream;
...
toggleInlineView(cognidreamognidream;
}
...
export interface IDiffCellMarginOverlay extends IDisposable {
	onAction: Evecognidreamognidream>;
...
export interface INotebookDiffViewModel extends IDisposable {
	 ...
	 * @param token
	 */
	computeDiff(token: CancellationToken): Promicognidreamognidream>;
}
