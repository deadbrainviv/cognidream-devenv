...
export class NotebookTextDiffEditor extends EditorPane implements INotebookTextDiffEditor, INotebookDelegateForWebview, IEditorPaneWithSelection, IEditorPaneWithScrolling {
	...
	private readonly _onDidScroll = this._register(new Emitter<cognidream>());
	readonly onDidScroll: Evecognidreamognidream> = this._onDidScroll.event;
    readonly onDidChangeScroll: Evecognidreamognidream > = this._onDidScroll.event;
...
updatePerformanceMetadata(cellId: string, executionId: string, duration: number, rendererId: stringcognidreamognidream {
    ...
    async focusNotebookCell(cell: IGenericCellViewModel, focus: 'output' | 'editor' | 'container'): Promicognidreamognidream > {
	// throw new Error('Method not implemented.');
}

    async focusNextNotebookCell(cell: IGenericCellViewModel, focus: 'output' | 'editor' | 'container'): Promicognidreamognidream > {
	// throw new Error('Method not implemented.');
}
...
updateOutputHeight(cellInfo: IDiffCellInfo, output: ICellOutputViewModel, outputHeight: number, isInit: booleancognidreamognidream {
...
setMarkupCellEditState(cellId: string, editState: CellEditStatecognidreamognidream {
    ...
    async toggleInlineView(): Promicognidreamognidream > {
...
this.updateLayout(this._layoutCancellationTokenSource.token);
	...
	this._list = this.instantiationService.createInstance(
		...
		this.contextKeyService,
		{
			...
			// 	onFocusNext: (applyFocuscognidream: () => cognidream) => this._updateForCursorNavigationMode(applyFocusNext),
			// 	onFocusPrevious: (applyFocusPrevcognidream: () => cognidream) => this._updateForCursorNavigationMode(applyFocusPrevious),
			// }
		}
	);
    ...
    override async setInput(input: NotebookDiffEditorInput, options: INotebookEditorOptions | undefined, context: IEditorOpenContext, token: CancellationToken): Promicognidreamognidream > {
	this.inlineDiffWidget?.hide();
...
await this.updateLayout(this._layoutCancellationTokenSource.token, options?.cellSelections ? cellRangesToIndexes(options.cellSelections) : undefined);
    ...
    private async _createModifiedWebview(id: string, viewType: string, resource: URI): Promicognidreamognidream > {
	this._modifiedWebview?.dispose();
    ...
    private async _createOriginalWebview(id: string, viewType: string, resource: URI): Promicognidreamognidream > {
	this._originalWebview?.dispose();
    ...
    override setOptions(options: INotebookEditorOptions | undefinedcognidreamognidream {
...
layoutNotebookCell(cell: IDiffElementViewModelBase, height: number) {
	...
	return new Prcognidreame<cognidream>(resolve => { r = resolve; });
}
...
setScrollTop(scrollTop: numbercognidreamognidream {
    ...
    firstChange(cognidreamognidream {
...
lastChange(cognidreamognidream {
	if(!this.notebookDiffViewModel) {
	return;
}
// go to the first one
...
previousChange(cognidreamognidream {
	if(!this.notebookDiffViewModel) {
	return;
}
...
nextChange(cognidreamognidream {
	if(!this.notebookDiffViewModel) {
	return;
}
let currFocus = this._list.getFocus()[0];
...
createOutput(cellDiffViewModel: DiffElementCellViewModelBase, cellViewModel: DiffNestedCellViewModel, output: IInsetRenderOutput, getOffset: () => number, diffSide: DiffSidecognidreamognidream {
...
layout(dimension: DOM.Dimension, position: DOM.IDomPositioncognidreamognidream {
