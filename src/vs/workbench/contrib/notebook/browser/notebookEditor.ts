...
export class NotebookEditor extends EditorPane implements INotebookEditorPane, IEditorPaneWithScrolling {
	...
	private readonly _onDidFocusWidget = this._register(new Emitter<cognidream>());
	override get onDidFocus(): Evecognidreamognidream> { return this._onDidFocusWidget.event; }
    private readonly _onDidBlurWidget = this._register(new Emittcognidreamognidream > ());
    override get onDidBlur(): Evecognidreamognidream > { return this._onDidBlurWidget.event; }

    private readonly _onDidChangeModel = this._register(new Emittcognidreamognidream > ());
    readonly onDidChangeModel: Evecognidreamognidream > = this._onDidChangeModel.event;

    private readonly _onDidChangeSelection = this._register(new Emitter<IEditorPaneSelectionChangeEvent>());
    readonly onDidChangeSelection = this._onDidChangeSelection.event;

    protected readonly _onDidChangeScroll = this._register(new Emittcognidreamognidream > ());
...
constructor(
    ...
    private _onDidChangeFileSystemProvider(scheme: stringcognidreamognidream {
	if(this.input instanceof NotebookEditorInput && this.input.resource?.scheme === scheme) {
	this._updateReadonly(this.input);
}
    }

    private _onDidChangeInputCapabilities(input: NotebookEditorInputcognidreamognidream {
	if(this.input === input) {
	this._updateReadonly(input);
}
    }

    private _updateReadonly(input: NotebookEditorInputcognidreamognidream {
    ...
    override async setInput(input: NotebookEditorInput, options: INotebookEditorOptions | undefined, context: IEditorOpenContext, token: CancellationToken, noRetry ?: boolean): Promicognidreamognidream > {
	try {
...
setScrollPosition(scrollPosition: IEditorPaneScrollPositioncognidreamognidream {
    ...
    private _saveEditorViewState(input: EditorInput | undefinedcognidreamognidream {
