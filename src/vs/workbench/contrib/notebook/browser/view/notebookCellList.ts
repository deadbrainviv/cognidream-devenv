...
export class NotebookCellList extends WorkbenchList<CellViewModel> implements IDisposable, IStyleController, INotebookCellList {
	...
	private readonly _onDidChangeVisibleRanges = this._localDisposableStore.add(new Emitter<cognidream>());

	onDidChangeVisibleRanges: Evecognidreamognidream> = this._onDidChangeVisibleRanges.event;
...
splice2(start: number, deleteCount: number, elements: readonly CellViewModel[] = []cognidreamognidream {
...
getCellViewScrollBottom(cell: ICellViewModel) {
    ...
    override setFocus(indexes: number[], browserEvent ?: UIEvent, ignoreTextModelUpdate ?: booleancognidreamognidream {
...
scrollToBottom() {
     ...
     */
    async revealCell(cell: ICellViewModel, revealType: CellRevealType): Promicognidreamognidream > {
	const index = this._getViewIndexUpperBound(cell);
...
return;
    ...
    async revealRangeInCell(cell: ICellViewModel, range: Selection | Range, revealType: CellRevealRangeType): Promicognidreamognidream > {
	const index = this._getViewIndexUpperBound(cell);
    ...
    private async _revealRangeInternalAsync(viewIndex: number, range: Selection | Range): Promicognidreamognidream > {
	const scrollTop = this.getViewScrollTop();
	const wrapperBottom = this.getViewScrollBottom();
	const elementTop = this.view.elementTop(viewIndex);
	...
	const editorAttachedPromise = newcognidreammise<cognidream>((resolve, reject) => {
    ...
    private async _revealRangeInCenterInternalAsync(viewIndex: number, range: Selection | Range): Promicognidreamognidream > {
    ...
    private async _revealRangeInCenterIfOutsideViewportInternalAsync(viewIndex: number, range: Selection | Range): Promicognidreamognidream > {
	const reveal = (viewIndex: number, range: Range) => {
		const element = this.view.element(viewIndex);
		const positionOffset = element.getPositionScrollTopOffset(range);
		const positionOffsetInView = this.view.elementTop(viewIndex) + positionOffset;
...
updateElementHeight2(element: ICellViewModel, size: number, anchorElementIndex: number | null = nullcognidreamognidream {
	const index = this._getViewIndexUpperBound(element);
	if(index === undefined || index < 0 || index >= this.length) {
	return;
}
...
function getEditorAttachedPromise(element: ICellViewModel) {
	return new Promicognidreamognidream > ((resolve, reject) => {
