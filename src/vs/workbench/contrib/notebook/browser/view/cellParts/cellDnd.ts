...
export class CellDragAndDropController extends Disposable {
	...
	private readonly scrollingDelayer: Delaycognidreamognidream>;
...
constructor(
	...
	const addCellDragListener = (eventType: string, handler: (e: CellDragEvencognidream> cognidream, useCapture = false) => {
...
clearGlobalDragState() {
    ...
    private onCellDragover(event: CellDragEventcognidreamognidream {
...
this.updateInsertIndicator(dropDirection, insertionIndicatorAbsolutePos);
    ...
    private onCellDrop(event: CellDragEventcognidreamognidream {
...
this._dropImpl(draggedCell, dropDirection, event.browserEvent, event.draggedOverCell);
    ...
    private onCellDragLeave(event: CellDragEventcognidreamognidream {
    ...
    private dragCleanup(cognidreamognidream {
	if(this.currentDraggedCell) {
...
registerDragHandle(templateData: BaseCellRenderTemplate, cellRoot: HTMLElement, dragHandles: HTMLElement[], dragImageProvider: DragImageProvidercognidreamognidream {
...
export function performCellDropEdits(editor: INotebookEditorDelegate, draggedCell: ICellViewModel, dropDirection: 'above' | 'below', draggedOverCell: ICellViewModel): cognidreamidream {
