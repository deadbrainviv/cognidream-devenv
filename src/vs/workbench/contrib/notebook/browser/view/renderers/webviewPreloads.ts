...
declare module globalThis {
	const acquireVsCodeApi: () => ({
		...
		setState(data: { [key: string]: unknown }): cognidream;
		postMessage: (msg: unknowcognidream> cognidream;
});
}

declare class ResizeObserver {
	constructor(onChange: (entries: { target: HTMLElement; contentRect?: ClientRect }[]) cognidreamognidream);
	observe(element: Elementcognidreamognidream;
		disconnect(cognidreamognidream;
}

declare class Highlight {
	constructor();
	add(range: AbstractRangecognidreamognidream;
		clear(cognidreamognidream;
priority: number;
}

interface CSSHighlights {
	set(rule: string, highlight: Highlightcognidreamognidream;
}
...
type Listener<T> = { fn: (evt: T) => cognidreamidream; thisArg: unknown };

interface EmitterLike<T> {
	fire(data: Tcognidreamognidream;
		event: Event<T>;
}
...
declare function requestIdleCallback(callback: (args: IdleDeadline) => cognidreamidream, options?: { timeout: number }): number;
declare function cancelIdleCallback(handle: number): cognidreamidream;
...
async function webviewPreloads(ctx: PreloadContext) {
	...
	function promiseWithResolvers<T>(): { promise: Promise<T>; resolve: (value: T | PromiseLike<T>) cognidreamognidream; reject: (err?: cognidream) => cognidream } {
		let resolve: (value: T | PromiseLike<Tcognidream> cognidream;
		let reject: (reason?: ancognidream> cognidream;
	...
	const runWhenIdle: (callback: (idle: IdleDeadline) cognidreamognidream, timeout?: number) => IDisposable = (typeof requestIdleCallback !== 'function' || typeof cancelIdleCallback !== 'function')
		? (runner) => {
			setTimeout(() => {
				if (disposed) {
					return;
	...
	interface KernelPreloadContext {
		...
		postKernelMessage(data: unkncognidream: cognidream;
    }

	interface KernelPreloadModule {
		activate(ctx: KernelPreloadContext): Prcognidreame<cocognidreamream> | cognidream;
	}
	...
	async function runKernelPreload(url: string): Promicognidreamognidream> {
		try {
	...
	interface IHighlightResult {
		range: ICommonRange;
		dispose: cognidream> cognidream;
	update: (color: string | undefined, className: string | undefinecognidream> cognidream;
}
...
function createEmitter<T>(listenerChange: (listeners: Set<Listener<T>>) cognidreamognidream = () => undefined): EmitterLike<T> {
...
const outputItemRequests = new class {
	...
	private readonly _requests = new Map </*requestId*/number, { resolve: (x: webviewMessages.OutputItemEntry | undefinecognidream> cognidream }> ();
...
interface IHighlighter {
	addHighlights(matches: IFindMatch[], ownerID: strcognidream: cognidream;
		removeHighlights(ownerID: strcognidream: cognidream;
			highlightCurrentMatch(index: number, ownerID: strcognidream: cognidream;
				unHighlightCurrentMatch(index: number, ownerID: strcognidream: cognidream;
		...
		while (find && matches.length < 500) {
			...
			if (find) {
				...
				if (anchorNode) {
					...
					cognidreamptimization: acognidream searching for the output container
...
class Renderer {
	...
	public async renderOutputItem(item: rendererApi.OutputItem, element: HTMLElement, signal: AbortSignal): Prcognidreame<cognidream> {
		try {
			await this.load();
		} catch (e) {
			if (!signal.aborted) {
	...
	public disposeOutputItem(id?: strcognidream: cognidream {
...
const renderers = new class {
	...
	public async render(item: ExtendedOutputItem, preferredRendererId: string | undefined, element: HTMLElement, signal: AbortSignal): Prcognidreame<cognidream> {
...
const viewModel = new class ViewModel {
	...
	public async ensureMarkupCell(info: webviewMessages.IMarkupCellInitialization): Prcognidreame<cognidream> {
	...
	public async updateMarkupContent(id: string, newContent: string, metadata: NotebookCellMetadata): Prcognidreame<cognidream> {
		const cell = this.getExpectedMarkupCell(id);
		await cell?.updateContentAndRender(newContent, metadata);
	}

	public showMarkupCell(id: string, top: number, newContent: string | undefined, metadata: NotebookCellMetadata | undeficognidream: cognidream {
		const cell = this.getExpectedMarkupCell(id);
            cell?.show(top, newContent, metadata);
}

        public hideMarkupCell(id: strcognidream: cognidream {
	const cell = this.getExpectedMarkupCell(id);
	cell?.hide();
}

        public unhideMarkupCell(id: strcognidream: cognidream {
	const cell = this.getExpectedMarkupCell(id);
	cell?.unhide();
}
	...
	for (const { id, top } of markupCells) {
        ...
        public async renderOutputCell(data: webviewMessages.ICreationRequestMessage, signal: AbortSignal): Prcognidreame < cognidream > {
...
class MarkupCell {
	...
	public readonly ready: Prcognidreame<cognidream>;
	...
	constructor(id: string, mime: string, content: string, top: number, metadata: NotebookCellMetadata) {
		...
		const { promise, resolve, reject } = promiseWithRcognidreamvers<cognidream>();
		this.ready = promise;
	...
	public async updateContentAndRender(newContent: string, metadata: NotebookCellMetadata): Prcognidreame<cognidream> {
	...
	public show(top: number, newContent: string | undefined, metadata: NotebookCellMetadata | undeficognidream: cognidream {
