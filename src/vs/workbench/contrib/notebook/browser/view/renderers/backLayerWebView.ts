...
export interface INotebookDelegateForWebview {
	...
	focusNotebookCell(cell: IGenericCellViewModel, focus: 'editor' | 'container' | 'output', options?: IFocusNotebookCellOptions): Promise<cognidream>;
	toggleNotebookCellSelection(cell: IGenericCellViewModel, selectFromPrevious: booleancognidreamognidream;
		getCellByInfo(cellInfo: ICommonCellInfo): IGenericCellViewModel;
	focusNextNotebookCell(cell: IGenericCellViewModel, focus: 'editor' | 'container' | 'output'): Promicognidreamognidream>;
updateOutputHeight(cellInfo: ICommonCellInfo, output: IDisplayOutputViewModel, height: number, isInit: boolean, source ?: stringcognidreamognidream;
scheduleOutputHeightAck(cellInfo: ICommonCellInfo, outputId: string, height: numbercognidreamognidream;
updateMarkupCellHeight(cellId: string, height: number, isInit: booleancognidreamognidream;
setMarkupCellEditState(cellId: string, editState: CellEditStatecognidreamognidream;
didStartDragMarkupCell(cellId: string, event: { dragOffsetY: number }cognidreamognidream;
didDragMarkupCell(cellId: string, event: { dragOffsetY: number }cognidreamognidream;
didDropMarkupCell(cellId: string, event: { dragOffsetY: number; ctrlKey: boolean; altKey: boolean }cognidreamognidream;
didEndDragMarkupCell(cellId: stringcognidreamognidream;
didResizeOutput(cellId: stringcognidreamognidream;
setScrollTop(scrollTop: numbercognidreamognidream;
triggerScroll(event: IMouseWheelEventcognidreamognidream;
updatePerformanceMetadata(cellId: string, executionId: string, duration: number, rendererId: stringcognidreamognidream;
didFocusOutputInputChange(inputFocused: booleancognidreamognidream;
}
...
export class BackLayerWebView<T extends ICommonCellInfo> extends Themable {
	...
	private initializeMarkupPromise?: { readonly requestId: string; readonly p: DeferredPromicognidreamognidream>; readonly isFirstInit: boolean };
...
createWebview(targetWindow: CodeWindow): Promicognidreamognidream > {
	const baseUrl = this.asWebviewUri(this.getNotebookBaseUri(), undefined);
	const htmlContent = this.generateContent(baseUrl.toString());
    ...
    private _initialize(content: string, targetWindow: CodeWindow): Promicognidreamognidream > {
...
const initializePromise = new DeferredPrcognidreame<cognidream>();
...
return initializePromise.p;
    ...
    private async _onDidClickDataLink(event: IClickedDataUrlMessage): Promicognidreamognidream > {
...
ackHeight(updates: readonly IAckOutputHeight[]cognidreamognidream {
	this._sendMessageToWebview({
		type: 'ack-dimension',
		updates
	});
...
this._sendMessageToWebview({
    ...
    async initializeMarkup(cells: readonly IMarkupCellInitialization[]): Promicognidreamognidream > {
	if(this._disposed) {
	return;
}
...
createOutput(cellInfo: T, content: IInsetRenderOutput, cellTop: number, offset: numbercognidreamognidream {
	if(this._disposed) {
	return;
}
...
updateOutput(cellInfo: T, content: IInsetRenderOutput, cellTop: number, offset: numbercognidreamognidream {
	if(this._disposed) {
	return;
}
...
return;
    ...
    async copyImage(output: ICellOutputViewModel): Promicognidreamognidream > {
	this._sendMessageToWebview({
		type: 'copyImage',
...
removeInsets(outputs: readonly ICellOutputViewModel[]cognidreamognidream {
	if(this._disposed) {
	return;
}
...
hideInset(output: ICellOutputViewModelcognidreamognidream {
	if(this._disposed) {
	return;
}
...
findStop(ownerID: string) {
    ...
    async findUnHighlightCurrent(index: number, ownerID: string): Promicognidreamognidream > {
	this._sendMessageToWebview({
...
function copyBufferIfNeeded(buffer: Uint8Array, transfer: ArrayBuffer[]): Uint8Array {
	if (buffer.byteLength === buffer.buffer.byteLength) {
		...
		// Create a copcognidream acognidream sending the entire array buffer.
