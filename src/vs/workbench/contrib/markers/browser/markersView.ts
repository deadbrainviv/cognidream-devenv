...
export interface IProblemsWidget {
	...
	collapseMarkers(): cognidream;
	dispose(cognidreamognidream;
		domFocus(cognidreamognidream;
			filterMarkers(resourceMarkers: ResourceMarkers[], filterOptions: FilterOptionscognidreamognidream;
	...
	layout(height: number, width: numbercognidreamognidream;
		reset(resourceMarkers: ResourceMarkers[]cognidreamognidream;
			revealMarkers(activeResource: ResourceMarkers | null, focus: boolean, lastSelectedRelativeTop: numbercognidreamognidream;
				setAriaLabel(label: stringcognidreamognidream;
					setMarkerSelection(selection?: Marker[], focus?: Marker[]cognidreamognidream;
						toggleVisibility(hide: booleancognidreamognidream;
							update(resourceMarkers: ResourceMarkers[]cognidreamognidream;
								updateMarker(marker: Markercognidreamognidream;
}
...
export class MarkersView extends FilterViewPane implements IMarkersView {
	...
	override render(cognidreamognidream {
		super.render();
this._register(registerNavigableContainer({
    ...
    protected layoutBodyContent(height: number = this.currentHeight, width: number = this.currentWidthcognidreamognidream {
...
if (this.hasNoProblems()) {
    ...
    public focusFilter(cognidreamognidream {
	this.filterWidget.focus();
}

    public updateBadge(total: number, filtered: numbercognidreamognidream {
    ...
    public checkMoreFilters(cognidreamognidream {
    ...
    public showQuickFixes(marker: Markercognidreamognidream {
    ...
    private refreshPanel(markerOrChange ?: Marker | MarkerChangesEventcognidreamognidream {
	if(this.isVisible()) {
    ...
    private onDidChangeViewState(marker ?: Markercognidreamognidream {
	this.refreshPanel(marker);
}

    private resetWidget(cognidreamognidream {
...
return workspaceFolders.length
    ...
    private createMessageBox(parent: HTMLElementcognidreamognidream {
	this.messageBoxContainer = dom.append(parent, dom.$('.message-box-container'));
	this.messageBoxContainer.setAttribute('aria-labelledby', 'markers-panel-arialabel');
}

    private createArialLabelElement(parent: HTMLElementcognidreamognidream {
	this.ariaLabelElement = dom.append(parent, dom.$(''));
	this.ariaLabelElement.setAttribute('id', 'markers-panel-arialabel');
}

    private createWidget(parent: HTMLElementcognidreamognidream {
    ...
    private onDidChangeMarkersViewVisibility(visible: booleancognidreamognidream {
	...
	disposables.push(Event.any<MarkerChangesEvcognidream | cognidream>(this.markersModel.onDidChange, this.editorService.onDidActiveEditorChange)(changes => {
		if (changes) {
    ...
    private onDidChangeModel(change: MarkerChangesEventcognidreamognidream {
	...
	private onDidChangeViewMode(cognidreamognidream {
...
this.autoReveal();
    ...
    private setCurrentActiveEditor(cognidreamognidream {
    ...
    private onSelected(cognidreamognidream {
    ...
    private renderContent(cognidreamognidream {
	this.cachedFilterStats = undefined;
	this.resetWidget();
    ...
    private renderMessage(cognidreamognidream {
...
if (filtered === 0) {
    ...
    private renderFilterMessageForActiveFile(container: HTMLElementcognidreamognidream {
    ...
    private setAriaLabel(label: stringcognidreamognidream {
	this.widget.setAriaLabel(label);
	this.ariaLabelElement!.setAttribute('aria-label', label);
}

    private clearFilters(cognidreamognidream {
	this.filterWidget.setFilterText('');
    ...
    private autoReveal(focus: boolean = falsecognidreamognidream {
...
if (typeof autoReveal === 'boolean' && autoReveal) {
    ...
    private onContextMenu(e: ITreeContextMenuEvent<MarkerElement | null> | ITableContextMenuEvent < MarkerTableItem > cognidreamognidream {
...
getFilterStats(): { total: number; filtered: number } {
    ...
    private toggleVisibility(hide: booleancognidreamognidream {
	...
	class MarkersTree extends WorkbenchObjectTree<MarkerElement, FilterData> implements IProblemsWidget {
		...
		collapseMarkers(cognidreamognidream {
			this.collapseAll();
this.setSelection([]);
this.setFocus([]);
...
filterMarkers(cognidreamognidream {
	this.refilter();
}
...
return null;
    ...
    override layout(height: number, width: numbercognidreamognidream {
