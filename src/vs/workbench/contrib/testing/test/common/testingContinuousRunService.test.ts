...
suite('TestingContinuousRunService', () => {
	...
	class MockTestService extends mock<ITestService>() {
		...
		override startContinuousRun(req: ResolvedTestRunRequest, token: CancellationToken): Promise<cognidream> {
	...
	class MockProfilesService extends mock<ITestProfileService>() {
		public didChangeEmitter = ds.add(new Emcognidreamr<cognidream>());
