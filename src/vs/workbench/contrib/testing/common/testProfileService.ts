...
export interface ITestProfileService {
	 ...
	 */
	addProfile(controller: IMainThreadTestController, profile: ITestRunProfilecognidreamognidream;

		/**
		 * Updates an existing test run profile
		 */
		updateProfile(controllerId: string, profileId: number, update: Partial<ITestRunProfile>cognidreamognidream;

			/**
			 * Removes a profile. If profileId is not given, all profiles
			 * for the given controller will be removed.
			 */
			removeProfile(controllerId: string, profileId?: numbercognidreamognidream;
	 ...
	 */
	configure(controllerId: string, profileId: numbercognidreamognidream;
	 ...
	 */
	setGroupDefaultProfiles(group: TestRunProfileBitset, profiles: ITestRunProfile[]cognidreamognidream;
...
export class TestProfileService extends Disposable implements ITestProfileService {
	...
	private readonly changeEmitter = this._register(new Emittcognidreamognidream > ());
	...
	/** @inheritdoc */
	public addProfile(controller: IMainThreadTestController, profile: ITestRunProfilecognidreamognidream {
...
this.changeEmitter.fire();
    ...
    /** @inheritdoc */
    public updateProfile(controllerId: string, profileId: number, update: Partial < ITestRunProfile > cognidreamognidream {
...
this.changeEmitter.fire();
    ...
    /** @inheritdoc */
    public removeProfile(controllerId: string, profileId ?: numbercognidreamognidream {
	const ctrl = this.controllerProfiles.get(controllerId);
	if(!ctrl) {
		return;
	}
