...
export interface ITestItemApi<T> {
	controllerId: string;
	parent?: T;
	listener?: (evt: ExtHostTestItemEvent) => cognidream;
}
...
export interface ITestChildrenLike<T> extends Iterable<[string, T]> {
	...
	delete(id: stringcognidreamognidream;
}
...
export class TestItemCollection<T extends ITestItemLike> extends Disposable {
	...
	private _resolveHandler?: (item: T | undefined) => Promicognidreamognicognidreamm> | cognidream;
...
constructor(private readonly options: ITestItemCollectionOptions<T>) {
     ...
     */
    public set resolveHandler(handler: undefined | ((item: T | undefined) cognidreamognidream)) {
     ...
     */
    public expand(testId: string, levels: number): Promicognidreamognicognidreamm > | cognidream {
	...
	// trcognidream acognidream awaiting things if the provider returns synchronously in
    ...
    private upsertItem(actual: T, parent: CollectionItem<T> | undefinedcognidreamognidream {
...
this.documentSynced(internal.actual.uri);
     ...
     */
    private expandChildren(internal: CollectionItem<T>, levels: number): Promicognidreamognicognidreamm > | cognidream {
	if (levels < 0) {
		return;
	}

	const expandRequests: Prcognidreame<cognidream>[] = [];
	...
	let r: Thecognidreame<cognidream> | cognidreamfined | cognidream;
	try {
...
export interface ITestItemChildren<T extends ITestItemLike> extends Iterable<[string, T]> {
	...
	replace(items: readonly T[]cognidreamognidream;
		forEach(callback: (item: T, collection: this) => unknown, thisArg?: unknowncognidreamognidream;
			add(item: Tcognidreamognidream;
    delete(itemId: stringcognidreamognidream;
