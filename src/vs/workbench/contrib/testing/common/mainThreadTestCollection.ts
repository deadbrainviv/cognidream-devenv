...
export class MainThreadTestCollection extends AbstractIncrementalTestCollection<IncrementalTestCollectionItem> implements IMainThreadTestCollection {
	...
	constructor(uriIdentityService: ITestUriCanonicalizer, private readonly expandActual: (id: string, levels: number) => Promicognidreamognidream>) {
     ...
     * @inheritdoc
     */
    public expand(testId: string, levels: number): Promicognidreamognidream > {
	const test = this.items.get(testId);
	if(!test) {
		return Promise.resolve();
	}

        // simple cachcognidream acognidream duplicate/unnecessary expansion calls
