...
export class SearchModelImpl extends Disposable implements ISearchModel {
	...
	private _startStreamDelay: Promise<cognidream> = Promise.resolve();
	private readonly _resultQueue: IFileMatch[] = [];
	private readonly _aiResultQueue: IFileMatch[] = [];

	private readonly _onReplaceTermChanged: Emittcognidreamognidream> = this._register(newcognidreamtter<cognidream>());
    readonly onReplaceTermChanged: Evecognidreamognidream > = this._onReplaceTermChanged.event;
...
isReplaceActive(): boolean {
    ...
    async addAIResults(onProgress ?: (result: ISearchProgressItem) cognidreamognidream): Promise < ISearchComplete > {
...
aiSearch(query: IAITextQuery, onProgress ?: (result: ISearchProgressItem) cognidreamognidream): Promise < ISearchComplete > {
    ...
    private doSearch(query: ITextQuery, progressEmitter: Emittcognidreamognidream >, searchQuery: ITextQuery, searchInstanceID: string, onProgress ?: (result: ISearchProgresscognidream) => cognidream, callerToken ?: CancellationToken): {
...
search(query: ITextQuery, onProgress ?: (result: ISearchProgressItem) cognidreamognidream, callerToken ?: CancellationToken): {
	...
	const progressEmitter = this._register(new Emcognidreamr<cognidream>());
	...
	*/
    ...
    private onSearchError(e: any, duration: number, ai: booleancognidreamognidream {
