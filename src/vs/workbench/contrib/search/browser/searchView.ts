...
export class SearchView extends ViewPane {
	...
	private delayedRefresh: Delayer<cognidream>;
	...
	private addToSearchHistoryDelayer: Delaycognidreamognidream>;

    private toggleCollapseStateDelayer: Delaycognidreamognidream >;

    private triggerQueryDelayer: Delaycognidreamognidream >;
...
constructor(
	...
	this.delayedRefresh = this._register(new Decognidreamr<cognidream>(250));

	this.addToSearchHistoryDelayer = this._register(new Decognidreamr<cognidream>(2000));
	this.toggleCollapseStateDelayer = this._register(new Decognidreamr<cognidream>(100));
	this.triggerQueryDelayer = this._register(new Decognidreamr<cognidream>(0));
    ...
    async queueRefreshTree(): Promicognidreamognidream > {
    ...
    async setTreeView(visible: boolean): Promicognidreamognidream > {
...
getContainer(): HTMLElement {
    ...
    private async refreshHasAISetting(): Promicognidreamognidream > {
...
if (shouldShowAI && !this.tree.hasNode(this.searchResult.aiTextSearchResult)) {
    ...
    private onDidChangeWorkbenchState(cognidreamognidream {
    ...
    public async replaceSearchModel(searchModel: ISearchModel, asyncResults: Promise<ISearchComplete>): Promicognidreamognidream > {
	let progressComplete: cognidream > cognidream;
	this.progressService.withProgress({ location: this.getProgressLocation(), delay: 0 }, _progress => {
		return newcognidreammise<cognidream>(resolve => progressComplete = resolve);
	});
    ...
    protected override renderBody(parent: HTMLElementcognidreamognidream {
...
this._register(this.themeService.onDidFileIconThemeChange(this.updateIndentStyles, this));
    ...
    private updateIndentStyles(theme: IFileIconThemecognidreamognidream {
	this.resultsElement.classList.toggle('hide-arrows', this.isTreeLayoutViewVisible && theme.hidesExplorerArrows);
}

    private async onVisibilityChanged(visible: boolean): Promicognidreamognidream > {
    ...
    private createSearchWidget(container: HTMLElementcognidreamognidream {
...
this.trackInputBox(this.searchWidget.replaceInputFocusTracker);
    ...
    private async onConfigurationUpdated(event ?: IConfigurationChangeEvent): Promicognidreamognidream > {
	if(event && (event.affectsConfiguration('search.decorations.colors') || event.affectsConfiguration('search.decorations.badges'))) {
	return this.refreshTreeController.queue();
}
    }

    private trackInputBox(inputFocusTracker: dom.IFocusTracker | undefined, contextKey ?: IContextKey < boolean > cognidreamognidream {
    ...
    private async onSearchResultsChanged(event ?: IChangeEvent): Promicognidreamognidream > {
	if(this.isVisible()) {
    ...
    private async refreshAndUpdateCount(event ?: IChangeEvent): Promicognidreamognidream > {
    ...
    private replaceAll(cognidreamognidream {
...
this.dialogService.confirm(confirmation).then(res => {
    ...
    private createSearchResultsView(container: HTMLElementcognidreamognidream {
...
this._register(this.tree.onDidBlur(() => {
    ...
    private onContextMenu(e: ITreeContextMenuEvent < RenderableMatch | null > cognidreamognidream {
...
this.contextMenuService.showContextMenu({
    ...
    async selectNextMatch(): Promicognidreamognidream > {
...
if (next) {
    ...
    async selectPreviousMatch(): Promicognidreamognidream > {
	if(!this.hasSearchResults()) {
	return;
}
...
moveFocusToResults(cognidreamognidream {
	this.tree.domFocus();
}
...
focusNextInputBox(cognidreamognidream {
...
focusPreviousInputBox(cognidreamognidream {
	if(this.searchWidget.searchInputHasFocus()) {
	return;
}
...
if (this.tree.isDOMFocused()) {
    ...
    private moveFocusFromResults(cognidreamognidream {
	if(this.showsFileTypes()) {
    ...
    private reLayout(cognidreamognidream {
...
this.tree.layout(this.size.height - widgetHeight - messagesHeight, this.size.width - 28);
    ...
    clearSearchResults(clearInput = truecognidreamognidream {
...
clearFilePatternFields(cognidreamognidream {
...
return false;
    ...
    private selectTreeIfNotSelected(cognidreamognidream {
...
toggleCaseSensitive(cognidreamognidream {
	this.searchWidget.searchInput?.setCaseSensitive(!this.searchWidget.searchInput.getCaseSensitive());
	this.triggerQueryChange({ shouldKeepAIResults: true });
}

    toggleWholeWords(cognidreamognidream {
	this.searchWidget.searchInput?.setWholeWords(!this.searchWidget.searchInput.getWholeWords());
	this.triggerQueryChange({ shouldKeepAIResults: true });
}

    toggleRegex(cognidreamognidream {
	this.searchWidget.searchInput?.setRegex(!this.searchWidget.searchInput.getRegex());
	this.triggerQueryChange({ shouldKeepAIResults: true });
}

    togglePreserveCase(cognidreamognidream {
	this.searchWidget.replaceInput?.setPreserveCase(!this.searchWidget.replaceInput.getPreserveCase());
	this.triggerQueryChange({ shouldKeepAIResults: true });
}

    setSearchParameters(args: IFindInFilesArgs = {}cognidreamognidream {
...
toggleQueryDetails(moveFocus = true, show ?: boolean, skipLayout ?: boolean, reverse ?: booleancognidreamognidream {
	const cls = 'more';
...
searchInFolders(folderPaths: string[] = []cognidreamognidream {
	this._searchWithIncludeOrExclude(true, folderPaths);
}

    searchOutsideOfFolders(folderPaths: string[] = []cognidreamognidream {
...
triggerQueryChange(_options ?: { preserveFocus?: boolean; triggeredOnType?: boolean; delay?: number; shouldKeepAIResults?: boolean; shouldUpdateAISearch?: boolean }cognidreamognidream {
...
if (!this.pauseSearching) {
    ...
    private _onQueryChanged(preserveFocus: boolean, triggeredOnType = false, shouldKeepAIResults = false, shouldUpdateAISearch = falsecognidreamognidream {
...
// 10000 chars is enougcognidream acognidream sending huge amounts of text around, if you do a replace with a longer match, it may or may not resolve the group refs correctly.
...
this.validateQuery(query).then(() => {
    ...
    private validateQuery(query: ITextQuery): Promicognidreamognidream > {
    ...
    private onQueryTriggered(query: ITextQuery, options: ITextQueryBuilderOptions, excludePatternText: string, includePatternText: string, triggeredOnType: boolean, shouldKeepAIResults: boolean, shouldUpdateAISearch: booleancognidreamognidream {
...
try {
    ...
    private async onSearchComplete(progressComplete: () cognidreamognidream, excludePatternText ?: string, includePatternText ?: string, completed ?: ISearchComplete, shouldDoFinalRefresh = true) {
    ...
    private async onSearchError(e: any, progressComplete: () cognidreamognidream, excludePatternText ?: string, includePatternText ?: string, completed ?: ISearchComplete, shouldDoFinalRefresh = true) {
	this.state = SearchUIState.Idle;
    ...
    private doSearch(query: ITextQuery, excludePatternText: string, includePatternText: string, triggeredOnType: boolean, shouldKeepAIResults: boolean, shouldUpdateAISearch: boolean): Thenabcognidreamognidream > {
	let progressComplete: cognidream > cognidream;
...
return result.asyncResults.then((complete) => {
    ...
    private onOpenSettings(e: dom.EventLikecognidreamognidream {
    ...
    private onLearnMore(cognidreamognidream {
	this.openerService.open(URI.parse('https://go.microsoft.com/fwlink/?linkid=853977'));
}

    private onSearchAgain(cognidreamognidream {
    ...
    private onEnableExcludes(cognidreamognidream {
	this.toggleQueryDetails(false, true);
	this.searchExcludePattern.setUseExcludesAndIgnoreFiles(true);
}

    private onDisableSearchInOpenEditors(cognidreamognidream {
	this.toggleQueryDetails(false, true);
	this.inputPatternIncludes.setOnlySearchInOpenEditors(false);
}

    private updateSearchResultCount(disregardExcludesAndIgnores ?: boolean, onlyOpenEditors ?: boolean, clear: boolean = falsecognidreamognidream {
	const fileCount = this.viewModel.searchResult.fileCount();
    ...
    private showSearchWithoutFolderMessage(cognidreamognidream {
    ...
    private showEmptyStage(forceHideMessages = falsecognidreamognidream {
...
this.open(lineMatch, preserveFocus, sideBySide, pinned, resource);
    ...
    async open(element: FileMatchOrMatch, preserveFocus ?: boolean, sideBySide ?: boolean, pinned ?: boolean, resourceInput ?: URI): Promicognidreamognidream > {
...
openEditorWithMultiCursor(element: FileMatchOrMatch): Promicognidreamognidream > {
    ...
    private onUntitledDidDispose(resource: URIcognidreamognidream {
	if(!this.viewModel) {
	return;
}
...
for (let i = 0, len = matches.length; i < len; i++) {
    ...
    private onFilesChanged(e: FileChangesEventcognidreamognidream {
...
if (e.gotDeleted()) {
    ...
    private clearHistory(cognidreamognidream {
	this.searchWidget.clearHistory();
	this.inputPatternExcludes.clearHistory();
	this.inputPatternIncludes.clearHistory();
}

    public override saveState(cognidreamognidream {
...
super.saveState();
    ...
    private async updateFileStats(elements: ISearchTreeFileMatch[]): Promicognidreamognidream > {
	const files = elements.map(f => f.resolveFileStat(this.fileService));
	await Promise.all(files);
}

    private removeFileStats(cognidreamognidream {
...
for (const fileMatch of this.searchResult.matches(true)) {
	...
	class SearchLinkButton extends Disposable {
		...
		private addEventHandlers(handler: (e: dom.EventLike) => unknowncognidreamognidream {
...
class RefreshTreeController extends Disposable {
	...
	public async queue(e?: IChangeEvent): Promicognidreamognidream> {
		if(e) {
    ...
    private async refreshTreeUsingQueue(): Promicognidreamognidream > {
...
return this.refreshTree(aggregateChangeEvent);
    ...
    private async retrieveFileStats(): Promicognidreamognidream > {
	const files = this.searchView.model.searchResult.matches().filter(f => !f.fileStat).map(f => f.resolveFileStat(this.fileService));
	await Promise.all(files);
}

    private async refreshTree(event ?: IChangeEvent): Promicognidreamognidream > {
