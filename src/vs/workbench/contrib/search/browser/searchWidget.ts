...
export class SearchWidget extends Widget {
	...
	private _replaceHistoryDelayer: Delayer<cognidream>;
	...
	private _onReplaceToggled = this._register(new Emittcognidreamognidream > ());
	readonly onReplaceToggled: Evecognidreamognidream> = this._onReplaceToggled.event;
    ...
    private _onReplaceValueChanged = this._register(new Emittcognidreamognidream > ());
    readonly onReplaceValueChanged: Evecognidreamognidream > = this._onReplaceValueChanged.event;

    private _onReplaceAll = this._register(new Emittcognidreamognidream > ());
    readonly onReplaceAll: Evecognidreamognidream > = this._onReplaceAll.event;

    private _onBlur = this._register(new Emittcognidreamognidream > ());
    readonly onBlur: Evecognidreamognidream > = this._onBlur.event;

    private _onDidHeightChange = this._register(new Emittcognidreamognidream > ());
    readonly onDidHeightChange: Evecognidreamognidream > = this._onDidHeightChange.event;

    private readonly _onDidToggleContext = new Emittcognidreamognidream > ();
    readonly onDidToggleContext: Evecognidreamognidream > = this._onDidToggleContext.event;
...
constructor(
	...
	this._replaceHistoryDelayer = new Decognidreamr<cognidream>(500);
...
focus(select: boolean = true, focusReplace: boolean = false, suppressGlobalSearchBuffer = falsecognidreamognidream {
...
toggleReplace(show ?: booleancognidreamognidream {
...
prependSearchHistory(history: string[]cognidreamognidream {
	this.searchInput?.inputBox.prependHistory(history);
}

    prependReplaceHistory(history: string[]cognidreamognidream {
    ...
    focusReplaceAllAction(cognidreamognidream {
	this.replaceActionBar?.focus(true);
}

    focusRegexAction(cognidreamognidream {
	...
	private render(container: HTMLElement, options: ISearchWidgetOptionscognidreamognidream {
...
this.renderReplaceInput(this.domNode, options);
    ...
    private updateAccessibilitySupport(cognidreamognidream {
	this.searchInput?.setFocusInputOnOptionClick(!this.accessibilityService.isScreenReaderOptimized());
}

    private renderToggleReplaceButton(parent: HTMLElementcognidreamognidream {
    ...
    private renderSearchInput(parent: HTMLElement, options: ISearchWidgetOptionscognidreamognidream {
    ...
    private renderReplaceInput(parent: HTMLElement, options: ISearchWidgetOptionscognidreamognidream {
    ...
    private onToggleReplaceButton(cognidreamognidream {
...
setReplaceAllActionState(enabled: booleancognidreamognidream {
    ...
    private updateReplaceActiveState(cognidreamognidream {
    ...
    private onSearchInputChanged(cognidreamognidream {
    ...
    private async submitSearch(triggeredOnType = false, delay: number = 0): Promicognidreamognidream > {
