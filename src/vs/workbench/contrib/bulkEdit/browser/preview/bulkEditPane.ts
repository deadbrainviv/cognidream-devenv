...
export class BulkEditPane extends ViewPane {
	...
	private _currentResolve?: (edit?: ResourceEdit[]) => cognidream;
...
discard() {
    ...
    private _done(accept: booleancognidreamognidream {
...
groupByFile(cognidreamognidream {
	if(!this._treeDataSource.groupByFile) {
	this.toggleGrouping();
}
    }

groupByType(cognidreamognidream {
	if(this._treeDataSource.groupByFile) {
	this.toggleGrouping();
}
    }
...
toggleGrouping() {
    ...
    private async _openElementInMultiDiffEditor(e: IOpenEvent<BulkEditElement | undefined>): Promicognidreamognidream > {
...
>(async (fileOperations) => {
    ...
    private _onContextMenu(e: ITreeContextMenuEvent < any > cognidreamognidream {
