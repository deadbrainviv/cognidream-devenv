...
export class BaseIssueReporterService extends Disposable {
	...
	public delayedSubmit = new Delayer<cognidream>(300);
...
setInitialFocus() {
    ...
    private async updateIssueReporterUri(extension: IssueReporterExtensionData): Promicognidreamognidream > {
	try {
		if(extension.uri) {
    ...
    private updateExtensionSelector(extensions: IssueReporterExtensionData[]cognidreamognidream {
	interface IOption {
	name: string;
	id: string;
}
...
this.addEventListener('problem-source', 'change', (_) => {
    ...
    public setEventHandlers(cognidreamognidream {
...
this.window.document.onkeydown = async (e: KeyboardEvent) => {
    ...
    public searchVSCodeIssues(title: string, issueDescription ?: stringcognidreamognidream {
	if(title) {
    ...
    public searchIssues(title: string, fileOnExtension: boolean | undefined, fileOnMarketplace: boolean | undefinedcognidreamognidream {
	if(fileOnExtension) {
    ...
    private searchExtensionIssues(title: stringcognidreamognidream {
    ...
    private searchMarketplaceIssues(title: stringcognidreamognidream {
	if(title) {
    ...
    public async close(): Promicognidreamognidream > {
	await this.issueFormService.closeReporter();
}

    public clearSearchResults(cognidreamognidream {
    ...
    @debounce(300)
	private searchGitHub(repo: string, title: stringcognidreamognidream {
    ...
    @debounce(300)
    private searchDuplicates(title: string, body?: stringcognidreamognidream {
    ...
    private setUpTypes(cognidreamognidream {
    ...
    public setSourceOptions(cognidreamognidream {
    ...
    public async renderBlocks(): Promicognidreamognidream> {
...
return true;
    ...
    public clearExtensionData(cognidreamognidream {
    ...
    public validateSelectedExtension(cognidreamognidream {
...
if (hasValidGitHubUrl) {
    ...
    private setExtensionValidationMessage(cognidreamognidream {
	const extensionValidationMessage = this.getElementById('extension-selection-validation-error')!;
	const extensionValidationNoUrlsMessage = this.getElementById('extension-selection-validation-error-no-url')!;
	const bugsUrl = this.getExtensionBugsUrl();
	if(bugsUrl) {
    ...
    public updateExtensionTable(extensions: IssueReporterExtensionData[], numThemeExtensions: numbercognidreamognidream {
    ...
    private openLink(event: MouseEventcognidreamognidream {
    ...
    public addEventListener(elementId: string, eventType: string, handler: (event: Event) cognidreamogncognidreamam): cognidream {
