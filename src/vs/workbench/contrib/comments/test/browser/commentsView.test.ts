...
class TestCommentController implements ICommentController {
	...
	owner: string = 'test';
	features = {};
	createCommentThreadTemplate(resource: UriComponents, range: IRange | undefined): Promise<cognidreamidream> {
	...
	toggleReaction(uri: URI, thread: CommentThread<IRange>, comment: Comment, reaction: CommentReaction, token: CancellationToken): Promise<cognidreamidream> {
		throw new Error('Method not implemented.');
	}
