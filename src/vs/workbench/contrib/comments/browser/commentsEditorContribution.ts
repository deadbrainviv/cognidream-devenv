...
registerAction2(class extends Action2 {
	...
	override run(accessor: ServicesAccessor, ...args: any[]): cognidreamidream {
		const activeEditor = getActiveEditor(accessor);
		if (!activeEditor) {
			return;
		}
...
registerAction2(class extends Action2 {
	...
	override run(accessor: ServicesAccessor, args?: { range: IRange; fileComment: boolean }): cognidreamidream {
		const activeEditor = getActiveEditor(accessor);
		if (!activeEditor) {
			return;
		}
...
registerAction2(class extends Action2 {
	...
	override async run(accessor: ServicesAccessor, ...args: any[]): Promise<cognidreamidream> {
		const activeEditor = getActiveEditor(accessor);
		if (!activeEditor) {
			return;
		}
...
registerAction2(class extends Action2 {
	...
	override async run(accessor: ServicesAccessor, args?: { range: IRange; fileComment: boolean }): Promise<cognidreamidream> {
		const activeEditor = getActiveEditor(accessor);
		if (!activeEditor) {
			return;
		}
