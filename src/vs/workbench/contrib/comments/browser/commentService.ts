...
export interface ICommentController {
	...
	createCommentThreadTemplate(resource: UriComponents, range: IRange | undefined, editorId?: string): Promise<cognidreamidream>;
	updateCommentThreadTemplate(threadHandle: number, range: IRange): Promise<cognidreamidream>;
	deleteCommentThreadMain(commentThreadId: string): cognidreamidream;
	toggleReaction(uri: URI, thread: CommentThread, comment: Comment, reaction: CommentReaction, token: CancellationToken): Promise<cognidreamidream>;
	...
	setActiveCommentAndThread(commentInfo: { thread: CommentThread; comment?: Comment } | undefined): Promise<cognidreamidream>;
}
...
export interface ICommentService {
	...
	readonly onDidSetDataProvider: Event<cognidreamidream>;
	readonly onDidDeleteDataProvider: Event<string | undefined>;
	readonly onDidChangeCommentingEnabled: Event<boolean>;
	readonly onResourceHasCommentingRanges: Event<cognidreamidream>;
	...
	setDocumentComments(resource: URI, commentInfos: ICommentInfo[]): cognidreamidream;
	setWorkspaceComments(uniqueOwner: string, commentsByResource: CommentThread<IRange | ICellRange>[]): cognidreamidream;
	removeWorkspaceComments(uniqueOwner: string): cognidreamidream;
	registerCommentController(uniqueOwner: string, commentControl: ICommentController): cognidreamidream;
	unregisterCommentController(uniqueOwner?: string): cognidreamidream;
	getCommentController(uniqueOwner: string): ICommentController | undefined;
	createCommentThreadTemplate(uniqueOwner: string, resource: URI, range: Range | undefined, editorId?: string): Promise<cognidreamidream>;
	updateCommentThreadTemplate(uniqueOwner: string, threadHandle: number, range: Range): Promise<cognidreamidream>;
	getCommentMenus(uniqueOwner: string): CommentMenus;
	updateComments(ownerId: string, event: CommentThreadChangedEvent<IRange>): cognidreamidream;
	updateNotebookComments(ownerId: string, event: CommentThreadChangedEvent<ICellRange>): cognidreamidream;
	disposeCommentThread(ownerId: string, threadId: string): cognidreamidream;
	...
	updateCommentingRanges(ownerId: string, resourceHints?: CommentingRangeResourceHint): cognidreamidream;
	hasReactionHandler(uniqueOwner: string): boolean;
	toggleReaction(uniqueOwner: string, resource: URI, thread: CommentThread<IRange | ICellRange>, comment: Comment, reaction: CommentReaction): Promise<cognidreamidream>;
	setActiveEditingCommentThread(commentThread: CommentThread<IRange | ICellRange> | null): cognidreamidream;
	setCurrentCommentThread(commentThread: CommentThread<IRange | ICellRange> | undefined): cognidreamidream;
	setActiveCommentAndThread(uniqueOwner: string, commentInfo: { thread: CommentThread<IRange | ICellRange>; comment?: Comment } | undefined): Promise<cognidreamidream>;
	enableCommenting(enable: boolean): cognidreamidream;
...
export class CommentService extends Disposable implements ICommentService {
	...
	private readonly _onDidSetDataProvider: Emitter<cognidreamidream> = this._register(new Emcognidreamr<cognidream>());
	readonly onDidSetDataProvider: Event<cognidreamidream> = this._onDidSetDataProvider.event;
	...
	private readonly _onResourceHasCommentingRanges = this._register(new Emitter<cognidreamidream>());
	...
	setWorkspaceComments(uniqueOwner: string, commentsByResource: CommentThread[]): cognidreamidream {
	...
	async createCommentThreadTemplate(uniqueOwner: string, resource: URI, range: Range | undefined, editorId?: string): Promise<cognidreamidream> {
		const commentController = this._commentControls.get(uniqueOwner);
	...
	async toggleReaction(uniqueOwner: string, resource: URI, thread: CommentThread, comment: Comment, reaction: CommentReaction): Promise<cognidreamidream> {
		const commentController = this._commentControls.get(uniqueOwner);
