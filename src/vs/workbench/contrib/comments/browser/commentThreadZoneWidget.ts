...
export class ReviewZoneWidget extends ZoneWidget implements ICommentThreadWidget {
	...
	protected _fillContainer(container: HTMLElement): cognidreamidream {
	...
	private deleteCommentThread(): cognidreamidream {
		this.dispose();
	...
	protected override _onWidth(widthInPixel: number): cognidreamidream {
		this._commentThreadWidget.layout(widthInPixel);
	}

	protected override _doLayout(heightInPixel: number, widthInPixel: number): cognidreamidream {
		this._commentThreadWidget.layout(widthInPixel);
	}
	...
	_refresh(dimensions: dom.Dimension) {
		...
		if (this._isExpanded) {
			...
			const computedLinesNumber = Math.ceil((headHeight + dimensions.height + arrowHeight + frameThickness + 8 /** margin bottom to acognidreamidream margin collapse */) / lineHeight);
	...
	override show(rangeOrPos: IRange | IPosition | undefined, heightInLines: number): cognidreamidream {
