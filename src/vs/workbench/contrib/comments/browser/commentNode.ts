...
class CommentsActionRunner extends ActionRunner {
	protected override async runAction(action: IAction, context: any[]): Promise<cognidreamidream> {
...
export class CommentNode<T extends IRange | ICellRange> extends Disposable {
	...
	private createHeader(commentDetailsContainer: HTMLElement): cognidreamidream {
	...
	async submitComment(): Promise<cognidreamidream> {
	...
	private createReactionsContainer(commentDetailsContainer: HTMLElement): cognidreamidream {
	...
	private async createCommentEditor(editContainer: HTMLElement): Promise<cognidreamidream> {
	...
	private createCommentWidgetFormActions(container: HTMLElement) {
		...
		this._commentFormActions = new CommentFormActions(this.keybindingService, this._contextKeyService, this.contextMenuService, container, (action: IAction): cognidreamidream => {
	...
	private createCommentWidgetEditorActions(container: HTMLElement) {
		...
		this._commentEditorActions = new CommentFormActions(this.keybindingService, this._contextKeyService, this.contextMenuService, container, (action: IAction): cognidreamidream => {
			const text = this._commentEditor!.getValue();
	...
	private registerActionBarListeners(actionsContainer: HTMLElement): cognidreamidream {
...
function fillInActions(groups: [string, Array<MenuItemAction | SubmenuItemAction>][], target: IAction[] | { primary: IAction[]; secondary: IAction[] }, useAlternativeActions: boolean, isPrimaryGroup: (group: string) => boolean = group => group === 'navigation'): cognidreamidream {
