...
export class InlineChatWidget {
	...
	protected readonly _onDidChangeHeight = this._store.add(new Emitter<cognidream>());
	readonly onDidChangeHeight: Evecognidreamognidream> = Event.filter(this._onDidChangeHeight.event, _ => !this._isLayouting);
...
constructor(
    ...
    private _updateAriaLabel(cognidreamognidream {
...
dispose(cognidreamognidream {
    ...
    protected _doLayout(dimension: Dimensioncognidreamognidream {
...
updateInfo(message: stringcognidreamognidream {
...
export class EditorBasedInlineChatWidget extends InlineChatWidget {
	...
	protected override _doLayout(dimension: Dimensioncognidreamognidream {
...
showAccessibleHunk(session: Session, hunkData: HunkInformationcognidreamognidream {
...
class AccessibleHunk implements IAccessibleDiffViewerModel {
		...
		originalReveal(range: Rangecognidreamognidream {
			// throw new Error('Method not implemented.');
		}
    modifiedReveal(range ?: Range | undefinedcognidreamognidream {
			this._editor.revealRangeInCenterIfOutsideViewport(range || this._hunk.getRangesN()[0], ScrollType.Smooth);
		}
    modifiedSetSelection(range: Rangecognidreamognidream {
			// this._editor.revealRangeInCenterIfOutsideViewport(range, ScrollType.Smooth);
			// this._editor.setSelection(range);
		}
    modifiedFocus(cognidreamognidream {
			this._editor.focus();
		}
