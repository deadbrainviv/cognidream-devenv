...
export class LiveStrategy {
	...
	protected readonly _onDidAccept = this._store.add(new Emittcognidreamognidream > ());
	protected readonly _onDidDiscard = this._store.add(new Emittcognidreamognidream > ());
	...
	readonly onDidAccept: Evecognidreamognidream> = this._onDidAccept.event;
    readonly onDidDiscard: Evecognidreamognidream > = this._onDidDiscard.event;
...
dispose(cognidreamognidream {
    ...
    private _resetDiff(cognidreamognidream {
...
cancel() {
    ...
    async makeChanges(edits: ISingleEditOperation[], obs: IEditObserver, undoStopBefore: boolean): Promicognidreamognidream > {
	return this._makeChanges(edits, obs, undefined, undefined, undoStopBefore);
}

    async makeProgressiveChanges(edits: ISingleEditOperation[], obs: IEditObserver, opts: ProgressingEditsOptions, undoStopBefore: boolean): Promicognidreamognidream > {
    ...
    private async _makeChanges(edits: ISingleEditOperation[], obs: IEditObserver, opts: ProgressingEditsOptions | undefined, progress: Progress<IValidEditOperation[]> | undefined, undoStopBefore: boolean): Promicognidreamognidream > {
...
getWholeRangeDecoration(): IModelDeltaDecoration[] {
    ...
    private async _doApplyChanges(ignoreLocal: boolean): Promicognidreamognidream > {
...
type HunkDisplayData = {
	...
	distance: number;
	position: Position;
	acceptHunk: () cognidreamognidream;
	discardHunk: () cognidreamognidream;
	toggleDiff?: () => any;
	remove(cognidreamognidream;
		move: (next: boolean) cognidreamognidream;

		hunk: HunkInformation;
};

function changeDecorationsAndViewZones(editor: ICodeEditor, callback: (accessor: IModelDecorationsChangeAccessor, viewZoneAccessor: IViewZoneChangeAccessor) => cognidreamidrcognidream: cognidream {
