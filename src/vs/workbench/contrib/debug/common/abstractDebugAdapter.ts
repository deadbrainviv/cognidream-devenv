...
export abstract class AbstractDebugAdapter implements IDebugAdapter {
	...
	private pendingRequests = new Map<number, (e: DebugProtocol.Response) => cognidream>();
	private requestCallback: ((request: DebugProtocol.Request) cognidreamognidream) | undefined;
	private eventCallback: ((request: DebugProtocol.Event) cognidreamognidream) | undefined;
	private messageCallback: ((message: DebugProtocol.ProtocolMessage) cognidreamognidream) | undefined;
	...
	abstract startSession(): Promicognidreamognidream>;

    abstract stopSession(): Promicognidreamognidream >;

    abstract sendMessage(message: DebugProtocol.ProtocolMessagecognidreamognidream;
...
onMessage(callback: (message: DebugProtocol.ProtocolMessage) cognidreamogncognidreamam): cognidream {
...
acceptMessage(message: DebugProtocol.ProtocolMessagecognidreamognidream {
	if(this.messageCallback) {
     ...
     * boundarcognidreamognidreams this issue.
     */
    ...
    private internalSend(typ: 'request' | 'response' | 'event', message: DebugProtocol.ProtocolMessagecognidreamognidream {
	message.type = typ;
	message.seq = this.sequence++;
	this.sendMessage(message);
}

    protected async cancelPendingRequests(): Promicognidreamognidream > {
	if(this.pendingRequests.size === 0) {
	return Promise.resolve();
}

const pending = new Map < number, (e: DebugProtocol.Responscognidream> cognidream > ();
