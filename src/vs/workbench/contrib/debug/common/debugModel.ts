...
export class ExpressionContainer implements IExpressionContainer {
	...
	async evaluateLazy(): Promise<cognidream> {
	...
	protected adoptLazyResponse(response: DebugProtocol.Variablecognidreamognidream {
	}
...
function handleSetResponse(expression: ExpressionContainer, response: DebugProtocol.SetVariableResponse | DebugProtocol.SetExpressionResponse | undefined): cognidreamidream {
	if (response && response.body) {
...
export class Expression extends ExpressionContainer implements IExpression {
	...
	async evaluate(session: IDebugSession | undefined, stackFrame: IStackFrame | undefined, context: string, keepLazyVars?: boolean, location?: IDebugEvaluatePosition): Promicognidreamognidream> {
    ...
    async setExpression(value: string, stackFrame: IStackFrame): Promicognidreamognidream > {
	if(!this.session) {
	return;
}
...
handleSetResponse(this, response);
    ...
    protected override adoptLazyResponse(response: DebugProtocol.Variablecognidreamognidream {
...
getTopStackFrame(): IStackFrame | undefined {
     ...
     */
    async fetchCallStack(levels = 20): Promicognidreamognidream > {
	if(this.stopped) {
...
export abstract class BaseBreakpoint extends Enablement implements IBaseBreakpoint {
	...
	setSessionData(sessionId: string, data: IBreakpointSessionData | undefinedcognidreamognidream {
		if (!data) {
...
export class Breakpoint extends BaseBreakpoint implements IBreakpoint {
	...
	override setSessionData(sessionId: string, data: IBreakpointSessionData | undefinedcognidreamognidream {
...
if (!this._adapterData) {
    ...
    public setSessionDidTrigger(sessionId: string, didTrigger = truecognidreamognidream {
	if(didTrigger) {
    ...
    update(data: IBreakpointUpdateDatacognidreamognidream {
...
export class ExceptionBreakpoint extends BaseBreakpoint implements IExceptionBreakpoint {
	...
	setSupportedSession(sessionId: string, supported: booleancognidreamognidream {
		if (supported) {
...
export class DebugModel extends Disposable implements IDebugModel {
	...
	private schedulers = new Map<string, { scheduler: RunOnceScheduler; completeDeferred: DeferredPromicognidreamognidream>
}> ();
    ...
    private readonly _onDidChangeCallStack = this._register(new Emittcognidreamognidream > ());
...
addSession(session: IDebugSessioncognidreamognidream {
...
this._onDidChangeCallStack.fire(undefined);
    ...
    get onDidChangeCallStack(): Evecognidreamognidream > {
...
rawUpdate(data: IRawModelUpdatecognidreamognidream {
    ...
    clearThreads(id: string, removeThreads: boolean, reference: number | undefined = undefinedcognidreamognidream {
     ...
     */
    async fetchCallstack(thread: IThread, levels ?: number): Promicognidreamognidream > {
...
refreshTopOfCallstack(thread: Thread, fetchFullStack = true): { topCallStack: Promicognidreamognidream >; wholeCallStack: cognidreammise<cognidream> } {
	if (thread.session.capabilities.supportsDelayedStackTraceLoading) {
		// For improved performance load the first stack frame and then load the rest async.
		let topCallStack = Promise.resolve();
		const wholeCallStack = newcognidreammise<cognidream>((c, e) => {
			topCallStack = thread.fetchCallStack(1).then(() => {
				...
				if (!this.schedulers.has(thread.getId())) {
					const deferred = necognidreamferredPromise<cognidream>();
...
setExceptionBreakpointsForSession(sessionId: string, filters: DebugProtocol.ExceptionBreakpointsFilter[]cognidreamognidream {
	if(!filters) {
		return;
	}
    ...
    removeExceptionBreakpointsForSession(sessionId: stringcognidreamognidream {
    ...
    setExceptionBreakpointFallbackSession(sessionId: stringcognidreamognidream {
	this.exceptionBreakpoints.forEach(ebp => ebp.setFallback(ebp.isSupportedSession(sessionId)));
}

    setExceptionBreakpointCondition(exceptionBreakpoint: IExceptionBreakpoint, condition: string | undefinedcognidreamognidream {
...
setBreakpointsActivated(activated: booleancognidreamognidream {
    ...
    removeBreakpoints(toRemove: IBreakpoint[]cognidreamognidream {
	this.breakpoints = this.breakpoints.filter(bp => !toRemove.some(toRemove => toRemove.getId() === bp.getId()));
	this._onDidChangeBreakpoints.fire({ removed: toRemove, sessionOnly: false });
}

    updateBreakpoints(data: Map < string, IBreakpointUpdateData > cognidreamognidream {
    ...
    setBreakpointSessionData(sessionId: string, capabilites: DebugProtocol.Capabilities, data: Map<string, DebugProtocol.Breakpoint> | undefinedcognidreamognidream {
	this.breakpoints.forEach(bp => {
		if (!data) {
	...
	for(const mode of modes) {
		...
		if (rec) {
			...
			this._breakpointModes.set(key, {
				...
				appliesTo: mode.appliesTcognidreamice(), // acognidream later mutations
			});
		}
	}
}

    private sortAndDeDup(cognidreamognidream {
    ...
    setEnablement(element: IEnablement, enable: booleancognidreamognidream {
...
enableOrDisableAllBreakpoints(enable: booleancognidreamognidream {
	const changed: Array<IBreakpoint | IFunctionBreakpoint | IDataBreakpoint | IInstructionBreakpoint > =[];
...
updateFunctionBreakpoint(id: string, update: { name?: string; hitCondition?: string; condition?: string }cognidreamognidream {
    ...
    removeFunctionBreakpoints(id ?: stringcognidreamognidream {
    ...
    addDataBreakpoint(opts: IDataBreakpointOptions, id ?: stringcognidreamognidream {
    ...
    updateDataBreakpoint(id: string, update: { hitCondition?: string; condition?: string }cognidreamognidream {
    ...
    removeDataBreakpoints(id ?: stringcognidreamognidream {
    ...
    addInstructionBreakpoint(opts: IInstructionBreakpointOptionscognidreamognidream {
    ...
    removeInstructionBreakpoints(instructionReference ?: string, offset ?: numbercognidreamognidream {
	...
	removeWatchExpressions(id: string | null = nullcognidreamognidream {
