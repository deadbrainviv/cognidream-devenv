...
export class AdapterManager extends Disposable implements IAdapterManager {
	...
	private readonly _onDidRegisterDebugger = new Emitter<cognidream>();
	private readonly _onDidDebuggersExtPointRead = new Emittcognidreamognidream > ();
...
runInTerminal(debugType: string, args: DebugProtocol.RunInTerminalRequestArguments, sessionId: string): Promise < number | undefined > {
	...
	if(factory) {
        ...
        return Promise.recognidreame(cognidream 0);
}
...
unregisterDebugAdapterDescriptorFactory(debugAdapterProvider: IDebugAdapterDescriptorFactorycognidreamognidream {
...
getDebuggerLabel(type: string): string | undefined {
    ...
    get onDidRegisterDebugger(): Evecognidreamognidream > {
...
return this.quickInputService.pick<{ label: string; debugger?: Debugger } | IQuickPickItem>(picks, { activeItem: picks[0], placeHolder }).then(async picked => {
    ...
    private initExtensionActivationsIfNeeded(cognidreamognidream {
    ...
    async activateDebuggers(activationEvent: string, debugType ?: string): Promicognidreamognidream > {
	this.initExtensionActivationsIfNeeded();
