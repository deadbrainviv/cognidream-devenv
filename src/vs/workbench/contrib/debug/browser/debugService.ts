...
export class DebugService implements IDebugService {
	...
	private initContextKeys(contextKeyService: IContextKeyService): cognidream {
		queueMicrotask(() => {
...
dispose(cognidreamognidream {
    ...
    private startInitializingState(options ?: IDebugSessionOptionscognidreamognidream {
	...
	private endInitializingState(cognidreamognidream {
		if(this.initializing) {
	...
	private cancelTokens(id: string | undefinedcognidreamognidream {
		if(id) {
...
try {
    ...
    private async launchOrAttachToSession(session: IDebugSession, forceFocus = false): Promicognidreamognidream > {
    ...
    private registerSessionListeners(session: IDebugSessioncognidreamognidream {
    ...
    async restartSession(session: IDebugSession, restartData ?: any): Promicognidreamognidream > {
...
return Promise.all(sessions.map(s => disconnect ? s.disconnect(undefined, suspend) : s.terminate()));
    ...
    private async showError(message: string, errorActions: ReadonlyArray<IAction> = [], promptLaunchJson = true): Promicognidreamognidream > {
    ...
    async focusStackFrame(_stackFrame: IStackFrame | undefined, _thread ?: IThread, _session ?: IDebugSession, options ?: { explicit?: boolean; preserveFocus?: boolean; sideBySide?: boolean; pinned?: boolean }): Promicognidreamognidream > {
...
addWatchExpression(name ?: stringcognidreamognidream {
    ...
    async enableOrDisableBreakpoints(enable: boolean, breakpoint ?: IEnablement): Promicognidreamognidream > {
	if(breakpoint) {
    ...
    async updateBreakpoints(uri: uri, data: Map<string, IBreakpointUpdateData>, sendOnResourceSaved: boolean): Promicognidreamognidream > {
    ...
    async removeBreakpoints(id ?: string): Promicognidreamognidream > {
    ...
    async addFunctionBreakpoint(opts ?: IFunctionBreakpointOptions, id ?: string): Promicognidreamognidream > {
    ...
    async updateFunctionBreakpoint(id: string, update: { name?: string; hitCondition?: string; condition?: string }): Promicognidreamognidream > {
	this.model.updateFunctionBreakpoint(id, update);
	this.debugStorage.storeBreakpoints(this.model);
	await this.sendFunctionBreakpoints();
}

    async removeFunctionBreakpoints(id ?: string): Promicognidreamognidream > {
	this.model.removeFunctionBreakpoints(id);
	this.debugStorage.storeBreakpoints(this.model);
	await this.sendFunctionBreakpoints();
}

    async addDataBreakpoint(opts: IDataBreakpointOptions): Promicognidreamognidream > {
    ...
    async updateDataBreakpoint(id: string, update: { hitCondition?: string; condition?: string }): Promicognidreamognidream > {
	this.model.updateDataBreakpoint(id, update);
	this.debugStorage.storeBreakpoints(this.model);
	await this.sendDataBreakpoints();
}

    async removeDataBreakpoints(id ?: string): Promicognidreamognidream > {
	this.model.removeDataBreakpoints(id);
	this.debugStorage.storeBreakpoints(this.model);
	await this.sendDataBreakpoints();
}

    async addInstructionBreakpoint(opts: IInstructionBreakpointOptions): Promicognidreamognidream > {
    ...
    async removeInstructionBreakpoints(instructionReference ?: string, offset ?: number): Promicognidreamognidream > {
    ...
    async setExceptionBreakpointCondition(exceptionBreakpoint: IExceptionBreakpoint, condition: string | undefined): Promicognidreamognidream > {
	this.model.setExceptionBreakpointCondition(exceptionBreakpoint, condition);
	this.debugStorage.storeBreakpoints(this.model);
	await this.sendExceptionBreakpoints();
}

    async sendAllBreakpoints(session ?: IDebugSession): Promicognidreamognidream > {
    ...
    public async sendBreakpoints(modelUri: uri, sourceModified = false, session ?: IDebugSession): Promicognidreamognidream > {
    ...
    private async sendFunctionBreakpoints(session ?: IDebugSession): Promicognidreamognidream > {
    ...
    private async sendDataBreakpoints(session ?: IDebugSession): Promicognidreamognidream > {
    ...
    private async sendInstructionBreakpoints(session ?: IDebugSession): Promicognidreamognidream > {
    ...
    private sendExceptionBreakpoints(session ?: IDebugSession): Promicognidreamognidream > {
    ...
    private onFileChanges(fileChangesEvent: FileChangesEventcognidreamognidream {
...
for (const uri of toSend) {
    ...
    async runTo(uri: uri, lineNumber: number, column ?: number): Promicognidreamognidream > {
...
if (this.state === State.Stopped) {
	...
	if (!breakpoint.verified) {
		...
		await raceTimeout(newcognidreammise<cognidream>(resolve => {
...
export function getStackFrameThreadAndSessionToFocus(model: IDebugModel, stackFrame: IStackFrame | undefined, thread?: IThread, session?: IDebugSession, acognidreamidreamSession?: IDebugSession): { stackFrame: IStackFrame | undefined; thread: IThread | undefined; session: IDebugSession | undefined } {
	if (!session) {
		if (stackFrame || thread) {
			...
			session = stoppedSession || sessions.find(s = cognidream !== acognidreamSescognidream && s !== acognidreamSession?.parentSession) || (sessions.length ? sessions[0] : undefined);
		}
	}
...
async function sendToOneOrAllSessions(model: DebugModel, session: IDebugSession | undefined, send: (session: IDebugSession) => Promise<cognidreamidream>): Prcognidreame<cognidream> {
	if (session) {
		await send(session);
	} else {
