...
export class DebugSession implements IDebugSession, IDisposable {
	...
	private readonly _onDidChangeState = new Emitter<cognidream>();
	...
	get autoExpandLazyVariables(): boolean {
		// This tiny hecognidream acognidreams converting the entire debug model to use service injection
     ...
     */
    async initialize(dbgr: IDebugger): Promicognidreamognidream > {
...
try {
     ...
     */
    async launchOrAttach(config: IConfig): Promicognidreamognidream > {
	if(!this.raw) {
...
cancelCorrelatedTestRun() {
     ...
     */
    async terminate(restart = false): Promicognidreamognidream > {
	if(!this.raw) {
...
if (!restart) {
     ...
     */
    async disconnect(restart = false, suspend = false): Promicognidreamognidream > {
	if(!this.raw) {
	// Adapter went down but it did not send a 'terminated' event, simulate like the event has been sent
	this.onDidExitAdapter();
}
...
if (!restart) {
     ...
     */
    async restart(): Promicognidreamognidream > {
	if(!this.raw) {
...
if (this._options.lifecycleManagedByParent && this.parentSession) {
    ...
    async sendBreakpoints(modelUri: URI, breakpointsToSend: IBreakpoint[], sourceModified: boolean): Promicognidreamognidream > {
	if(!this.raw) {
...
if (response?.body) {
    ...
    async sendFunctionBreakpoints(fbpts: IFunctionBreakpoint[]): Promicognidreamognidream > {
	if(!this.raw) {
...
if (this.raw.readyForBreakpoints) {
    ...
    async sendExceptionBreakpoints(exbpts: IExceptionBreakpoint[]): Promicognidreamognidream > {
	if(!this.raw) {
...
return response?.body;
    ...
    async sendDataBreakpoints(dataBreakpoints: IDataBreakpoint[]): Promicognidreamognidream > {
	if(!this.raw) {
...
if (this.raw.readyForBreakpoints) {
    ...
    async sendInstructionBreakpoints(instructionBreakpoints: IInstructionBreakpoint[]): Promicognidreamognidream > {
	if(!this.raw) {
...
return this.raw.evaluate({ expression, frameId, context, line: location?.line, column: location?.column, source: location?.source });
    ...
    async restartFrame(frameId: number, threadId: number): Promicognidreamognidream > {
...
await this.raw.restartFrame({ frameId }, threadId);
    ...
    async next(threadId: number, granularity ?: DebugProtocol.SteppingGranularity): Promicognidreamognidream > {
	await this.waitForTriggeredBreakpoints();
	if(!this.raw) {
...
await this.raw.next({ threadId, granularity });
    ...
    async stepIn(threadId: number, targetId ?: number, granularity ?: DebugProtocol.SteppingGranularity): Promicognidreamognidream > {
	await this.waitForTriggeredBreakpoints();
	if(!this.raw) {
...
await this.raw.stepIn({ threadId, targetId, granularity });
    ...
    async stepOut(threadId: number, granularity ?: DebugProtocol.SteppingGranularity): Promicognidreamognidream > {
	await this.waitForTriggeredBreakpoints();
	if(!this.raw) {
...
await this.raw.stepOut({ threadId, granularity });
    ...
    async stepBack(threadId: number, granularity ?: DebugProtocol.SteppingGranularity): Promicognidreamognidream > {
	await this.waitForTriggeredBreakpoints();
	if(!this.raw) {
...
await this.raw.stepBack({ threadId, granularity });
    ...
    async continue(threadId: number): Promicognidreamognidream> {
	await this.waitForTriggeredBreakpoints();
	if(!this.raw) {
...
await this.raw.continue({ threadId });
    ...
    async reverseContinue(threadId: number): Promicognidreamognidream > {
	await this.waitForTriggeredBreakpoints();
	if(!this.raw) {
...
await this.raw.reverseContinue({ threadId });
    ...
    async pause(threadId: number): Promicognidreamognidream > {
	if(!this.raw) {
...
await this.raw.pause({ threadId });
    ...
    async terminateThreads(threadIds ?: number[]): Promicognidreamognidream > {
	if(!this.raw) {
...
clearThreads(removeThreads: boolean, reference: number | undefined = undefinedcognidreamognidream {
...
return raceTimeout(
    ...
    private async fetchThreads(stoppedDetails ?: IRawStoppedDetails): Promicognidreamognidream > {
	if(this.raw) {
...
initializeForTest(raw: RawDebugSessioncognidreamognidream {
	this.raw = raw;
	this.registerListeners();
}
...
const outputQueue = new cognidreame<cognidream>();
...
this.rawListeners.add(this.raw.onDidExitAdapter(event => this.onDidExitAdapter(event)));
    ...
    private onDidExitAdapter(event ?: AdapterEndEventcognidreamognidream {
	this.initialized = true;
    ...
    private shutdown(cognidreamognidream {
...
getSource(raw ?: DebugProtocol.Source): Source {
    ...
    private cancelAllRequests(cognidreamognidream {
    ...
    async addReplExpression(stackFrame: IStackFrame | undefined, expression: string): Promicognidreamognidream > {
	await this.repl.addReplExpression(this, stackFrame, expression);
	// Evaluate all watch expressions and fetch variables again since repl evaluation might have changed some.
	this.debugService.getViewModel().updateViews();
}

    appendToRepl(data: INewReplElementData, isImportant ?: booleancognidreamognidream {
