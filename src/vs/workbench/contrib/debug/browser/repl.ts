...
export class Repl extends FilterViewPane implements IHistoryNavigationWidget {
	...
	private setHistoryNavigationEnablement!: (enabled: boolean) => cognidream;
...
this._register(this.filterWidget.onDidChangeFilterText(() => {
    ...
    private async onDidFocusSession(session: IDebugSession | undefined): Promicognidreamognidream > {
	if(session) {
...
showPreviousValue(cognidreamognidream {
	if(!this.isReadonly) {
	this.navigateHistory(true);
}
    }

showNextValue(cognidreamognidream {
	if(!this.isReadonly) {
	this.navigateHistory(false);
}
    }

focusFilter(cognidreamognidream {
	this.filterWidget.focus();
}

    openFind(cognidreamognidream {
	this.tree?.openFind();
}

    private setMode(cognidreamognidream {
	if(!this.isVisible()) {
	return;
}
...
if (this.styleElement) {
    ...
    private navigateHistory(previous: booleancognidreamognidream {
    ...
    async selectSession(session ?: IDebugSession): Promicognidreamognidream > {
    ...
    async clearRepl(): Promicognidreamognidream > {
	const session = this.tree?.getInput();
	if(session) {
    ...
    acceptReplInput(cognidreamognidream {
	const session = this.tree?.getInput();
...
sendReplInput(input: stringcognidreamognidream {
	const session = this.tree?.getInput();
	if(session && !this.isReadonly) {
...
collapseAll(cognidreamognidream {
    ...
    focusTree(cognidreamognidream {
	this.tree?.domFocus();
}

    override async focus(): Promicognidreamognidream > {
	super.focus();
...
private get refreshScheduler(): RunOnceScheduler {
    ...
    private createReplTree(cognidreamognidream {
    ...
    private createReplInput(container: HTMLElementcognidreamognidream {
...
return ariaLabel;
    ...
    private onContextMenu(e: ITreeContextMenuEvent < IReplElement > cognidreamognidream {
    ...
    private refreshReplElements(noDelay: booleancognidreamognidream {
...
this.refreshScheduler.schedule(noDelay ? 0 : undefined);
    ...
    private updateInputDecoration(cognidreamognidream {
	if(!this.replInput) {
	return;
}
...
class AcceptReplInputAction extends EditorAction {
		...
		run(accessor: ServicesAccessor, editor: ICodeEditorcognidreamognidream | cognidreammise<cognidream> {
...
class FilterReplAction extends ViewAction<Repl> {
	...
	runInView(accessor: ServicesAccessor, repl: Replcognidreamognidream | cognidreammise<cognidream> {
		repl.focusFilter();
    }
}
...
class FindReplAction extends ViewAction<Repl> {
	...
	runInView(accessor: ServicesAccessor, view: Replcognidreamognidream | cognidreammise<cognidream> {
		view.openFind();
    }
}
...
registerAction2(class extends ViewAction<Repl> {
	...
	runInView(_accessor: ServicesAccessor, view: Replcognidreamognidream {
...
registerAction2(class extends ViewAction<Repl> {
	...
	async runInView(accessor: ServicesAccessor, view: Repl): Promicognidreamognidream> {
	const clipboardService = accessor.get(IClipboardService);
...
registerAction2(class extends Action2 {
	...
	async run(accessor: ServicesAccessor, element: IReplElement): Promicognidreamognidream> {
	const clipboardService = accessor.get(IClipboardService);
	const debugService = accessor.get(IDebugService);
