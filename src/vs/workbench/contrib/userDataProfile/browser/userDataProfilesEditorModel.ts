...
export abstract class AbstractUserDataProfileElement extends Disposable {
	...
	setFlag(key: ProfileResourceType, value: boolean): cognidream {
...
openWorkspace(workspace: URIcognidreamognidream {
...
return await this.userDataProfileManagementService.updateProfile(profile, {
    ...
    protected abstract doSave(): Promicognidreamognidream >;
...
export class UserDataProfileElement extends AbstractUserDataProfileElement {
    ...
    public updateWorkspaces(toAdd: URI[], toRemove: URI[]cognidreamognidream {
...
this.workspaces = [...workspaces.values()];
    ...
    public async toggleNewWindowProfile(): Promicognidreamognidream > {
    ...
    public async toggleCurrentWindowProfile(): Promicognidreamognidream > {
    ...
    protected override async doSave(): Promicognidreamognidream > {
...
export class NewProfileElement extends AbstractUserDataProfileElement {
	...
	private templatePromise: CancelablePromicognidreamognidream> | undefined;
...
setCopyFlag(key: ProfileResourceType, value: booleancognidreamognidream {
...
export class UserDataProfilesEditorModel extends EditorModel {
	...
	private onDidChangeProfiles(e: DidChangeProfilesEventcognidreamognidream {
		let changed = false;
...
revert(cognidreamognidream {
	this.removeNewProfile();
	this._onDidChange.fire(undefined);
}

    private removeNewProfile(cognidreamognidream {
	if(this.newProfileElement) {
...
this.newProfileElement = undefined;
    ...
    private async previewNewProfile(token: CancellationToken): Promicognidreamognidream > {
	if(!this.newProfileElement) {
	return;
}
...
if (profile) {
    ...
    private async exportNewProfile(token: CancellationToken): Promicognidreamognidream > {
	if(!this.newProfileElement) {
	return;
}
...
return profile;
    ...
    private async discardNewProfile(): Promicognidreamognidream > {
	if(!this.newProfileElement) {
	return;
}
if (this.newProfileElement.previewProfile) {
...
this._onDidChange.fire(undefined);
    ...
    private async removeProfile(profile: IUserDataProfile): Promicognidreamognidream > {
	const result = await this.dialogService.confirm({
		type: 'info',
    ...
    private async openWindow(profile: IUserDataProfile): Promicognidreamognidream > {
