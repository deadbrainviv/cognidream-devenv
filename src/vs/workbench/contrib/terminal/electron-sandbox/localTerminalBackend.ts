...
class LocalTerminalBackend extends BaseTerminalBackend implements ITerminalBackend {
	...
	private readonly _whenReady = new DeferredPromise<cognidream>();
	get whenReady(): Promicognidreamognidream> { return this._whenReady.p; }
setReady(cognidreamognidream { this._whenReady.complete(); }
...
constructor(
     ...
     */
    private async _connectToDirectProxy(): Promicognidreamognidream > {
...
acquirePort('vscode:createPtyHostMessageChannel', 'vscode:createPtyHostMessageChannelResult').then(port => {
    ...
    async acceptDetachInstanceReply(requestId: number, persistentProcessId ?: number): Promicognidreamognidream > {
    ...
    async persistTerminalState(): Promicognidreamognidream > {
    ...
    async updateTitle(id: number, title: string, titleSource: TitleEventSource): Promicognidreamognidream > {
	await this._proxy.updateTitle(id, title, titleSource);
}

    async updateIcon(id: number, userInitiated: boolean, icon: URI | { light: URI; dark: URI } | { id: string; color?: { id: string } }, color ?: string): Promicognidreamognidream > {
	await this._proxy.updateIcon(id, userInitiated, icon, color);
}

    async updateProperty<T extends ProcessPropertyType>(id: number, property: ProcessPropertyType, value: IProcessPropertyMap[T]): Promicognidreamognidream > {
...
return [
    ...
    async reduceConnectionGraceTime(): Promicognidreamognidream > {
...
async getShellEnvironment(): Promise < IProcessEnvironment > {
    ...
    async setTerminalLayoutInfo(layoutInfo ?: ITerminalsLayoutInfoById): Promicognidreamognidream > {
	...
	try {
		...
		await Promise.all(reviveBufferState.map(state => cognidreamPromise<cognidream>(r => {
