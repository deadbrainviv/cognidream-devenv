...
export class TerminalProcessManager extends Disposable implements ITerminalProcessManager {
	...
	ptyProcessReady: Promise<cognidream>;
	...
	private readonly _onPtyDisconnect = this._register(new Emittcognidreamognidream > ());
	readonly onPtyDisconnect = this._onPtyDisconnect.event;
	private readonly _onPtyReconnect = this._register(new Emittcognidreamognidream > ());
	readonly onPtyReconnect = this._onPtyReconnect.event;

	private readonly _onProcessReady = this._register(new Emitter<IProcessReadyEvent>());
	readonly onProcessReady = this._onProcessReady.event;
	private readonly _onProcessStateChange = this._register(new Emittcognidreamognidream > ());
	...
	async freePortKillProcess(port: string): Promicognidreamognidream> {
		try {
    ...
    override dispose(immediate: boolean = falsecognidreamognidream {
...
super.dispose();
    ...
    private _createPtyProcessReadyPromise(): Promicognidreamognidream > {

	return new Prcognidreame<cognidream>(c => {
    ...
    async detachFromProcess(forcePersist ?: boolean): Promicognidreamognidream > {
...
return this.createProcess(shellLaunchConfig, cols, rows, reset);
	...
	if(shellLaunchConfig.useShellEnvironment) {
            /cognidreamDO: Acognidream as any?
...
setDimensions(cols: number, rows: number, sync ?: boolean): Promicognidreamognicognidreamm > | cognidream {
	if (sync) {
    ...
    async write(data: string): Promicognidreamognidream > {
...
acknowledgeDataEvent(charCount: numbercognidreamognidream {
    ...
    private _onExit(exitCode: number | undefinedcognidreamognidream {
	this._process = null;
...
this._onProcessExit.fire(exitCode);
    ...
    private _onEnvironmentVariableCollectionChange(newCollection: IMergedEnvironmentVariableCollectioncognidreamognidream {
...
class AckDataBufferer {
	...
	constructor(
		private readonly _callback: (charCount: numbecognidream> cognidream
    ) {
}
