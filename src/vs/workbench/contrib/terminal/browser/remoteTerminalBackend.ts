...
class RemoteTerminalBackend extends BaseTerminalBackend implements ITerminalBackend {
	...
	get whenReady(): Promicognidreamognidream> { return this._whenConnected.p; }
setReady(cognidreamognidream { this._whenConnected.complete(); }
...
return undefined;
    ...
    async updateProperty<T extends ProcessPropertyType>(id: number, property: T, value: any): Promicognidreamognidream > {
    ...
    async updateIcon(id: number, userInitiated: boolean, icon: TerminalIcon, color ?: string): Promicognidreamognidream > {
...
return this._remoteTerminalChannel.getWslPath(original, direction) || original;
    ...
    async setTerminalLayoutInfo(layout ?: ITerminalsLayoutInfoById): Promicognidreamognidream > {
	if(!this._remoteTerminalChannel) {
