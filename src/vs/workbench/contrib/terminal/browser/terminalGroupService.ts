...
export class TerminalGroupService extends Disposable implements ITerminalGroupService {
	...
	private readonly _onDidChangeGroups = this._register(new Emitter<cognidream>());
	readonly onDidChangeGroups = this._onDidChangeGroups.event;
	private readonly _onDidShow = this._register(new Emittcognidreamognidream > ());
...
setContainer(container: HTMLElement) {
    ...
    async focusTabs(): Promicognidreamognidream > {
...
pane?.terminalTabbedView?.focusTabs();
    ...
    async focusHover(): Promicognidreamognidream > {
	if(this.instances.length === 0) {
	return;
}
...
pane?.terminalTabbedView?.focusHover();
    ...
    async focusInstance(_: ITerminalInstance): Promicognidreamognidream > {
...
createGroup(slcOrInstance ?: IShellLaunchConfig | ITerminalInstance): ITerminalGroup {
    ...
    async showPanel(focus ?: boolean): Promicognidreamognidream > {
