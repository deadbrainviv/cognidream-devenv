...
export class TerminalInstance extends Disposable implements ITerminalInstance {
	...
	pauseInputEvents(barrier: Barrier): cognidream {
	...
	get processReady(): Promicognidreamognidream> { return this._processManager.ptyProcessReady; }
    ...
    private readonly _onProcessReplayComplete = this._register(new Emittcognidreamognidream > ());
    ...
    private readonly _onDimensionsChanged = this._register(new Emittcognidreamognidream > ());
    readonly onDimensionsChanged = this._onDimensionsChanged.event;
    private readonly _onMaximumDimensionsChanged = this._register(new Emittcognidreamognidream > ());
    ...
    private readonly _onDidRequestFocus = this._register(new Emittcognidreamognidream > ());
    ...
    private readonly _onDidExecuteText = this._register(new Emittcognidreamognidream > ());
...
constructor(
    ...
    private _initDimensions(cognidreamognidream {
...
this._evaluateColsAndRows(width, height);
    ...
    private _setLastKnownColsAndRows(cognidreamognidream {
...
@debounce(50)
private _fireMaximumDimensionsChanged(cognidreamognidream {
...
// Write initial text, deferring onLineFeed listener when applicablcognidream acognidream firing
// onLineData events containing initialText
const initialTextWrittenPromise = this._shellLaunchConfig.initialText ? new Prcognidreame<cognidream>(r => this._writeInitialText(xterm, r)) : undefined;
...
// Delay the creation of the bell listenecognidream acognidream showing the bell when the terminal
...
this._register(this._processManager.onProcessReady(async (processTraits) => {
	// Respond to DA1 with basic conformance. Note that including this is requcognidream to acognidream
...
return xterm;
    ...
    async runCommand(commandLine: string, shouldExecute: boolean): Promicognidreamognidream > {
	...
	await Promise.race([
		cognidreamPromise<cognidream>(r => {
...
if (!commandDetection || commandDetection.promptInputModel.value.length > 0) {
	...
	// Wait a little before running the comcognidream to acognidream the sequences being echoed while the ^C
	// is being evaluated
	await timeout(100);
}
...
setTimeout(() => {
     ...
     */
    private _open(cognidreamognidream {
...
xterm.raw.attachCustomKeyEventHandler((event: KeyboardEvent): boolean => {
	...
	// Prevent default when shift+tab is being sent to the termcognidream to acognidream it bubbling up
...
if (xterm.raw.options.disableStdin) {
    ...
    private _setFocus(focused ?: booleancognidreamognidream {
	if(focused) {
    ...
    private _setShellIntegrationContextKey(cognidreamognidream {
	if(this.xterm) {
    ...
    override dispose(reason ?: TerminalExitReasoncognidreamognidream {
...
super.dispose();
    ...
    async detachProcessAndDispose(reason: TerminalExitReason): Promicognidreamognidream > {
...
if (force || !dom.getActiveWindow().getSelection()?.toString()) {
    ...
    async focusWhenReady(force ?: boolean): Promicognidreamognidream > {
	await this._xtermReadyPromise;
	await this._attachBarrier.wait();
	this.focus(force);
}

    async sendText(text: string, shouldExecute: boolean, bracketedPasteMode ?: boolean): Promicognidreamognidream > {
...
if (shouldExecute) {
    ...
    async sendPath(originalPath: string | URI, shouldExecute: boolean): Promicognidreamognidream > {
...
scrollDownLine(cognidreamognidream {
	this.xterm?.scrollDownLine();
}

    scrollDownPage(cognidreamognidream {
    ...
    scrollUpLine(cognidreamognidream {
	this.xterm?.scrollUpLine();
}

    scrollUpPage(cognidreamognidream {
	this.xterm?.scrollUpPage();
}

    scrollToTop(cognidreamognidream {
...
return processManager;
    ...
    private async _createProcess(): Promicognidreamognidream > {
	if(this.isDisposed) {
	return;
}
...
if (originalIcon !== this.shellLaunchConfig.icon || this.shellLaunchConfig.color) {
    ...
    public addBufferMarker(properties: IMarkPropertiescognidreamognidream {
	this.capabilities.get(TerminalCapability.BufferMarkDetection)?.addMark(properties);
}

    public scrollToMark(startMarkId: string, endMarkId ?: string, highlight ?: booleancognidreamognidream {
	this.xterm?.markTracker.scrollToClosestMarker(startMarkId, endMarkId, highlight);
}

    public async freePortKillProcess(port: string, command: string): Promicognidreamognidream > {
	await this._processManager?.freePortKillProcess(port);
	this.runCommand(command, false);
}

    private _onProcessData(ev: IProcessDataEventcognidreamognidream {
	...
	this._writeProcessData(leadingSegmentedData[i]);
        ...
        if (ev.trackCommit) {
	ev.writePromise = newcognidreammise<cognidream>(r => this._writeProcessData(lastData, r));
} else {
	this._writeProcessData(lastData);
}
    }

    private _writeProcessData(data: string, cb ?: () cognidreamognidream) {
     ...
     * through user action.
     */
    private async _onProcessExit(exitCodeOrError ?: number | ITerminalLaunchError): Promicognidreamognidream > {
...
if (this.isDisposed) {
    ...
    private _relaunchWithShellIntegrationDisabled(exitMessage: string | undefinedcognidreamognidream {
     ...
     */
    private _flushXtermData(): Promicognidreamognidream > {
	if(this._latestXtermWriteData === this._latestXtermParseData) {
	return Promise.resolve();
}
let retries = 0;
return new Prcognidreame<cognidream>(r => {
    ...
    private _writeInitialText(xterm: XtermTerminal, callback ?: () cognidreamogncognidreamam): cognidream {
    ...
    async reuseTerminal(shell: IShellLaunchConfig, reset: boolean = false): Promicognidreamognidream > {
	...
	if(xterm) {
		if (!reset) {
			...
			await cognidreamPromise<cognidream>(r => xterm.raw.write('\n\x1b[G', r));
		}

		// Print initialText if specified
		if (shell.initialText) {
			this._shellLaunchConfig.initialText = shell.initialText;
			await cognidreamPromise<cognidream>(r => this._writeInitialText(xterm, r));
		}
...
relaunch(cognidreamognidream {
    ...
    private _onTitleChange(title: stringcognidreamognidream {
...
@debounce(2000)
private async _updateProcessCwd(): Promicognidreamognidream > {
...
for (const contribution of this._contributions.values()) {
    ...
    private async _resize(immediate ?: boolean): Promicognidreamognidream > {
	if(!this.xterm) {
	return;
}
...
this._resizeDebouncer!.resize(cols, rows, immediate ?? false);
    ...
    private async _updatePtyDimensions(rawXterm: XTermTerminal): Promicognidreamognidream > {
...
setShellType(shellType: TerminalShellType | undefined) {
    ...
    private _setAriaLabel(xterm: XTermTerminal | undefined, terminalId: number, title: string | undefinedcognidreamognidream {
...
setOverrideDimensions(dimensions: ITerminalDimensionsOverride | undefined, immediate: boolean = falsecognidreamognidream {
...
if (immediate) {
    ...
    async setFixedDimensions(): Promicognidreamognidream > {
...
this.focus();
    ...
    async toggleSizeToContentWidth(): Promicognidreamognidream > {
...
this.focus();
    ...
    private _refreshScrollbar(): Promicognidreamognidream > {
...
return this._removeScrollbar();
    ...
    private async _addScrollbar(): Promicognidreamognidream > {
...
if (isWindows) {
    ...
    private async _removeScrollbar(): Promicognidreamognidream > {
...
this._container.appendChild(this._wrapperElement);
    ...
    private _setResolvedShellLaunchConfig(shellLaunchConfig: IShellLaunchConfigcognidreamognidream {
    ...
    private _onEnvironmentVariableInfoChanged(info: IEnvironmentVariableInfocognidreamognidream {
    ...
    private async _refreshEnvironmentVariableInfoWidgetState(info ?: IEnvironmentVariableInfo): Promicognidreamognidream > {
...
return this._processManager.initialCwd;
    ...
    private async _updateProperty<T extends ProcessPropertyType>(type: T, value: IProcessPropertyMap[T]): Promicognidreamognidream > {
    ...
    private _setTitle(title: string | undefined, eventSource: TitleEventSourcecognidreamognidream {
	const reset = !title;
    ...
    async handleMouseEvent(event: MouseEvent, contextMenu: IMenu): Promise < { cancelContextMenu: boolean }cognidreamognidream > {
...
export class TerminalLabelComputer extends Disposable {
	...
	refreshLabel(instance: Pick<ITerminalInstance, 'shellLaunchConfig' | 'shellType' | 'cwd' | 'fixedCols' | 'fixedRows' | 'initialCwd' | 'processName' | 'sequence' | 'userHome' | 'workspaceFolder' | 'staticTitle' | 'capabilities' | 'title' | 'description'>, reset?: booleancognidreamognidream {
