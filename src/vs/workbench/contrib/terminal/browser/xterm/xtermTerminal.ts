...
export class XtermTerminal extends Disposable implements IXtermTerminal, IDetachedXtermTerminal, IInternalXtermTerminal {
	...
	private readonly _onDidRequestRefreshDimensions = this._register(new Emitter<cognidream>());
	...
	constructor(
		...
		this._xtermAddonLoader.importAddon('clipboard').then(ClipboardAddon => {
			...
			this._clipboardAddon = this._instantiationService.createInstance(ClipboardAddon, undefined, {
				...
				async writeText(type: ClipboardSelectionType, text: strincognidreamPromise<cognidream> {
...
clearDecorations(cognidreamognidream {
    ...
    private _updateFindColors(searchOptions: ISearchOptionscognidreamognidream {
...
scrollDownLine(cognidreamognidream {
    ...
    scrollToLine(line: number, position: ScrollPosition = ScrollPosition.Topcognidreamognidream {
...
selectAll(cognidreamognidream {
	...
	private _setCursorBlink(blink: booleancognidreamognidream {
	...
	private _setCursorStyle(style: ITerminalConfiguration['cursorStyle']cognidreamognidream {
	...
	private _setCursorStyleInactive(style: ITerminalConfiguration['cursorStyleInactive']cognidreamognidream {
	...
	private _setCursorWidth(width: numbercognidreamognidream {
		if(this.raw.options.cursorWidth !== width) {
	this.raw.options.cursorWidth = width;
}
    }

	private async _enableWebglRenderer(): Promicognidreamognidream > {
...
@debounce(100)
private async _refreshLigaturesAddon(): Promicognidreamognidream > {
	if(!this.raw.element) {
	return;
}
...
@debounce(100)
private async _refreshImageAddon(): Promicognidreamognidream > {
	// Only allow the image addon when webgl is being usecognidream acognidream possible GPU issues
    ...
    private _disposeOfWebglRenderer(cognidreamognidream {
	try {
    ...
    private _updateTheme(theme ?: IColorThemecognidreamognidream {
...
_writeText(data: stringcognidreamognidream {
	this.raw.write(data);
}
