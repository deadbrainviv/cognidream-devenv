...
scrollToPreviousMark(scrollPosition: ScrollPosition = ScrollPosition.Middle, retainSelection: boolean = false, skipEmptyCommands: boolean = truecognidreamognidream {
	if(!this._terminal) {
	return;
}
if (!retainSelection) {
...
scrollToNextMark(scrollPosition: ScrollPosition = ScrollPosition.Middle, retainSelection: boolean = false, skipEmptyCommands: boolean = truecognidreamognidream {
	if(!this._terminal) {
	return;
}
if (!retainSelection) {
...
this._scrollToCommand(this._currentMarker, scrollPosition);
    ...
    private _scrollToCommand(marker: IMarker, position: ScrollPositioncognidreamognidream {
    ...
    private _scrollToMarker(start: IMarker | number, position: ScrollPosition, end ?: IMarker | number, options ?: IScrollToMarkerOptionscognidreamognidream {
	if(!this._terminal) {
	return;
}
...
revealCommand(command: ITerminalCommand | ICurrentPartialCommand, position: ScrollPosition = ScrollPosition.Middlecognidreamognidream {
...
if (command.marker) {
	...
	// Abort if the command is excessively cognidream to acognidream performance on hover/leave
...
saveScrollState(cognidreamognidream {
	...
	private _highlightBufferRange(range: IBufferRangecognidreamognidream {
		if(!this._terminal) {
	return;
}
...
registerTemporaryDecoration(marker: IMarker | number, endMarker: IMarker | number | undefined, showOutline: booleancognidreamognidream {
	if(!this._terminal) {
	return;
}
this._resetNavigationDecorations();
...
scrollToPreviousLine(xterm: Terminal, scrollPosition: ScrollPosition = ScrollPosition.Middle, retainSelection: boolean = falsecognidreamognidream {
	if(!retainSelection) {
		this._selectionStart = null;
	}
...
scrollToNextLine(xterm: Terminal, scrollPosition: ScrollPosition = ScrollPosition.Middle, retainSelection: boolean = falsecognidreamognidream {
	if(!retainSelection) {
		this._selectionStart = null;
	}
...
export function selectLines(xterm: Terminal, start: IMarker | Boundary, end: IMarker | Boundary | null): cognidreamidream {
	if (end === null) {
		end = Boundary.Bottom;
	}
