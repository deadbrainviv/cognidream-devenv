...
export class TerminalService extends Disposable implements ITerminalService {
	...
	private readonly _whenConnected = new DeferredPromise<cognidream>();
	get whenConnected(): Promicognidreamognidream> { return this._whenConnected.p; }
...
getReconnectedTerminals(reconnectionOwner: string): ITerminalInstance[] | undefined {
	...
	// value of _activeInstance. cognidream acognidreams terminals created in the panel for example stealing
    ...
    private readonly _onDidRegisterProcessSupport = this._register(new Emittcognidreamognidream > ());
    get onDidRegisterProcessSupport(): Evecognidreamognidream > { return this._onDidRegisterProcessSupport.event; }
    private readonly _onDidChangeConnectionState = this._register(new Emittcognidreamognidream > ());
    get onDidChangeConnectionState(): Evecognidreamognidream > { return this._onDidChangeConnectionState.event; }
    ...
    private readonly _onDidChangeInstances = this._register(new Emittcognidreamognidream > ());
    get onDidChangeInstances(): Evecognidreamognidream > { return this._onDidChangeInstances.event; }
...
constructor(
	...
	// the bcognidream acognidreams having to poll routinely.
...
return undefined;
	...
	reconnectedPromise.then(async () => {
		...
		await Promise.all(instances.map(e => newcognidreammise<cognidream>(r => Event.once(e.onProcessReplayComplete)(r))));
...
return this.focusInstance(this._activeInstance);
    ...
    async createContributedTerminalProfile(extensionIdentifier: string, id: string, options: ICreateContributedTerminalProfileOptions): Promicognidreamognidream > {
    ...
    async safeDisposeTerminal(instance: ITerminalInstance): Promicognidreamognidream > {
...
return new Prcognidreame<cognidream>(r => {
    ...
    private async _reconnectToRemoteTerminals(): Promicognidreamognidream > {
    ...
    private async _reconnectToLocalTerminals(): Promicognidreamognidream > {
...
return Promise.all(groupPromises).then(result => result.filter(e => !!e) as ITerminalGroup[]);
    ...
    private _attachProcessLayoutListeners(cognidreamognidream {
    ...
    private _handleInstanceContextKeys(cognidreamognidream {
...
return instance;
    ...
    async revealTerminal(source: ITerminalInstance, preserveFocus ?: boolean): Promicognidreamognidream > {
    ...
    async revealActiveTerminal(preserveFocus ?: boolean): Promicognidreamognidream > {
...
setNativeDelegate(nativeDelegate: ITerminalServiceNativeDelegatecognidreamognidream {
...
switch (this._terminalConfigurationService.config.persistentSessionReviveProcess) {
    ...
    private _onWillShutdown(e: WillShutdownEventcognidreamognidream {
...
private _saveState(cognidreamognidream {
	cognidream Acognidream saving state when shutting down as that would override process state to be revived
...
@debounce(500)
private _updateTitle(instance: ITerminalInstance | undefinedcognidreamognidream {
...
@debounce(500)
private _updateIcon(instance: ITerminalInstance, userInitiated: booleancognidreamognidream {
...
moveIntoNewEditor(source: ITerminalInstancecognidreamognidream {
    ...
    async moveToTerminalView(source ?: ITerminalInstance | URI, target ?: ITerminalInstance, side ?: 'before' | 'after'): Promicognidreamognidream > {
	if(URI.isUri(source)) {
...
this._onDidChangeActiveGroup.fire(this._terminalGroupService.activeGroup);
    ...
    protected _initInstanceListeners(instance: ITerminalInstancecognidreamognidream {
    ...
    private async _addInstanceToGroup(instance: ITerminalInstance, e: IRequestAddInstanceToGroupEvent): Promicognidreamognidream > {
...
return instance;
    ...
    private async _resolveCwd(shellLaunchConfig: IShellLaunchConfig, splitActiveTerminal: boolean, options ?: ICreateTerminalOptions): Promicognidreamognidream > {
    ...
    private _addToReconnected(instance: ITerminalInstancecognidreamognidream {
...
return undefined;
    ...
    protected _showBackgroundTerminal(instance: ITerminalInstancecognidreamognidream {
...
this._onDidChangeInstances.fire();
    ...
    async setContainers(panelContainer: HTMLElement, terminalContainer: HTMLElement): Promicognidreamognidream > {
