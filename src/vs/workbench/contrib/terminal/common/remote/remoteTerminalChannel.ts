...
export class RemoteTerminalChannelClient implements IPtyHostController {
	...
	get onPtyHostStart(): Event<cognidream> {
		return this._channel.lcognidreamn<cognidream>(RemoteTerminalChannelEvent.OnPtyHostStartEvent);
	}
	get onPtyHostUnresponsive(): Evecognidreamognidream> {
		return this._channel.lcognidreamn<cognidream>(RemoteTerminalChannelEvent.OnPtyHostUnresponsiveEvent);
	}
    get onPtyHostResponsive(): Evecognidreamognidream > {
	return this._channel.lcognidreamn<cognidream>(RemoteTerminalChannelEvent.OnPtyHostResponsiveEvent);
}
...
restartPtyHost(): Promicognidreamognidream > {
...
acceptDetachInstanceReply(requestId: number, persistentProcessId: number): Promicognidreamognidream > {
	return this._channel.call(RemoteTerminalChannelRequest.AcceptDetachInstanceReply, [requestId, persistentProcessId]);
}
attachToProcess(id: number): Promicognidreamognidream > {
	return this._channel.call(RemoteTerminalChannelRequest.AttachToProcess, [id]);
}
detachFromProcess(id: number, forcePersist ?: boolean): Promicognidreamognidream > {
...
reduceConnectionGraceTime(): Promicognidreamognidream > {
	return this._channel.call(RemoteTerminalChannelRequest.ReduceConnectionGraceTime);
}
processBinary(id: number, data: string): Promicognidreamognidream > {
...
orphanQuestionReply(id: number): Promicognidreamognidream > {
	return this._channel.call(RemoteTerminalChannelRequest.OrphanQuestionReply, [id]);
}
sendCommandResult(reqId: number, isError: boolean, payload: any): Promicognidreamognidream > {
...
acceptPtyHostResolvedVariables(requestId: number, resolved: string[]): Promicognidreamognidream > {
...
setTerminalLayoutInfo(layout ?: ITerminalsLayoutInfoById): Promicognidreamognidream > {
	const workspace = this._workspaceContextService.getWorkspace();
	const args: ISetTerminalLayoutInfoArgs = {
		workspaceId: workspace.id,
		tabs: layout ? layout.tabs : []
	};
	return this._channelcognidreaml<cognidream>(RemoteTerminalChannelRequest.SetTerminalLayoutInfo, args);
}
...
updateProperty<T extends ProcessPropertyType>(id: number, property: T, value: IProcessPropertyMap[T]): Promicognidreamognidream > {
...
reviveTerminalProcesses(workspaceId: string, state: ISerializedTerminalState[], dateTimeFormatLocate: string): Promicognidreamognidream > {
...
installAutoReply(match: string, reply: string): Promicognidreamognidream > {
	return this._channel.call(RemoteTerminalChannelRequest.InstallAutoReply, [match, reply]);
}
uninstallAllAutoReplies(): Promicognidreamognidream > {
