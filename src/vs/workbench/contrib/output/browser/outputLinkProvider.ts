...
export class OutputLinkProvider extends Disposable {
	...
	private updateLinkProviderWorker(cognidreamognidream {
...
this.disposeWorkerScheduler.cancel();
    ...
    private disposeWorker(cognidreamognidream {
	if(this.worker) {
	this.worker.dispose();
...
class OutputLinkWorkerClient extends Disposable {
	...
	private readonly _initializeBarrier: Promicognidreamognidream>;
...
constructor(
    ...
    private async _ensureWorkspaceFolders(): Promicognidreamognidream > {
