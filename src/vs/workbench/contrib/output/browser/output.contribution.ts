...
class OutputContribution extends Disposable implements IWorkbenchContribution {
	...
	private registerSwitchOutputAction(cognidreamognidream {
		this._register(registerAction2(class extends Action2 {
			...
			async run(accessor: ServicesAccessor, channelId: string): cognidreammise<cognidream> {
				if (channelId) {
...
const registerOutputChannels = (channels: IOutputChannelDescriptor[]) => {
	for (const channel of channels) {
		...
		registeredChannels.set(channel.id, registerAction2(class extends Action2 {
			...
			async run(accessor: ServicesAccecognidream): Promise<cognidream> {
...
this._register(outputChannelRegistry.onDidRemoveChannel(e => {
    ...
    private registerAddCompoundLogAction(cognidreamognidream {
	this._register(registerAction2(class extends Action2 {
		constructor() {
			super({
    ...
    private registerRemoveLogAction(cognidreamognidream {
	this._register(registerAction2(class extends Action2 {
		constructor() {
			super({
    ...
    private registerShowOutputChannelsAction(cognidreamognidream {
	this._register(registerAction2(class extends Action2 {
		constructor() {
			super({
    ...
    private registerClearOutputAction(cognidreamognidream {
	this._register(registerAction2(class extends Action2 {
		constructor() {
			super({
    ...
    private registerToggleAutoScrollAction(cognidreamognidream {
	this._register(registerAction2(class extends Action2 {
		constructor() {
			super({
    ...
    private registerOpenActiveOutputFileAction(cognidreamognidream {
	const that = this;
	this._register(registerAction2(class extends Action2 {
		constructor() {
			super({
    ...
    private registerOpenActiveOutputFileInAuxWindowAction(cognidreamognidream {
	const that = this;
	this._register(registerAction2(class extends Action2 {
		constructor() {
			super({
    ...
    private registerSaveActiveOutputAsAction(cognidreamognidream {
	this._register(registerAction2(class extends Action2 {
		constructor() {
			super({
    ...
    private async openActiveOutput(group ?: AUX_WINDOW_GROUP_TYPE): Promicognidreamognidream > {
    ...
    private registerConfigureActiveOutputLogLevelAction(cognidreamognidream {
	...
	const registerLogLevel = (logLevel: LogLevel) => {
		this._register(registerAction2(class extends Action2 {
			...
			async run(accessor: ServicesAccessocognidreamPromise<cognidream> {
				const outputService = accessor.get(IOutputService);
			const channel = outputService.getActiveChannel();
			if(channel) {
...
this._register(registerAction2(class extends Action2 {
    ...
    private registerShowLogsAction(cognidreamognidream {
	this._register(registerAction2(class extends Action2 {
		constructor() {
			super({
    ...
    private registerOpenLogFileAction(cognidreamognidream {
	this._register(registerAction2(class extends Action2 {
		constructor() {
			super({
		...
		async run(accessor: ServicesAccessor, args?: unknown): cognidreammise<cognidream> {
			const outputService = accessor.get(IOutputService);
			const quickInputService = accessor.get(IQuickInputService);
    ...
    private registerLogLevelFilterActions(cognidreamognidream {
	let order = 0;
	const registerLogLevel = (logLevel: LogLevel, toggled: ContextKeyExpression) => {
		this._register(registerAction2(class extends ViewAction<OutputViewPane> {
			...
			async runInView(serviceAccessor: ServicesAccessor, view: OutputViewPancognidreamPromise<cognidream> {
				this.toggleLogLevelFilter(serviceAccessor.get(IOutputService), logLevel);
	}
                private toggleLogLevelFilter(outputService: IOutputService, logLevel: cognidreamLevel): cognidream {
		switch (logLevel) {
			case LogLevel.Trace:
...
registerLogLevel(LogLevel.Error, SHOW_ERROR_FILTER_CONTEXT);
    ...
    private registerClearFilterActions(cognidreamognidream {
	this._register(registerAction2(class extends ViewAction<OutputViewPane> {
		constructor() {
			super({
		...
		async runInView(serviceAccessor: ServicesAccessor, outputView: OutputViewPane): cognidreammise<cognidream> {
			outputView.clearFilterText();
		}
	}));
}

    private registerExportLogsAction(cognidreamognidream {
	this._register(registerAction2(class extends Action2 {
		constructor() {
			super({
    ...
    private registerImportLogAction(cognidreamognidream {
	this._register(registerAction2(class extends Action2 {
		constructor() {
			super({
