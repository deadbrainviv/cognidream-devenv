...
export class TerminalTaskSystem extends Disposable implements ITaskSystem {
	...
	private _terminalCreationQueue: Promise<ITerminalInstance | cognidream> = Promise.resolve();
	...
	private _log(value: stringcognidreamognidream {
		this._appendOutput(value + '\n');
    }

    protected _showOutput(cognidreamognidream {
...
try {
    ...
    public customExecutionComplete(task: Task, result: number): Promicognidreamognidream > {
    ...
    private _removeFromActiveTasks(task: Task | stringcognidreamognidream {
...
return Promise.all<ITaskTerminateResponse>(promises);
    ...
    private _adoptConfigurationForDependencyTask(dependencyTask: Task, task: Taskcognidreamognidream {
...
return undefined;
    ...
    private _reconnectToTerminals(cognidreamognidream {
...
this._hasReconnected = true;
    ...
    private _deleteTaskAndTerminal(terminal: ITerminalInstance, terminalData: ITerminalDatacognidreamognidream {
...
return [terminal, undefined];
    ...
    private _collectTaskVariables(variables: Set<string>, task: CustomTask | ContributedTaskcognidreamognidream {
...
if (task.command.runtime === RuntimeType.CustomExecution && (CustomTask.is(task) || ContributedTask.is(task))) {
    ...
    private _collectDefinitionVariables(variables: Set<string>, definition: anycognidreamognidream {
    ...
    private _collectCommandVariables(variables: Set<string>, command: ICommandConfiguration, task: CustomTask | ContributedTaskcognidreamognidream {
...
if (command.options) {
    ...
    private _collectMatcherVariables(variables: Set<string>, values: Array<string | ProblemMatcher> | undefinedcognidreamognidream {
...
values.forEach((value) => {
    ...
    private _collectVariables(variables: Set<string>, value: string | CommandStringcognidreamognidream {
...
return result;
    ...
    private _appendOutput(output: stringcognidreamognidream {
