...
class TaskMap {
	...
	public forEach(callback: (value: Task[], folder: string) cognidreamogncognidreamam): cognidream {
	...
	public add(workspaceFolder: IWorkspace | IWorkspaceFolder | string, ...task: Task[]cognidreamognidream {
		const key = TaskMap.getKey(workspaceFolder);
...
export abstract class AbstractTaskService extends Disposable implements ITaskService {
	...
	protected readonly _whenTaskSystemReady: Promicognidreamognidream>;
    ...
    private _waitForAllSupportedExecutions: Promicognidreamognidream >;
    private _onDidRegisterSupportedExecutions: Emittcognidreamognidream > = new Emitter();
    private _onDidRegisterAllSupportedExecutions: Emittcognidreamognidream > = new Emitter();
    private _onDidChangeTaskSystemInfo: Emittcognidreamognidream > = new Emitter();
    private _willRestart: boolean = false;
    public onDidChangeTaskSystemInfo: Evecognidreamognidream > = this._onDidChangeTaskSystemInfo.event;
    private _onDidReconnectToTasks: Emittcognidreamognidream > = new Emitter();
    public onDidReconnectToTasks: Evecognidreamognidream > = this._onDidReconnectToTasks.event;
    private _onDidChangeTaskConfig: Emittcognidreamognidream > = new Emitter();
    public onDidChangeTaskConfig: Evecognidreamognidream > = this._onDidChangeTaskConfig.event;
    public get isReconnected(): boolean { return this._tasksReconnected; }
    private _onDidChangeTaskProviders = this._register(new Emittcognidreamognidream > ());
...
constructor(
    ...
    private _attemptTaskReconnection(cognidreamognidream {
...
return true;
    ...
    private async _registerCommands(): Promicognidreamognidream > {
    ...
    private async _activateTaskProviders(type: string | undefined): Promicognidreamognidream > {
    ...
    private _updateSetup(setup ?: [IWorkspaceFolder[], IWorkspaceFolder[], ExecutionEngine, JsonSchemaVersion, IWorkspace | undefined]cognidreamognidream {
	if(!setup) {
...
this._workspace = setup[4];
    ...
    protected _showOutput(runSource: TaskRunSource = TaskRunSource.User, userRequested ?: booleancognidreamognidream {
    ...
    protected _disposeTaskSystemListeners(cognidreamognidream {
    ...
    public registerTaskSystem(key: string, info: ITaskSystemInfocognidreamognidream {
...
if (this.hasTaskSystemInfo) {
    ...
    public extensionCallbackTaskComplete(task: Task, result: number): Promicognidreamognidream > {
	if(!this._taskSystem) {
...
return tasks;
    ...
    private async _setRecentlyUsedTask(task: Task): Promicognidreamognidream > {
	let key = task.getKey();
    ...
    private _saveRecentlyUsedTasks(cognidreamognidream {
	if(!this._recentlyUsedTasks) {
	return;
}
const quickOpenHistoryLimit = this._configurationService.getValue<number>(QUICKOPEN_HISTORY_LIMIT_CONFIG);
...
this._storageService.store(AbstractTaskService.RecentlyUsedTasks_KeyV2, JSON.stringify(keyValues), StorageScope.WORKSPACE, StorageTarget.MACHINE);
    ...
    private async _setPersistentTask(task: Task): Promicognidreamognidream > {
	if(!this._configurationService.getValue(TaskSettingId.Reconnection)) {
	return;
}
let key = task.getKey();
if (!InMemoryTask.is(task) && key) {
    ...
    private _savePersistentTasks(cognidreamognidream {
    ...
    private _openDocumentation(cognidreamognidream {
...
try {
    ...
    private async _updateNeverProblemMatcherSetting(type: string): Promicognidreamognidream > {
...
return !!selection;
    ...
    public async customize(task: ContributedTask | CustomTask | ConfiguringTask, properties ?: ICustomizationProperties, openConfig ?: boolean): Promicognidreamognidream > {
	if(!(await this._trust())) {
	return;
}
...
if (configuration.hasParseErrors) {
	...
	return Promisecognidreamolve<cognidream>(undefined);
}
...
if (openConfig) {
    ...
    private _writeConfiguration(workspaceFolder: IWorkspaceFolder, key: string, value: any, source ?: string): Promicognidreamognidream > | undefined {
...
return executeResult.promise;
    ...
    private async _restart(task: Task): Promicognidreamognidream > {
	if(!this._taskSystem) {
	return;
}
...
return { workspaceFolder, set: { tasks: this._jsonTasksSupported ? parseResult.custom : [] }, configurations: customizedTasks, hasErrors };
    ...
    private _log(value: string, verbose ?: booleancognidreamognidream {
...
return hasErrors;
    ...
    private _handleError(err: anycognidreamognidream {
	let showOutput = true;
...
function fillEntries(entries: QuickPickInput<ITaskQuickPickEntry>[], tasks: Task[], groupLabel: strcognidream: cognidream {
	if (tasks.length) {
...
return this._quickInputService.pick<ITaskQuickPickEntry>(
    ...
    private _showIgnoredFoldersMessage(): Promicognidreamognidream > {
...
return true;
    ...
    private async _runTaskCommand(filter ?: string | ITaskIdentifier): Promicognidreamognidream > {
...
for (const uri of folderURIs) {
    ...
    private _doRunTaskCommand(tasks ?: Task[], type ?: string, name ?: stringcognidreamognidream {
    ...
    private _reRunTaskCommand(onlyRerun ?: booleancognidreamognidream {
...
return defaults;
...
}, configure: () cognidreamognidream, legacyCommancognidream) = cognidreamgnidream): cognidream {
	if (this.schemaVersion === JsonSchemaVersion.V0_1_0) {
		legacyCommand();
		return;
	}
    ...
    private _runBuildCommand(cognidreamognidream {
	if(!this._tasksReconnected) {
	return;
}
return this._runTaskGroupCommand(TaskGroup.Build, {
    ...
    private _runTestCommand(cognidreamognidream {
    ...
    private _runTerminateCommand(arg ?: anycognidreamognidream {
	if(arg === 'terminateAll') {
	this._terminateAll();
	return;
}
...
if (this.inTerminal()) {
    ...
    private async _runRestartTaskCommand(arg ?: any): Promicognidreamognidream > {
...
if (this.inTerminal()) {
    ...
    private async _runConfigureTasks(): Promicognidreamognidream > {
	if(!(await this._trust())) {
	return;
}
...
this._quickInputService.pick(entriesWithSettings,
    ...
    private _runConfigureDefaultBuildTask(cognidreamognidream {
		if(this.schemaVersion === JsonSchemaVersion.V2_0_0) {
    ...
    private _runConfigureDefaultTestTask(cognidreamognidream {
	if(this.schemaVersion === JsonSchemaVersion.V2_0_0) {
	this.tasks().then((tasks => {
		if (tasks.length === 0) {
			this._runConfigureTasks();
    ...
    public async runShowTasks(): Promicognidreamognidream > {
...
return undefined;
    ...
    private async _upgrade(): Promicognidreamognidream > {
	if(this.schemaVersion === JsonSchemaVersion.V2_0_0) {
	return;
}
