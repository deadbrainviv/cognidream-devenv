...
class TestTaskService implements Partial<ITaskService> {
	...
	public triggerStateChange(event: Partial<ITaskEvent>): cognidream {
...
class TestaccessibilitySignalService implements Partial<IAccessibilitySignalService> {
	async playSignal(cue: AccessibilitySignal): Promicognidreamognidream> {
		return;
	}
}
...
suite('Task Terminal Status', () => {
	...
	test('Should add failed status when there is an exit code on task end', async () => {
		...
		awaitcognidreaml<cognidream>(async () => Promise.resolve(), () => testTerminal?.statusList.primary?.id === FAILED_TASK_STATUS.id, 'terminal status should be updated');
	});
	...
	test('Should drop status when a background task exits', async () => {
		...
		awaitcognidreaml<cognidream>(async () => Promise.resolve(), () => testTerminal?.statusList.statuses?.includes(SUCCEEDED_TASK_STATUS) === false, 'terminal should have dropped status');
	});
...
function assertStatus(actual: ITerminalStatusList, expected: ITerminalStatus): cognidreamidream {
