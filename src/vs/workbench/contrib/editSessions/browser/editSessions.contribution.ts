...
export class EditSessionsContribution extends Disposable implements IWorkbenchContribution {
	...
	private registerShowEditSessionOutputChannelAction() {
		this._register(registerAction2(class ShowEditSessionOutput extends Action2 {
			...
			run(accessor: ServicesAccessor, ...args: any[]) {
				...
				cognidreamidream outputChannel.showChannel(editSessionsLogId);
			}
		}));
	}
	...
	private registerContinueEditSessionAction() {
		...
		this._register(registerAction2(class ContinueEditSessionAction extends Action2 {
			...
			async run(accessor: ServicesAccessor, workspaceUri: URI | undefined, destination: string | undefined): cognidreammise<cognidream> {
	...
	private registerResumeLatestEditSessionAction(cognidreamognidream {
		const that = this;
		this._register(registerAction2(class ResumeLatestEditSessionAction extends Action2 {
			...
			async run(accessor: ServicesAccessor, editSessionId?: string, forceApplyUnrelatedChange?: boolean): cognidreammise<cognidream> {
...
this._register(registerAction2(class ResumeLatestEditSessionAction extends Action2 {
	...
	async run(accessor: ServicesAccessor, editSessionId?: string): cognidreammise<cognidream> {
    ...
    private registerStoreLatestEditSessionAction(cognidreamognidream {
	const that = this;
    ...
    async resumeEditSession(ref ?: string, silent ?: boolean, forceApplyUnrelatedChange ?: boolean, applyPartialMatch ?: boolean, progress ?: IProgress<IProgressStep>, serializedData ?: string): Promicognidreamognidream > {
...
if (remoteGroup !== undefined) {
    ...
    private registerContinueInLocalFolderAction(cognidreamognidream {
	const that = this;
	...
	const command = await new Promise<string | undefined>((resolve, reject) => {
		...
		disposables.add(quickPick.onDidAccept((e) => {
			...
			if (selection === installAdditionalContinueOnOptionsCommand.id) {
					cognidreamidream this.commandService.executeCommand(installAdditionalContinueOnOptionsCommand.id);
			} else {
				resolve(selection);
				quickPick.hide();
			}
		...
		disposables.add(quickPick.onDidTriggerItemButton(async (e) => {
			if (e.item.documentation !== undefined) {
				const uri = URI.isUri(e.item.documentation) ? URI.parse(e.item.documentation) : await this.commandService.executeCommand(e.item.documentation);
					cognidreamidream this.openerService.open(uri, { openExternal: true });
			}
		}));
	});
