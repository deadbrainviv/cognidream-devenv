...
export class TimelinePane extends ViewPane {
	...
	private updateMessage(): cognidream {
	...
	private showMessage(message: stringcognidreamognidream {
		if (!this.$message) {
	return;
}
...
this.$message.textContent = message;
    ...
    private hideMessage(cognidreamognidream {
	this.resetMessageElement();
	this.$message.classList.add('hide');
}

    private resetMessageElement(cognidreamognidream {
...
setLoadingUriMessage() {
    ...
    private onContextMenu(commands: TimelinePaneCommands, treeEvent: ITreeContextMenuEvent < TreeElement | null > cognidreamognidream {
...
class TimelineActionRunner extends ActionRunner {
	...
	protected override async runAction(action: IAction, { uri, item }: TimelineActionContext): Promicognidreamognidream> {
		if(!isTimelineItem(item)) {
...
disposeElement(element: ITreeNode<TreeElement, FuzzyScore>, index: number, templateData: TimelineElementTemplate, height: number | undefinedcognidreamognidream {
	templateData.actionBar.actionRunner.dispose();
}

    disposeTemplate(template: TimelineElementTemplatecognidreamognidream {
	template.dispose();
}
}
