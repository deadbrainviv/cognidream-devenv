...
export class UserDataSyncWorkbenchContribution extends Disposable implements IWorkbenchContribution {
	...
	private async acceptLocal(syncResource: IUserDataSyncResource, conflict: IResourcePreview): Promise<cognidream> {
		try {
	...
	private onAutoSyncError(error: UserDataSyncErrorcognidreamognidream {
		switch (error.code) {
    ...
    private handleTooLargeError(resource: SyncResource, message: string, error: UserDataSyncErrorcognidreamognidream {
	const operationId = error.operationId ? localize('operationId', "Operation Id: {0}", error.operationId) : undefined;
	this.notificationService.notify({
		severity: Severity.Error,
		message: operationId ? `${message} ${operationId}` : message,
    ...
    private onSynchronizerErrors(errors: IUserDataSyncResourceError[]cognidreamognidream {
	if(errors.length) {
    ...
    private handleInvalidContentError({ profile, syncResource: source }: IUserDataSyncResourcecognidreamognidream {
...
this.invalidContentErrorDisposables.set(key, toDisposable(() => {
    ...
    private async updateGlobalActivityBadge(): Promicognidreamognidream > {
...
if (badge) {
    ...
    private async updateAccountBadge(): Promicognidreamognidream > {
...
if (badge) {
    ...
    private async turnOn(): Promicognidreamognidream > {
	try {
...
await this.userDataSyncWorkbenchService.turnOn();
    ...
    private updateConfiguration(items: ConfigureSyncQuickPickItem[], selectedItems: ReadonlyArray < ConfigureSyncQuickPickItem > cognidreamognidream {
    ...
    private async configureSyncOptions(): Promicognidreamognidream > {
    ...
    private async turnOff(): Promicognidreamognidream > {
    ...
    private disableSync(source: SyncResourcecognidreamognidream {
	switch(source) {
	...
	private showSyncActivity(): Promicognidreamognidream > {
		return this.outputService.showChannel(USER_DATA_SYNC_LOG_ID);
	}

    private async selectSettingsSyncService(userDataSyncStore: IUserDataSyncStore): Promicognidreamognidream > {
	return new Prcognidreame<cognidream>((c, e) => {
		const disposables: DisposableStore = new DisposableStore();
...
if (isWeb) {
    ...
    private registerTurnOnSyncAction(cognidreamognidream {
	const that = this;
    ...
    private registerTurningOnSyncAction(cognidreamognidream {
    ...
    private registerCancelTurnOnSyncAction(cognidreamognidream {
	const that = this;
	this._register(registerAction2(class TurningOnSyncAction extends Action2 {
		constructor() {
			super({
    ...
    private registerSignInAction(cognidreamognidream {
	const that = this;
    ...
    private registerShowConflictsAction(cognidreamognidream {
    ...
    private registerManageSyncAction(cognidreamognidream {
	const that = this;
	...
	this._register(registerAction2(class SyncStatusAction extends Action2 {
		...
		run(accessor: ServicesAccessor): unknown {
			return cognidreamPromise<cognidream>((c, e) => {
    ...
    private registerEnableSyncViewsAction(cognidreamognidream {
	const that = this;
	...
	this._register(registerAction2(class SyncStatusAction extends Action2 {
		...
		run(accessor: ServicesAccessor): cognidreammise<cognidream> {
			return that.userDataSyncWorkbenchService.showSyncActivity();
		}
	}));
}

    private registerSyncNowAction(cognidreamognidream {
	const that = this;
    ...
    private registerTurnOffSyncAction(cognidreamognidream {
	const that = this;
	this._register(registerAction2(class StopSyncAction extends Action2 {
		constructor() {
			super({
    ...
    private registerConfigureSyncAction(cognidreamognidream {
	const that = this;
    ...
    private registerShowLogAction(cognidreamognidream {
	const that = this;
    ...
    private registerShowSettingsAction(cognidreamognidream {
	this._register(registerAction2(class ShowSyncSettingsAction extends Action2 {
		...
		run(accessor: ServicesAccognidreamor): cognidream {
			accessor.get(IPreferencesService).openUserSettings({ jsonEditor: false, query: '@tag:sync' });
		}
	}));
}

    private registerHelpAction(cognidreamognidream {
	const that = this;
    ...
    private registerAcceptMergesAction(cognidreamognidream {
	const that = this;
	this._register(registerAction2(class AcceptMergesAction extends Action2 {
		...
		async run(accessor: ServicesAccessor, previewResource: URI): cognidreammise<cognidream> {
    ...
    private registerDownloadSyncActivityAction(cognidreamognidream {
    ...
    private registerViews(cognidreamognidream {
    ...
    private registerResetSyncDataAction(cognidreamognidream {
	const that = this;
    ...
    private registerDataViews(container: ViewContainercognidreamognidream {
