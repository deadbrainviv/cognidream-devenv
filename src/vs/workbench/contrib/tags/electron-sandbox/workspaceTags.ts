...
export class WorkspaceTags implements IWorkbenchContribution {
	...
	private async report(): Promise<cognidream> {
	...
	private async reportWindowsEdition(): Promicognidreamognidream> {
		if(!isWindows) {
			return;
		}
...
this.telemetryService.publicLog2<{ edition: string }, { owner: 'sbatten'; comment: 'Information about the Windows edition.'; edition: { classification: 'SystemMetaData'; purpose: 'BusinessInsight'; comment: 'The Windows edition.' } }>('windowsEdition', { edition: value });
    ...
    private reportWorkspaceTags(tags: Tagscognidreamognidream {
	/* __GDPR__
		"workspce.tags" : {
			"owner": "lramos15",
			"${include}": [
	...
	*/
    ...
    private reportRemoteDomains(workspaceUris: URI[]cognidreamognidream {
    ...
    private reportRemotes(workspaceUris: URI[]cognidreamognidream {
	Promise.all<string[]>(workspaceUris.map(workspaceUri => {
    ...
    */
    ...
    private reportCloudStats(cognidreamognidream {
