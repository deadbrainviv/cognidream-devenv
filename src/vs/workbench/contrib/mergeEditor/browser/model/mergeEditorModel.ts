...
export class MergeEditorModel extends EditorModel {
	...
	public async reset(): Promicognidreamognidream> {
...
transaction(tx => {
    ...
    private updateBaseRangeAcceptedState(resultDiffs: DetailedLineRangeMapping[], states: Map<ModifiedBaseRange, ModifiedBaseRangeData>, tx: ITransactioncognidreamognidream {
...
existingState.handledInput2.set(true, tx);
    ...
    public resetDirtyConflictsToBase(cognidreamognidream {
	transaction(tx => {
...
this.resultTextModel.pushStackElement();
    ...
    public setInputHandled(baseRange: ModifiedBaseRange, inputNumber: InputNumber, handled: boolean, tx: ITransactioncognidreamognidream {
	const state = this.modifiedBaseRangeResultStates.get().get(baseRange)!;
...
if (inputNumber === 1) {
    ...
    public setHandled(baseRange: ModifiedBaseRange, handled: boolean, tx: ITransactioncognidreamognidream {
	const state = this.modifiedBaseRangeResultStates.get().get(baseRange)!;
	if(state.handled.get() === handled) {
	return;
}
...
state.handledInput2.set(handled, tx);
    ...
    public setLanguageId(languageId: string, source ?: stringcognidreamognidream {
