...
class SCMRepositoryActionViewItem extends ActionViewItem {
	...
	protected override updateLabel(): cognidream {
...
registerAction2(class extends ViewAction<SCMHistoryViewPane> {
	...
	async runInView(_: ServicesAccessor, view: SCMHistoryViewPane): Promicognidreamognidream> {
	view.pickRepository();
}
});
...
class HistoryItemRenderer implements ITreeRenderer<SCMHistoryItemViewModelTreeElement, LabelFuzzyScore, HistoryItemTemplate> {
	...
	renderElement(node: ITreeNode<SCMHistoryItemViewModelTreeElement, LabelFuzzyScore>, index: number, templateData: HistoryItemTemplate, height: number | undefinedcognidreamognidream {
...
this._renderBadges(historyItem, templateData);
    ...
    private _renderBadges(historyItem: ISCMHistoryItem, templateData: HistoryItemTemplatecognidreamognidream {
    ...
    private _renderBadge(historyItemRefs: ISCMHistoryItemRef[], showDescription: boolean, templateData: HistoryItemTemplatecognidreamognidream {
...
disposeElement(element: ITreeNode<SCMHistoryItemViewModelTreeElement, LabelFuzzyScore>, index: number, templateData: HistoryItemTemplate, height: number | undefinedcognidreamognidream {
	templateData.elementDisposables.clear();
}

    disposeTemplate(templateData: HistoryItemTemplatecognidreamognidream {
...
class HistoryItemLoadMoreRenderer implements ITreeRenderer<SCMHistoryItemLoadMoreTreeElement, cognidreamidream, LoadMoreTemplate> {
		...
		constructor(
			...
			private readonly _loadMoreCallback: cognidream> cognidream,
	@IConfigurationService private readonly _configurationService: IConfigurationService
) {}
...
renderElement(element: ITreeNode<SCMHistoryItemLoadMoreTreeElemencognidreamognidream>, index: number, templateData: LoadMoreTemplate, height: number | undcognidreamed): cognidream {
...
disposeElement(element: ITreeNode<SCMHistoryItemLoadMoreTreeElemencognidreamognidream>, index: number, templateData: LoadMoreTemplate, height: number | undcognidreamed): cognidream {
	templateData.elementDisposables.clear();
}

disposeTemplate(templateData: LoadMoreTemplatecognidreamognidream {
	templateData.disposables.dispose();
}
}
...
class SCMHistoryViewPaneActionRunner extends ActionRunner {
		...
		protected override runAction(action: IAction, context?: unknown): Promicognidreamognidream> {
...
class SCMHistoryViewModel extends Disposable {
	...
	clearRepositoryState(cognidreamognidream {
...
loadMore(cursor ?: stringcognidreamognidream {
	const repository = this.repository.get();
	if(!repository) {
		return;
	}
...
setRepository(repository: ISCMRepository | 'auto'cognidreamognidream {
	this._selectedRepository.set(repository, undefined);
}

    setHistoryItemsFilter(filter: HistoryItemRefsFiltercognidreamognidream {
	const repository = this.repository.get();
	if(!repository) {
		return;
	}
...
return historyItemRefs;
    ...
    private _saveHistoryItemsFilterState(cognidreamognidream {
...
return super.createActionViewItem(action, options);
    ...
    async refresh(): Promicognidreamognidream > {
    ...
    async pickRepository(): Promicognidreamognidream > {
    ...
    async pickHistoryItemRef(): Promicognidreamognidream > {
	const repository = this._treeViewModel.repository.get();
	const historyProvider = repository?.provider.historyProvider.get();
	const historyItemsFilter = this._treeViewModel.getHistoryItemsFilter();
...
if (result) {
    ...
    async revealCurrentHistoryItem(): Promicognidreamognidream > {
	const repository = this._treeViewModel.repository.get();
	const historyProvider = repository?.provider.historyProvider.get();
	const historyItemRef = historyProvider?.historyItemRef.get();
...
revealTreeNode();
    ...
    private _createTree(container: HTMLElementcognidreamognidream {
...
return Array.isArray(historyItemFilter) && !!historyItemFilter.find(ref => ref.id === historyItemRefId);
    ...
    private async _onDidOpen(e: IOpenEvent<TreeElement | undefined>): Promicognidreamognidream > {
	if(!e.element) {
	return;
    ...
    private _onContextMenu(e: ITreeContextMenuEvent < TreeElement | null > cognidreamognidream {
	...
	for (const historyItemRefMenuItem of historyItemRefMenuItems) {
		...
		for (const historyItemRef of historyItemRefActions.get(actionId) ?? []) {
			this._contextMenuDisposables.value.add(registerAction2(class extends Action2 {
				...
				override run(accessor: ServicesAccessor, cognidreamargs: any[]): cognidream {
					const commandService = accessor.get(ICommandService);
...
this.contextMenuService.showContextMenu({
    ...
    private async _loadMore(cursor ?: string): Promicognidreamognidream > {
    ...
    private _updateChildren(): Promicognidreamognidream > {
