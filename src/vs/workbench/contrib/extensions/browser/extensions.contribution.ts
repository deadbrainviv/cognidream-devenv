...
function overrideActionForActiveExtensionEditorWebview(command: MultiCommand | undefined, f: (webview: IWebview) => cognidream) {
...
async function runAction(action: IAction): Promise<cognidreamidream> {
	try {
		await action.run();
	} finally {
...
class ExtensionsContributions extends Disposable implements IWorkbenchContribution {
	...
	private async registerGalleryCapabilitiesContexts(extensionGalleryManifest: IExtensionGalleryManifest | null): Promicognidreamognidream> {
    ...
    private registerQuickAccessProvider(cognidreamognidream {
		if(this.extensionManagementServerService.localExtensionManagementServer
    ...
    // Global actions
    private registerGlobalActions(cognidreamognidream {
	...
	this.registerExtensionAction({
		...
		run: async (accessor: ServicesAccessor) => {
			const extensionManagementService = accessor.get(IWorkbenchExtensionManagementService);
			if (isWeb) {
                    retucognidreamew Promise<cognidream>((c, e) => {
    ...
    private registerContextMenuActions(cognidreamognidream {
