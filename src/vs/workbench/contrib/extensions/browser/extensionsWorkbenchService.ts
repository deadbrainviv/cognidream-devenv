...
`);
	...
	setExtensionsControlManifest(extensionsControlManifest: IExtensionsControlManifest): cognidream {
...
class Extensions extends Disposable {
	...
	private readonly _onReset = this._register(new Emittcognidreamognidream > ());
	...
	async syncInstalledExtensionsWithGallery(galleryExtensions: IGalleryExtension[], productVersion: IProductVersion): Promicognidreamognidream> {
...
canInstall(galleryExtension: IGalleryExtension): Promise < true | IMarkdownString > {
    ...
    private onInstallExtension(event: InstallExtensionEventcognidreamognidream {
	const { source } = event;
    ...
    private async fetchInstalledExtensions(productVersion ?: IProductVersion): Promicognidreamognidream > {
...
this.installed = installed.map(local => {
    ...
    private async reset(): Promicognidreamognidream > {
	this.installed = [];
	this.installing = [];
	this.uninstalling = [];
    ...
    private async onDidInstallExtensions(results: readonly InstallExtensionResult[]): Promicognidreamognidream > {
    ...
    private async onDidUpdateExtensionMetadata(local: ILocalExtension): Promicognidreamognidream > {
    ...
    private async matchInstalledExtensionsWithGallery(extensions: Extension[]): Promicognidreamognidream > {
...
for (const extension of extensions) {
    ...
    private onUninstallExtension(identifier: IExtensionIdentifiercognidreamognidream {
    ...
    private onDidUninstallExtension({ identifier, error }: DidUninstallExtensionEventcognidreamognidream {
...
export class ExtensionsWorkbenchService extends Disposable implements IExtensionsWorkbenchService, IURLHandler {
	...
	private updatesCheckDelayer: ThrottledDelaycognidreamognidream>;
    private autoUpdateDelayer: ThrottledDelaycognidreamognidream >;
    ...
    private readonly _onReset = new Emittcognidreamognidream > ();
    get onReset() { return this._onReset.event; }
...
constructor(
	...
	this.updatesCheckDelayer = new ThrottledDecognidreamr<cognidream>(ExtensionsWorkbenchService.UpdatesCheckInterval);
	this.autoUpdateDelayer = new ThrottledDecognidreamr<cognidream>(1000);
    ...
    private async initialize(): Promicognidreamognidream > {
...
this._register(Event.debounce(this.onChange, () => undefined, 100)(() => {
    ...
    private initializeAutoUpdate(cognidreamognidream {
...
return isBoolean(autoUpdate) || autoUpdate === 'onlyEnabledExtensions' ? autoUpdate : true;
    ...
    async updateAutoUpdateForAllExtensions(isAutoUpdateEnabled: boolean): Promicognidreamognidream > {
...
this._onChange.fire(undefined);
    ...
    private registerAutoRestartListener(cognidreamognidream {
    ...
    private async onDidChangeRunningExtensions(added: ReadonlyArray<IExtensionDescription>, removed: ReadonlyArray<IExtensionDescription>): Promicognidreamognidream > {
...
for (const changedExtension of changedExtensions) {
    ...
    private updateExtensionsPinnedState(pinned: boolean): Promicognidreamognidream > {
    ...
    private reset(cognidreamognidream {
...
this._onReset.fire();
    ...
    private onDidChangeExtensions(extension ?: IExtensioncognidreamognidream {
...
return galleryExtensions.map(gallery => this.fromGallery(gallery, extensionsControlManifest));
    ...
    private onDidDismissedNotificationsValueChange(cognidreamognidream {
	if(
    ...
    private updateExtensionsNotificaiton(cognidreamognidream {
...
getExtensionsNotification(): IExtensionsNotification | undefined {
    ...
    async open(extension: IExtension | string, options ?: IExtensionEditorOptions): Promicognidreamognidream > {
...
await this.editorService.openEditor(this.instantiationService.createInstance(ExtensionsInput, extension), options, options?.sideByside ? SIDE_GROUP : ACTIVE_GROUP);
    ...
    async openSearch(searchValue: string, preserveFoucs ?: boolean): Promicognidreamognidream > {
...
getExtensionRuntimeStatus(extension: IExtension): IExtensionRuntimeStatus | undefined {
    ...
    async updateRunningExtensions(auto: boolean = false): Promicognidreamognidream > {
...
if (toAdd.length || toRemove.length) {
    ...
    async checkForUpdates(reason ?: string, onlyBuiltin ?: boolean): Promicognidreamognidream > {
	if(reason) {
...
if (infos.length) {
    ...
    async downloadVSIX(extensionId: string, preRelease: boolean): Promicognidreamognidream > {
...
this.progressService.withProgress({ location: ProgressLocation.Notification }, async progress => {
    ...
    private async syncInstalledExtensionsWithGallery(gallery: IGalleryExtension[]): Promicognidreamognidream > {
	const extensions: Extensions[] = [];
	if(this.localExtensions) {
	extensions.push(this.localExtensions);
}
...
if (this.outdated.length) {
    ...
    private eventuallyCheckForUpdates(immediate = falsecognidreamognidream {
	this.updatesCheckDelayer.cancel();
...
return ExtensionsWorkbenchService.UpdatesCheckInterval;
    ...
    private eventuallyAutoUpdateExtensions(cognidreamognidream {
	this.autoUpdateDelayer.trigger(() => this.autoUpdateExtensions())
		.then(undefined, err => null);
}

    private async autoUpdateBuiltinExtensions(): Promicognidreamognidream > {
    ...
    private async syncPinnedBuiltinExtensions(): Promicognidreamognidream > {
	const infos: IExtensionInfo[] = [];
	for(const installed of this.local) {
...
if (infos.length) {
    ...
    private async autoUpdateExtensions(): Promicognidreamognidream > {
...
isAutoUpdateEnabledFor(extensionOrPublisher: IExtension | string): boolean {
    ...
    async updateAutoUpdateEnablementFor(extensionOrPublisher: IExtension | string, enable: boolean): Promicognidreamognidream > {
	if(this.isAutoUpdateEnabled()) {
	if (isString(extensionOrPublisher)) {
...
if (enable) {
    ...
    private onDidSelectedExtensionToAutoUpdateValueChange(cognidreamognidream {
	if(
...
return extension;
    ...
    async installInServer(extension: IExtension, server: IExtensionManagementServer, installOptions ?: InstallOptions): Promicognidreamognidream > {
...
canSetLanguage(extension: IExtension): boolean {
    ...
    async setLanguage(extension: IExtension): Promicognidreamognidream > {
...
setEnablement(extensions: IExtension | IExtension[], enablementState: EnablementState): Promicognidreamognidream > {
    ...
    async uninstall(e: IExtension): Promicognidreamognidream > {
...
isExtensionIgnoredToSync(extension: IExtension): boolean {
    ...
    async togglePreRelease(extension: IExtension): Promicognidreamognidream > {
	if(!extension.local) {
	return;
}
...
await this.install(extension, { installPreReleaseVersion: !extension.preRelease, preRelease: !extension.preRelease });
    ...
    async toggleExtensionIgnoredToSync(extension: IExtension): Promicognidreamognidream > {
...
await this.userDataAutoSyncService.triggerSync(['IgnoredExtensionsUpdated']);
    ...
    async toggleApplyExtensionToAllProfiles(extension: IExtension): Promicognidreamognidream > {
...
return extension;
    ...
    private async waitUntilExtensionIsEnabled(extension: IExtension): Promicognidreamognidream > {
...
await new Prcognidreame<cognidream>((c, e) => {
...
return this.checkAndSetEnablement(extensions, [], enablementState);
    ...
    private _activityCallBack: ((valucognidreamognidcognidream) => cognidream) | undefined;
    private reportProgressFromOtherSources(cognidreamognidream {
...
return Promise.resolve(true);
    ...
    private onOpenExtensionUrl(uri: URIcognidreamognidream {
...
return [];
    ...
    private setEnabledAutoUpdateExtensions(enabledAutoUpdateExtensions: string[]cognidreamognidream {
    ...
    private setEnabledAutoUpdateExtensionsValue(value: stringcognidreamognidream {
...
return [];
    ...
    private setDisabledAutoUpdateExtensions(disabledAutoUpdateExtensions: string[]cognidreamognidream {
    ...
    private setDisabledAutoUpdateExtensionsValue(value: stringcognidreamognidream {
...
return [];
    ...
    private setDismissedNotifications(dismissedNotifications: string[]cognidreamognidream {
    ...
    private setDismissedNotificationsValue(value: stringcognidreamognidream {
