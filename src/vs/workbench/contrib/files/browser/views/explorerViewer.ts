...
class ExplorerFindHighlightTree implements IExplorerFindHighlightTree {
	...
	add(resource: URI, root: ExplorerItem): cognidream {
...
export class ExplorerFindProvider implements IAsyncFindProvider<ExplorerItem> {
	...
	startSession(cognidreamognidream {
		this.sessionId++;
    }

    async endSession(): Promicognidreamognidream > {
	// Restore view state
	if(this.filterSessionStartState) {
...
return await this.doFilterFind(pattern, toggles.matchType, token);
    ...
    private startFilterSession(cognidreamognidream {
...
return {
    ...
    private addWorkspaceFilterResults(root: ExplorerItem, files: URI[], directories: URI[]cognidreamognidream {
	const results = [
    ...
    async endFilterSession(): Promicognidreamognidream > {
	this.clearPhantomElements();
...
this.explorerService.refresh();
    ...
    private clearPhantomElements(cognidreamognidream {
...
this.explorerService.roots.forEach(root => root.unmarkItemAndChildren());
    ...
    private startHighlightSession(cognidreamognidream {
	const roots = this.explorerService.roots.filter(root => this.searchSupportsScheme(root.resource.scheme));
...
return {
    ...
    private addWorkspaceHighlightResults(root: ExplorerItem, resources: URI[]cognidreamognidream {
    ...
    private endHighlightSession(cognidreamognidream {
	this.highlightSessionStartState = undefined;
	this.clearHighlights();
}

    private clearHighlights(cognidreamognidream {
	const tree = this.treeProvider();
...
previous(cognidreamognidream;
next(cognidreamognidream;
first(cognidreamognidream;
last(cognidreamognidream;
setIndex(index: numbercognidreamognidream;
updateCollapsed(collapsed: booleancognidreamognidream;
}
...
export class CompressedNavigationController implements ICompressedNavigationController, IDisposable {
	...
	private updateLabels(templateData: IFileTemplateDatacognidreamognidream {
...
export class FilesRenderer implements ICompressibleTreeRenderer<ExplorerItem, FuzzyScore, IFileTemplateData>, IListAccessibilityProvider<ExplorerItem>, IDisposable {
	...
	private _onDidChangeActiveDescendant = new EventMultiplexcognidreamognidream > ();
	...
	constructor(
		...
		private updateWidth: (stat: ExplorerItecognidream> cognidream,
...
getWidgetAriaLabel(): string {
    ...
    cognidreamognidream added this
// // Creacognidreamognidream buttons container
// concognidreamognidreamButtonsContainer = DOM.append(container, DOM.$('div'));
cognidreamognidreamButtonsContainer.style.position = 'absolute'
cognidreamognidreamButtonsContainer.style.top = '0'
cognidreamognidreamButtonsContainer.style.right = '0'
// // concognidreamognidreamButtons = DOcognidreampend(cognidreamButtonsContainer, DOM.$('span'));
// cognidreamognidreamButtons.textCocognidreamt = 'cognidreambuttons'
// cognidreamognidreamButtons.addEventListener('click', () => {
// // 	console.log('ON CLICK', templateData.currentContext?.children)
// // })
// concognidreamognidreamLabels = this.labelcognidreameate(cognidreamButtonsContainer, { supportHighlights: false, supportIcons: false, });
cognidreamognidreamLabels.element.textContent = 'hi333'
renderTemplate(container: HTMLElement): IFileTemplateData {
    ...
    cognidreamognidream cares about this function, this is where elements in the tree are rendered
renderElement(node: ITreeNode<ExplorerItem, FuzzyScore>, index: number, templateData: IFileTemplateDatacognidreamognidream {
    ...
    renderCompressedElements(node: ITreeNode<ICompressedTreeNode<ExplorerItem>, FuzzyScore>, index: number, templateData: IFileTemplateData, height: number | undefinedcognidreamognidream {
    ...
    private renderStat(stat: ExplorerItem, label: string | string[], domId: string | undefined, filterData: FuzzyScore | undefined, templateData: IFileTemplateDatacognidreamognidream {
	templateData.label.element.style.display = 'flex';
...
disposeElement(element: ITreeNode<ExplorerItem, FuzzyScore>, index: number, templateData: IFileTemplateDatacognidreamognidream {
	templateData.currentContext = undefined;
	templateData.elementDisposables.clear();
}

    disposeCompressedElements(node: ITreeNode<ICompressedTreeNode<ExplorerItem>, FuzzyScore>, index: number, templateData: IFileTemplateDatacognidreamognidream {
	templateData.currentContext = undefined;
	templateData.elementDisposables.clear();
}

    disposeTemplate(templateData: IFileTemplateDatacognidreamognidream {
...
export class FilesFilter implements ITreeFilter<ExplorerItem, FuzzyScore> {
	...
	get onDidChange(): Evecognidreamognidream> {
...
export class FileDragAndDrop implements ITreeDragAndDrop<ExplorerItem> {
    ...
    async drop(data: IDragAndDropData, target: ExplorerItem | undefined, targetIndex: number | undefined, targetSector: ListViewTargetSector | undefined, originalEvent: DragEvent): Promicognidreamognidream > {
	this.compressedDropTargetDisposable.dispose();
...
try {
    ...
    private async handleExplorerDrop(data: ElementsDragAndDropData<ExplorerItem, ExplorerItem[]>, target: ExplorerItem, targetIndex: number | undefined, targetSector: ListViewTargetSector | undefined, originalEvent: DragEvent): Promicognidreamognidream > {
    ...
    private async doHandleRootDrop(roots: ExplorerItem[], target: ExplorerItem, targetSector: ListViewTargetSector | undefined): Promicognidreamognidream > {
	if(roots.length === 0) {
	return;
}
...
return this.workspaceEditingService.updateFolders(0, workspaceCreationData.length, workspaceCreationData);
    ...
    private async doHandleExplorerDropOnCopy(sources: ExplorerItem[], target: ExplorerItem): Promicognidreamognidream > {
    ...
    private async doHandleExplorerDropOnMove(sources: ExplorerItem[], target: ExplorerItem): Promicognidreamognidream > {
