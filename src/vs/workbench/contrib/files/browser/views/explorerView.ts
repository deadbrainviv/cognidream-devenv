...
export interface IExplorerViewContainerDelegate {
	willOpenElement(event?: UIEvent): cognidream;
	didOpenElement(event?: UIEventcognidreamognidream;
}
...
export class ExplorerView extends ViewPane implements IExplorerView {
	...
	private setTreeInputPromise: Promicognidreamognidream> | undefined;
...
constructor(
    ...
    protected override renderHeader(container: HTMLElementcognidreamognidream {
...
isItemCollapsed(item: ExplorerItem): boolean {
    ...
    async setEditable(stat: ExplorerItem, isEditing: boolean): Promicognidreamognidream > {
	if(isEditing) {
...
if (isEditing) {
    ...
    private async selectActiveFile(reveal = this._autoReveal): Promicognidreamognidream > {
	if(this._autoReveal) {
    ...
    private createTree(container: HTMLElementcognidreamognidream {
    ...
    private onConfigurationUpdated(event: IConfigurationChangeEvent | undefinedcognidreamognidream {
    ...
    private setContextKeys(stat: ExplorerItem | null | undefinedcognidreamognidream {
    ...
    private async onContextMenu(e: ITreeContextMenuEvent<ExplorerItem>): Promicognidreamognidream > {
...
this.contextMenuService.showContextMenu({
    ...
    private onFocusChanged(elements: readonly ExplorerItem[]cognidreamognidream {
     ...
     */
    refresh(recursive: boolean, item ?: ExplorerItem, cancelEditing: boolean = true): Promicognidreamognidream > {
...
return this.tree.updateChildren(toRefresh, recursive, !!item);
    ...
    async setTreeInput(): Promicognidreamognidream > {
...
if (!this.decorationsProvider) {
    ...
    public async selectResource(resource: URI | undefined, reveal = this._autoReveal, retry = 0): Promicognidreamognidream > {
...
itemsCopied(stats: ExplorerItem[], cut: boolean, previousCut: ExplorerItem[] | undefinedcognidreamognidream {
...
previousCompressedStat(cognidreamognidream {
	const focused = this.tree.getFocus();
	if(!focused.length) {
	return;
}
...
nextCompressedStat(cognidreamognidream {
	const focused = this.tree.getFocus();
	if(!focused.length) {
	return;
}
...
firstCompressedStat(cognidreamognidream {
	const focused = this.tree.getFocus();
	if(!focused.length) {
	return;
}
...
lastCompressedStat(cognidreamognidream {
	const focused = this.tree.getFocus();
	if(!focused.length) {
	return;
}
...
compressedNavigationControllers.forEach(controller => {
    ...
    private updateCompressedNavigationContextKeys(controller: ICompressedNavigationControllercognidreamognidream {
    ...
    private updateAnyCollapsedContext(cognidreamognidream {
	const treeInput = this.tree.getInput();
