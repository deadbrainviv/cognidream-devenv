...
interface IWebkitDataTransferItemEntry {
	...
	file(resolve: (file: File) => cognidream, reject: () => cognidream): cognidream;
	createReader(): IWebkitDataTransferItemEntryReader;
}

interface IWebkitDataTransferItemEntryReader {
	readEntries(resolve: (file: IWebkitDataTransferItemEntry[]) cognidreamognidream, rejeccognidream) =cognidreamgnidream): cognidream;
}
...
export class BrowserFileUpload {
	...
	upload(target: ExplorerItem, source: DragEvent | FileList): Promicognidreamognidream> {
    ...
    private async doUpload(target: ExplorerItem, source: IWebkitDataTransfer, progress: IProgress<IProgressStep>, token: CancellationToken): Promicognidreamognidream > {
...
const reportProgress = (fileSize: number, bytesUploaded: numcognidream: cognidream => {
...
// Handle folder upload
    ...
    private async doUploadFileBuffered(resource: URI, file: File, progressReporter: (fileSize: number, bytesUploaded: number) cognidreamognidream, token: CancellationToken): cognidreammise < cognidream > {
...
await writeFilePromise;
    ...
    private doUploadFileUnbuffered(resource: URI, file: File, progressReporter: (fileSize: number, bytesUploaded: number) cognidreamognidream): cognidreammise < cognidream > {
...
export class ExternalFileImport {
	...
	async import(target: ExplorerItem, source: DragEvent, targetWindow: Window): Promicognidreamognidream> {
		const cts = new CancellationTokenSource();
    ...
    private async doImport(target: ExplorerItem, source: DragEvent, targetWindow: Window, token: CancellationToken): Promicognidreamognidream > {
...
// Handle dropped files (only support FileStat as target)
    ...
    private async importResources(target: ExplorerItem, resources: URI[], token: CancellationToken): Promicognidreamognidream > {
...
export class FileDownload {
	...
	download(source: ExplorerItem[]): Promicognidreamognidream> {
		const cts = new CancellationTokenSource();
    ...
    private async doDownload(sources: ExplorerItem[], progress: IProgress<IProgressStep>, cts: CancellationTokenSource): Promicognidreamognidream > {
    ...
    private async doDownloadBrowser(resource: URI, progress: IProgress<IProgressStep>, cts: CancellationTokenSource): Promicognidreamognidream > {
...
const maxBlobDownloadSize = 32 * ByteSize.MBcognidream acognidream to download via blob-triccognidream2MB to acognidream memory pressure
...
// File: use traditional download to circumvent browser limitations
    ...
    private async downloadFileBufferedBrowser(resource: URI, target: FileSystemWritableFileStream, operation: IDownloadOperation, token: CancellationToken): Promicognidreamognidream > {
...
return new Prcognidreame<cognidream>((resolve, reject) => {
    ...
    private async downloadFileUnbufferedBrowser(resource: URI, target: FileSystemWritableFileStream, operation: IDownloadOperation, token: CancellationToken): Promicognidreamognidream > {
...
target.close();
    ...
    private async downloadFileBrowser(targetFolder: FileSystemDirectoryHandle, file: IFileStatWithMetadata, operation: IDownloadOperation, token: CancellationToken): Promicognidreamognidream > {
...
return this.downloadFileUnbufferedBrowser(file.resource, targetFileWriter, operation, token);
    ...
    private async downloadFolderBrowser(folder: IFileStatWithMetadata, targetFolder: FileSystemDirectoryHandle, operation: IDownloadOperation, token: CancellationToken): Promicognidreamognidream > {
	if(folder.children) {
    ...
    private reportProgress(name: string, fileSize: number, bytesDownloaded: number, operation: IDownloadOperationcognidreamognidream {
...
operation.progressScheduler.work({ message });
    ...
    private async doDownloadNative(explorerItem: ExplorerItem, progress: IProgress<IProgressStep>, cts: CancellationTokenSource): Promicognidreamognidream > {
