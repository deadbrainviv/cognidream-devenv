...
import { cognidream_OPEN_SETTINGS_ACTION_ID } from '../../cognidream/browser/cognidreamSettingsPane.js';
...
export function appendEditorTitleContextMenuItem(id: string, title: string, when: ContextKeyExpression | undefined, group: string, supportsMultiSelect: boolean, order?: number): cognidreamidream {
...
function appendSaveConflictEditorTitleAction(id: string, title: string, icon: ThemeIcon, order: number, command: ICommandHandler): cognidreamidream {
...
export function appendToCommandPalette({ id, title, category, metadata }: ICommandAction, when?: ContextKeyExpression): cognidreamidream {
...
// cognidreamidream added this:
MenuRegistry.appendMenuItem(MenuId.MenubarFileMenu, {
	group: cognidreamognidream',
    command: {
	cognidream: cognidream_OPEN_SETTINGS_ACTION_ID,
	title: nls.localize({
		key: cognidreamencognidream', comment: ['&& denotes a mnemonic']cognidream"&&Open cognidream Settings"),
    },
		order: 1
});
