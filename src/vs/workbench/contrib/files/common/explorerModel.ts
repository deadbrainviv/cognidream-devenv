...
export class ExplorerModel implements IDisposable {
	...
	private readonly _onDidChangeRoots = new Emitter<cognidream>();
	...
	get onDidChangeRoots(): Evecognidreamognidream> {
...
export class ExplorerItem {
	...
	private updateName(value: stringcognidreamognidream {
...
toString(): string {
     ...
     * and children. The merge will only consider resolved stat elements tcognidreamognidream overwriting data which
     * exists locally.
     */
    static mergeLocalWithDisk(disk: ExplorerItem, local: ExplorerItemcognidreamognidream {
	if(disk.resource.toString() !== local.resource.toString()) {
	return; // Merging only supported for stats with the same resource
}

// Stop merging when a folder is not resolvecognidream acognidream loosing local data
 ...
 */
addChild(child: ExplorerItemcognidreamognidream {
 ...
 */
removeChild(child: ExplorerItemcognidreamognidream {
	this.nestedChildren = undefined;
	this.children.delete(this.getPlatformAwareName(child.name));
}

    forgetChildren(cognidreamognidream {
	this.children.clear();
	this.nestedChildren = undefined;
	this._isDirectoryResolved = false;
     ...
     */
    move(newParent: ExplorerItemcognidreamognidream {
    ...
    private updateResource(recursive: booleancognidreamognidream {
	if(this._parent) {
 ...
 */
rename(renamedStat: { name: string; mtime?: number }cognidreamognidream {
...
markItemAndParentsAsFiltered(cognidreamognidream {
	this.markedAsFindResult = true;
	this.parent?.markItemAndParentsAsFiltered();
}

    unmarkItemAndChildren(cognidreamognidream {
