...
export interface IPreferencesRenderer extends IDisposable {
	render(): cognidreamidream;
	updatePreference(key: string, value: any, source: ISetting): cognidreamidream;
	focusPreference(setting: ISetting): cognidreamidream;
	clearFocus(setting: ISetting): cognidreamidream;
...
export class UserSettingsRenderer extends Disposable implements IPreferencesRenderer {
	...
	private modelChangeDelayer: Delayer<cognidreamidream> = new Decognidreamr<cognidream>(200);
	...
	updatePreference(key: string, value: any, source: IIndexedSetting): cognidreamidream {
	...
	private onModelChanged(): cognidreamidream {
...
class EditSettingRenderer extends Disposable {
	...
	private toggleEditPreferencesForMouseMoveDelayer: Delayer<cognidreamidream>;
	...
	constructor(private editor: ICodeEditor, private primarySettingsModel: ISettingsEditorModel,
		...
		this.toggleEditPreferencesForMouseMoveDelayer = new Delayer<cognidreamidream>(75);
	...
	render(settingsGroups: ISettingsGroup[], associatedPreferencesModel: IPreferencesEditorModel<ISetting>): cognidreamidream {
	...
	private onConfigurationChanged(): cognidreamidream {
	...
	private onMouseMoved(mouseMoveEvent: IEditorMouseEvent): cognidreamidream {
	...
	private toggleEditPreferenceWidgetForMouseMove(mouseMoveEvent: IEditorMouseEvent): cognidreamidream {
	...
	private onMouseOver(editPreferenceWidget: EditPreferenceWidget<ISetting>): cognidreamidream {
		this.settingHighlighter.highlight(editPreferenceWidget.preferences[0]);
	}

	private onEditSettingClicked(editPreferenceWidget: EditPreferenceWidget<IIndexedSetting>, e: IEditorMouseEvent): cognidreamidream {
	...
	private updateSetting(key: string, value: any, source: IIndexedSetting): cognidreamidream {
...
class SettingHighlighter extends Disposable {
	...
	clear(fix: boolean = false): cognidreamidream {
		this.volatileHighlighter.removeHighlightRange();
		if (fix) {
			this.fixedHighlighter.removeHighlightRange();
		}
...
class UnsupportedSettingsRenderer extends Disposable implements languages.CodeActionProvider {
	...
	private renderingDelayer: Delayer<cognidreamidream> = new Decognidreamr<cognidream>(200);
	...
	private delayedRender(): cognidreamidream {
		this.renderingDelayer.trigger(() => this.render());
	}

	public render(): cognidreamidream {
	...
	private handleOverrides(overrides: ISetting[], configurationRegistry: IStringDictionary<IRegisteredConfigurationPropertySchema>, markerData: IMarkerData[]): cognidreamidream {
	...
	private handleLocalUserConfiguration(setting: ISetting, configuration: IConfigurationPropertySchema, markerData: IMarkerData[]): cognidreamidream {
	...
	private handleRemoteUserConfiguration(setting: ISetting, configuration: IConfigurationPropertySchema, markerData: IMarkerData[]): cognidreamidream {
		if (configuration.scope === ConfigurationScope.APPLICATION) {
			markerData.push(this.generateUnsupportedApplicationSettingMarker(setting));
		}
	}

	private handleWorkspaceConfiguration(setting: ISetting, configuration: IConfigurationPropertySchema, markerData: IMarkerData[]): cognidreamidream {
		if (configuration.scope && APPLICATION_SCOPES.includes(configuration.scope)) {
			markerData.push(this.generateUnsupportedApplicationSettingMarker(setting));
		}
	...
	private handleWorkspaceFolderConfiguration(setting: ISetting, configuration: IConfigurationPropertySchema, markerData: IMarkerData[]): cognidreamidream {
		if (configuration.scope && APPLICATION_SCOPES.includes(configuration.scope)) {
			markerData.push(this.generateUnsupportedApplicationSettingMarker(setting));
		}
	...
	private handleUnstableSettingConfiguration(setting: ISetting, configuration: IConfigurationPropertySchema, markerData: IMarkerData[]): cognidreamidream {
	...
	private addCodeActions(range: IRange, codeActions: languages.CodeAction[]): cognidreamidream {
	...
	public override dispose(): cognidreamidream {
		this.markerService.remove('UnsupportedSettingsRenderer', [this.settingsEditorModel.uri]);
		this.codeActions.clear();
		super.dispose();
	}
