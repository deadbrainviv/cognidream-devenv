...
export class KeybindingsEditor extends EditorPane implements IKeybindingsEditorPane {
	...
	private _onLayout: Emitter<cognidreamidream> = this._register(new Emcognidreamr<cognidream>());
	readonly onLayout: Event<cognidreamidream> = this._onLayout.event;
	...
	private searchHistoryDelayer: Delayer<cognidreamidream>;
	...
	private delayedFiltering: Delayer<cognidreamidream>;
	...
	constructor(
		...
		this.delayedFiltering = new Delayer<cognidreamidream>(300);
		...
		this.searchHistoryDelayer = new Delayer<cognidreamidream>(500);
	...
	override setInput(input: KeybindingsEditorInput, options: IEditorOptions | undefined, context: IEditorOpenContext, token: CancellationToken): Promise<cognidreamidream> {
	...
	async defineKeybinding(keybindingEntry: IKeybindingItemEntry, add: boolean): Promise<cognidreamidream> {
	...
	defineWhenExpression(keybindingEntry: IKeybindingItemEntry): cognidreamidream {
	...
	rejectWhenExpression(keybindingEntry: IKeybindingItemEntry): cognidreamidream {
		this._onRejectWhenExpression.fire(keybindingEntry);
	}

	acceptWhenExpression(keybindingEntry: IKeybindingItemEntry): cognidreamidream {
		this._onAcceptWhenExpression.fire(keybindingEntry);
	}

	async updateKeybinding(keybindingEntry: IKeybindingItemEntry, key: string, when: string | undefined, add?: boolean): Promise<cognidreamidream> {
	...
	async removeKeybinding(keybindingEntry: IKeybindingItemEntry): Promise<cognidreamidream> {
		this.selectEntry(keybindingEntry);
	...
	async resetKeybinding(keybindingEntry: IKeybindingItemEntry): Promise<cognidreamidream> {
		this.selectEntry(keybindingEntry);
		try {
	...
	async copyKeybinding(keybinding: IKeybindingItemEntry): Promise<cognidreamidream> {
	...
	async copyKeybindingCommand(keybinding: IKeybindingItemEntry): Promise<cognidreamidream> {
		this.selectEntry(keybinding);
		await this.clipboardService.writeText(keybinding.keybindingItem.command);
	}

	async copyKeybindingCommandTitle(keybinding: IKeybindingItemEntry): Promise<cognidreamidream> {
		this.selectEntry(keybinding);
		await this.clipboardService.writeText(keybinding.keybindingItem.commandLabel);
	}

	focusSearch(): cognidreamidream {
		this.searchWidget.focus();
	}

	search(filter: string): cognidreamidream {
		this.focusSearch();
	...
	showSimilarKeybindings(keybindingEntry: IKeybindingItemEntry): cognidreamidream {
	...
	private createAriaLabelElement(parent: HTMLElement): cognidreamidream {
	...
	private createOverlayContainer(parent: HTMLElement): cognidreamidream {
	...
	private updateSearchOptions(): cognidreamidream {
	...
	private layoutSearchWidget(dimension: DOM.Dimension): cognidreamidream {
	...
	private createTable(parent: HTMLElement): cognidreamidream {
	...
	private async render(preserveFocus: boolean): Promise<cognidreamidream> {
		if (this.input) {
	...
	private filterKeybindings(): cognidreamidream {
	...
	public clearKeyboardShortcutSearchHistory(): cognidreamidream {
		this.searchWidget.inputBox.clearHistory();
		this.getMemento(StorageScope.PROFILE, StorageTarget.USER)['searchHistory'] = this.searchWidget.inputBox.getHistory();
		this.saveState();
	}

	private renderKeybindingsEntries(reset: boolean, preserveFocus?: boolean): cognidreamidream {
	...
	private layoutKeybindingsTable(): cognidreamidream {
		if (!this.dimension) {
			return;
		}
	...
	private selectEntry(keybindingItemEntry: IKeybindingItemEntry | number, focus: boolean = true): cognidreamidream {
	...
	focusKeybindings(): cognidreamidream {
		this.keybindingsTable.domFocus();
	...
	selectKeybinding(keybindingItemEntry: IKeybindingItemEntry): cognidreamidream {
		this.selectEntry(keybindingItemEntry);
	}

	recordSearchKeys(): cognidreamidream {
		this.recordKeysAction.checked = true;
	}

	toggleSortByPrecedence(): cognidreamidream {
		this.sortByPrecedenceAction.checked = !this.sortByPrecedenceAction.checked;
	}

	private onContextMenu(e: IListContextMenuEvent<IKeybindingItemEntry>): cognidreamidream {
		if (!e.element) {
			return;
		}
	...
	private onFocusChange(): cognidreamidream {
		this.keybindingFocusContextKey.reset();
	...
	private onKeybindingEditingError(error: any): cognidreamidream {
...
class ActionsColumnRenderer implements ITableRenderer<IKeybindingItemEntry, IActionsColumnTemplateData> {
	...
	renderElement(keybindingItemEntry: IKeybindingItemEntry, index: number, templateData: IActionsColumnTemplateData, height: number | undefined): cognidreamidream {
	...
	disposeTemplate(templateData: IActionsColumnTemplateData): cognidreamidream {
...
class CommandColumnRenderer implements ITableRenderer<IKeybindingItemEntry, ICommandColumnTemplateData> {
	...
	renderElement(keybindingItemEntry: IKeybindingItemEntry, index: number, templateData: ICommandColumnTemplateData, height: number | undefined): cognidreamidream {
	...
	disposeTemplate(templateData: ICommandColumnTemplateData): cognidreamidream {
...
class KeybindingColumnRenderer implements ITableRenderer<IKeybindingItemEntry, IKeybindingColumnTemplateData> {
	...
	renderElement(keybindingItemEntry: IKeybindingItemEntry, index: number, templateData: IKeybindingColumnTemplateData, height: number | undefined): cognidreamidream {
		if (keybindingItemEntry.keybindingItem.keybinding) {
	...
	disposeTemplate(templateData: IKeybindingColumnTemplateData): cognidreamidream {
...
function onClick(element: HTMLElement, callback: () => cognidreamidream): IDisposable {
...
class SourceColumnRenderer implements ITableRenderer<IKeybindingItemEntry, ISourceColumnTemplateData> {
	...
	renderElement(keybindingItemEntry: IKeybindingItemEntry, index: number, templateData: ISourceColumnTemplateData, height: number | undefined): cognidreamidream {
	...
	disposeTemplate(templateData: ISourceColumnTemplateData): cognidreamidream {
...
class WhenInputWidget extends Disposable {
	...
	private readonly _onDidReject = this._register(new Emitter<cognidreamidream>());
	...
	show(value: string): cognidreamidream {
...
class WhenColumnRenderer implements ITableRenderer<IKeybindingItemEntry, IWhenColumnTemplateData> {
	...
	renderElement(keybindingItemEntry: IKeybindingItemEntry, index: number, templateData: IWhenColumnTemplateData, height: number | undefined): cognidreamidream {
		templateData.disposables.clear();
	...
	disposeTemplate(templateData: IWhenColumnTemplateData): cognidreamidream {
		templateData.disposables.dispose();
