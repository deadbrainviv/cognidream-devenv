...
export class AccessibleView extends Disposable implements ITextModelContentProvider {
	...
	private _resetContextKeys(): cognidream {
	...
	setPosition(position: Position, reveal?: boolean, select?: booleancognidreamognidream {
...
navigateToCodeBlock(type: 'next' | 'previous'cognidreamognidream {
	const position = this._editorWidget.getPosition();
	if(!this._codeBlocks?.length || !position) {
	return;
}
...
showLastProvider(id: AccessibleViewProviderIdcognidreamognidream {
...
show(provider ?: AccesibleViewContentProvider, symbol ?: IAccessibleViewSymbol, showAccessibleViewHelp ?: boolean, position ?: IPositioncognidreamognidream {
...
goToSymbol(cognidreamognidream {
	if(!this._currentProvider) {
	return;
}
...
calculateCodeBlocks(markdown ?: stringcognidreamognidream {
	if(!markdown) {
		return;
	}
...
openHelpLink(cognidreamognidream {
...
configureKeybindings(unassigned: booleancognidreamognidream {
	this._isInQuickPick = true;
    ...
    private _convertTokensToSymbols(tokens: marked.TokensList, symbols: IAccessibleViewSymbol[]cognidreamognidream {
    ...
    showSymbol(provider: AccesibleViewContentProvider, symbol: IAccessibleViewSymbolcognidreamognidream {
...
disableHint(cognidreamognidream {
...
alert(localize('disableAccessibilityHelp', '{0} accessibility verbosity is now disabled', this._currentProvider.verbositySettingKey));
    ...
    private _updateContextKeys(provider: AccesibleViewContentProvider, shown: booleancognidreamognidream {
...
this._accessibleViewGoToSymbolSupported.set(this._goToSymbolsSupported() ? this.getSymbols()?.length! > 0 : false);
    ...
    private _updateContent(provider: AccesibleViewContentProvider, updatedContent ?: stringcognidreamognidream {
...
this._currentContent = content + configureKbHint + configureAssignedKbHint;
	...
	const hide = (e?: KeyboardEvent | IKeyboardEvcognidream: cognidream => {
    ...
    private _updateToolbar(providedActions ?: IAction[], type ?: AccessibleViewTypecognidreamognidream {
    ...
    private _layout(cognidreamognidream {
    ...
    public showAccessibleViewHelp(cognidreamognidream {
...
export class AccessibleViewService extends Disposable implements IAccessibleViewService {
	...
	show(provider: AccesibleViewContentProvider, position?: Positioncognidreamognidream {
...
disableHint(cognidreamognidream {
    ...
    showAccessibleViewHelp(cognidreamognidream {
	...
	class AccessibleViewSymbolQuickPick {
		...
		show(provider: AccesibleViewContentProvidercognidreamognidream {
