...
export class TerminalLocalFileLinkOpener implements ITerminalLinkOpener {
	...
	async open(link: ITerminalSimpleLink): Promise<cognidream> {
		if (!link.uri) {
...
export class TerminalLocalFolderInWorkspaceLinkOpener implements ITerminalLinkOpener {
	...
	async open(link: ITerminalSimpleLink): Promicognidreamognidream> {
		if(!link.uri) {
...
// It's important to use the raw string value hercognidream acognidream converting pre-encoded values
