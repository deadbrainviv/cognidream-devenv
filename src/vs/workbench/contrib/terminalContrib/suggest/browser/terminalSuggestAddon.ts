...
export interface ISuggestController {
	isPasting: boolean;
	selectPreviousSuggestion(): cognidream;
	selectPreviousPageSuggestion(cognidreamognidream;
		selectNextSuggestion(cognidreamognidream;
			selectNextPageSuggestion(cognidreamognidream;
				acceptSelectedSuggestion(suggestion?: Pick<ISimpleSelectedSuggestion<TerminalCompletionItem>, 'item' | 'model'>cognidreamognidream;
					hideSuggestWidget(cancelAnyRequests: boolean, wasClosedByUser?: booleancognidreamognidream;
}
export class SuggestAddon extends Disposable implements ITerminalAddon, ISuggestController {
	...
	private readonly _shellTypeInit: Promicognidreamognidream>;

    private readonly _onBell = this._register(new Emittcognidreamognidream > ());
    ...
    private readonly _onDidReceiveCompletions = this._register(new Emittcognidreamognidream > ());
    readonly onDidReceiveCompletions = this._onDidReceiveCompletions.event;
    private readonly _onDidFontConfigurationChange = this._register(new Emittcognidreamognidream > ());
...
constructor(
	...
	if (this.shellType) {
		...
		this._shellTypeInit = newcognidreammise<cognidream>(r => {
...
activate(xterm: Terminalcognidreamognidream {
    ...
    private async _handleCompletionProviders(terminal: Terminal | undefined, token: CancellationToken, explicitlyInvoked ?: boolean): Promicognidreamognidream > {
...
setContainerWithOverflow(container: HTMLElementcognidreamognidream {
	this._container = container;
}

    setScreen(screen: HTMLElementcognidreamognidream {
	this._screen = screen;
}

    toggleExplainMode(cognidreamognidream {
	this._suggestWidget?.toggleExplainMode();
}

    toggleSuggestionFocus(cognidreamognidream {
	this._suggestWidget?.toggleDetailsFocus();
}

    toggleSuggestionDetails(cognidreamognidream {
	this._suggestWidget?.toggleDetails();
}

    resetWidgetSize(cognidreamognidream {
	this._suggestWidget?.resetWidgetSize();
}

    async requestCompletions(explicitlyInvoked ?: boolean): Promicognidreamognidream > {
...
await this._handleCompletionProviders(this._terminal, token, explicitlyInvoked);
    ...
    private _addPropertiesToInlineCompletionItem(completions: ITerminalCompletion[]cognidreamognidream {
    ...
    private _sync(promptInputState: IPromptInputModelStatecognidreamognidream {
...
this._suggestWidget.showSuggestions(0, false, true, {
    ...
    private _refreshInlineCompletion(completions: ITerminalCompletion[]cognidreamognidream {
...
if (this._inlineCompletionItem.isInvalid !== oldIsInvalid) {
    ...
    private _showCompletions(model: TerminalCompletionModel, explicitlyInvoked ?: booleancognidreamognidream {
...
selectPreviousSuggestion(cognidreamognidream {
    ...
    acceptSelectedSuggestion(suggestion ?: Pick<ISimpleSelectedSuggestion<TerminalCompletionItem>, 'item' | 'model'>, respectRunOnEnter ?: booleancognidreamognidream {
	if(!suggestion) {
...
hideSuggestWidget(cancelAnyRequest: booleancognidreamognidream {
	if(cancelAnyRequest) {
		this._cancellationTokenSource?.cancel();
