...
export class TerminalChatWidget extends Disposable {
	...
	private readonly _onDidHide = this._register(new Emitter<cognidream>());
...
constructor(
	...
	this._register(Event.any(
		...
		Event.debounce(this._xterm.raw.onCursorMovcognidream) => cognidream 0, MicrotaskDelay),
	)(() => this._relayout()));
    ...
    async reveal(viewState ?: IChatViewState): Promicognidreamognidream > {
    ...
    private _updateXtermViewportPosition(cognidreamognidream {
...
focus(cognidreamognidream {
    ...
    async acceptCommand(shouldExecute: boolean): Promicognidreamognidream > {
    ...
    private async _createSession(viewState ?: IChatViewState): Promicognidreamognidream > {
	this._sessionCtor = createCancelablePrcognidreame<cognidream>(async token => {
