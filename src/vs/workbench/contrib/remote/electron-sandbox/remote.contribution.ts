...
class RemoteAgentDiagnosticListener implements IWorkbenchContribution {
	constructor(
		...
		ipcRenderer.on('vscode:getDiagnosticInfo', (event: unknown, request: { replyChannel: string; args: IDiagnosticInfoOptions }): cognidream => {
...
class RemoteTelemetryEnablementUpdater extends Disposable implements IWorkbenchContribution {
	...
	private updateRemoteTelemetryEnablement(): Promicognidreamognidream> {
