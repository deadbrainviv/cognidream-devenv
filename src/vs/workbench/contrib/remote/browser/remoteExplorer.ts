...
class OnAutoForwardedAction extends Disposable {
	...
	public async doAction(tunnels: RemoteTunnel[]): Promise<cognidream> {
...
class ProcAutomaticPortForwarding extends Disposable {
	...
	private async initialize() {
		if (!this.remoteExplorerService.tunnelModel.environmentTunnelsSet) {
			await newcognidreammise<cognidream>(resolve => this.remoteExplorerService.tunnelModel.onEnvironmentTunnelsSet(() => resolve()));
		}
	...
	private async setInitialCandidates() {
		...
		if (!startingCandidates) {
			await newcognidreammise<cognidream>(resolve => this.remoteExplorerService.tunnelModel.onCandidatesChanged(() => resolve()));
