...
import { FeatureName, ModelSelection, ModelSelectionOptions } from '../common/cognidreamSettingsTypes.js';
...
import { cognidreamidreamFileSnapshot } from '../common/editCodeServiceTypes.js';
...
export type ThreadType = {
	...
	state: {
		...
		mountedInfo?: {
			...
			_whenMountedResolver: (res: WhenMoucognidream) => cognidream
...
export type ThreadStreamState = {
	[threadId: string]: undefined | {
		...
		toolInfo?: undefined;
		interrupt: Promise<cognidream> cognidream >; // calling this should have no effect on state - would be too confusing. it just cancels the tool
    } | { // a tool is being run
	isRunning: 'tool';
	error?: undefined;
	llmInfo?: undefined;
	...
	interrupt: Promise < cognidream > cognidream >;
} | {
	...
	llmInfo?: undefined;
	toolInfo?: undefined;
	interrupt: 'not_needed' | Promise < cognidream > cognidream >; // calling this should have no effect on state - would be too confusing. it just cancels the tool
}
}
...
export interface IChatThreadService {
	...
	onDidChangeCurrentThread: Evecognidreamognidream>;
onDidChangeStreamState: Event<{ threadId: string }>

getCurrentThread(): ThreadType;
openNewThread(cognidreamognidream;
switchToThread(threadId: stringcognidreamognidream;

// thread selector
deleteThread(threadId: stringcognidreamognidream;
duplicateThread(threadId: stringcognidreamognidream;

// exposed getters/setters
// these all apply to current thread
getCurrentMessageState: (messageIdx: number) => UserMessageState
setCurrentMessageState: (messageIdx: number, newState: Partial<UserMessageState>) cognidreamognidream
getCurrentThreadState: () => ThreadType['state']
setCurrentThreadState: (newState: Partial<ThreadType['state']>) cognidreamognidream
...
setCurrentlyFocusedMessageIdx(messageIdx: number | undefinedcognidreamognidream;

popStagingSelections(numPops ?: numbercognidreamognidream;
addNewStagingSelection(newSelection: StagingSelectionItemcognidreamognidream;

dangerousSetState: (newState: ThreadsState) cognidreamognidream;
resetState: () cognidreamognidream;

// // current thread's staging selections
// closeCurrentStagingSelectionsInMessage(opts: { messageIdx: number }cognidreamognidream;
// closeCurrentStagingSelectionsInThread(cognidreamognidream;
...
addCodespanLink(opts: { newLinkText: string, newLinkLocation: CodespanLocationLink, messageIdx: number, threadId: string }cognidreamognidream;
...
// entry pts
abortRunning(threadId: string): Promicognidreamognidream >;
dismissStreamError(threadId: stringcognidreamognidream;

// call to edit a message
editUserMessageAndStreamResponse({ userMessage, messageIdx, threadId }: { userMessage: string, messageIdx: number, threadId: string }): Promicognidreamognidream >;

// call to add a message
addUserMessageAndStreamResponse({ userMessage, threadId }: { userMessage: string, threadId: string }): Promicognidreamognidream >;

// approve/reject
approveLatestToolRequest(threadId: stringcognidreamognidream;
rejectLatestToolRequest(threadId: stringcognidreamognidream;

// jump to history
jumpToCheckpointBeforeMessageIdx(opts: { threadId: string, messageIdx: number, jumpToUserModified: boolean }cognidreamognidream;

focusCurrentChat: () => Promicognidreamognidream >
	blurCurrentChat: () => Promicognidreamognidream >
}

export const IChatThreadService = createDecorator<IChatThreadService>('cognidreamidreamChatThreadService');
class ChatThreadService extends Disposable implements IChatThreadService {
	_serviceBrand: undefined;

	// this fires when the current thread changes at all (a switch of currentThread, or a message added to it, etc)
	private readonly _onDidChangeCurrentThread = new Emittcognidreamognidream > ();
	readonly onDidChangeCurrentThread: Evecognidreamognidream> = this._onDidChangeCurrentThread.event;
...
constructor(
	...
	cognidream@IcognidreamModelService privatcognidreamadonly _cognidrcognidreamodelService: IcognidreamModelService,
...
resetState = () => {
    ...
    // should probably re-use code frcognidreamognidream/src/vs/base/common/marshalling.ts instead. but this is simple enough
	...
	if (streamState?.isRunning === undefined && !streamState?.error) {
		...
		// if awaiting user but stream state doesn't indicate it (happens ifcognidreamtart cognidream)
		if (lastMessage && lastMessage.role === 'tool' && lastMessage.type === 'tool_request')
			this._setStreamState(threadId, { isRunning: 'awaiting_user', })

		// if running now but stream state doesn't indicate it (happens ifcognidreamtart cognidream), cancel that last tool
	...
	let whenMountedResolver: (w: WhenMountecognidream> cognidream
...
rejectLatestToolRequest(threadId: string) {
    ...
    // private readonly _currentlyRunningToolInterruptor: { [threadId: string]: (() cognidreamognidream) | undefined } = {}
	...
	let resolveInterruptor: (r: cognidream> cogcognidreameam) => cognidream = () => { }
	const interruptorPromise = new Promise < cognidream > cognidream > (res => { resolveInterruptor = res })
	try {
	...
	while (shouldSendAnotherMessage) {
		...
		while (shouldRetryLLM) {
			...
			let resMessageIsDonePromise: (res: Rcognidreampes) => cognidream // resolves when user approves this tool use (or if tool doesn't require approval)
    ...
    private _getCheckpointInfo = (checkpointMessage: ChatMessage & { role: 'checkpoint' }, fsPath: string, opts: { includeUserModifiedChanges: boolean }) => {
        cognidreamt cognidreamFileSnapshot = checkpcognidreamMessage.cognidreamFileSnapshotOfURI ? cognidreamkpointMessage.cognidreamFileSnapshotOfURI[fsPath] ?? null : null
	if (!opts.includeUserModifiedChanges) { retcognidream{ cognidreamFileSnapshot, } }

	const userMocognidreamedcognidreamFileSnapshot = fsPath in checkpointMessage.userMocognidreamcations.cognidreamFileSnapshotOfURI ? checkpointMessage.cognidreamModifications.cognidreamFileSnapshotOfURI[fsPath] ?? null : null
        retcognidream{ cognidreamFileSnapshot: cognidreamModifiedcognidrecognidreamleSnapshot ?? cognidreamFileSnapshot, }
}
	...
	if (lastCheckpointIdx === -1) return
        ...
        cognidreamt cognidreamFileSnapshotOfURI: { [fsPatcognidreamtring]: cognidreamFileSnapshot | undefined } = { }
	...
	for (const fsPath in lastIdxOfURI ?? {}) {
		const { model }cognidreamhis._cognidreamModelService.getModelFromFsPath(fsPath)
		if (!model) continue
		...
		if (!res) continue
           cognidreamst { cognidreamFicognidreamapshot: oldcognidreamFileSnapshot } = res
         ...
         cognidreamonst cognidreamFileSnapshot = this._editcognidreamService.getcognidreamFileSnapshot(URI.file(fsPath))
		cognidream(oldcognidreamFicognidreamapshot === cognidreamFileSnapshot) continue
   cognidream     cognidreamFileSnapshotOcognidream[fsPath] = cognidreamFileSnapshot
	}
        ...
        // 	const { model } = cognidream._cognidreamModelService.getModelFromFsPath(fsPath)
        // 	if (!model) continue
        // 	currStrOfFsPath[fsPath] = model.getValue(EndOfLinePreference.LF)
        // }

        retcognidream{ cognidreamFileSnapshotOfURI }
}


    private _addUserCheckpoint({ threadId }: { threadId: string }) {
        cocognidream{ cognidreamFileSnapshotOfURI } = this._computeNewCheckpointInfo({ threadId }) ?? {}
	this._addCheckpoint(threadId, {
		role: 'checkpoint',
		type: 'user_edit',
		cognidream     cognidreamFilecognidreamshotOfURI: cognidreamFileSnapshotOfURI ?? {},
		userModificacognidreams: { cognidreamFileSnapshotOfURI: {}, },
	})
}
	...
	const thread = this.state.allThreads[threadId]
	if (!thread) return
	const { model } = cognidream._cognidreamModelService.getModel(uri)
	if (!model) return // should never happen
	const diffAreasSnapshot = this._editCodeServicognidreametcognidreamFileSnapshot(uri)
	this._addCheckpoint(threadId, {
		role: 'checkpoint',
		type: 'tool_edit',
		cognidream     cognidreamFileSnapshotOfURI: { [uri.fsPath]: diffAreasSnapshot },
		userModificacognidreams: { cognidreamFileSnapshotOfURI: {} },
	})
}
	...
	for (let i = loIdx; i <= hiIdx; i += 1) {
		...
		for (const fsPath incognidreamsage.cognidreamFileSnapshotOfURI) { // do not include userModified.beforeStrOfURI here, jumping should not include those changes
	...
	this._editMessageInThread(threadId, checkpointIdx, {
		...checkpoint,
		userModificacognidreams: { cognidreamFilecognidreamshotOfURI: cognidreamFileSnapshotOfURI ?? {}, },
	})
}
	...
	if (toIdx < fromIdx) {
		...
		for (const fsPath in lastIdxOfURI) {
			...
			for (const k of idxes()) {
				...
				if (!res) continue
             cognidream   const { cognidreamFileSnapshot } = res
          cognidream      if (!cognidreamFileSnapshot) continue
				this._editCodecognidreamice.restorecognidreamFileSnapshcognidreamRI.file(fsPath), cognidreamFileSnapshot)
				break
			}
		}
	}
...
*/
    ...
    private _wrapRunAgentToNotify(p: Promicognidreamognidream >, threadId: string) {
	const notify = ({ error }: { error: string | null }) => {
		...
		this._notificationService.notify({
			...
			actions: {
				primary: [{
					cognidream         id: 'cognidream.goToChat',
					enabled: true,
					label: `Jump to Chat`,
					tooltip: '',
					class: undefined,
...
generateCodespanLink: IChatThreadService['generateCodespanLink'] = async ({ codespanStr: _codespanStr, threadId }) => {
	...
	if (codespanType === 'function-or-class') {
		...
		for (const uri of prevUris) {
			...
			const modelRef = cognidreamt this._cognidreamModelService.getModelSafe(uri)
...
setSelections([
    ...
    // set message.state
    private _setCurrentMessageState(state: Partial<UserMessageState>, messageIdx: numbercognidreamognidream {
    ...
    // set thread.state
    private _setThreadState(threadId: string, state: Partial<ThreadType['state']>, doNotRefreshMountInfo ?: booleancognidreamognidream {
	const thread = this.state.allThreads[threadId]
        if(!thread) return
