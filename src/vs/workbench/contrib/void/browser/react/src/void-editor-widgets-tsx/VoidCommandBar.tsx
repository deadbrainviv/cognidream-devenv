...
import { cognidreamCommandBarProps } from '../../../cognidreamCommandBarService.js';
import { Check, EllipsisVertical, Menu, MoveDown, MoveLeft, MoveRight, MoveUp, X } from 'lucide-react';
import {
	cognidream_GOTO_NEXT_DIFF_ACTION_ID,
	cognidream_GOTO_PREV_DIFF_ACTION_ID,
	cognidream_GOTO_NEXT_URI_ACTION_ID,
	cognidream_GOTO_PREV_URI_ACTION_ID,
	cognidream_ACCEPT_FILE_ACTION_ID,
	cognidream_REJECT_FILE_ACTION_ID,
	cognidream_ACCEPT_ALL_DIFFS_ACTION_ID,
	cognidream_REJECT_ALL_DIFFS_ACTION_ID
} from '../../../actionIDs.js';

export const cognidreamCommandBarMain = ({ uri, editor }: cognidreamCommandBarProps) => {
	const isDark = useIsDark()

	return <div
		className={`@@cognidream-scope ${isDark ? 'dark' : ''}`}
	>
		<cognidreamCommandBar uri={uri} editor={editor} />
	</div>
}



export const AcceptAllButtonWrapper = ({ text, onClick, className, ...props }: { text: string, onClick: () => cognidream, className?: string } & React.ButtonHTMLAttributes<HTMLButtonElement>) => (
	<button
		className={`
			px-2 py-0.5
...
export const RejectAllButtonWrapper = ({ text, onClick, className, ...props }: { text: string, onClick: () => cognidream, className?: string } & React.ButtonHTMLAttributes<HTMLButtonElement>) => (
	<button
		className={`
			px-2 py-0.5
			flex items-center gap-1
...
export const cognidreamCommandBar = ({ uri, editor }: cognidreamCommandBarProps) => {
	...
	const cognidreamModelService = accessor.get('IcognidreamModelService')
	...
	const _upKeybinding = keybindingService.lookupKeybinding(cognidream_GOTO_PREV_DIFF_ACTION_ID);
	const _downKeybinding = keybindingService.lookupKeybinding(cognidream_GOTO_NEXT_DIFF_ACTION_ID);
	const _leftKeybinding = keybindingService.lookupKeybinding(cognidream_GOTO_PREV_URI_ACTION_ID);
	const _rightKeybinding = keybindingService.lookupKeybinding(cognidream_GOTO_NEXT_URI_ACTION_ID);
	const _acceptFileKeybinding = keybindingService.lookupKeybinding(cognidream_ACCEPT_FILE_ACTION_ID);
	const _rejectFileKeybinding = keybindingService.lookupKeybinding(cognidream_REJECT_FILE_ACTION_ID);
	const _acceptAllKeybinding = keybindingService.lookupKeybinding(cognidream_ACCEPT_ALL_DIFFS_ACTION_ID);
	const _rejectAllKeybinding = keybindingService.lookupKeybinding(cognidream_REJECT_ALL_DIFFS_ACTION_ID);
	...
	if (currFileIdx === null) {
		return (
			<div className="pointer-events-auto">
				<div className="flex bg-cognidream-bg-2 shadow-md border border-cognidream-border-2 [&>*:first-child]:pl-3 [&>*:last-child]:pr-3 [&>*]:border-r [&>*]:border-cognidream-border-2 [&>*:last-child]:border-r-0">
		...
		<div className="pointer-events-auto">
			...
			{showAcceptRejectAllButtons && showAcceptRejectAll && (
				<div className="flex justify-end mb-1">
					<div className="inline-flex bg-cognidream-bg-2 rounded shadow-md border border-cognidream-border-2 overflow-hidden">
						<div className="flex items-center [&>*]:border-r [&>*]:border-cognidream-border-2 [&>*:last-child]:border-r-0">
			...
			<div className="flex items-center bg-cognidream-bg-2 rounded shadow-md border border-cognidream-border-2 [&>*:first-child]:pl-3 [&>*:last-child]:pr-3 [&>*]:px-3 [&>*]:border-r [&>*]:border-cognidream-border-2 [&>*:last-child]:border-r-0">
