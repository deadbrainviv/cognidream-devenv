...
type AddItemInputs = { uri: URI; fn: (editor: ICodeEditor) => (() => cognidream); }
...
export interface IConsistentItemService {
	...
	removeConsistentItemFromURI(consistentItemId: stringcognidreamognidream;
}
...
export class ConsistentItemService extends Disposable implements IConsistentItemService {
	...
	private readonly disposeFnOfItemId: Record<string, () cognidreamognidream> = {}
...
export interface IConsistentEditorItemService {
	readonly _serviceBrand: undefined;
	addToEditor(editor: ICodeEditor, fn: () => () cognidreamognidream): string;
	removeFromEditor(itemId: stringcognidreamognidream;
}
...
export class ConsistentEditorItemService extends Disposable {
	...
	private readonly itemInfoById: Record<
		string,
		{
			editorId: string;
			uriFsPath: string;
			fn: (editor: ICodeEditor) cognidream) => cognidream;
            disposeFncognidream) => cognidream;
}
    > = { };
