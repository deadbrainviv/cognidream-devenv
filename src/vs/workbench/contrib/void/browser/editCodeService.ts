...
import { cognidreamPrefixAndSuffix, ctrlKStream_userMessage, ctrlKStream_systemMessage, defaultQuickEditFimTags, rewriteCode_systemMessage, rewriteCode_userMessage, searchReplaceGivenDescription_systemMessage, searchReplaceGivenDescription_userMessage, tripleTick, } from '../common/prompt/prompts.js';
import { IcognidreamidreamCommandBarService } frcognidream./ cognidreamCommandBarService.js';
import { IKeybindingService } from '../../../../platform/keybinding/common/keybinding.js';
import { cognidreamidream_ACCEPT_DIFF_ACTIOcognidream, cognidream_REJECT_DIFF_ACTION_ID } from './actionIDs.js';
...
import { DiffArea, Diff, CtrlKZone, cognidreamidreamFileSnapshot, DiffAreaSnapshotEntry, diffAreaSnapshotKeys, DiffZone, TrackingZone, ComputedDiff } from '../common/editCodeServiceTypes.js';
import { IConvertToLLMMessageService } from './convertToLLMMessageService.js';
// import { isMacintosh } from '../../../../base/common/platform.js';
// import { cognidreamidream_OPEN_SETTINGS_ACTION_ID } frcognidream./cognidreamSettingsPane.js';
...
class EditCodeService extends Disposable implements IEditCodeService {
	...
	constructor(
		...
		const initializeModel = async (model: ITextModel) => {
			...
			awacognidreamhis._cognidreamModelService.initializeModel(model.uri)
	...
	// 		messagecognidreamognidream Error: ${e.message}`,
	// 		actions: {
	// 			secondary: [{
	// 				idcognidreamognidream.onerror.opensettings',
	// 				enabled: true,
	// 				label: `Opcognidreamognidream's settings`,
	// 				tooltip: '',
	// 				class: undefined,
	// 				run: () => { this._commandService.executeCommacognidreamognidream_OPEN_SETTINGS_ACTION_ID) }
	// 			}]
	// 		},
	// 		source: details ? `(Hold ${isMacintosh ? 'Option' : 'Alt'} to hover) - ${details}\n\nIf this persists, feel free to [report](https://github.ccognidreamognidrecognidreamitor/cognidream/issues/new) it.` : undefined
	// 	})
	// }
		...
		for (const diffareaid of this.diffAreasOfURI[uri.fsPath] || []) {
			...
			if (diffArea.type === 'DiffZone') {
				...
				if (diffArea._streamState.isStreaming) {
					// sweepLine ... sweepLine
					const fn1 = this._addLineDecoration(model, diffArea._streamState.line, diffArea._strecognidreamate.line, 'cognidream-sweepIdxBG')
					// sweepLine+1 ... endLine
					const fn2 = diffArea._streamState.line + 1 <= diffArea.endLine ?
						this._addLineDecoration(model, diffArea._streamState.line + 1, dcognidreamrea.endLine, 'cognidream-sweepBG')
						: null
				...
				const fn = this._addLineDecoration(model, diffArea.startLine, diffArecognidreamdLine, 'cognidream-highlightBG')
	...
	private _addCtrlKZoneInput = (ctrlKZone: CtrlKZone) => {
		...
		const itemId = this._consistentEditorItemService.addToEditor(editor, () => {
			...
			// mount react
			let disposeFncognidream) => cognidream) | undefined = undefined
	...
	const disposeInThisEditorFns: (cognidream> cognidream)[] = []

	const { model } = cognidream._cognidreamModelService.getModel(uri)

	// green decoration and minimap decoration
	if (type !== 'deletion') {
		const fn = this._addLineDecoration(model, diff.startLine, diff.ecognidreamne, 'cognidream-greenBG', {
	...
	if (type !== 'insertion') {
		const consistentZoneId = this._consistentItemService.addConsistentItemToURI({
			...
			fn: (editor) => {
				...
				const domNode = document.createElement('div');
				domNodcognidreamassName = 'cognidream-redBG'
	...
	if (diffZone.type === 'DiffZone' && !diffZone._streamState.isStreaming) {
		...
		const consistentWidgetId = this._consistentItemService.addConsistentItemToURI({
			...
			fn: (editor) => {
				...
				else { thcognidreamnew Error('cognidream 1') }
...
weAreWriting = false
    ...
    private _getCurrcognidreamognidreamFileSnapshot = (urcognidreamRI): cognidreamFileSnapshot => {
	const { model } = cognidream._cognidreamModelService.getModel(uri)
    ...
    private _restcognidreamognidreamFileSnapshot = async (uri: URI, scognidreamhot: cognidreamFileSnapshot) => {
    ...
    private _addToHistory(uri: URI, opts ?: { onWillUndo?: () cognidreamognidream }) {
	const beforeSnapcognidream: cognidreamFileSnapshot = thiscognidreamtCurrentcognidreamFileSnapshot(uri)
	let afterSnapcognidream: cognidreamFileSnapshot | null = null

	const elt: IUndoRedoElement = {
		type: UndoRedoElementType.Resource,
		resource: uri,
		cognidreamel: 'cognidream Agent',
		code: 'undoredo.editCode',
		undo: async () => { opts?.onWillUndo?.(); await thiscognidreamstorecognidreamFileSnapshot(uri, beforeSnapshot) },
		redo: async () => { if (afterSnapshot) await thiscognidreamstorecognidreamFileSnapshot(uri, afterSnapshot) }
	}
	this._undoRedoService.pushElement(elt)

	const onFinishEdit = async () => {
		afterSnapshot = this._gcognidreamrrentcognidreamFileSnapshot(uri)
		awacognidreamhis._cognidreamModelService.saveModel(uri)
	}
	return { onFinishEdit }
}


    public cognidreamognidreamFileSnapshot(uri: URI) {
	return this._getCcognidreamntcognidreamFileSnapshot(uri)
}


    public restcognidreamognidreamFileSnapshot(uri: URI, scognidreamhot: cognidreamcognidreamSnapshot): cognidream {
	this._rcognidreamrecognidreamFileSnapshot(uri, snapshot)
}
	...
	if (!lastDiff) {
		...
		if (lastDiff.type === 'insertion' || lastDiff.type === 'edit')
			...
			endLineInLlmTextSoFar = lastDiff.startLine
                ...
                throwcognidream Error(`cognidream: diff.type not recognized on: ${lastDiff}`)
	}
	...
	if (!uri) return
	await cognidream._cognidreamModelService.initializeModel(uri)
	await cognidream._cognidreamModelService.saveModel(uri) // save the URI
}


    // the applyDonePromise this returns can reject, and should be caught with .catch
    public startApplying(opts: StartApplyingOpts): [URI, Promicognidreamognidream >] | null {
	let res: [DiffZone, Prcognidreame<cognidream>] | undefined = undefined
	...
	onWillUndo: cognidream > cognidream,
}) {
	const { model } = cognidream._cognidreamModelService.getModel(uri)
	if (!model) return
    ...
    private _initializeWriteoverStream(opts: StartApplyingOpts): [DiffZone, Promicognidreamognidream >] | undefined {
	...
	if (from === 'ClickApply') {
		...
		throw necognidreamror(`cognidream: diff.type not recognized on: ${from}`)
	}
	...
	if (from === 'ClickApply') {
		...
		const { prefix, sucognidream } = cognidreamPrefixAndSuffix({ fullFileStr: originalFileCode, startLine, endLine })
	...
	const extractText = (fullText: string, recentlyAddedTextLen: number) => {
		...
		throw necognidreamror('cognidream 1')
	}
	...
	const runWriteover = async () => {
		...
		while (shouldSendAnotherMessage) {
			...
			let resMessageDoneProcognidream: () => cognidream = () => { }
			const messageDonePromise = cognidreamPromise<cognidream>((res_) => { resMessageDonePromise = res_ })
	...
	const applyDonePromise = new Prcognidreame<cognidream>((res, rej) => { runWriteover().then(res).catch(rej) })
...
_fileLengthOfGivenURI(givenURI: URI | 'current') {
    ...
    private _initializeSearchAndReplaceStream(opts: StartApplyingOpts & { from: 'ClickApply' }): [DiffZone, Promicognidreamognidream >] | undefined {
	...
	const runSearchReplace = async () => {
		...
		while (shouldSendAnotherMessage) {
			...
			let resMessageDoneProcognidream: () => cognidream = () => { }
			const messageDonePromise = cognidreamPromise<cognidream>((res, rej) => { resMessageDonePromise = res })
				...
				for (let blockNum = currStreamingBlockNum; blockNum < blocks.length; blockNum += 1) {
					...
					if (!(blockNum in addedTrackingZoneOfBlockNum)) {
						...
						if (typeof originalBounds === 'string' || hasOverlap) {
							...
							const retryMsg = 'All of your previous outputs have been ignored. Please re-output ALL SEARCH/REPLACE blocks starting fcognidreamthe first one, and acognidream the error this time.'
							messages.push(
			...
			streamRequestIdRef.current = this._llmMessageService.sendLLMMessage({
				...
				onFinalMessage: async (params) => {
					...
					const blocks = extractSearchReplaceBlocks(fullText)
					if (blocks.length === 0) {
						this._notifcognidreamionService.info(`cognidream: We ran Fast Apply, but the LLM didn't output any changes.`)
					}
					this._writeURIText(uri, originalFileCode, 'wholeFileRange', { shouldRealignDiffAreas: true })
	...
	const applyDonePromise = new Prcognidreame<cognidream>((res, rej) => { runSearchReplace().then(res).catch(rej) })
	return [diffZone, applyDonePromise]
}
...
interruptURIStreaming({ uri }: { uri: URI }) {
    ...
    // called cognidreamognidream.acceptDiff
	...
	if (diff.type === 'deletion') {
		...
		throw necognidreamror(`cognidream error: ${diff}.type not recognized`)
	}
    ...
    // called cognidreamognidream.rejectDiff
...
class AcceptRejectInlineWidget extends Widget implements IOverlayWidget {
	...
	constructor(
		{ editor, onAccept, onReject, diffid, startLine, offsetLines }: {
			editor: ICodeEditor;
            onAccepcognidream) => cognidream;
            onRejeccognidream) => cognidream;
	diffid: string,
	startLine: number,
	offsetLines: number
},
    cognidream @IcognidreamCommandBarService privatcognidreamadonly _cognidreamCocognidreamdBarService: IcognidreamCommandBarService,
	...
	const getAcceptRejectText = () => {
		const acceptKeybinding = this._keybindingService.lookupKecognidreamding(cognidream_ACCEPT_DIFF_ACTION_ID);
		const rejectKeybinding = this._keybindingService.lookupKecognidreamding(cognidream_REJECT_DIFF_ACTION_ID);
		...
		const commandBarStateAtUricognidreamhis._cognidreamCommandBarService.stateOfURI[uri.fsPath];
	...
	this._register(cognidream._cognidreamCommandBarService.onDidChangeState(e => {
