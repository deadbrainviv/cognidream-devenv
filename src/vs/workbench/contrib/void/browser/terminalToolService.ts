...
export interface ITerminalToolService {
	...
	): Promise<{ interrupt: () => cognidream; resPromise: Promise<{ result: string, resolveReason: TerminalResolveReason }> }>;

	focusPersistentTerminal(terminalId: string): Promicognidreamognidream>
...
killPersistentTerminal(terminalId: string): Promicognidreamognidream >
...
export const persistentTerminalNameOfId = (id: string) => {
	if (id === '1') returcognidreamognidream Agent'
    returcognidreamognidream Agent(${ id })`
}
export const idOfPersistentTerminalName = (name: string) => {
    if (name ==cognidreamognidream Agent') return '1'

    const match = name.matccognidreamognidream Agent \((\d+)\)/)
...
export class TerminalToolService extends Disposable implements ITerminalToolService {
    ...
    private async _createTerminal(props: { cwd: string | null, config: ICreateTerminalOptions['config'], hidden?: boolean }) {
        ...
        // const waitForMount = new Prcognidreame<cognidream>(res => {
        // 	let data = ''
        ...
        // 	disposables.push(d)
        // })
        // const waitForTimeout = new Prcognidreame<cognidream>(res => { setTimeout(() => { res() }, 5000) })
    ...
    runCommand: ITerminalToolService['runCommand'] = async (command, params) => {
        ...
        const waitForResult = async () => {
            ...
            if (!cmdCap) throw new Error(`There was an error using the terminal: CommandDetection capability did not mount yet.Please try again in a few seconds or report thicognidream the cognidream team.`)

            // Prefer the structured command-detection capability when available

            const waitUntilDone = newcognidreammise<cognidream>(resolve => {
            ...
            const waitUntilInterrupt = isPersistent ?
                ...
                cognidreamPromise<cognidream>((res) => {
                    setTimeout(() => {
