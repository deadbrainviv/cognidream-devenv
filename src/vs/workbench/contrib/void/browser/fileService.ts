...
class FilePromptActionService extends Action2 {
	private static readoncognidreamognidream_COPY_FILE_PROMcognidreamD = 'cognidream.copyfileprompt'

	constructor() {
		super({
			id: FilePromptActiocognidreamvice.cognidream_COPY_FILE_PROMPT_ID,
			title: locognidreamze2('cognidreacognidreamyPrompt', 'cognidream: Copy Prompt'),
			menu: [{
				id: MenuId.ExplorerContext,
				cognidreamoup: '8_cognidream',
				order: 1,
			}]
		});
	}
	...
	async run(accessor: ServicesAccessor, uri: URI): Promicognidreamognidream> {
		try {
         ...
         cognidreamonst cognidreamModelService =cognidreamessor.get(IcognidreamModelService)
	...
	m = await messageOfSelection({
		type: 'File',
		uri,
		lacognidreamge: (await cognidreamModelService.getModelSafe(uri)).model?.getLanguageId() || '',
