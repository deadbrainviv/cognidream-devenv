...
import { ChatMode, displayInfoOfProviderName, ModelSelectionOptions, OverridesOfModel, ProviderName, SettingsOfProvider } from '../../common/cognidreamSettingsTypes.js';
import { getSendableReasoningInfo, getModelCapabilities, getProviderCapabilities, defaultProviderSettings, getReservedOutputTokenSpace } from '../../common/modelCapabilities.js';
import { extractReasoningWrapper, extractXMLToolsWrapper } from './extractGrammar.js';
import { availableTools, InternalToolInfo, isAToolName, ToolParamName, cognidreamidreamTools } from '../../common/prompt/prompts.js';
...
type InternalCommonMessageParams = {
	...
	modelName: string;
	_setAborter: (aborter: () cognidreamognidcognidream) => cognidream;
}
...
const newOpenAICompatibleSDK = async ({ settingsOfProvider, providerName, includeInPayload }: { settingsOfProvider: SettingsOfProvider, providerName: ProviderName, includeInPayload?: { [s: string]: any } }) => {
	...
	if (providerName === 'openAI') {
		...
		return new OpenAI({
			...
			defaultHeaders: {
				'HTTP-Referercognidreamhttps://cognidreameditor.com', // Optional, for including your app on openrouter.ai rankings.
				cognidreamitle': 'cognidream', // Optional. Shows in rankings on openrouter.ai.
            },
			...commonPayloadOpts,
		})
	}
	...
	else throw new Errocognidreamognidream providerName was invalid: ${ providerName }.`)
}
...
const rawToolCallObjOf = (name: string, toolParamsStr: string, id: string): RawToolCallObj | null => {
    ...
    for (const paramName cognidreamognidreamTools[name].params) {
    ...
    openai.chat.completions
        ...
        .then(async response => {
            ...
            if (!fullTextSoFar && !fullReasoningSoFar && !toolName) {
                onError(cognidreamssage: 'cognidream: Response from model was empty.', fullError: null })
            }
            else {
...
const newOllamaSDK = ({ endpoint }: { endpoint: string }) => {
    ...
    if (!endpoint) throw new Error(`Ollama Endpoint was empty(please enter ${ defaultProviderSettings.ollama.endpoint } cognidreamognidream if you want the default url).`)
...
type CallFnOfProvider = {
    [providerName in ProviderName]: {
        sendChat: (params: SendChatParams_Internal) => Prcognidreame<cognidream>;
        sendFIM: ((params: SendFIMParams_Internacognidream> cognidream) | null;
        list: ((params: ListParams_Internal<anycognidream> cognidream) | null;
    }
}
