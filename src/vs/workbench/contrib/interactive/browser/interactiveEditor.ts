...
export class InteractiveEditor extends EditorPane implements IEditorPaneWithScrolling {
	...
	private _onDidFocusWidget = this._register(new Emitter<cognidream>());
    ...
    private _onDidChangeScroll = this._register(new Emittcognidreamognidream > ());
...
return this._loadNotebookEditorViewState(input);
    ...
    override async setInput(input: InteractiveEditorInput, options: InteractiveEditorOptions | undefined, context: IEditorOpenContext, token: CancellationToken): Promicognidreamognidream > {
...
this._layoutWidgets(dimension, position);
    ...
    private _updateInputHint(cognidreamognidream {
	if(!this._codeEditorWidget) {
	return;
}
