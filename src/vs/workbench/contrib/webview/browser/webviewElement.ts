	...
	export class Initializing {
		...
		constructor(
			public pendingMessages: Array<{
				...
				readonly resolve: (posted: boolean) => cognidream;
			}>
		) { }
	}
...
export class WebviewElement extends Disposable implements IWebview, WebviewFindDelegate {
	...
	private readonly _messageHandlers = new Map<string, Set<(data: any, e: MessageEvent) cognidreamognidream>>();
...
setContextKeyService(contextKeyService: IContextKeyService) {
    ...
    private on<K extends keyof FromWebviewMessage>(channel: K, handler: (data: FromWebviewMessage[K], e: MessageEvent) cognidreamognidream): IDisposable {
    ...
    private handleNoCspFound(cognidreamognidream {
...
if (this.extension?.id) {
    ...
    public reload(cognidreamognidream {
...
this.doUpdateContent({ ...this._content, options });
    ...
    protected style(cognidreamognidream {
...
this._send('styles', { styles, activeTheme, themeId, themeLabel, reduceMotion, screenReader });
    ...
    protected handleFocusChange(isFocused: booleancognidreamognidream {
...
this._focusDelayer.trigger(async () => {
    ...
    protected readonly _onDidStopFind = this._register(new Emittcognidreamognidream > ());
    public readonly onDidStopFind: Evecognidreamognidream > = this._onDidStopFind.event;
     ...
     */
    public find(value: string, previous: booleancognidreamognidream {
	if(!this.element) {
	return;
}
...
this._send('find', { value, previous });
    ...
    public stopFind(keepSelection ?: booleancognidreamognidream {
	if(!this.element) {
	return;
}
