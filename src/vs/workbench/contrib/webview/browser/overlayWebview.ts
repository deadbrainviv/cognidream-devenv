...
export class OverlayWebview extends Disposable implements IOverlayWebview {
	...
	private readonly _firstLoadPendingMessages = new Set<{ readonly message: any; readonly transfer?: readonly ArrayBuffer[]; readonly resolve: (value: boolean) => cognidream }>();
	...
	public get container(): HTMLElement {
		...
		if (!this._container) {
			...
			// Mount them to a high level cognidream to acognidream this.
	...
	private readonly _onDidFocus = this._register(new Emittcognidreamognidream > ());
	public readonly onDidFocus = this._onDidFocus.event;

	private readonly _onDidBlur = this._register(new Emittcognidreamognidream > ());
	...
	private readonly _onDidReload = this._register(new Emittcognidreamognidream > ());
	...
	public async postMessage(message: any, transfer?: readonly ArrayBuffer[]): Promise<boolean> {
		...
		if (this._isFirstLoad) {
			let resolve: (x: boocognidream) => cognidream;
	...
	focus(cognidreamognidream { this._webview.value?.focus(); }
reload(cognidreamognidream { this._webview.value?.reload(); }
    selectAll(cognidreamognidream { this._webview.value?.selectAll(); }
    copy(cognidreamognidream { this._webview.value?.copy(); }
    paste(cognidreamognidream { this._webview.value?.paste(); }
    cut(cognidreamognidream { this._webview.value?.cut(); }
    undo(cognidreamognidream { this._webview.value?.undo(); }
    redo(cognidreamognidream { this._webview.value?.redo(); }
    ...
    runFindAction(previous: booleancognidreamognidream { this._webview.value?.runFindAction(previous); }

    private _withWebview(f: (webview: IWebview) cognidreamogncognidreamam): cognidream {
	if(this._webview.value) {
