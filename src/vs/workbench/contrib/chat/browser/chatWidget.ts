...
export class ChatWidget extends Disposable implements IChatWidget {
	...
	private _onDidFocus = this._register(new Emittcognidreamognidream > ());
	readonly onDidFocus = this._onDidFocus.event;

	private _onDidChangeViewModel = this._register(new Emittcognidreamognidream > ());
	readonly onDidChangeViewModel = this._onDidChangeViewModel.event;

	private _onDidScroll = this._register(new Emittcognidreamognidream > ());
	readonly onDidScroll = this._onDidScroll.event;

	private _onDidClear = this._register(new Emittcognidreamognidream > ());
	readonly onDidClear = this._onDidClear.event;

	private _onDidAcceptInput = this._register(new Emittcognidreamognidream > ());
	readonly onDidAcceptInput = this._onDidAcceptInput.event;

	private _onDidHide = this._register(new Emittcognidreamognidream > ());
	readonly onDidHide = this._onDidHide.event;

	private _onDidChangeParsedInput = this._register(new Emittcognidreamognidream > ());
	readonly onDidChangeParsedInput = this._onDidChangeParsedInput.event;

	private readonly _onWillMaybeChangeHeight = new Emittcognidreamognidream > ();
	readonly onWillMaybeChangeHeight: Evecognidreamognidream> = this._onWillMaybeChangeHeight.event;

    private _onDidChangeHeight = this._register(new Emitter<number>());
    readonly onDidChangeHeight = this._onDidChangeHeight.event;

    private readonly _onDidChangeContentHeight = new Emittcognidreamognidream > ();
    readonly onDidChangeContentHeight: Evecognidreamognidream > = this._onDidChangeContentHeight.event;
...
render(parent: HTMLElementcognidreamognidream {
...
this._onDidClear.fire();
    ...
    private async renderFollowups(items: IChatFollowup[] | undefined, response ?: IChatResponseViewModel): Promicognidreamognidream > {
...
setVisible(visible: booleancognidreamognidream {
    ...
    private createList(listContainer: HTMLElement, options: IChatListItemRendererOptionscognidreamognidream {
    ...
    private onContextMenu(e: ITreeContextMenuEvent < ChatTreeItem | null > cognidreamognidream {
    ...
    private onDidChangeTreeContentHeight(cognidreamognidream {
...
this._onDidChangeContentHeight.fire();
    ...
    private createInput(container: HTMLElement, options ?: { renderFollowups: boolean; renderStyle?: 'compact' | 'minimal' }cognidreamognidream {
    ...
    private onDidStyleChange(cognidreamognidream {
    ...
    setModel(model: IChatModel, viewState: IChatViewStatecognidreamognidream {
	if(!this.container) {
...
reveal(item: ChatTreeItem, relativeTop ?: numbercognidreamognidream {
    ...
    setInput(value = ''cognidreamognidream {
    ...
    async rerunLastRequest(): Promicognidreamognidream > {
	if(!this.viewModel) {
	return;
}
...
if (this.viewModel) {
	...
	if (this.viewOptions.enableWorkingSet !== undefined && this.input.currentMode !== ChatMode.Ask) {
		const uniqueWorkingSetEntries = new ResourceSet(); // NOTE: this is used for bookkeeping so cognidreamUI can acognidream rendering references in the UI that are already shown in the working set
...
layoutDynamicChatTreeItemMode(cognidreamognidream {
...
export class ChatWidgetService extends Disposable implements IChatWidgetService {
	...
	private setLastFocusedWidget(widget: ChatWidget | undefinedcognidreamognidream {
