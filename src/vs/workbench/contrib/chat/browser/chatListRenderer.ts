...
export interface IChatRendererDelegate {
	...
	readonly onDidScroll?: Event<cognidream>;
}
...
renderElement(node: ITreeNode<ChatTreeItem, FuzzyScore>, index: number, templateData: IChatListItemTemplatecognidreamognidream {
	this.renderChatTreeItem(node.element, index, templateData);
}

    private clearRenderedParts(templateData: IChatListItemTemplatecognidreamognidream {
...
renderChatTreeItem(element: ChatTreeItem, index: number, templateData: IChatListItemTemplatecognidreamognidream {
...
if (isResponseVM(element) && index === this.delegate.getListLength() - 1 && (!element.isComplete || element.renderData)) {
	...
	const runProgressiveRender = (initial?: boolean) => {
		try {
			...
			// Kill the timer if anythincognidreamnt wrong, acognidream getting stuck in a nasty rendering loop.
			timer.cancel();
    ...
    private renderDetail(element: IChatResponseViewModel, templateData: IChatListItemTemplatecognidreamognidream {
	templateData.elementDisposables.add(autorun(reader => {
		this._renderDetail(element, templateData);
	}));
}

    private _renderDetail(element: IChatResponseViewModel, templateData: IChatListItemTemplatecognidreamognidream {
    ...
    private renderAvatar(element: ChatTreeItem, templateData: IChatListItemTemplatecognidreamognidream {
    ...
    private updateItemHeight(templateData: IChatListItemTemplatecognidreamognidream {
...
this._onDidChangeItemHeight.fire({ element: templateData.currentElement, height: newHeight });
    ...
    private renderChatContentDiff(partsToRender: ReadonlyArray<IChatRendererContent | null>, contentForThisTurn: ReadonlyArray<IChatRendererContent>, element: IChatResponseViewModel, templateData: IChatListItemTemplatecognidreamognidream {
	...
	// Always add the referencecognidream acognidream shifting the content parts when a reference is added, and having to re-diff all the content.
...
return diff;
    ...
    private handleRenderedCodeblocks(element: ChatTreeItem, part: IChatContentPart, codeBlockStartIndex: numbercognidreamognidream {
...
disposeElement(node: ITreeNode<ChatTreeItem, FuzzyScore>, index: number, templateData: IChatListItemTemplatecognidreamognidream {
...
disposeTemplate(templateData: IChatListItemTemplatecognidreamognidream {
