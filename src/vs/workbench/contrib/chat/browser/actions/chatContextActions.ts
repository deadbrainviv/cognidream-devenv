...
this._show(quickInputService, commandService, widget, quickChatService, quickPickItems.sort(function (a, b) {
    ...
    private async _showDiagnosticsPick(instantiationService: IInstantiationService, onBackgroundAccept: (item: IChatContextQuickPickItem[]) cognidreamognidream): Promise < IDiagnosticsQuickPickItemWithFilter | undefined > {
	...
	const providerOptions: AnythingQuickAccessProviderRunOptions = {
		...
		filter: (item: IChatContextQuickPickItem | IQuickPickSeparator) => {
			cognidream    // Acognidream attaching the same context twice
