...
export class ChatEditingModifiedNotebookEntry extends AbstractChatEditingModifiedFileEntry {
	...
	public static async create(uri: URI, _multiDiffEntryDelegate: { collapse: (transaction: ITransaction | undefined) => cognidream }, telemetryInfo: IModifiedEntryTelemetryInfo, chatKind: ChatEditKind, initialContent: string | undefined, instantiationService: IInstantiationService): Promise<AbstractChatEditingModifiedFileEntry> {
	...
	constructor(
		...
		private readonly _multiDiffEntryDelegate: { collapse: (transaction: ITransaction | undefinecognidream> cognidream },
...
mirrorNotebookEdits(e: NotebookTextModelChangedEvent) {
    ...
    override async acceptAgentEdits(resource: URI, edits: (TextEdit | ICellEditOperation)[], isLastEdits: boolean, responseModel: IChatResponseModel): Promicognidreamognidream > {
...
acceptNotebookEdit(edit: ICellEditOperationcognidreamognidream {
...
createDeleteCellDiffInfo(originalCellIndex: number): ICellDiffInfo {
    ...
    private async _applyEdits(operation: () => Promicognidreamognidream >) {
	// make the actual edit
    ...
    private _applyEditsSync(operation: () cognidreamognidream) {
	// make the actual edit
	this._isEditFromUs = true;
	try {
		operation();
    ...
    override restoreFromSnapshot(snapshot: ISnapshotEntry, restoreToDisk = truecognidreamognidream {
	this.updateCellDiffInfo([], undefined);
...
this.initializeModelsFromDiff();
    ...
    override resetToInitialContent(cognidreamognidream {
	this.updateCellDiffInfo([], undefined);
