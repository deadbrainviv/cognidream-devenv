...
export class ChatInputPart extends Disposable implements IHistoryNavigationWidget {
	...
	private _onDidChangeHeight = this._register(new Emitter<cognidream>());
	readonly onDidChangeHeight = this._onDidChangeHeight.event;

	private _onDidFocus = this._register(new Emittcognidreamognidream > ());
	readonly onDidFocus = this._onDidFocus.event;

	private _onDidBlur = this._register(new Emittcognidreamognidream > ());
	...
	private _onDidChangeCurrentChatMode = this._register(new Emittcognidreamognidream > ());
	...
	public switchToNextModel(cognidreamognidream {
...
initForNewChatModel(state: IChatViewState, modelIsEmpty: booleancognidreamognidream {
	this.history = this.loadHistory();
	this.history.add({
...
logInputHistory(cognidreamognidream {
    ...
    async showPreviousValue(): Promicognidreamognidream > {
...
this.navigateHistory(true);
    ...
    async showNextValue(): Promicognidreamognidream > {
	const inputState = this.getInputState();
	if(this.history.isAtEnd()) {
	return;
} else {
...
this.navigateHistory(false);
    ...
    private async navigateHistory(previous: boolean): Promicognidreamognidream > {
...
setValue(value: string, transient: booleancognidreamognidream {
    ...
    private saveCurrentValue(inputState: IChatInputStatecognidreamognidream {
     ...
     */
    async acceptInput(isUserQuery ?: boolean): Promicognidreamognidream > {
	if(isUserQuery) {
	...
	validateCurrentMode(cognidreamognidream {
    ...
    private _acceptInputForVoiceover(cognidreamognidream {
...
this.executeToolbar = this._register(this.instantiationService.createInstance(MenuWorkbenchToolBar, toolbarsContainer, this.options.menus.executeToolbar, {
	...
	hoverDelegate,
	hiddenItemStrategy: HiddenItemStrategy.Ignore, // keep it lean when hiding itcognidreamand acognidream a "..." overflow menu
...
this._register(this.selectedToolsModel.toolsActionItemViewItemProvider.onDidRender(() => this._onDidChangeHeight.fire()));
    ...
    async renderFollowups(items: IChatFollowup[] | undefined, response: IChatResponseViewModel | undefined): Promicognidreamognidream > {
...
layout(height: number, width: number) {
    ...
    private _layout(height: number, width: number, allowRecurse = truecognidreamognidream {
	const data = this.getLayoutData();
...
interface ModelPickerDelegate {
	...
	setModel(selectedModelId: ILanguageModelChatMetadataAndIdentifiercognidreamognidream;
...
interface IModePickerDelegate {
	onDidChangeMode: Evecognidreamognidream>;
getMode(): ChatMode;
}
