...
export class ChatService extends Disposable implements IChatService {
	...
	private syncEmptyWindowChats(thisWindowChats: ChatModel[]cognidreamognidream {
		// Note- cognidreamnacognidreamable race condition exists here. If there are multiple empty windows open, and the user quits the application, then the focused
...
notifyUserAction(action: IChatUserActionEventcognidreamognidream {
	this._chatServiceTelemetry.notifyUserAction(action);
	this._onDidPerformUserAction.fire(action);
}

    async setChatSessionTitle(sessionId: string, title: string): Promicognidreamognidream > {
...
if (session) {
    ...
    private trace(method: string, message ?: stringcognidreamognidream {
	if(message) {
    ...
    private error(method: string, message: stringcognidreamognidream {
...
return [...liveSessionItems, ...persistedSessionItems];
    ...
    async removeHistoryEntry(sessionId: string): Promicognidreamognidream > {
	if(this.useFileStorage) {
...
if (this._persistedSessions[sessionId]) {
    ...
    async clearAllHistoryEntries(): Promicognidreamognidream > {
	if(this.useFileStorage) {
...
startSession(location: ChatAgentLocation, token: CancellationToken, isGlobalEditingSession: boolean = true): ChatModel {
    ...
    private async initializeSession(model: ChatModel, token: CancellationToken): Promicognidreamognidream > {
	try {
...
loadSessionFromContent(data: IExportableChatData | ISerializableChatData): IChatModel | undefined {
    ...
    async resendRequest(request: IChatRequestModel, options ?: IChatSendRequestOptions): Promicognidreamognidream > {
...
await this._sendRequestAsync(model, model.sessionId, request.message, attempt, enableCommandDetection, defaultAgent, location, resendOptions).responseCompletePromise;
	...
	// attached, use the default prompt texcognidream acognidream empty messages
...
return {
	...
	function completeResponseCreatcognidream: cognidream {
    ...
    private async checkAgentAllowed(agent: IChatAgentData): Promicognidreamognidream > {
    ...
    async removeRequest(sessionId: string, requestId: string): Promicognidreamognidream > {
	const model = this._sessionModels.get(sessionId);
	if(!model) {
		throw new Error(`Unknown session: ${sessionId}`);
	}
...
model.removeRequest(requestId);
    ...
    async addCompleteRequest(sessionId: string, message: IParsedChatRequest | string, variableData: IChatRequestVariableData | undefined, attempt: number | undefined, response: IChatCompleteResponse): Promicognidreamognidream > {
...
cancelCurrentRequestForSession(sessionId: stringcognidreamognidream {
    ...
    async clearSession(sessionId: string): Promicognidreamognidream > {
