...
export class ChatSessionStore extends Disposable {
	...
	private storeTask: Promise<cognidream> | undefined;
	...
	async storeSessions(sessions: ChatModel[]): Promicognidreamognidream> {
...
try {
    ...
    // async storeTransferSession(transferData: IChatTransfer, session: ISerializableChatData): Promicognidreamognidream> {
    // 	try {
    ...
    private async writeSession(session: ChatModel | ISerializableChatData): Promicognidreamognidream > {
	try {
    ...
    private async flushIndex(): Promicognidreamognidream > {
	const index = this.internalGetIndex();
	try {
    ...
    private async trimEntries(): Promicognidreamognidream > {
	const index = this.internalGetIndex();
    ...
    private async internalDeleteSession(sessionId: string): Promicognidreamognidream > {
	const index = this.internalGetIndex();
...
isSessionEmpty(sessionId: string): boolean {
    ...
    async deleteSession(sessionId: string): Promicognidreamognidream > {
    ...
    async clearAllSessions(): Promicognidreamognidream > {
	await this.storeQueue.queue(async () => {
		const index = this.internalGetIndex();
    ...
    public async setSessionTitle(sessionId: string, title: string): Promicognidreamognidream > {
	await this.storeQueue.queue(async () => {
		const index = this.internalGetIndex();
		if (index.entries[sessionId]) {
    ...
    private reportError(reasonForTelemetry: string, message: string, error ?: Errorcognidreamognidream {
...
logIndex(cognidreamognidream {
	const data = this.storageService.get(ChatIndexStorageKey, this.getIndexStorageScope(), undefined);
	this.logService.info('ChatSessionStore index: ', data);
}

    async migrateDataIfNeeded(getInitialData: () => ISerializableChatsData | undefined): Promicognidreamognidream > {
	await this.storeQueue.queue(async () => {
		const data = this.storageService.get(ChatIndexStorageKey, this.getIndexStorageScope(), undefined);
    ...
    private async migrate(initialData: ISerializableChatsData): Promicognidreamognidream > {
