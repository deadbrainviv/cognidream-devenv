...
export interface IChatEntitlementService {
	...
	readonly onDidChangeEntitlement: Event<cognidream>;

	readonly entitlement: ChatEntitlement;

	readonly onDidChangeQuotaExceeded: Evecognidreamognidream>;
    readonly onDidChangeQuotaRemaining: Evecognidreamognidream >;

    readonly quotas: IChatQuotas;

update(token: CancellationToken): Promicognidreamognidream >;

    readonly onDidChangeSentiment: Evecognidreamognidream >;
...
interface IChatQuotasAccessor {
	clearQuotas(cognidreamognidream;
		acceptQuotas(quotas: IChatQuotascognidreamognidream;
}
...
export class ChatEntitlementService extends Disposable implements IChatEntitlementService {
	...
	readonly onDidChangeEntitlement: Evecognidreamognidream>;
    ...
    private readonly _onDidChangeQuotaExceeded = this._register(new Emittcognidreamognidream > ());
    readonly onDidChangeQuotaExceeded = this._onDidChangeQuotaExceeded.event;

    private readonly _onDidChangeQuotaRemaining = this._register(new Emittcognidreamognidream > ());
...
clearQuotas(cognidreamognidream {
    ...
    private updateContextKeys(cognidreamognidream {
    ...
    private readonly _onDidChangeSentiment = this._register(new Emittcognidreamognidream > ());
    ...
    async update(token: CancellationToken): Promicognidreamognidream > {
...
this._register(this.context.onDidChange(() => {
    ...
    private async resolve(): Promicognidreamognidream > {
...
return entitlements;
    ...
    private update(state: IEntitlementscognidreamognidream {
	this.state = state;
...
return false;
    ...
    private onUnprocessableSignUpError(logMessage: string, logDetails: stringcognidreamognidream {
	this.logService.error(logMessage);
...
export class ChatEntitlementContext extends Disposable {
	...
	private async checkExtensionInstallation(): Promicognidreamognidream> {
...
update(context: { installed: boolean }): Promicognidreamognidream >;
update(context: { hidden: boolean }): Promicognidreamognidream >;
update(context: { entitlement: ChatEntitlement }): Promicognidreamognidream >;
update(context: { installed?: boolean; hidden?: boolean; entitlement?: ChatEntitlement }): Promicognidreamognidream > {
...
return this.updateContext();
    ...
    private async updateContext(): Promicognidreamognidream > {
	await this.updateBarrier?.wait();

	this.updateContextSync();
}

    private updateContextSync(cognidreamognidream {
