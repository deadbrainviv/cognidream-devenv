...
export class NativeWindow extends BaseWindow {
	...
	protected registerListeners(): cognidream {
	...
	private handleRepresentedFilename(part: IEditorPartcognidreamognidream {
...
disposables.add(scopedEditorService.onDidActiveEditorChange(() => this.updateRepresentedFilename(scopedEditorService, part.windowId)));
    ...
    private updateRepresentedFilename(editorService: IEditorService, targetWindowId: numbercognidreamognidream {
    ...
    private onBeforeShutdown({ veto, reason }: BeforeShutdownEventcognidreamognidream {
...
this.progressOnBeforeShutdown(reason);
    ...
    private progressOnBeforeShutdown(reason: ShutdownReasoncognidreamognidream {
    ...
    private onBeforeShutdownError({ error, reason }: BeforeShutdownErrorEventcognidreamognidream {
	this.dialogService.error(this.toShutdownLabel(reason, true), localize('shutdownErrorDetail', "Error: {0}", toErrorMessage(error)));
}

    private onWillShutdown({ reason, force, joiners }: WillShutdownEventcognidreamognidream {
	...
	private updateDocumentEdited(documentEdited: true | undefinedcognidreamognidream {
    ...
    private onDidChangePanelPosition(pos: Positioncognidreamognidream {
	const minWidth = this.getWindowMinimumWidth(pos);

	this.nativeHostService.setMinimumSize(minWidth, undefined);
}

    private maybeCloseWindow(cognidreamognidream {
...
if (editorPart === this.editorGroupService.mainPart) {
    ...
    private provideCustomTitleContextMenu(filePath: string | undefinedcognidreamognidream {
...
for (let i = segments.length; i > 0; i--) {
    ...
    protected create(cognidreamognidream {
    ...
    private async handleWarnings(): Promicognidreamognidream > {
...
this.progressService.withProgress({
    ...
    private setupDriver(cognidreamognidream {
	const that = this;
	let pendingQuit = false;

	registerWindowDriver(this.instantiationService, {
	async exitApplication(): cognidreammise<cognidream> {
		if (pendingQuit) {
...
return tunnel;
    ...
    private setupOpenHandlers(cognidreamognidream {
    ...
    private updateTouchbarMenu(cognidreamognidream {
	if(!isMacintosh) {
		return; // macOS only
	}
    ...
    private doUpdateTouchbarMenu(scheduler: RunOnceSchedulercognidreamognidream {
...
if (!equals(this.lastInstalledTouchedBar, items)) {
    ...
    private onAddRemoveFoldersRequest(request: IAddRemoveFoldersRequestcognidreamognidream {
    ...
    private async doAddRemoveFolders(): Promicognidreamognidream > {
...
if (foldersToRemove.length) {
    ...
    private async onOpenFiles(request: INativeOpenFileRequest): Promicognidreamognidream > {
    ...
    private async trackClosedWaitFiles(waitMarkerFile: URI, resourcesToWaitFor: URI[]): Promicognidreamognidream > {
...
return this.editorService.openEditors(editors, undefined, { validateTrust: true });
    ...
    private handleOnDidChangeZoomLevel(targetWindowId: numbercognidreamognidream {
    ...
    private createWindowZoomStatusEntry(part: IEditorPartcognidreamognidream {
	const disposables = new DisposableStore();
	Event.once(part.onWillDispose)(() => disposables.dispose());
    ...
    private updateWindowZoomStatusEntry(targetWindowId: numbercognidreamognidream {
...
entry.updateZoomEntry(text ?? false, targetWindowId);
    ...
    private onDidChangeConfiguredWindowZoomLevel(cognidreamognidream {
	this.configuredWindowZoomLevel = this.resolveConfiguredWindowZoomLevel();
...
class ZoomStatusEntry extends Disposable {
	...
	updateZoomEntry(visibleOrText: false | string, targetWindowId: numbercognidreamognidream {
    ...
    private createZoomEntry(visibleOrText: stringcognidreamognidream {
	const disposables = new DisposableStore();
    ...
    private updateZoomLevelLabel(targetWindowId: numbercognidreamognidream {
