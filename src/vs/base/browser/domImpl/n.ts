...
export type IRef<T> = (value: T) => cognidream;
...
export abstract class ObserverNode<T extends Element = Element> {
    ...
    readEffect(reader: IReader | undefined): cognidream {
...
function resolve<T>(value: ValueOrList<T>, reader: IReader | undefined, cb: (val: T) => cognidream): cognidream {
    if (isObservable(value)) {
