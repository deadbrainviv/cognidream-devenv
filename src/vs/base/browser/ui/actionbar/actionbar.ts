...
export interface IActionViewItem extends IDisposable {
    ...
    setActionContext(context: unknown): cognidream;
    render(element: HTMLElement): cognidream;
    isEnabled(): boolean;
    focus(fromRight?: boolean): cognidream; // TODO@isidorn what is this?
    blur(): cognidream;
    showHover?(): cognidream;
}
...
export class ActionBar extends Disposable implements IActionRunner {
    ...
    private readonly _onDidCancel = this._register(new Emitter<cognidream>({ onWillAddFirstListener: () => this.cancelHasListener = true }));
    ...
    private refreshRole(): cognidream {
        if (this.length() >= 1) {
            this.actionsList.setAttribute('role', this.options.ariaRole || 'toolbar');
        } else {
    ...
    private updateFocusedItem(): cognidream {
    ...
    push(arg: IAction | ReadonlyArray<IAction>, options: IActionOptions = {}): cognidream {
    ...
    focus(index?: number): cognidream;
    focus(selectFirst?: boolean): cognidream;
    focus(arg?: number | boolean): cognidream {
    ...
    protected updateFocus(fromRight?: boolean, preventScroll?: boolean, forceFocus: boolean = false): cognidream {
        if (typeof this.focusedItem === 'undefined') {
    ...
    private doTrigger(event: StandardKeyboardEvent): cognidream {
        if (typeof this.focusedItem === 'undefined') {
    ...
    async run(action: IAction, context?: unknown): Promise<cognidream> {
