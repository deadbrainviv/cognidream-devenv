...
class TableListRenderer<TRow> implements IListRenderer<TRow, RowTemplateData> {
    ...
    renderElement(element: TRow, index: number, templateData: RowTemplateData, height: number | undefined): cognidream {
        for (let i = 0; i < this.columns.length; i++) {
    ...
    disposeElement(element: TRow, index: number, templateData: RowTemplateData, height: number | undefined): cognidream {
        for (let i = 0; i < this.columns.length; i++) {
            const renderer = this.renderers[i];
    ...
    disposeTemplate(templateData: RowTemplateData): cognidream {
        for (let i = 0; i < this.columns.length; i++) {
            const renderer = this.renderers[i];
    ...
    layoutColumn(index: number, size: number): cognidream {
...
export class Table<TRow> implements ISpliceable<TRow>, IDisposable {
    ...
    get onDidFocus(): Event<cognidream> { return this.list.onDidFocus; }
    get onDidBlur(): Event<cognidream> { return this.list.onDidBlur; }
    ...
    get onDidDispose(): Event<cognidream> { return this.list.onDidDispose; }
    ...
    resizeColumn(index: number, percentage: number): cognidream {
        const size = Math.round((percentage / 100.00) * this.cachedWidth);
        this.splitview.resizeView(index, size);
    }

    updateOptions(options: ITableOptionsUpdate): cognidream {
        this.list.updateOptions(options);
    }

    splice(start: number, deleteCount: number, elements: readonly TRow[] = []): cognidream {
    ...
    style(styles: ITableStyles): cognidream {
    ...
    reveal(index: number, relativeTop?: number): cognidream {
