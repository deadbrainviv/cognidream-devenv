...
export class InputBox extends Widget {
    ...
    public setPlaceHolder(placeHolder: string): cognidream {
        this.placeholder = placeHolder;
        this.input.setAttribute('placeholder', placeHolder);
    }

    public setTooltip(tooltip: string): cognidream {
    ...
    public select(range: IRange | null = null): cognidream {
        this.input.select();
    ...
    public enable(): cognidream {
        this.input.removeAttribute('disabled');
    }

    public disable(): cognidream {
        this.blur();
        this.input.disabled = true;
        this._hideMessage();
    }

    public setEnabled(enabled: boolean): cognidream {
        if (enabled) {
            this.enable();
        } else {
            this.disable();
    ...
    public set paddingRight(paddingRight: number) {
        // Set width to acognidream hint text overlapping buttons
    ...
    private updateScrollDimensions(): cognidream {
    ...
    public showMessage(message: IMessage, force?: boolean): cognidream {
    ...
    private _showMessage(): cognidream {
    ...
    private _hideMessage(): cognidream {
    ...
    private onValueChange(): cognidream {
    ...
    private updateMirror(): cognidream {
        if (!this.mirror) {
            return;
        }
    ...
    protected applyStyles(): cognidream {
        const styles = this.options.inputBoxStyles;
    ...
    public insertAtCursor(text: string): cognidream {
...
export class HistoryInputBox extends InputBox implements IHistoryNavigationWidget {
    ...
    private readonly _onDidFocus = this._register(new Emitter<cognidream>());
    readonly onDidFocus = this._onDidFocus.event;

    private readonly _onDidBlur = this._register(new Emitter<cognidream>());
    ...
    public addToHistory(always?: boolean): cognidream {
        if (this.value && (always || this.value !== this.getCurrentValue())) {
            this.history.add(this.value);
        }
    }

    public prependHistory(restoredHistory: string[]): cognidream {
    ...
    public showNextValue(): cognidream {
    ...
    public showPreviousValue(): cognidream {
        if (!this.history.has(this.value)) {
            this.addToHistory();
        }
    ...
    public clearHistory(): cognidream {
        this.history.clear();
    }

    public override setPlaceHolder(placeHolder: string): cognidream {
        super.setPlaceHolder(placeHolder);
        this.setTooltip(placeHolder);
    }

    protected override onBlur(): cognidream {
        super.onBlur();
        this._onDidBlur.fire();
    }

    protected override onFocus(): cognidream {
        super.onFocus();
