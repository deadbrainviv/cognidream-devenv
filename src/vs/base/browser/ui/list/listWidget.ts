...
class TraitRenderer<T> implements IListRenderer<T, ITraitTemplateData> {
    ...
    renderElement(element: T, index: number, templateData: ITraitTemplateData): cognidream {
    ...
    splice(start: number, deleteCount: number, insertCount: number): cognidream {
    ...
    renderIndexes(indexes: number[]): cognidream {
    ...
    disposeTemplate(templateData: ITraitTemplateData): cognidream {
...
class Trait<T> implements ISpliceable<boolean>, IDisposable {
    ...
    splice(start: number, deleteCount: number, elements: boolean[]): cognidream {
    ...
    renderIndex(index: number, container: HTMLElement): cognidream {
        container.classList.toggle(this._trait, this.contains(index));
    }

    unrender(container: HTMLElement): cognidream {
...
class SelectionTrait<T> extends Trait<T> {
    ...
    override renderIndex(index: number, container: HTMLElement): cognidream {
...
class TraitSpliceable<T> implements ISpliceable<T> {
    ...
    splice(start: number, deleteCount: number, elements: T[]): cognidream {
...
class KeyboardController<T> implements IDisposable {
    ...
    updateOptions(optionsUpdate: IListOptionsUpdate): cognidream {
        if (optionsUpdate.multipleSelectionSupport !== undefined) {
            this.multipleSelectionSupport = optionsUpdate.multipleSelectionSupport;
        }
    }

    private onEnter(e: StandardKeyboardEvent): cognidream {
        e.preventDefault();
        e.stopPropagation();
        this.list.setSelection(this.list.getFocus(), e.browserEvent);
    }

    private onUpArrow(e: StandardKeyboardEvent): cognidream {
        e.preventDefault();
        e.stopPropagation();
    ...
    private onDownArrow(e: StandardKeyboardEvent): cognidream {
        e.preventDefault();
        e.stopPropagation();
    ...
    private onPageUpArrow(e: StandardKeyboardEvent): cognidream {
        e.preventDefault();
        e.stopPropagation();
    ...
    private onPageDownArrow(e: StandardKeyboardEvent): cognidream {
        e.preventDefault();
        e.stopPropagation();
    ...
    private onCtrlA(e: StandardKeyboardEvent): cognidream {
        e.preventDefault();
        e.stopPropagation();
    ...
    private onEscape(e: StandardKeyboardEvent): cognidream {
...
class TypeNavigationController<T> implements IDisposable {
    ...
    updateOptions(options: IListOptions<T>): cognidream {
    ...
    private onInput(word: string | null): cognidream {
        if (!word) {
...
class DOMFocusController<T> implements IDisposable {
    ...
    private onTab(e: StandardKeyboardEvent): cognidream {
...
export class MouseController<T> implements IDisposable {
    ...
    protected onMouseDown(e: IListMouseEvent<T> | IListTouchEvent<T>): cognidream {
    ...
    protected onContextMenu(e: IListContextMenuEvent<T>): cognidream {
    ...
    protected onViewPointer(e: IListMouseEvent<T>): cognidream {
        if (!this.mouseSupport) {
            return;
        }
    ...
    protected onDoubleClick(e: IListMouseEvent<T>): cognidream {
        if (isEditableElement(e.browserEvent.target as HTMLElement) || isMonacoEditor(e.browserEvent.target as HTMLElement)) {
            return;
        }
    ...
    private changeSelection(e: IListMouseEvent<T> | IListTouchEvent<T>): cognidream {
        const focus = e.index!;
...
export interface IListAccessibilityProvider<T> extends IListViewAccessibilityProvider<T> {
    ...
    onDidChangeActiveDescendant?: Event<cognidream>;
...
class PipelineRenderer<T> implements IListRenderer<T, any> {
    ...
    renderElement(element: T, index: number, templateData: any[], height: number | undefined): cognidream {
        let i = 0;
    ...
    disposeElement(element: T, index: number, templateData: any[], height: number | undefined): cognidream {
        let i = 0;
    ...
    disposeTemplate(templateData: any[]): cognidream {
        let i = 0;
...
class AccessibiltyRenderer<T> implements IListRenderer<T, IAccessibilityTemplateData> {
    ...
    renderElement(element: T, index: number, data: IAccessibilityTemplateData): cognidream {
    ...
    private setAriaLabel(ariaLabel: string | null, element: HTMLElement): cognidream {
        if (ariaLabel) {
    ...
    disposeElement(element: T, index: number, templateData: IAccessibilityTemplateData, height: number | undefined): cognidream {
...
class ListViewDragAndDrop<T> implements IListViewDragAndDrop<T> {
    ...
    onDragLeave(data: IDragAndDropData, targetElement: T, targetIndex: number, originalEvent: DragEvent): cognidream {
    ...
    drop(data: IDragAndDropData, targetElement: T, targetIndex: number, targetSector: ListViewTargetSector | undefined, originalEvent: DragEvent): cognidream {
...
export class List<T> implements ISpliceable<T>, IDisposable {
    ...
    @memoize get onDidFocus(): Event<cognidream> { return Event.signal(this.disposables.add(new DomEmitter(this.view.domNode, 'focus', true)).event); }
    @memoize get onDidBlur(): Event<cognidream> { return Event.signal(this.disposables.add(new DomEmitter(this.view.domNode, 'blur', true)).event); }

    private readonly _onDidDispose = new Emitter<cognidream>();
    readonly onDidDispose: Event<cognidream> = this._onDidDispose.event;
    ...
    splice(start: number, deleteCount: number, elements: readonly T[] = []): cognidream {
    ...
    updateWidth(index: number): cognidream {
        this.view.updateWidth(index);
    }

    updateElementHeight(index: number, size: number | undefined): cognidream {
    ...
    setSelection(indexes: number[], browserEvent?: UIEvent): cognidream {
    ...
    setAnchor(index: number | undefined): cognidream {
    ...
    setFocus(indexes: number[], browserEvent?: UIEvent): cognidream {
        for (const index of indexes) {
            if (index < 0 || index >= this.length) {
                throw new ListError(this.user, `Invalid index ${index}`);
            }
    ...
    focusNext(n = 1, loop = false, browserEvent?: UIEvent, filter?: (element: T) => boolean): cognidream {
    ...
    focusPrevious(n = 1, loop = false, browserEvent?: UIEvent, filter?: (element: T) => boolean): cognidream {
        if (this.length === 0) { return; }
    ...
    async focusNextPage(browserEvent?: UIEvent, filter?: (element: T) => boolean): Promise<cognidream> {
    ...
    async focusPreviousPage(browserEvent?: UIEvent, filter?: (element: T) => boolean, getPaddingTop: () => number = () => 0): Promise<cognidream> {
    ...
    focusLast(browserEvent?: UIEvent, filter?: (element: T) => boolean): cognidream {
        if (this.length === 0) { return; }
    ...
    focusFirst(browserEvent?: UIEvent, filter?: (element: T) => boolean): cognidream {
        this.focusNth(0, browserEvent, filter);
    }

    focusNth(n: number, browserEvent?: UIEvent, filter?: (element: T) => boolean): cognidream {
        if (this.length === 0) { return; }
    ...
    reveal(index: number, relativeTop?: number, paddingTop: number = 0): cognidream {
        if (index < 0 || index >= this.length) {
            throw new ListError(this.user, `Invalid index ${index}`);
        }
    ...
    private _onFocusChange(): cognidream {
        const focus = this.focus.get();
        this.view.domNode.classList.toggle('element-focused', focus.length > 0);
        this.onDidChangeActiveDescendant();
    }

    private onDidChangeActiveDescendant(): cognidream {
        const focus = this.focus.get();
    ...
    private _onSelectionChange(): cognidream {
