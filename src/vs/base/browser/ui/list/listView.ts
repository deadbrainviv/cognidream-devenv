...
export class ElementsDragAndDropData<T, TContext = cognidream> implements IDragAndDropData {
...
export class NativeDragAndDropData implements IDragAndDropData {
    ...
    update(dataTransfer: DataTransfer): cognidream {
...
export interface IListView<T> extends ISpliceable<T>, IDisposable {
    ...
    updateOptions(options: IListViewOptionsUpdate): cognidream;
    getScrollTop(): number;
    setScrollTop(scrollTop: number, reuseAnimation?: boolean): cognidream;
    getScrollLeft(): number;
    setScrollLeft(scrollLeft: number): cognidream;
    delegateScrollFromMouseWheelEvent(browserEvent: IMouseWheelEvent): cognidream;
    delegateVerticalScrollbarPointerDown(browserEvent: PointerEvent): cognidream;
    updateWidth(index: number): cognidream;
    updateElementHeight(index: number, size: number | undefined, anchorIndex: number | null): cognidream;
    rerender(): cognidream;
...
export class ListView<T> implements IListView<T> {
    ...
    private scrollableElementWidthDelayer = new Delayer<cognidream>(50);
    ...
    private _setupFocusObserver(container: HTMLElement): cognidream {
    ...
    private _splice(start: number, deleteCount: number, elements: readonly T[] = []): T[] {
        ...
        // try to reuse rows, acognidream removing them from DOM
    ...
    protected eventuallyUpdateScrollDimensions(): cognidream {
    ...
    private eventuallyUpdateScrollWidth(): cognidream {
    ...
    private updateScrollWidth(): cognidream {
        if (!this.horizontalScrolling) {
            return;
        }
    ...
    protected render(previousRenderRange: IRange, renderTop: number, renderHeight: number, renderLeft: number | undefined, scrollWidth: number | undefined, updateItemsInDOM: boolean = false): cognidream {
    ...
    private insertItemInDOM(index: number, row?: IRow): cognidream {
        const item = this.items[index];
    ...
    private measureItemWidth(item: IItem<T>): cognidream {
    ...
    private updateItemInDOM(item: IItem<T>, index: number): cognidream {
    ...
    private removeItemFromDOM(index: number): cognidream {
        const item = this.items[index];
        item.dragStartDisposable.dispose();
        item.checkedDisposable.dispose();
    ...
    private onScroll(e: ScrollEvent): cognidream {
        try {
            const previousRenderRange = this.getRenderRange(this.lastRenderTop, this.lastRenderHeight);
    ...
    private onTouchChange(event: GestureEvent): cognidream {
    ...
    private onDragStart(element: T, uri: string, event: DragEvent): cognidream {
    ...
    private onDragLeave(event: IListDragEvent<T>): cognidream {
        this.onDragLeaveTimeout.dispose();
    ...
    private onDrop(event: IListDragEvent<T>): cognidream {
        if (!this.canDrop) {
            return;
        }
    ...
    private onDragEnd(event: DragEvent): cognidream {
        this.canDrop = false;
        this.teardownDragAndDropScrollTopAnimation();
        this.clearDragOverFeedback();
        this.domNode.classList.remove('dragging');
    ...
    private clearDragOverFeedback(): cognidream {
        this.currentDragFeedback = undefined;
    ...
    private setupDragAndDropScrollTopAnimation(event: DragEvent | MouseEvent): cognidream {
    ...
    private animateDragAndDropScrollTop(viewTop: number): cognidream {
    ...
    private teardownDragAndDropScrollTopAnimation(): cognidream {
        this.dragOverAnimationStopDisposable.dispose();
     ...
     */
    protected _rerender(renderTop: number, renderHeight: number, inSmoothScrolling?: boolean): cognidream {
        ...
        while (true) {
            ...
            if (!didChange) {
                ...
                if (typeof anchorElementIndex === 'number') {
                    ...
                    // animation, and then reuse it with a new target (to acognidream prolonging the scroll)
