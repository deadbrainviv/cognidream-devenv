...
export class MenuBar extends Disposable {
    ...
    push(arg: MenuBarMenu | MenuBarMenu[]): cognidream {
    ...
    updateMenu(menu: MenuBarMenu): cognidream {
    ...
    private updateOverflowAction(): cognidream {
        ...
        // If we are only showing the overflow, add this class to acognidream taking up space
        this.container.classList.toggle(overflowMenuOnlyClass, this.numMenusShown === 0);
    }

    private updateLabels(titleElement: HTMLElement, buttonElement: HTMLElement, label: string): cognidream {
    ...
    update(options?: IMenuBarOptions): cognidream {
        if (options) {
    ...
    private registerMnemonic(menuIndex: number, mnemonic: string): cognidream {
        this.mnemonics.set(mnemonic.toLocaleLowerCase(), menuIndex);
    }

    private hideMenubar(): cognidream {
    ...
    private showMenubar(): cognidream {
    ...
    private setUnfocusedState(): cognidream {
    ...
    private updateMnemonicVisibility(visible: boolean): cognidream {
        if (this.menus) {
            this.menus.forEach(menuBarMenu => {
    ...
    private onModifierKeyToggled(modifierKeyStatus: DOM.IModifierKeyStatus): cognidream {
    ...
    private cleanupCustomMenu(): cognidream {
        if (this.focusedMenu) {
    ...
    private showCustomMenu(menuIndex: number, selectFirst = true): cognidream {
