...
export abstract class BreadcrumbsItem {
    abstract dispose(): cognidream;
    abstract equals(other: BreadcrumbsItem): boolean;
    abstract render(container: HTMLElement): cognidream;
}
...
export class BreadcrumbsWidget {
    ...
    layout(dim: dom.Dimension | undefined): cognidream {
    ...
    private _style(styleElement: HTMLStyleElement, style: IBreadcrumbsWidgetStyles): cognidream {
        let content = '';
    ...
    setFocused(item: BreadcrumbsItem | undefined, payload?: any): cognidream {
        this._focus(this._items.indexOf(item!), payload);
    }

    focusPrev(payload?: any): cognidream {
        if (this._focusedItemIdx > 0) {
            this._focus(this._focusedItemIdx - 1, payload);
        }
    }

    focusNext(payload?: any): cognidream {
        if (this._focusedItemIdx + 1 < this._nodes.length) {
            this._focus(this._focusedItemIdx + 1, payload);
        }
    }

    private _focus(nth: number, payload: any): cognidream {
    ...
    reveal(item: BreadcrumbsItem): cognidream {
    ...
    revealLast(): cognidream {
        this._reveal(this._items.length - 1, false);
    }

    private _reveal(nth: number, minimal: boolean): cognidream {
    ...
    setSelection(item: BreadcrumbsItem | undefined, payload?: any): cognidream {
        this._select(this._items.indexOf(item!), payload);
    }

    private _select(nth: number, payload: any): cognidream {
    ...
    setItems(items: BreadcrumbsItem[]): cognidream {
    ...
    private _render(start: number): cognidream {
        let didChange = false;
    ...
    private _renderItem(item: BreadcrumbsItem, container: HTMLDivElement): cognidream {
    ...
    private _onClick(event: IMouseEvent): cognidream {
        if (!this._enabled) {
            return;
        }
