...
function findAdjacentBoxLeafNodes<T extends IView>(boxNode: GridNode<T>, direction: Direction, boundary: Boundary): GridLeafNode<T>[] {
    ...
    function _(boxNode: GridNode<T>, direction: Direction, boundary: Boundary): cognidream {
...
export class Grid<T extends IView = IView> extends Disposable {
    ...
    style(styles: IGridStyles): cognidream {
     ...
     */
    addView(newView: T, size: number | Sizing, referenceView: T, direction: Direction): cognidream {
    ...
    private addViewAt(newView: T, size: number | DistributeSizing | InvisibleSizing, location: GridLocation): cognidream {
        if (this.views.has(newView)) {
            throw new Error('Can\'t add same view twice');
        }
    ...
    protected _addView(newView: T, size: number | GridViewSizing, location: GridLocation): cognidream {
     ...
     */
    removeView(view: T, sizing?: Sizing): cognidream {
     ...
     */
    moveView(view: T, sizing: number | Sizing, referenceView: T, direction: Direction): cognidream {
     ...
     */
    moveViewTo(view: T, location: GridLocation): cognidream {
        const sourceLocation = this.getViewLocation(view);
        const [sourceParentLocation, from] = tail(sourceLocation);
     ...
     */
    swapViews(from: T, to: T): cognidream {
     ...
     */
    resizeView(view: T, size: IViewSize): cognidream {
        const location = this.getViewLocation(view);
     ...
     * @param view The {@link IView view}.
     */
    setViewVisible(view: T, visible: boolean): cognidream {
        const location = this.getViewLocation(view);
    ...
    private onDidSashReset(location: GridLocation): cognidream {
...
export class SerializableGrid<T extends ISerializableView> extends Grid<T> {
    ...
    override layout(width: number, height: number, top: number = 0, left: number = 0): cognidream {
...
export function sanitizeGridNodeDescriptor<T>(nodeDescriptor: GridNodeDescriptor<T>, rootNode: boolean): cognidream {
