...
export interface IView {
     ...
     */
    layout(width: number, height: number, top: number, left: number): cognidream;
     ...
     */
    setVisible?(visible: boolean): cognidream;
...
class BranchNode implements ISplitView<ILayoutContext>, IDisposable {
    ...
    private _onDidScroll = new Emitter<cognidream>();
    private onDidScrollDisposable: IDisposable = Disposable.None;
    readonly onDidScroll: Event<cognidream> = this._onDidScroll.event;
    ...
    style(styles: IGridViewStyles): cognidream {
        this._styles = styles;
    ...
    layout(size: number, offset: number, ctx: ILayoutContext | undefined): cognidream {
    ...
    addChild(node: Node, size: number | Sizing, index: number, skipLayout?: boolean): cognidream {
    ...
    moveChild(from: number, to: number): cognidream {
    ...
    swapChildren(from: number, to: number): cognidream {
        from = validateIndex(from, this.children.length);
        to = validateIndex(to, this.children.length);
    ...
    resizeChild(index: number, size: number): cognidream {
        index = validateIndex(index, this.children.length);
    ...
    distributeViewSizes(recursive = false): cognidream {
    ...
    setChildVisible(index: number, visible: boolean): cognidream {
        index = validateIndex(index, this.children.length);
    ...
    private updateBoundarySashes(): cognidream {
    ...
    private onDidChildrenChange(): cognidream {
        this.updateChildrenEvents();
        this._onDidChange.fire(undefined);
    }

    private updateChildrenEvents(): cognidream {
    ...
    private updateSplitviewEdgeSnappingEnablement(): cognidream {
...
/**
 * Creates a latched event that acognidreams being fired when the view
...
class LeafNode implements ISplitView<ILayoutContext>, IDisposable {
    ...
    readonly onDidScroll: Event<cognidream> = Event.None;
    ...
    private _layout(width: number, height: number, top: number, left: number): cognidream {
...
export class GridView implements IDisposable {
    ...
    private _onDidScroll = new Relay<cognidream>();
     ...
     */
    layout(width: number, height: number, top: number = 0, left: number = 0): cognidream {
     ...
     */
    addView(view: IView, size: number | Sizing, location: GridLocation): cognidream {
     ...
     */
    moveView(parentLocation: GridLocation, from: number, to: number): cognidream {
        if (this.hasMaximizedView()) {
            this.exitMaximizedView();
        }
     ...
     */
    swapViews(from: GridLocation, to: GridLocation): cognidream {
        if (this.hasMaximizedView()) {
            this.exitMaximizedView();
        }
     ...
     */
    resizeView(location: GridLocation, size: Partial<IViewSize>): cognidream {
        if (this.hasMaximizedView()) {
            this.exitMaximizedView();
        }
     ...
     * @param location The {@link GridLocation location} of the view.
     */
    expandView(location: GridLocation): cognidream {
        if (this.hasMaximizedView()) {
            this.exitMaximizedView();
        }
    ...
    maximizeView(location: GridLocation) {
        ...
        function hideAllViewsBut(parent: BranchNode, exclude: LeafNode): cognidream {
    ...
    exitMaximizedView(): cognidream {
        ...
        function showViewsInReverseOrder(parent: BranchNode): cognidream {
     ...
     * in the entire grid.
     */
    distributeViewSizes(location?: GridLocation): cognidream {
        if (this.hasMaximizedView()) {
            this.exitMaximizedView();
        }
     ...
     * @param location The {@link GridLocation location} of the view.
     */
    setViewVisible(location: GridLocation, visible: boolean): cognidream {
        if (this.hasMaximizedView()) {
            this.exitMaximizedView();
            return;
        }
    ...
    private _deserialize(root: ISerializedBranchNode, orientation: Orientation, deserializer: IViewDeserializer<ISerializableView>, orthogonalSize: number): cognidream {
     ...
     */
    getViewMap(map: Map<IView, HTMLElement>, node?: Node): cognidream {
        if (!node) {
            node = this.root;
        }
