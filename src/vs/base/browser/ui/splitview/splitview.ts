...
export interface IView<TLayoutContext = undefined> {
     ...
     */
    layout(size: number, offset: number, context: TLayoutContext | undefined): cognidream;
...
abstract class ViewItem<TLayoutContext, TView extends IView<TLayoutContext>> {
    ...
    setVisible(visible: boolean, size?: number): cognidream {
    ...
    layout(offset: number, layoutContext: TLayoutContext | undefined): cognidream {
    ...
    abstract layoutContainer(offset: number): cognidream;
...
class VerticalViewItem<TLayoutContext, TView extends IView<TLayoutContext>> extends ViewItem<TLayoutContext, TView> {
    ...
    layoutContainer(offset: number): cognidream {
...
export class SplitView<TLayoutContext = undefined, TView extends IView<TLayoutContext> = IView<TLayoutContext>> extends Disposable {
    ...
    style(styles: ISplitViewStyles): cognidream {
     ...
     */
    addView(view: TView, size: number | Sizing, index = this.viewItems.length, skipLayout?: boolean): cognidream {
     ...
     */
    moveView(from: number, to: number): cognidream {
        if (this.state !== State.Idle) {
            throw new Error('Cant modify splitview');
        }
     ...
     * @param to The target index.
     */
    swapViews(from: number, to: number): cognidream {
        if (this.state !== State.Idle) {
            throw new Error('Cant modify splitview');
        }
     ...
     */
    setViewVisible(index: number, visible: boolean): cognidream {
        if (index < 0 || index >= this.viewItems.length) {
            throw new Error('Index out of bounds');
        }
     ...
     */
    layout(size: number, layoutContext?: TLayoutContext): cognidream {
    ...
    private onSashChange({ current }: ISashEvent): cognidream {
    ...
    private onSashEnd(index: number): cognidream {
    ...
    private onViewChange(item: ViewItem<TLayoutContext, TView>, size: number | undefined): cognidream {
     ...
     */
    resizeView(index: number, size: number): cognidream {
        if (index < 0 || index >= this.viewItems.length) {
            return;
        }
     ...
     */
    distributeViewSizes(): cognidream {
    ...
    private doAddView(view: TView, size: number | Sizing, index = this.viewItems.length, skipLayout?: boolean): cognidream {
        if (this.state !== State.Idle) {
            throw new Error('Cant modify splitview');
        }
    ...
    private relayout(lowPriorityIndexes?: number[], highPriorityIndexes?: number[]): cognidream {
    ...
    private distributeEmptySpace(lowPriorityIndex?: number): cognidream {
        const contentSize = this.viewItems.reduce((r, i) => r + i.size, 0);
    ...
    private layoutViews(): cognidream {
    ...
    private updateScrollableElement(): cognidream {
    ...
    private updateSashEnablement(): cognidream {
        let previous = false;
