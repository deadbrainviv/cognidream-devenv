...
export abstract class Pane extends Disposable implements IView {
    ...
    style(styles: IPaneStyles): cognidream {
        this.styles = styles;
    ...
    protected updateHeader(): cognidream {
        if (!this.header) {
            return;
        }
        const expanded = !this.headerVisible || this.isExpanded();
    ...
    protected abstract renderHeader(container: HTMLElement): cognidream;
    protected abstract renderBody(container: HTMLElement): cognidream;
    protected abstract layoutBody(height: number, width: number): cognidream;
}
...
class PaneDraggable extends Disposable {
    ...
    private onDragStart(e: DragEvent): cognidream {
    ...
    private onDragEnter(e: DragEvent): cognidream {
    ...
    private onDragLeave(e: DragEvent): cognidream {
        if (!this.context.draggable || this.context.draggable === this) {
            return;
        }
    ...
    private onDragEnd(e: DragEvent): cognidream {
    ...
    private onDrop(e: DragEvent): cognidream {
        if (!this.context.draggable) {
            return;
        }
...
export class PaneView extends Disposable {
    ...
    addPane(pane: Pane, size: number, index = this.splitview.length): cognidream {
    ...
    removePane(pane: Pane): cognidream {
    ...
    movePane(from: Pane, to: Pane): cognidream {
    ...
    resizePane(pane: Pane, size: number): cognidream {
        const index = this.paneItems.findIndex(item => item.pane === pane);
    ...
    flipOrientation(height: number, width: number): cognidream {
    ...
    private setupAnimation(): cognidream {
        if (typeof this.animationTimer === 'number') {
            getWindow(this.element).clearTimeout(this.animationTimer);
        }
    ...
    private focusPrevious(): cognidream {
