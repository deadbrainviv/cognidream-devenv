...
class TreeNodeListDragAndDrop<T, TFilterData, TRef> implements IListDragAndDrop<ITreeNode<T, TFilterData>> {
    ...
    drop(data: IDragAndDropData, targetNode: ITreeNode<T, TFilterData> | undefined, targetIndex: number | undefined, targetSector: ListViewTargetSector | undefined, originalEvent: DragEvent): cognidream {
        this.autoExpandDisposable.dispose();
        this.autoExpandNode = undefined;
    ...
    onDragEnd(originalEvent: DragEvent): cognidream {
...
export class ComposedTreeDelegate<T, N extends { element: T }> implements IListVirtualDelegate<N> {
    ...
    setDynamicHeight(element: N, height: number): cognidream {
...
export class TreeRenderer<T, TFilterData, TRef, TTemplateData> implements IListRenderer<ITreeNode<T, TFilterData>, ITreeListTemplateData<TTemplateData>> {
    ...
    updateOptions(options: ITreeRendererOptions = {}): cognidream {
    ...
    renderElement(node: ITreeNode<T, TFilterData>, index: number, templateData: ITreeListTemplateData<TTemplateData>, height: number | undefined): cognidream {
    ...
    disposeElement(node: ITreeNode<T, TFilterData>, index: number, templateData: ITreeListTemplateData<TTemplateData>, height: number | undefined): cognidream {
    ...
    disposeTemplate(templateData: ITreeListTemplateData<TTemplateData>): cognidream {
        this.renderer.disposeTemplate(templateData.templateData);
    }

    private onDidChangeTwistieState(element: T): cognidream {
    ...
    private onDidChangeNodeTwistieState(node: ITreeNode<T, TFilterData>): cognidream {
    ...
    private _renderIndentGuides(node: ITreeNode<T, TFilterData>, templateData: ITreeListTemplateData<TTemplateData>): cognidream {
    ...
    private _onDidChangeActiveNodes(nodes: ITreeNode<T, TFilterData>[]): cognidream {
        if (!this.shouldRenderIndentGuides) {
            return;
        }
...
class FindWidget<T, TFilterData> extends Disposable {
    ...
    readonly _onDidDisable = new Emitter<cognidream>();
    ...
    setToggleState(id: string, checked: boolean): cognidream {
    ...
    showMessage(message: IMessage): cognidream {
        this.findInput.showMessage(message);
    }

    clearMessage(): cognidream {
        this.findInput.clearMessage();
    }

    override async dispose(): Promise<cognidream> {
...
export abstract class AbstractFindController<T, TFilterData> implements IDisposable {
    ...
    protected onDidChangeValue(pattern: string): cognidream {
        this._pattern = pattern;
    ...
    protected abstract applyPattern(pattern: string): cognidream;

    protected onDidToggleChange(e: ITreeFindToggleChangeEvent): cognidream {
        this.toggles.set(e.id, e.isChecked);
    }

    protected updateToggleState(id: string, checked: boolean): cognidream {
        this.toggles.set(id, checked);
        this.widget?.setToggleState(id, checked);
    }

    protected renderMessage(showNotFound: boolean, warningMessage?: string): cognidream {
        if (showNotFound) {
    ...
    protected alertResults(results: number): cognidream {
        if (!results) {
...
export class FindController<T, TFilterData> extends AbstractFindController<T, TFilterData> {
    ...
    updateOptions(optionsUpdate: IAbstractTreeOptionsUpdate = {}): cognidream {
    ...
    protected applyPattern(pattern: string): cognidream {
        this.tree.refilter();
    ...
    protected override onDidToggleChange(e: ITreeFindToggleChangeEvent): cognidream {
...
class StickyScrollWidget<T, TFilterData, TRef> implements IDisposable {
    ...
    setState(state: StickyScrollState<T, TFilterData, TRef> | undefined): cognidream {
    ...
    private renderState(state: StickyScrollState<T, TFilterData, TRef>): cognidream {
        this._previousStateDisposables.clear();
    ...
    private setVisible(visible: boolean): cognidream {
...
class StickyScrollFocus<T, TFilterData, TRef> extends Disposable {
    ...
    private handleContextMenu(e: IListContextMenuEvent<ITreeNode<T, TFilterData>>): cognidream {
    ...
    private onKeyDown(e: KeyboardEvent): cognidream {
    ...
    private onMouseDown(e: IListMouseEvent<ITreeNode<T, TFilterData>>): cognidream {
        const target = e.browserEvent.target as HTMLElement;
        if (!isStickyScrollContainer(target) && !isStickyScrollElement(target)) {
            return;
        }
    ...
    updateElements(elements: HTMLElement[], state: StickyScrollState<T, TFilterData, TRef> | undefined): cognidream {
    ...
    private setFocusedElement(stickyIndex: number): cognidream {
    ...
    private removeFocus(): cognidream {
    ...
    private setFocus(newFocusIndex: number): cognidream {
        if (0 > newFocusIndex) {
    ...
    private toggleElementFocus(element: HTMLElement, focused: boolean): cognidream {
        this.toggleElementActiveFocus(element, focused && this.domHasFocus);
        this.toggleElementPassiveFocus(element, focused);
    }

    private toggleCurrentElementActiveFocus(focused: boolean): cognidream {
        if (this.focusedIndex === -1) {
            return;
        }
...
export interface IAbstractTreeOptions<T, TFilterData = cognidream> extends IAbstractTreeOptionsUpdate, IListOptions<T> {
...
function dfs<T, TFilterData>(node: ITreeNode<T, TFilterData>, fn: (node: ITreeNode<T, TFilterData>) => cognidream): cognidream {
    fn(node);
...
class Trait<T> {
    ...
    set(nodes: ITreeNode<T, any>[], browserEvent?: UIEvent): cognidream {
    ...
    private _set(nodes: ITreeNode<T, any>[], silent: boolean, browserEvent?: UIEvent): cognidream {
        this.nodes = [...nodes];
    ...
    onDidModelSplice({ insertedNodes, deletedNodes }: ITreeModelSpliceEvent<T, any>): cognidream {
...
class TreeNodeListMouseController<T, TFilterData, TRef> extends MouseController<ITreeNode<T, TFilterData>> {
    ...
    protected override onViewPointer(e: IListMouseEvent<ITreeNode<T, TFilterData>>): cognidream {
    ...
    private handleStickyScrollMouseEvent(e: IListMouseEvent<ITreeNode<T, TFilterData>>, node: ITreeNode<T, TFilterData>): cognidream {
    ...
    protected override onDoubleClick(e: IListMouseEvent<ITreeNode<T, TFilterData>>): cognidream {
    ...
    protected override onMouseDown(e: IListMouseEvent<ITreeNode<T, TFilterData>> | IListTouchEvent<ITreeNode<T, TFilterData>>): cognidream {
        const target = e.browserEvent.target as HTMLElement;
        if (!isStickyScrollContainer(target) && !isStickyScrollElement(target)) {
            super.onMouseDown(e);
            return;
    ...
    protected override onContextMenu(e: IListContextMenuEvent<ITreeNode<T, TFilterData>>): cognidream {
        const target = e.browserEvent.target as HTMLElement;
        if (!isStickyScrollContainer(target) && !isStickyScrollElement(target)) {
...
class TreeNodeList<T, TFilterData, TRef> extends List<ITreeNode<T, TFilterData>> {
    ...
    override splice(start: number, deleteCount: number, elements: readonly ITreeNode<T, TFilterData>[] = []): cognidream {
    ...
    override setFocus(indexes: number[], browserEvent?: UIEvent, fromAPI = false): cognidream {
    ...
    override setSelection(indexes: number[], browserEvent?: UIEvent, fromAPI = false): cognidream {
    ...
    override setAnchor(index: number | undefined, fromAPI = false): cognidream {
...
export abstract class AbstractTree<T, TFilterData, TRef> implements IDisposable {
    ...
    get onDidFocus(): Event<cognidream> { return this.view.onDidFocus; }
    get onDidBlur(): Event<cognidream> { return this.view.onDidBlur; }

    private readonly onDidSwapModel = this.disposables.add(new Emitter<cognidream>());
    private readonly onDidChangeModelRelay = this.disposables.add(new Relay<cognidream>());
    ...
    get onDidChangeModel(): Event<cognidream> { return Event.any(this.onDidChangeModelRelay.event, this.onDidSwapModel.event); }
    ...
    private readonly _onWillRefilter = new Emitter<cognidream>();
    readonly onWillRefilter: Event<cognidream> = this._onWillRefilter.event;
    ...
    get onDidDispose(): Event<cognidream> { return this.view.onDidDispose; }
    ...
    updateWidth(element: TRef): cognidream {
    ...
    layout(height?: number, width?: number): cognidream {
        this.view.layout(height, width);
    }

    style(styles: IListStyles): cognidream {
    ...
    expandTo(location: TRef): cognidream {
        this.model.expandTo(location);
    }

    triggerTypeNavigation(): cognidream {
    ...
    setAnchor(element: TRef | undefined): cognidream {
    ...
    setSelection(elements: TRef[], browserEvent?: UIEvent): cognidream {
        this.eventBufferer.bufferEvents(() => {
    ...
    setFocus(elements: TRef[], browserEvent?: UIEvent): cognidream {
        this.eventBufferer.bufferEvents(() => {
            const nodes = elements.map(e => this.model.getNode(e));
    ...
    focusNext(n = 1, loop = false, browserEvent?: UIEvent, filter: ((node: ITreeNode<T, TFilterData>) => boolean) | undefined = (isKeyboardEvent(browserEvent) && browserEvent.altKey) ? undefined : this.focusNavigationFilter): cognidream {
        this.view.focusNext(n, loop, browserEvent, filter);
    }

    focusPrevious(n = 1, loop = false, browserEvent?: UIEvent, filter: ((node: ITreeNode<T, TFilterData>) => boolean) | undefined = (isKeyboardEvent(browserEvent) && browserEvent.altKey) ? undefined : this.focusNavigationFilter): cognidream {
        this.view.focusPrevious(n, loop, browserEvent, filter);
    }

    focusNextPage(browserEvent?: UIEvent, filter: ((node: ITreeNode<T, TFilterData>) => boolean) | undefined = (isKeyboardEvent(browserEvent) && browserEvent.altKey) ? undefined : this.focusNavigationFilter): Promise<cognidream> {
        return this.view.focusNextPage(browserEvent, filter);
    }

    focusPreviousPage(browserEvent?: UIEvent, filter: ((node: ITreeNode<T, TFilterData>) => boolean) | undefined = (isKeyboardEvent(browserEvent) && browserEvent.altKey) ? undefined : this.focusNavigationFilter): Promise<cognidream> {
        return this.view.focusPreviousPage(browserEvent, filter, () => this.stickyScrollController?.height ?? 0);
    }

    focusLast(browserEvent?: UIEvent, filter: ((node: ITreeNode<T, TFilterData>) => boolean) | undefined = (isKeyboardEvent(browserEvent) && browserEvent.altKey) ? undefined : this.focusNavigationFilter): cognidream {
        this.view.focusLast(browserEvent, filter);
    }

    focusFirst(browserEvent?: UIEvent, filter: ((node: ITreeNode<T, TFilterData>) => boolean) | undefined = (isKeyboardEvent(browserEvent) && browserEvent.altKey) ? undefined : this.focusNavigationFilter): cognidream {
    ...
    reveal(location: TRef, relativeTop?: number): cognidream {
        this.model.expandTo(location);
    ...
    private onLeftArrow(e: StandardKeyboardEvent): cognidream {
        e.preventDefault();
        e.stopPropagation();
    ...
    private onRightArrow(e: StandardKeyboardEvent): cognidream {
        e.preventDefault();
        e.stopPropagation();
    ...
    private onSpace(e: StandardKeyboardEvent): cognidream {
        e.preventDefault();
        e.stopPropagation();
