...
export interface IDataTreeOptions<T, TFilterData = cognidream> extends IAbstractTreeOptions<T, TFilterData> {
    readonly sorter?: ITreeSorter<T>;
}

export class DataTree<TInput, T, TFilterData = cognidream> extends AbstractTree<T | null, TFilterData, T | null> {
    ...
    setInput(input: TInput | undefined, viewState?: AbstractTreeViewState): cognidream {
    ...
    updateChildren(element: TInput | T = this.input!): cognidream {
    ...
    resort(element: T | TInput = this.input!, recursive = true): cognidream {
        this.model.resort((element === this.input ? null : element) as T, recursive);
    }

    // View

    refresh(element?: T): cognidream {
    ...
    private _refresh(element: TInput | T, isCollapsed?: (el: T) => boolean | undefined, onDidCreateNode?: (node: ITreeNode<T, TFilterData>) => cognidream): cognidream {
        let onDidDeleteNode: ((node: ITreeNode<T, TFilterData>) => cognidream) | undefined;
