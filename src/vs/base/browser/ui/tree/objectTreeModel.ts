...
export type ITreeNodeCallback<T, TFilterData> = (node: ITreeNode<T, TFilterData>) => cognidream;

export interface IObjectTreeModel<T extends NonNullable<any>, TFilterData extends NonNullable<any> = cognidream> extends ITreeModel<T | null, TFilterData, T | null> {
    setChildren(element: T | null, children: Iterable<IObjectTreeElement<T>> | undefined, options?: IObjectTreeModelSetChildrenOptions<T, TFilterData>): cognidream;
    resort(element?: T | null, recursive?: boolean): cognidream;
}
...
export class ObjectTreeModel<T extends NonNullable<any>, TFilterData extends NonNullable<any> = cognidream> implements IObjectTreeModel<T, TFilterData> {
    ...
    rerender(element: T | null): cognidream {
        const location = this.getElementLocation(element);
        this.model.rerender(location);
    }

    resort(element: T | null = null, recursive = true): cognidream {
        if (!this.sorter) {
            return;
        }
    ...
    expandTo(element: T | null): cognidream {
        const location = this.getElementLocation(element);
