...
// Exported for tests
export interface IIndexTreeNode<T, TFilterData = cognidream> extends ITreeNode<T, TFilterData> {
...
export interface IIndexTreeModelSpliceOptions<T, TFilterData> {
     ...
     */
    onDidCreateNode?: (node: ITreeNode<T, TFilterData>) => cognidream;

    /**
     * Callback for when a node is deleted.
     */
    onDidDeleteNode?: (node: ITreeNode<T, TFilterData>) => cognidream;
}
...
export class IndexTreeModel<T extends Exclude<any, undefined>, TFilterData = cognidream> implements ITreeModel<T, TFilterData, number[]> {
    ...
    private spliceSimple(
        ...
        while (node) {
            if (node.visibility === TreeVisibility.Recurse) {
                // delayed to acognidream excessive refiltering, see #135941
    ...
    rerender(location: number[]): cognidream {
        if (location.length === 0) {
            throw new TreeError(this.user, 'Invalid tree location');
        }
    ...
    expandTo(location: number[]): cognidream {
    ...
    private _updateAncestorsRenderNodeCount(node: IIndexTreeNode<T, TFilterData> | undefined, diff: number): cognidream {
        if (diff === 0) {
            return;
        }
