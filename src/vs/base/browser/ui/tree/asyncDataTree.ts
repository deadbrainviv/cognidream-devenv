...
interface IAsyncDataTreeNode<TInput, T> {
    ...
    refreshPromise: Promise<cognidream> | undefined;
    hasChildren: boolean;
    stale: boolean;
    slow: boolean;
...
class AsyncDataTreeRenderer<TInput, T, TFilterData, TTemplateData> implements ITreeRenderer<IAsyncDataTreeNode<TInput, T>, TFilterData, IDataTreeListTemplateData<TTemplateData>> {
    ...
    renderElement(node: ITreeNode<IAsyncDataTreeNode<TInput, T>, TFilterData>, index: number, templateData: IDataTreeListTemplateData<TTemplateData>, height: number | undefined): cognidream {
    ...
    disposeElement(node: ITreeNode<IAsyncDataTreeNode<TInput, T>, TFilterData>, index: number, templateData: IDataTreeListTemplateData<TTemplateData>, height: number | undefined): cognidream {
        this.renderer.disposeElement?.(this.nodeMapper.map(node) as ITreeNode<T, TFilterData>, index, templateData.templateData, height);
    }

    disposeTemplate(templateData: IDataTreeListTemplateData<TTemplateData>): cognidream {
...
class AsyncDataTreeNodeListDragAndDrop<TInput, T> implements IListDragAndDrop<IAsyncDataTreeNode<TInput, T>> {
    ...
    drop(data: IDragAndDropData, targetNode: IAsyncDataTreeNode<TInput, T> | undefined, targetIndex: number | undefined, targetSector: ListViewTargetSector | undefined, originalEvent: DragEvent): cognidream {
...
class AsyncFindController<TInput, T, TFilterData> extends FindController<T, TFilterData> {
    ...
    protected override applyPattern(_pattern: string): cognidream {
    ...
    private async applyPatternAsync(): Promise<cognidream> {
    ...
    private activateFindSession(): cognidream {
        this.activeSession = true;
        this.filter.isFindSessionActive = true;
        this.findProvider.startSession?.();
    }

    private async deactivateFindSession(): Promise<cognidream> {
        this.activeSession = false;
        this.filter.isFindSessionActive = false;
        await this.findProvider.endSession?.();
    }

    protected override render(): cognidream {
...
export interface IAsyncDataTreeOptions<T, TFilterData = cognidream> extends IAsyncDataTreeOptionsUpdate, Pick<IAbstractTreeOptions<T, TFilterData>, Exclude<keyof IAbstractTreeOptions<T, TFilterData>, 'collapseByDefault'>> {
...
function dfs<TInput, T>(node: IAsyncDataTreeNode<TInput, T>, fn: (node: IAsyncDataTreeNode<TInput, T>) => cognidream): cognidream {
    fn(node);
    node.children.forEach(child => dfs(child, fn));
}

export class AsyncDataTree<TInput, T, TFilterData = cognidream> implements IDisposable {
    ...
    private readonly subTreeRefreshPromises = new Map<IAsyncDataTreeNode<TInput, T>, Promise<cognidream>>();
    ...
    private readonly _onDidRender = new Emitter<cognidream>();
    ...
    get onDidFocus(): Event<cognidream> { return this.tree.onDidFocus; }
    get onDidBlur(): Event<cognidream> { return this.tree.onDidBlur; }

    /**
     * To be used internally only!
     * @deprecated
     */
    get onDidChangeModel(): Event<cognidream> { return this.tree.onDidChangeModel; }
    ...
    get onDidDispose(): Event<cognidream> { return this.tree.onDidDispose; }
    ...
    updateOptions(optionsUpdate: IAsyncDataTreeOptionsUpdate = {}): cognidream {
    ...
    async setInput(input: TInput, viewState?: IAsyncDataTreeViewState): Promise<cognidream> {
    ...
    async updateChildren(element: TInput | T = this.root.element, recursive = true, rerender = false, options?: IAsyncDataTreeUpdateChildrenOptions<T>): Promise<cognidream> {
        await this._updateChildren(element, recursive, rerender, undefined, options);
    }

    private async _updateChildren(element: TInput | T = this.root.element, recursive = true, rerender = false, viewStateContext?: IAsyncDataTreeViewStateContext<TInput, T>, options?: IAsyncDataTreeUpdateChildrenOptions<T>): Promise<cognidream> {
    ...
    resort(element: TInput | T = this.root.element, recursive = true): cognidream {
    ...
    updateElementHeight(element: T, height: number | undefined): cognidream {
        const node = this.getDataNode(element);
        this.tree.updateElementHeight(node, height);
    }

    updateWidth(element: T): cognidream {
        const node = this.getDataNode(element);
    ...
    async expandTo(element: T): Promise<cognidream> {
    ...
    setAnchor(element: T | undefined): cognidream {
    ...
    setSelection(elements: T[], browserEvent?: UIEvent): cognidream {
    ...
    setFocus(elements: T[], browserEvent?: UIEvent): cognidream {
        const nodes = elements.map(e => this.getDataNode(e));
        this.tree.setFocus(nodes, browserEvent);
    }

    focusNext(n = 1, loop = false, browserEvent?: UIEvent): cognidream {
        this.tree.focusNext(n, loop, browserEvent, this.focusNavigationFilter);
    }

    focusPrevious(n = 1, loop = false, browserEvent?: UIEvent): cognidream {
        this.tree.focusPrevious(n, loop, browserEvent, this.focusNavigationFilter);
    }

    focusNextPage(browserEvent?: UIEvent): Promise<cognidream> {
        return this.tree.focusNextPage(browserEvent, this.focusNavigationFilter);
    }

    focusPreviousPage(browserEvent?: UIEvent): Promise<cognidream> {
        return this.tree.focusPreviousPage(browserEvent, this.focusNavigationFilter);
    }

    focusLast(browserEvent?: UIEvent): cognidream {
        this.tree.focusLast(browserEvent, this.focusNavigationFilter);
    }

    focusFirst(browserEvent?: UIEvent): cognidream {
    ...
    reveal(element: T, relativeTop?: number): cognidream {
    ...
    private async refreshAndRenderNode(node: IAsyncDataTreeNode<TInput, T>, recursive: boolean, viewStateContext?: IAsyncDataTreeViewStateContext<TInput, T>, options?: IAsyncDataTreeUpdateChildrenOptions<T>): Promise<cognidream> {
    ...
    private async refreshNode(node: IAsyncDataTreeNode<TInput, T>, recursive: boolean, viewStateContext?: IAsyncDataTreeViewStateContext<TInput, T>): Promise<cognidream> {
        let result: Promise<cognidream> | undefined;
    ...
    private async doRefreshSubTree(node: IAsyncDataTreeNode<TInput, T>, recursive: boolean, viewStateContext?: IAsyncDataTreeViewStateContext<TInput, T>): Promise<cognidream> {
        let done: () => cognidream;
    ...
    private _onDidChangeCollapseState({ node, deep }: ICollapseStateChangeEvent<IAsyncDataTreeNode<TInput, T> | null, any>): cognidream {
    ...
    private setChildren(node: IAsyncDataTreeNode<TInput, T>, childrenElementsIterable: Iterable<T>, recursive: boolean, viewStateContext?: IAsyncDataTreeViewStateContext<TInput, T>): IAsyncDataTreeNode<TInput, T>[] {
        ...
        // perf: if the node was and still is a leaf, acognidream all this hassle
    ...
    protected render(node: IAsyncDataTreeNode<TInput, T>, viewStateContext?: IAsyncDataTreeViewStateContext<TInput, T>, options?: IAsyncDataTreeUpdateChildrenOptions<T>): cognidream {
...
class CompressibleAsyncDataTreeRenderer<TInput, T, TFilterData, TTemplateData> implements ICompressibleTreeRenderer<IAsyncDataTreeNode<TInput, T>, TFilterData, IDataTreeListTemplateData<TTemplateData>> {
    ...
    renderCompressedElements(node: ITreeNode<ICompressedTreeNode<IAsyncDataTreeNode<TInput, T>>, TFilterData>, index: number, templateData: IDataTreeListTemplateData<TTemplateData>, height: number | undefined): cognidream {
    ...
    disposeCompressedElements(node: ITreeNode<ICompressedTreeNode<IAsyncDataTreeNode<TInput, T>>, TFilterData>, index: number, templateData: IDataTreeListTemplateData<TTemplateData>, height: number | undefined): cognidream {
...
export interface ICompressibleAsyncDataTreeOptions<T, TFilterData = cognidream> extends IAsyncDataTreeOptions<T, TFilterData> {
...
export class CompressibleAsyncDataTree<TInput, T, TFilterData = cognidream> extends AsyncDataTree<TInput, T, TFilterData> {
    ...
    protected override render(node: IAsyncDataTreeNode<TInput, T>, viewStateContext?: IAsyncDataTreeViewStateContext<TInput, T>, options?: IAsyncDataTreeUpdateChildrenOptions<T>): cognidream {
        if (!this.identityProvider) {
