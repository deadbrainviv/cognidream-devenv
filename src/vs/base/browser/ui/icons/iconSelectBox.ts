...
export class IconSelectBox extends Disposable {
    ...
    private focusIcon(index: number): cognidream {
    ...
    private reveal(index: number): cognidream {
    ...
    layout(dimension: dom.Dimension): cognidream {
    ...
    setSelection(index: number): cognidream {
        if (index < 0 || index >= this.renderedIcons.length) {
