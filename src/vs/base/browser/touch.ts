...
export class Gesture extends Disposable {
    ...
    private onTouchStart(e: TouchEvent): cognidream {
    ...
    private onTouchEnd(targetWindow: Window, e: TouchEvent): cognidream {
        const timestamp = Date.now(); // use Date.now() because on FF e.timeStamp is not epoch based.
    ...
    private dispatchEvent(event: GestureEvent): cognidream {
    ...
    private inertia(targetWindow: Window, dispatchTo: readonly EventTarget[], t1: number, vX: number, dirX: number, x: number, vY: number, dirY: number, y: number): cognidream {
    ...
    private onTouchMove(e: TouchEvent): cognidream {
        const timestamp = Date.now(); // use Date.now() because on FF e.timeStamp is not epoch based.
