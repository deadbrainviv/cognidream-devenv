...
class WrappedStyleElement {
    ...
    public setStyle(cssStyle: string): cognidream {
...
export function createStyleSheet(container: HTMLElement = mainWindow.document.head, beforeAppend?: (style: HTMLStyleElement) => cognidream, disposableStore?: DisposableStore): HTMLStyleElement {
...
export function createCSSRule(selector: string, cssText: string, style = getSharedStyleSheet()): cognidream {
    if (!style || !cssText) {
        return;
    }
...
export function removeCSSRulesContainingSelector(ruleName: string, style = getSharedStyleSheet()): cognidream {
    if (!style) {
        return;
    }
