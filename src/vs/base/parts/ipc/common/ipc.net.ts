...
export namespace SocketDiagnostics {
    ...
    export function traceSocketEvent(nativeObject: any, socketDebugLabel: string, type: SocketDiagnosticsEventType, data?: VSBuffer | Uint8Array | ArrayBuffer | ArrayBufferView | any): cognidream {
...
export interface ISocket extends IDisposable {
    onData(listener: (e: VSBuffer) => cognidream): IDisposable;
    onClose(listener: (e: SocketCloseEvent) => cognidream): IDisposable;
    onEnd(listener: () => cognidream): IDisposable;
...
export const enum ProtocolConstants {
    ...
    /**
     * Send a message every 5 seconds to acognidream that the connection is closed by the OS.
     */
...
class ProtocolReader extends Disposable {
    ...
    public acceptChunk(data: VSBuffer | null): cognidream {
...
class ProtocolWriter {
    ...
    private _writeSoon(header: VSBuffer, data: VSBuffer): cognidream {
    ...
    private _scheduleWriting(): cognidream {
...
export class Client<TContext = string> extends IPCClient<TContext> {
    ...
    get onDidDispose(): Event<cognidream> { return this.protocol.onDidDispose; }
...
export class BufferedEmitter<T> {
    ...
    private _deliverMessages(): cognidream {
    ...
    public fire(event: T): cognidream {
...
export class PersistentProtocol implements IMessagePassingProtocol {
    ...
    private readonly _onDidDispose = new BufferedEmitter<cognidream>();
    readonly onDidDispose: Event<cognidream> = this._onDidDispose.event;
    ...
    public beginAcceptReconnection(socket: ISocket, initialDataChunk: VSBuffer | null): cognidream {
    ...
    public endAcceptReconnection(): cognidream {
        this._isReconnecting = false;
    ...
    public acceptDisconnect(): cognidream {
        this._onDidDispose.fire();
    }

    private _receiveMessage(msg: ProtocolMessage): cognidream {
     ...
     */
    sendControl(buffer: VSBuffer): cognidream {
        const msg = new ProtocolMessage(ProtocolMessageType.Control, 0, 0, buffer);
        this._socketWriter.write(msg);
    }

    private _sendAckCheck(): cognidream {
    ...
    private _recvAckCheck(): cognidream {
    ...
    private _sendKeepAlive(): cognidream {
        this._incomingAckId = this._incomingMsgId;
