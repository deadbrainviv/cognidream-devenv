...
interface IHandler {
    (response: IRawResponse): cognidream;
}

export interface IMessagePassingProtocol {
    send(buffer: VSBuffer): cognidream;
...
export interface IChannelServer<TContext = string> {
    registerChannel(channelName: string, channel: IServerChannel<TContext>): cognidream;
}
...
interface IWriter {
    write(buffer: VSBuffer): cognidream;
}
...
export function serialize(writer: IWriter, data: any): cognidream {
...
export class ChannelServer<TContext = string> implements IChannelServer<TContext>, IDisposable {
    ...
    private sendResponse(response: IRawResponse): cognidream {
    ...
    private onRawMessage(message: VSBuffer): cognidream {
    ...
    private onPromise(request: IRawPromiseRequest): cognidream {
    ...
    private onEventListen(request: IRawEventListenRequest): cognidream {
        const channel = this.channels.get(request.channelName);
    ...
    private disposeActiveRequest(request: IRawRequest): cognidream {
    ...
    private collectPendingRequest(request: IRawPromiseRequest | IRawEventListenRequest): cognidream {
    ...
    private flushPendingRequests(channelName: string): cognidream {
...
export interface IIPCLogger {
    logIncoming(msgLength: number, requestId: number, initiator: RequestInitiator, str: string, data?: any): cognidream;
    logOutgoing(msgLength: number, requestId: number, initiator: RequestInitiator, str: string, data?: any): cognidream;
}
...
export class ChannelClient implements IChannelClient, IDisposable {
    ...
    private readonly _onDidInitialize = new Emitter<cognidream>();
    ...
    private requestPromise(channelName: string, name: string, arg?: any, cancellationToken = CancellationToken.None): Promise<unknown> {
        ...
        const result = new Promise((c, e) => {
            ...
            let uninitializedPromise: CancelablePromise<cognidream> | null = null;
    ...
    private sendRequest(request: IRawRequest): cognidream {
        switch (request.type) {
            case RequestType.Promise:
            case RequestType.EventListen: {
    ...
    private onBuffer(message: VSBuffer): cognidream {
        const reader = new BufferReader(message);
        const header = deserialize(reader);
        const body = deserialize(reader);
    ...
    private onResponse(response: IRawResponse): cognidream {
    ...
    @memoize
    get onDidInitializePromise(): Promise<cognidream> {
        return Event.toPromise(this.onDidInitialize);
    }

    private whenInitialized(): Promise<cognidream> {
        if (this.state === State.Idle) {
...
export interface ClientConnectionEvent {
    ...
    onDidClientDisconnect: Event<cognidream>;
}
...
function logWithColors(direction: string, totalLength: number, msgLength: number, req: number, initiator: RequestInitiator, str: string, data: any): cognidream {
    data = pretty(data);
...
export class IPCLogger implements IIPCLogger {
    ...
    public logOutgoing(msgLength: number, requestId: number, initiator: RequestInitiator, str: string, data?: any): cognidream {
        this._totalOutgoing += msgLength;
        logWithColors(this._outgoingPrefix, this._totalOutgoing, msgLength, requestId, initiator, str, data);
    }

    public logIncoming(msgLength: number, requestId: number, initiator: RequestInitiator, str: string, data?: any): cognidream {
