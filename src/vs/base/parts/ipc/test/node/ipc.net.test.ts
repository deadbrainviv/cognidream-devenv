...
class MessageStream extends Disposable {
    ...
    private _currentComplete: ((data: VSBuffer) => cognidream) | null;
...
class Ether {
    ...
    public write(from: 'a' | 'b', data: Buffer): cognidream {
        setTimeout(() => {
            if (from === 'a') {
                this._ab.push(data);
            } else {
...
flakySuite('IPC, create handle', () => {
    ...
    function testIPCHandle(handle: string): Promise<cognidream> {
...
suite('WebSocketNodeSocket', () => {
    ...
    class FakeNodeSocket extends Disposable {
        ...
        public write(data: VSBuffer): cognidream {
            this.writtenData.push(data);
        }

        public fireData(data: number[]): cognidream {
    ...
    function listenOnRandomPort(handler: (socket: Socket) => cognidream): Promise<Server> {
