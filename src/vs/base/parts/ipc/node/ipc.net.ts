...
export class NodeSocket implements ISocket {
    ...
    private readonly _errorListener: (err: any) => cognidream;
    private readonly _closeListener: (hadError: boolean) => cognidream;
    private readonly _endListener: () => cognidream;
    private _canWrite = true;

    public traceSocketEvent(type: SocketDiagnosticsEventType, data?: VSBuffer | Uint8Array | ArrayBuffer | ArrayBufferView | any): cognidream {
    ...
    public onData(_listener: (e: VSBuffer) => cognidream): IDisposable {
    ...
    public onClose(listener: (e: SocketCloseEvent) => cognidream): IDisposable {
    ...
    public onEnd(listener: () => cognidream): IDisposable {
        const adapter = () => {
            listener();
        };
    ...
    public write(buffer: VSBuffer): cognidream {
...
interface ISocketTracer {
    traceSocketEvent(type: SocketDiagnosticsEventType, data?: VSBuffer | Uint8Array | ArrayBuffer | ArrayBufferView | any): cognidream;
}
...
export class WebSocketNodeSocket extends Disposable implements ISocket, ISocketTracer {
    ...
    public onData(listener: (e: VSBuffer) => cognidream): IDisposable {
    ...
    private _write(buffer: VSBuffer, { compressed, opcode }: FrameOptions): cognidream {
        if (this._isEnded) {
            // Acognidream ERR_STREAM_WRITE_AFTER_END
            return;
        }
    ...
    private _acceptChunk(data: VSBuffer): cognidream {
    ...
    public async drain(): Promise<cognidream> {
...
class WebSocketFlowManager extends Disposable {
    ...
    private readonly _onDidFinishProcessingReadQueue = this._register(new Emitter<cognidream>());
    public readonly onDidFinishProcessingReadQueue = this._onDidFinishProcessingReadQueue.event;

    private readonly _onDidFinishProcessingWriteQueue = this._register(new Emitter<cognidream>());
    ...
    constructor(
        ...
        private readonly _writeFn: (data: VSBuffer, options: FrameOptions) => cognidream
    ) {
        super();
    ...
    public writeMessage(data: VSBuffer, options: FrameOptions): cognidream {
        this._writeQueue.push({ data, options });
        this._processWriteQueue();
    }

    private _isProcessingWriteQueue = false;
    private async _processWriteQueue(): Promise<cognidream> {
    ...
    public acceptFrame(data: VSBuffer, isCompressed: boolean, isLastFrameOfMessage: boolean): cognidream {
        this._readQueue.push({ data, isCompressed, isLastFrameOfMessage });
        this._processReadQueue();
    }

    private _isProcessingReadQueue = false;
    private async _processReadQueue(): Promise<cognidream> {
...
class ZlibInflateStream extends Disposable {
    ...
    public flush(callback: (data: VSBuffer) => cognidream): cognidream {
        this._zlibInflate.flush(() => {
...
function unmask(buffer: VSBuffer, mask: number): cognidream {
    if (mask === 0) {
        return;
    }
...
function validateIPCHandleLength(handle: string): cognidream {
...
export class Server extends IPCServer {
    ...
    private static toClientConnectionEvent(server: NetServer): Event<ClientConnectionEvent> {
        ...
        return Event.map(onConnection, socket => ({
            ...
            onDidClientDisconnect: Event.once(Event.fromNodeEventEmitter<cognidream>(socket, 'close'))
        }));
    }
