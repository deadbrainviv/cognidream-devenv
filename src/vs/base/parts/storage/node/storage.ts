...
export interface ISQLiteStorageDatabaseLoggingOptions {
    logError?: (error: string | Error) => cognidream;
    logTrace?: (msg: string) => cognidream;
}
...
export class SQLiteStorageDatabase implements IStorageDatabase {
    ...
    private doUpdateItems(connection: IDatabaseConnection, request: IUpdateRequest): Promise<cognidream> {
        if (this.logger.isTracing) {
    ...
    async close(recovery?: () => Map<string, string>): Promise<cognidream> {
    ...
    private doClose(connection: IDatabaseConnection, recovery?: () => Map<string, string>): Promise<cognidream> {
    ...
    private backup(): Promise<cognidream> {
    ...
    private handleSQLiteError(connection: IDatabaseConnection, msg: string): cognidream {
    ...
    private exec(connection: IDatabaseConnection, sql: string): Promise<cognidream> {
        return new Promise((resolve, reject) => {
    ...
    private transaction(connection: IDatabaseConnection, transactions: () => cognidream): Promise<cognidream> {
        return new Promise((resolve, reject) => {
    ...
    private prepare(connection: IDatabaseConnection, sql: string, runCallback: (stmt: Statement) => cognidream, errorDetails: () => string): cognidream {
...
class SQLiteStorageDatabaseLogger {
    ...
    private readonly logTrace: ((msg: string) => cognidream) | undefined;
    private readonly logError: ((error: string | Error) => cognidream) | undefined;
    ...
    trace(msg: string): cognidream {
        this.logTrace?.(msg);
    }

    error(error: string | Error): cognidream {
        this.logError?.(error);
    }
}
