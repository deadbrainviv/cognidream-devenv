...
export interface IStorageDatabase {
    ...
    updateItems(request: IUpdateRequest): Promise<cognidream>;

    optimize(): Promise<cognidream>;

    close(recovery?: () => Map<string, string>): Promise<cognidream>;
}
...
export interface IStorage extends IDisposable {
    ...
    set(key: string, value: StorageValue, external?: boolean): Promise<cognidream>;
    delete(key: string, external?: boolean): Promise<cognidream>;

    flush(delay?: number): Promise<cognidream>;
    whenFlushed(): Promise<cognidream>;

    optimize(): Promise<cognidream>;

    close(): Promise<cognidream>;
}
...
export class Storage extends Disposable implements IStorage {
    ...
    private readonly flushDelayer = this._register(new ThrottledDelayer<cognidream>(Storage.DEFAULT_FLUSH_DELAY));

    private pendingDeletes = new Set<string>();
    private pendingInserts = new Map<string, string>();

    private pendingClose: Promise<cognidream> | undefined = undefined;
    ...
    private onDidChangeItemsExternal(e: IStorageItemsChangeEvent): cognidream {
    ...
    private acceptExternal(key: string, value: string | undefined): cognidream {
    ...
    async set(key: string, value: string | boolean | number | null | undefined | object, external = false): Promise<cognidream> {
        if (this.state === StorageState.Closed) {
            return; // Return early if we are already closed
        }
    ...
    async delete(key: string, external = false): Promise<cognidream> {
        if (this.state === StorageState.Closed) {
            return; // Return early if we are already closed
        }
    ...
    async optimize(): Promise<cognidream> {
        if (this.state === StorageState.Closed) {
            return; // Return early if we are already closed
        }
    ...
    private async doClose(): Promise<cognidream> {
        ...
        // the DB is closed to acognidream corruption.
        //
    ...
    private async flushPending(): Promise<cognidream> {
        if (!this.hasPending) {
    ...
    async flush(delay?: number): Promise<cognidream> {
        if (
    ...
    private async doFlush(delay?: number): Promise<cognidream> {
    ...
    async whenFlushed(): Promise<cognidream> {
        if (!this.hasPending) {
            return; // return early if nothing to do
        }
...
export class InMemoryStorageDatabase implements IStorageDatabase {
    ...
    async updateItems(request: IUpdateRequest): Promise<cognidream> {
