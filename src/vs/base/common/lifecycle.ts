...
export interface IDisposableTracker {
    ...
    */
    trackDisposable(disposable: IDisposable): cognidream;

    /**
     * Is called when a disposable is registered as child of another disposable (e.g. {@link DisposableStore}).
     * If parent is `null`, the disposable is removed from its former parent.
    */
    setParent(child: IDisposable, parent: IDisposable | null): cognidream;

    /**
     * Is called after a disposable is disposed.
    */
    markAsDisposed(disposable: IDisposable): cognidream;
...
export class DisposableTracker implements IDisposableTracker {
    ...
    trackDisposable(d: IDisposable): cognidream {
...
export function setDisposableTracker(tracker: IDisposableTracker | null): cognidream {
...
export function markAsDisposed(disposable: IDisposable): cognidream {
    disposableTracker?.markAsDisposed(disposable);
}

function setParentOfDisposable(child: IDisposable, parent: IDisposable | null): cognidream {
    disposableTracker?.setParent(child, parent);
}

function setParentOfDisposables(children: IDisposable[], parent: IDisposable | null): cognidream {
 ...
 */
export function toDisposable(fn: () => cognidream): IDisposable {
...
export class DisposableStore implements IDisposable {
     ...
     */
    public delete<T extends IDisposable>(o: T): cognidream {
        if (!o) {
            return;
        }
        if ((o as unknown as DisposableStore) === this) {
     ...
     */
    public deleteAndLeak<T extends IDisposable>(o: T): cognidream {
        if (!o) {
            return;
        }
...
export abstract class ReferenceCollection<T> {
    ...
    protected abstract destroyReferencedObject(key: string, object: T): cognidream;
}
...
export function disposeOnReturn(fn: (store: DisposableStore) => cognidream): cognidream {
...
export class DisposableMap<K, V extends IDisposable = IDisposable> implements IDisposable {
    ...
    set(key: K, value: V, skipDisposeOnOverwrite = false): cognidream {
        if (this._isDisposed) {
     ...
     */
    deleteAndDispose(key: K): cognidream {
        this._store.get(key)?.dispose();
 ...
 */
export function thenIfNotDisposed<T>(promise: Promise<T>, then: (result: T) => cognidream): IDisposable {
    let disposed = false;
    promise.then(result => {
        if (disposed) {
            return;
