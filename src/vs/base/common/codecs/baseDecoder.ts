...
> extends ObservableDisposable implements ReadableStream<T> {
    ...
    private readonly _onEnd = this._register(new Emitter<cognidream>());
     ...
     */
    protected abstract onStreamData(data: K): cognidream;
     ...
     */
    private settledPromise = new DeferredPromise<cognidream>();
     ...
     */
    public get settled(): Promise<cognidream> {
     ...
     */
    private tryOnStreamData(data: K): cognidream {
        try {
    ...
    public on(event: 'data', callback: (data: T) => cognidream): cognidream;
    public on(event: 'error', callback: (err: Error) => cognidream): cognidream;
    public on(event: 'end', callback: () => cognidream): cognidream;
    public on(event: TStreamListenerNames, callback: unknown): cognidream {
        if (event === 'data') {
            return this.onData(callback as (data: T) => cognidream);
        }

        if (event === 'error') {
            return this.onError(callback as (error: Error) => cognidream);
        }

        if (event === 'end') {
            return this.onEnd(callback as () => cognidream);
        }
     ...
     * @throws if the decoder stream has already ended.
     */
    public onData(callback: (data: T) => cognidream): cognidream {
        assert(
            !this.ended,
     ...
     * @throws if the decoder stream has already ended.
     */
    public onError(callback: (error: Error) => cognidream): cognidream {
        assert(
            !this.ended,
     ...
     * @throws if the decoder stream has already ended.
     */
    public onEnd(callback: () => cognidream): cognidream {
        assert(
            !this.ended,
     ...
     */
    public removeAllListeners(): cognidream {
     ...
     */
    public removeListener(event: string, callback: Function): cognidream {
     ...
     */
    protected onStreamEnd(): cognidream {
        if (this._ended) {
            return;
        }
     ...
     */
    protected onStreamError(error: Error): cognidream {
