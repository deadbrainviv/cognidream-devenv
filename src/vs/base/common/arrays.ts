...
export function forEachAdjacent<T>(arr: T[], f: (item1: T | undefined, item2: T | undefined) => cognidream): cognidream {
    for (let i = 0; i <= arr.length; i++) {
        f(i === 0 ? undefined : arr[i - 1], i === arr.length ? undefined : arr[i]);
    }
}

export function forEachWithNeighbors<T>(arr: T[], f: (before: T | undefined, element: T, after: T | undefined) => cognidream): cognidream {
    for (let i = 0; i < arr.length; i++) {
...
export function sortedDiff<T>(before: ReadonlyArray<T>, after: ReadonlyArray<T>, compare: (a: T, b: T) => number): ISplice<T>[] {
    ...
    function pushSplice(start: number, deleteCount: number, toInsert: T[]): cognidream {
...
function topStep<T>(array: ReadonlyArray<T>, compare: (a: T, b: T) => number, result: T[], i: number, m: number): cognidream {
 ...
 */
export function move(array: unknown[], from: number, to: number): cognidream {
 ...
 */
export function insert<T>(array: T[], element: T): () => cognidream {
    array.push(element);
 ...
 */
export function shuffle<T>(array: T[], _seed?: number): cognidream {
    let rand: () => number;
 ...
 */
export function pushToStart<T>(arr: T[], value: T): cognidream {
 ...
 */
export function pushToEnd<T>(arr: T[], value: T): cognidream {
    const index = arr.indexOf(value);
...
export function pushMany<T>(arr: T[], items: ReadonlyArray<T>): cognidream {
    for (const item of items) {
        arr.push(item);
    }
}
 ...
 */
export function insertInto<T>(array: T[], start: number, newItems: T[]): cognidream {
...
export class CallbackIterable<T> {
    ...
    constructor(
        ...
        */
        public readonly iterate: (callback: (item: T) => boolean) => cognidream
    ) {
    }

    forEach(handler: (item: T) => cognidream) {
