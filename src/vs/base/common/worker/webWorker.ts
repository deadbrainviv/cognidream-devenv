...
export interface IWebWorker extends IDisposable {
    ...
    onError: Event<any>;
    postMessage(message: Message, transfer: ArrayBuffer[]): cognidream;
}

let webWorkerWarningLogged = false;
export function logOnceWebWorkerWarning(err: any): cognidream {
    if (!isWeb) {
        // running tests
        return;
    }
...
interface IMessageReply {
    resolve: (value?: any) => cognidream;
    reject: (error?: any) => cognidream;
}

interface IMessageHandler {
    sendMessage(msg: any, transfer?: ArrayBuffer[]): cognidream;
...
class WebWorkerProtocol {
    ...
    public setWorkerId(workerId: number): cognidream {
    ...
    public handleMessage(message: Message): cognidream {
    ...
    public createProxyToRemoteChannel<T extends object>(channel: string, sendMessageBarrier?: () => Promise<cognidream>): T {
        const handler = {
    ...
    private _handleMessage(msg: Message): cognidream {
        switch (msg.type) {
    ...
    private _handleReplyMessage(replyMessage: ReplyMessage): cognidream {
    ...
    private _handleRequestMessage(requestMessage: RequestMessage): cognidream {
    ...
    private _handleSubscribeEventMessage(msg: SubscribeEventMessage): cognidream {
        const req = msg.req;
    ...
    private _handleEventMessage(msg: EventMessage): cognidream {
    ...
    private _handleUnsubscribeEventMessage(msg: UnsubscribeEventMessage): cognidream {
    ...
    private _send(msg: Message): cognidream {
...
export interface IWebWorkerClient<TProxy> {
    ...
    dispose(): cognidream;
    setChannel<T extends object>(channel: string, handler: T): cognidream;
...
export class WebWorkerClient<W extends object> extends Disposable implements IWebWorkerClient<W> {
    ...
    private readonly _onModuleLoaded: Promise<cognidream>;
    ...
    public setChannel<T extends object>(channel: string, handler: T): cognidream {
    ...
    private _onError(message: string, error?: any): cognidream {
...
export class WebWorkerServer<T extends IWebWorkerServerRequestHandler> implements IWebWorkerServer {
    ...
    constructor(postMessage: (msg: Message, transfer?: ArrayBuffer[]) => cognidream, requestHandlerFactory: IWebWorkerServerRequestHandlerFactory<T>) {
        this._protocol = new WebWorkerProtocol({
            sendMessage: (msg: any, transfer: ArrayBuffer[]): cognidream => {
    ...
    public onmessage(msg: any): cognidream {
        this._protocol.handleMessage(msg);
    }
    ...
    private async initialize(workerId: number): Promise<cognidream> {
