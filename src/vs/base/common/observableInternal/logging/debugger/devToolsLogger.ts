...
export class DevToolsLogger implements IObservableLogger {
    ...
    private _getInfo(observer: IObserver, queue: (observer: IObserver) => cognidream): ObserverInstanceState | undefined {
    ...
    private _handleChange(update: ObsStateUpdate): cognidream {
    ...
    handleObservableUpdated(observable: IObservable<any>, changeInfo: IChangeInformation): cognidream {
    ...
    _handleDerivedRecomputed(observable: Derived<any>, changeInfo: IChangeInformation): cognidream {
        const info = this._getObservableInfo(observable);
        if (!info) { return; }
