...
export class Debouncer implements IDisposable {
    ...
    public debounce(fn: () => cognidream, timeoutMs: number): cognidream {
...
export class Throttler implements IDisposable {
    ...
    public throttle(fn: () => cognidream, timeoutMs: number): cognidream {
        if (this._timeout === undefined) {
            this._timeout = setTimeout(() => {
                this._timeout = undefined;
                fn();
...
export function deepAssign<T>(target: T, source: T): cognidream {
...
export function deepAssignDeleteNulls<T>(target: T, source: T): cognidream {
    for (const key in source) {
        if (source[key] === null) {
            delete target[key];
