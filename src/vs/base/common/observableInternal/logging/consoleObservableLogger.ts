...
export function logObservableToConsole(obs: IObservable<any>): cognidream {
...
export class ConsoleObservableLogger implements IObservableLogger {
    ...
    public addFilteredObj(obj: unknown): cognidream {
    ...
    handleObservableUpdated(observable: IObservable<unknown>, info: IChangeInformation): cognidream {
    ...
    _handleDerivedRecomputed(derived: Derived<unknown>, info: IChangeInformation): cognidream {
        if (!this._isIncluded(derived)) { return; }
    ...
    handleDerivedCleared(derived: Derived<unknown>): cognidream {
        if (!this._isIncluded(derived)) { return; }
    ...
    handleFromEventObservableTriggered(observable: FromEventObservable<any, any>, info: IChangeInformation): cognidream {
        if (!this._isIncluded(observable)) { return; }
    ...
    handleEndTransaction(): cognidream {
        this.indentation--;
    }
}
...
function consoleTextToArgs(text: ConsoleText): unknown[] {
    ...
    function process(t: ConsoleText): cognidream {
        if ('length' in t) {
            for (const item of t) {
                if (item) {
                    process(item);
