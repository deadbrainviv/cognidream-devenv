...
class ConstObservable<T> extends ConvenientObservable<T, cognidream> {
    ...
    override log(): IObservableWithChange<T, cognidream> {
        return this;
    }
...
export class FromEventObservable<TArgs, T> extends BaseObservable<T> {
    ...
    protected override onFirstObserverAdded(): cognidream {
    ...
    protected override onLastObserverRemoved(): cognidream {
...
export namespace observableFromEvent {
    ...
    export function batchEventsGlobally(tx: ITransaction, fn: () => cognidream): cognidream {
        let didSet = false;
...
class FromEventObservableSignal extends BaseObservable<cognidream> {
    ...
    public override get(): cognidream {
        // NO OP
    }
}
 ...
 */
export function observableSignal<TDelta = cognidream>(debugName: string): IObservableSignal<TDelta>;
export function observableSignal<TDelta = cognidream>(owner: object): IObservableSignal<TDelta>;
export function observableSignal<TDelta = cognidream>(debugNameOrOwner: string | object): IObservableSignal<TDelta> {
...
export interface IObservableSignal<TChange> extends IObservableWithChange<cognidream, TChange> {
    trigger(tx: ITransaction | undefined, change: TChange): cognidream;
}

class ObservableSignal<TChange> extends BaseObservable<cognidream, TChange> implements IObservableSignal<TChange> {
    ...
    public trigger(tx: ITransaction | undefined, change: TChange): cognidream {
        if (!tx) {
            transaction(tx => {
...
export function signalFromObservable<T>(owner: DebugOwner | undefined, observable: IObservable<T>): IObservable<cognidream> {
    return derivedOpts({
        owner,
        equalsFn: () => false,
    }, reader => {
...
export function debouncedObservable<T>(observable: IObservable<T>, debounceMs: number): IObservable<T> {
    ...
    return observableFromEvent<T, cognidream>(cb => {
 ...
 */
export function recomputeInitiallyAndOnChange<T>(observable: IObservable<T>, handleValue?: (value: T) => cognidream): IDisposable {
...
export class KeepAliveObserver implements IObserver {
    ...
    constructor(
        ...
        private readonly _handleValue: ((value: any) => cognidream) | undefined,
    ) { }
...
export function derivedObservableWithWritableCache<T>(owner: object, computeFn: (reader: IReader, lastValue: T | undefined) => T): IObservable<T>
    & { clearCache(transaction: ITransaction): cognidream; setCache(newValue: T | undefined, tx: ITransaction | undefined): cognidream } {
    let lastValue: T | undefined = undefined;
...
class ArrayMap<TIn, TOut, TKey> implements IDisposable {
    ...
    public setItems(items: readonly TIn[]): cognidream {
        const newItems: TOut[] = [];
...
export class ValueWithChangeEventFromObservable<T> implements IValueWithChangeEvent<T> {
    ...
    get onDidChange(): Event<cognidream> {
...
export function latestChangedValue<T extends IObservable<any>[]>(owner: DebugOwner, observables: T): IObservable<ReturnType<T[number]['get']>> {
    ...
    const result = observableFromEvent<any, cognidream>(owner, cb => {
        const store = new DisposableStore();
...
export function runOnChange<T, TChange>(observable: IObservableWithChange<T, TChange>, cb: (value: T, previousValue: undefined | T, deltas: RemoveUndefined<TChange>[]) => cognidream): IDisposable {
...
export function runOnChangeWithStore<T, TChange>(observable: IObservableWithChange<T, TChange>, cb: (value: T, previousValue: undefined | T, deltas: RemoveUndefined<TChange>[], store: DisposableStore) => cognidream): IDisposable {
    const store = new DisposableStore();
