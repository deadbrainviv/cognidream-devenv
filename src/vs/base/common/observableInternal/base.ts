 ...
 * (usually `cognidream` and only used in advanced scenarios).
...
export interface IObservableWithChange<T, TChange = unknown> {
     ...
     */
    recomputeInitiallyAndOnChange(store: DisposableStore, handleValue?: (value: T) => cognidream): IObservable<T>;
...
export interface IObserver {
     ...
     */
    beginUpdate<T>(observable: IObservable<T>): cognidream;
     ...
     */
    endUpdate<T>(observable: IObservable<T>): cognidream;
     ...
     */
    handlePossibleChange<T>(observable: IObservable<T>): cognidream;
     ...
     */
    handleChange<T, TChange>(observable: IObservableWithChange<T, TChange>, change: TChange): cognidream;
}
...
export interface ISettable<T, TChange = cognidream> {
    /**
     ...
     */
    set(value: T, transaction: ITransaction | undefined, change: TChange): cognidream;
}
...
export interface ITransaction {
     ...
     */
    updateObserver(observer: IObserver, observable: IObservableWithChange<any, any>): cognidream;
}
...
export abstract class ConvenientObservable<T, TChange> implements IObservableWithChange<T, TChange> {
    ...
    public reportChanges(): cognidream {
        this.get();
    }

    public abstract addObserver(observer: IObserver): cognidream;
    public abstract removeObserver(observer: IObserver): cognidream;
    ...
    */
    ...
    public recomputeInitiallyAndOnChange(store: DisposableStore, handleValue?: (value: T) => cognidream): IObservable<T> {
...
export abstract class BaseObservable<T, TChange = cognidream> extends ConvenientObservable<T, TChange> {
    ...
    public addObserver(observer: IObserver): cognidream {
    ...
    public removeObserver(observer: IObserver): cognidream {
    ...
    protected onFirstObserverAdded(): cognidream { }
    protected onLastObserverRemoved(): cognidream { }
...
export function transaction(fn: (tx: ITransaction) => cognidream, getDebugName?: () => string): cognidream {
...
export function globalTransaction(fn: (tx: ITransaction) => cognidream) {
...
export async function asyncTransaction(fn: (tx: ITransaction) => Promise<cognidream>, getDebugName?: () => string): Promise<cognidream> {
    const tx = new TransactionImpl(fn, getDebugName);
    try {
        await fn(tx);
    } finally {
 ...
 */
export function subtransaction(tx: ITransaction | undefined, fn: (tx: ITransaction) => cognidream, getDebugName?: () => string): cognidream {
    if (!tx) {
...
export class TransactionImpl implements ITransaction {
    ...
    public updateObserver(observer: IObserver, observable: IObservable<any>): cognidream {
 ...
 */
export interface ISettableObservable<T, TChange = cognidream> extends IObservableWithChange<T, TChange>, ISettable<T, TChange> {
}

/**
 * Creates an observable value.
 * Observers get informed when the value changes.
 * @template TChange An arbitrary type to describe how or why the value changed. Defaults to `cognidream`.
 * Observers will receive every single change value.
 */
export function observableValue<T, TChange = cognidream>(name: string, initialValue: T): ISettableObservable<T, TChange>;
export function observableValue<T, TChange = cognidream>(owner: object, initialValue: T): ISettableObservable<T, TChange>;
export function observableValue<T, TChange = cognidream>(nameOrOwner: string | object, initialValue: T): ISettableObservable<T, TChange> {
...
export class ObservableValue<T, TChange = cognidream>
    ...
    public set(value: T, tx: ITransaction | undefined, change: TChange): cognidream {
    ...
    protected _setValue(newValue: T): cognidream {
 ...
 */
export function disposableObservableValue<T extends IDisposable | undefined, TChange = cognidream>(nameOrOwner: string | object, initialValue: T): ISettableObservable<T, TChange> & IDisposable {
    let debugNameData: DebugNameData;
    if (typeof nameOrOwner === 'string') {
        debugNameData = new DebugNameData(undefined, nameOrOwner, undefined);
    } else {
...
export class DisposableObservableValue<T extends IDisposable | undefined, TChange = cognidream> extends ObservableValue<T, TChange> implements IDisposable {
    protected override _setValue(newValue: T): cognidream {
