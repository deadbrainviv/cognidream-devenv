...
export namespace Event {
     ...
     * `setTimeout`. The event is converted into a signal (`Event<cognidream>`) to acognidream additional object creation as a
     ...
     */
    export function defer(event: Event<unknown>, disposable?: DisposableStore): Event<cognidream> {
        return debounce<unknown, cognidream>(event, () => cognidream 0, 0, undefined, true, undefined, disposable);
    }
     ...
     * @param disposable A disposable store to add the new EventEmitter to.
     */
    export function forEach<I>(event: Event<I>, each: (i: I) => cognidream, disposable?: DisposableStore): Event<I> {
    ...
    /**
     * Given an event, returns the same event but typed as `Event<cognidream>`.
     */
    export function signal<T>(event: Event<T>): Event<cognidream> {
        return event as Event<any> as Event<cognidream>;
    }
    ...
    export function any(...events: Event<any>[]): Event<cognidream>;
    export function any<T>(...events: Event<T>[]): Event<T> {
        return (listener, thisArgs = null, disposables?) => {
    ...
    export function debounce<I, O>(event: Event<I>, merge: (last: O | undefined, event: I) => O, delay: number | typeof MicrotaskDelay = 100, leading = false, flushOnListenerRemove = false, leakWarningThreshold?: number, disposable?: DisposableStore): Event<O> {
        ...
        let doFire: (() => cognidream) | undefined;
    ...
    class ChainableSynthesis implements IChainableSythensis<any> {
        ...
        forEach(fn: (i: any) => cognidream): this {
            this.steps.push(v => {
                fn(v);
                return v;
            });
    ...
    export interface IChainableSythensis<T> {
        ...
        forEach(fn: (i: T) => cognidream): IChainableSythensis<T>;
    ...
    export interface DOMEventEmitter {
        addEventListener(event: string | symbol, listener: Function): cognidream;
        removeEventListener(event: string | symbol, listener: Function): cognidream;
    }
    ...
    class EmitterObserver<T> implements IObserver {
        ...
        beginUpdate<T>(_observable: IObservable<T>): cognidream {
            // assert(_observable === this.obs);
            this._counter++;
        }

        handlePossibleChange<T>(_observable: IObservable<T>): cognidream {
            // assert(_observable === this.obs);
        }

        handleChange<T, TChange>(_observable: IObservableWithChange<T, TChange>, _change: TChange): cognidream {
            // assert(_observable === this.obs);
            this._hasChanged = true;
        }

        endUpdate<T>(_observable: IObservable<T>): cognidream {
            // assert(_observable === this.obs);
            this._counter--;
            if (this._counter === 0) {
                this._observable.reportChanges();
     ...
     */
    export function fromObservableLight(observable: IObservable<unknown>): Event<cognidream> {
...
export interface EmitterOptions {
     ...
     */
    onListenerError?: (e: any) => cognidream;
    /**
...
export class EventProfiling {
    ...
    start(listenerCount: number): cognidream {
...
class LeakageMonitor {
    ...
    constructor(
        private readonly _errorHandler: (err: Error) => cognidream,
    ...
    check(stack: Stacktrace, listenerCount: number): undefined | (() => cognidream) {
...
type ListenerContainer<T> = UniqueContainer<(data: T) => cognidream>;
type ListenerOrListeners<T> = (ListenerContainer<T> | undefined)[] | ListenerContainer<T>;

const forEachListener = <T>(listeners: ListenerOrListeners<T>, fn: (c: ListenerContainer<T>) => cognidream) => {
...
export class Emitter<T> {
     ...
     * for event emitters (#185789), so we optimize that special case to acognidream
     ...
     *
     * The array listeners can be 'sparse', to acognidream reallocating the array
    ...
    private _deliver(listener: undefined | UniqueContainer<(value: T) => cognidream>, value: T) {
        if (!listener) {
            return;
        }
...
export class AsyncEmitter<T extends IWaitUntil> extends Emitter<T> {
    ...
    private _asyncDeliveryQueue?: LinkedList<[(ev: T) => cognidream, IWaitUntilData<T>]>;

    async fireAsync(data: IWaitUntilData<T>, token: CancellationToken, promiseJoin?: (p: Promise<unknown>, listener: Function) => Promise<unknown>): Promise<cognidream> {
        if (!this._listeners) {
            return;
        }
        ...
        while (this._asyncDeliveryQueue.size > 0 && !token.isCancellationRequested) {
            ...
            const event = <T>{
                ...data,
                token,
                waitUntil: (p: Promise<unknown>): cognidream => {
...
export class EventMultiplexer<T> implements IDisposable {
    ...
    private onFirstListenerAdd(): cognidream {
        this.hasListeners = true;
        this.events.forEach(e => this.hook(e));
    }

    private onLastListenerRemove(): cognidream {
        this.hasListeners = false;
        this.events.forEach(e => this.unhook(e));
    }

    private hook(e: { event: Event<T>; listener: IDisposable | null }): cognidream {
        e.listener = e.event(r => this.emitter.fire(r));
    }

    private unhook(e: { event: Event<T>; listener: IDisposable | null }): cognidream {
        e.listener?.dispose();
        e.listener = null;
    }
...
export class EventBufferer {
    ...
    bufferEvents<R = cognidream>(fn: () => R): R {
...
class ConstValueWithChangeEvent<T> implements IValueWithChangeEvent<T> {
    public readonly onDidChange: Event<cognidream> = Event.None;
