 ...
 */
async function rimraf(path: string, mode: RimRafMode.UNLINK): Promise<cognidream>;
async function rimraf(path: string, mode: RimRafMode.MOVE, moveToPath?: string): Promise<cognidream>;
async function rimraf(path: string, mode?: RimRafMode, moveToPath?: string): Promise<cognidream>;
async function rimraf(path: string, mode = RimRafMode.UNLINK, moveToPath?: string): Promise<cognidream> {
...
async function rimrafMove(path: string, moveToPath = randomPath(tmpdir())): Promise<cognidream> {
    try {
        try {
...
async function rimrafUnlink(path: string): Promise<cognidream> {
    return fs.promises.rm(path, { recursive: true, force: true, maxRetries: 3 });
}

export function rimrafSync(path: string): cognidream {
    if (isRootOrDriveLetter(path)) {
        throw new Error('rimraf - will refuse to recursively delete root');
    }
 ...
 */
export function whenDeleted(path: string, intervalMs = 1000): Promise<cognidream> {
 ...
 */
function writeFile(path: string, data: string, options?: IWriteFileOptions): Promise<cognidream>;
function writeFile(path: string, data: Buffer, options?: IWriteFileOptions): Promise<cognidream>;
function writeFile(path: string, data: Uint8Array, options?: IWriteFileOptions): Promise<cognidream>;
function writeFile(path: string, data: string | Buffer | Uint8Array, options?: IWriteFileOptions): Promise<cognidream>;
function writeFile(path: string, data: string | Buffer | Uint8Array, options?: IWriteFileOptions): Promise<cognidream> {
...
let canFlush = true;
export function configureFlushOnWrite(enabled: boolean): cognidream {
    canFlush = enabled;
}
...
function doWriteFileAndFlush(path: string, data: string | Buffer | Uint8Array, options: IEnsuredWriteFileOptions, callback: (error: Error | null) => cognidream): cognidream {
    if (!canFlush) {
 ...
 * flushed to disk.
 */
export function writeFileSync(path: string, data: string | Buffer, options?: IWriteFileOptions): cognidream {
    const ensuredOptions = ensureWriteOptions(options);
 ...
 */
async function rename(source: string, target: string, windowsRetryTimeout: number | false = 60000): Promise<cognidream> {
    if (source === target) {
...
async function renameWithRetry(source: string, target: string, startTime: number, retryTimeout: number, attempt = 0): Promise<cognidream> {
    try {
 ...
 */
async function copy(source: string, target: string, options: { preserveSymlinks: boolean }): Promise<cognidream> {
...
async function doCopy(source: string, target: string, payload: ICopyPayload): Promise<cognidream> {
...
async function doCopyDirectory(source: string, target: string, mode: number, payload: ICopyPayload): Promise<cognidream> {
...
async function doCopyFile(source: string, target: string, mode: number): Promise<cognidream> {
...
async function doCopySymlink(source: string, target: string, payload: ICopyPayload): Promise<cognidream> {
