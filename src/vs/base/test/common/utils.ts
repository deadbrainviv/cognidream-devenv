...
export type ValueCallback<T = any> = (value: T | Promise<T>) => cognidream;
...
export function suiteRepeat(n: number, description: string, callback: (this: any) => cognidream): cognidream {
    for (let i = 0; i < n; i++) {
...
export async function assertThrowsAsync(block: () => any, message: string | Error = 'Missing expected exception'): Promise<cognidream> {
    try {
        await block();
    } catch {
        return;
...
export function throwIfDisposablesAreLeaked(body: () => cognidream, logToConsole = true): cognidream {
...
export async function throwIfDisposablesAreLeakedAsync(body: () => Promise<cognidream>): Promise<cognidream> {
    const tracker = new DisposableTracker();
    setDisposableTracker(tracker);
    await body();
    setDisposableTracker(null);
