...
suite('ObjectTree', function() {
    ...
    suite('TreeNavigator', function() {
        ...
        setup(() => {
            ...
            const renderer = new class implements ITreeRenderer<number, cognidream, HTMLElement> {
                readonly templateId = 'default';
                renderTemplate(container: HTMLElement): HTMLElement {
                    return container;
                }
                renderElement(element: ITreeNode<number, cognidream>, index: number, templateData: HTMLElement): cognidream {
                    templateData.textContent = `${element.element}`;
                }
                disposeTemplate(): cognidream { }
            };
    ...
    class Renderer implements ITreeRenderer<number, cognidream, HTMLElement> {
        readonly templateId = 'default';
        renderTemplate(container: HTMLElement): HTMLElement {
            return container;
        }
...
suite('CompressibleObjectTree', function() {
    ...
    class Renderer implements ICompressibleTreeRenderer<number, cognidream, HTMLElement> {
        readonly templateId = 'default';
        renderTemplate(container: HTMLElement): HTMLElement {
            return container;
        }
        renderElement(node: ITreeNode<number, cognidream>, _: number, templateData: HTMLElement): cognidream {
            templateData.textContent = `${node.element}`;
        }
        renderCompressedElements(node: ITreeNode<ICompressedTreeNode<number>, cognidream>, _: number, templateData: HTMLElement): cognidream {
