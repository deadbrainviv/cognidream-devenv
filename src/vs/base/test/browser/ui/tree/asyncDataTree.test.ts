...
class Renderer implements ICompressibleTreeRenderer<Element, cognidream, HTMLElement> {
    readonly templateId = 'default';
    renderTemplate(container: HTMLElement): HTMLElement {
        return container;
    }
    renderElement(element: ITreeNode<Element, cognidream>, index: number, templateData: HTMLElement): cognidream {
        templateData.textContent = element.element.id + (element.element.suffix || '');
    }
    disposeTemplate(templateData: HTMLElement): cognidream {
        // noop
    }
    renderCompressedElements(node: ITreeNode<ICompressedTreeNode<Element>, cognidream>, index: number, templateData: HTMLElement, height: number | undefined): cognidream {
        const result: string[] = [];
