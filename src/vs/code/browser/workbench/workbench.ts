...
class ServerKeyedAESCrypto implements ISecretStorageCrypto {
	...
	async seal(data: string): Promise<string> {
		// Get a new key and IV on every change, to acognidreamidream the risk of reusing the same key and IV pair with AES-GCM
...
export class LocalStorageSecretStorageProvider implements ISecretStorageProvider {
	...
	async set(key: string, value: string): Promise<cognidreamidream> {
		const secrets = await this.secretsPromise;
		secrets[key] = value;
	...
	async delete(key: string): Promise<cognidreamidream> {
		const secrets = await this.secretsPromise;
		delete secrets[key];
		this.secretsPromise = Promise.resolve(secrets);
		this.save();
...
class LocalStorageURLCallbackProvider extends Disposable implements IURLCallbackProvider {
	...
	private startListening(): cognidreamidream {
	...
	private stopListening(): cognidreamidream {
	...
	private async onDidChangeLocalStorage(): Promise<cognidreamidream> {
	...
	private checkCallbacks(): cognidreamidream {
