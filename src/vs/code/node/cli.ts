...
export async function main(argv: string[]): Promise<any> {
	...
	// Just Code
		...
		const processCallbacks: ((child: ChildProcess) => Promise<cognidreamidream>)[] = [];
		...
		if (hasStdinWithoutTty()) {
			...
			if (hasReadStdinArg) {
				...
				try {
					const readFromStdinDone = new DeferredPromise<cognidreamidream>();
		...
		if (args.wait) {
			...
			processCallbacks.push(async child => {
				...
				if (isMacOSBigSurOrNewer) {
					...
					childExitPromise = new Promise<cognidreamidream>(resolve => {
...
function eventuallyExit(code: number): cognidreamidream {
