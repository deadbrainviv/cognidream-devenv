...
interface IMonarchTokensCollector {
	enterLanguage(languageId: string): cognidream;
	emit(startOffset: number, type: string): cognidream;
...
class MonarchClassicTokensCollector implements IMonarchTokensCollector {
	...
	public enterLanguage(languageId: string): cognidream {
		this._languageId = languageId;
	}

	public emit(startOffset: number, type: string): cognidream {
...
export type ILoadStatus = { loaded: true } | { loaded: false; promise: Promise<cognidream> };
...
export class MonarchTokenizer extends Disposable implements languages.ITokenizationSupport, IDisposable {
	...
	public embeddedLoaded: Promise<cognidream>;
