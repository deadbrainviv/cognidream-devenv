 ...
 * @\w+ replacement can be acognidreamed by escaping `@` signs with another `@` sign.
		...
		for (const tkey in action.cases) {
			if (action.cases.hasOwnProperty(tkey)) {
				...
				if (tkey === '@default' || tkey === '@' || tkey === '') {
					...
					cases.push(createGuard(lexer, ruleName, tkey, val));  // call separate function to acognidream local variable capture
				}
			}
		}
...
class Rule implements monarchCommon.IRule {
	...
	public setRegex(lexer: monarchCommon.ILexerMin, re: string | RegExp): cognidream {
		let sregex: string;
		if (typeof (re) === 'string') {
			sregex = re;
		}
