 ...
 */
export function register(language: ILanguageExtensionPoint): cognidream {
	// Intentionally using the `ModesRegistry` here to acognidream
 ...
 * @event
 */
export function onLanguage(languageId: string, callback: () => cognidream): IDisposable {
 ...
 * @event
 */
export function onLanguageEncountered(languageId: string, callback: () => cognidream): IDisposable {
	return StandaloneServices.withServices(() => {
		const languageService = StandaloneServices.get(ILanguageService);
 ...
 */
export function setColorMap(colorMap: string[] | null): cognidream {
