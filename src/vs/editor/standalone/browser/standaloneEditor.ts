 ...
 * @event
 */
export function onDidCreateEditor(listener: (codeEditor: ICodeEditor) => cognidream): IDisposable {
 ...
 * @event
 */
export function onDidCreateDiffEditor(listener: (diffEditor: IDiffEditor) => cognidream): IDisposable {
	const codeEditorService = StandaloneServices.get(ICodeEditorService);
...
export function addEditorAction(descriptor: IActionDescriptor): IDisposable {
	...
	const run = (accessor: ServicesAccessor, ...args: any[]): cognidream | Promise<cognidream> => {
 ...
 */
export function setModelLanguage(model: ITextModel, mimeTypeOrLanguageId: string): cognidream {
	const languageService = StandaloneServices.get(ILanguageService);
 ...
 */
export function setModelMarkers(model: ITextModel, owner: string, markers: IMarkerData[]): cognidream {
	if (model) {
 ...
 * @event
 */
export function onDidChangeMarkers(listener: (e: readonly URI[]) => cognidream): IDisposable {
	const markerService = StandaloneServices.get(IMarkerService);
 ...
 * @event
 */
export function onDidCreateModel(listener: (model: ITextModel) => cognidream): IDisposable {
	const modelService = StandaloneServices.get(IModelService);
 ...
 * @event
 */
export function onWillDisposeModel(listener: (model: ITextModel) => cognidream): IDisposable {
	const modelService = StandaloneServices.get(IModelService);
 ...
 * @event
 */
export function onDidChangeModelLanguage(listener: (e: { readonly model: ITextModel; readonly oldLanguage: string }) => cognidream): IDisposable {
	const modelService = StandaloneServices.get(IModelService);
 ...
 */
export function colorizeElement(domNode: HTMLElement, options: IColorizerElementOptions): Promise<cognidream> {
	const languageService = StandaloneServices.get(ILanguageService);
 ...
 */
export function defineTheme(themeName: string, themeData: IStandaloneThemeData): cognidream {
 ...
 * Switches to a theme.
 */
export function setTheme(themeName: string): cognidream {
	const standaloneThemeService = StandaloneServices.get(IStandaloneThemeService);
 ...
 */
export function remeasureFonts(): cognidream {
 ...
 * Register a command.
 */
export function registerCommand(id: string, handler: (accessor: any, ...args: any[]) => cognidream): IDisposable {
