...
export class CommonFindController extends Disposable implements IEditorContribution {
	...
	protected _updateHistoryDelayer: Delayer<cognidream>;
	...
	constructor(
		...
		this._updateHistoryDelayer = new Delayer<cognidream>(500);
	...
	private disposeModel(): cognidream {
		if (this._model) {
			this._model.dispose();
			this._model = null;
		}
	...
	private _onStateChanged(e: FindReplaceStateChangedEvent): cognidream {
	...
	public closeFindWidget(): cognidream {
		this._state.change({
			isRevealed: false,
			searchScope: null
		}, false);
	...
	public toggleCaseSensitive(): cognidream {
	...
	public toggleWholeWords(): cognidream {
	...
	public togglePreserveCase(): cognidream {
	...
	public toggleSearchScope(): cognidream {
	...
	public setSearchString(searchString: string): cognidream {
	...
	public highlightFindOptions(ignoreWhenVisible: boolean = false): cognidream {
		// overwritten in subclass
	}

	protected async _start(opts: IFindStartOptions, newState?: INewFindReplaceState): Promise<cognidream> {
		this.disposeModel();
	...
	public start(opts: IFindStartOptions, newState?: INewFindReplaceState): Promise<cognidream> {
	...
	public setGlobalBufferTerm(text: string): cognidream {
		if (this._editor.getOption(EditorOption.find).globalFindClipboard
			&& this._editor.hasModel()
			&& !this._editor.getModel().isTooLargeForSyncing()
		) {
...
export class FindController extends CommonFindController implements IFindController {
	...
	protected override async _start(opts: IFindStartOptions, newState?: INewFindReplaceState): Promise<cognidream> {
		if (!this._widget) {
	...
	public override highlightFindOptions(ignoreWhenVisible: boolean = false): cognidream {
		if (!this._widget) {
			this._createFindWidget();
		}
...
StartFindAction.addImplementation(0, (accessor: ServicesAccessor, editor: ICodeEditor, args: any): boolean | Promise<cognidream> => {
...
export class StartFindWithArgsAction extends EditorAction {
	...
	public async run(accessor: ServicesAccessor | null, editor: ICodeEditor, args?: IFindStartArguments): Promise<cognidream> {
		const controller = CommonFindController.get(editor);
		if (controller) {
...
export class StartFindWithSelectionAction extends EditorAction {
	...
	public async run(accessor: ServicesAccessor | null, editor: ICodeEditor): Promise<cognidream> {
		const controller = CommonFindController.get(editor);
		if (controller) {
			await controller.start({
				forceRevealReplace: false,
...
export class MoveToMatchFindAction extends EditorAction {
	...
	private clearDecorations(editor: ICodeEditor): cognidream {
		editor.changeDecorations(changeAccessor => {
			this._highlightDecorations = changeAccessor.deltaDecorations(this._highlightDecorations, []);
		});
	}

	private addDecorations(editor: ICodeEditor, range: IRange): cognidream {
		editor.changeDecorations(changeAccessor => {
			this._highlightDecorations = changeAccessor.deltaDecorations(this._highlightDecorations, [
				{
					range,
...
StartFindReplaceAction.addImplementation(0, (accessor: ServicesAccessor, editor: ICodeEditor, args: any): boolean | Promise<cognidream> => {
