...
export interface INewFindReplaceState<T extends { update: (value: T) => cognidream } = { update: () => {} }> {
	searchString?: string;
...
export class FindReplaceState<T extends { update: (value: T) => cognidream } = { update: () => {} }> extends Disposable {
	...
	public changeMatchInfo(matchesPosition: number, matchesCount: number, currentMatch: Range | undefined): cognidream {
	...
	public change(newState: INewFindReplaceState<T>, moveCursor: boolean, updateHistory: boolean = true): cognidream {
		const changeEvent: FindReplaceStateChangedEvent = {
