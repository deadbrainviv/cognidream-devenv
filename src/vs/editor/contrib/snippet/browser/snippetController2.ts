...
export class SnippetController2 implements IEditorContribution {
	...
	private _choiceCompletions?: { provider: CompletionItemProvider; enable(): cognidream; disable(): cognidream };
	...
	private _updateState(): cognidream {
	...
	private _handleChoice(): cognidream {
		if (!this._session || !this._editor.hasModel()) {
	...
	cancel(resetSelection: boolean = false): cognidream {
		this._inSnippet.reset();
		this._hasPrevTabstop.reset();
		this._hasNextTabstop.reset();
		this._snippetListener.clear();
