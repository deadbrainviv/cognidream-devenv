...
suite('SnippetParser', () => {
	...
	test('Marker, toTextmateString()', function () {
		...
		function assertTextsnippetString(input: string, expected: string): cognidream {
	...
	test('Marker, toTextmateString() <-> identity', function () {
		...
		function assertIdent(input: string): cognidream {
	...
	test('marker#len', () => {
		...
		function assertLen(template: string, ...lengths: number[]): cognidream {
	...
	test('Snippet parser freeze #53144', function () {
		const snippet = new SnippetParser().parse('${1/(cognidream$)|(.+)/${1:?-\treturn nil;}/}');
