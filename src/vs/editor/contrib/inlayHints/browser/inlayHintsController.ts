...
class InlayHintsCache {
	...
	set(model: ITextModel, value: InlayHintItem[]): cognidream {
		const key = InlayHintsCache._key(model);
...
export class InlayHintsController implements IEditorContribution {
	...
	private _cacheHintsForFastRestore(model: ITextModel): cognidream {
	...
	private _updateHintsDecorators(ranges: readonly Range[], items: readonly InlayHintItem[]): cognidream {
		...
		const addInjectedText = (item: InlayHintItem, ref: ClassNameReference, content: string, cursorStops: InjectedTextCursorStops, attachedData?: RenderedInlayHintLabelPart | object): cognidream => {
		...
		const addInjectedWhitespace = (item: InlayHintItem, isLast: boolean): cognidream => {
	...
	private _fillInColors(props: CssProperties, hint: languages.InlayHint): cognidream {
	...
	private _removeAllDecorations(): cognidream {
