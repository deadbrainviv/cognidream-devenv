...
interface IRenameWidget {
	...
	acceptInput(wantsPreview: boolean): cognidream;
	cancelInput(focusEditor: boolean, caller: string): cognidream;
...
export class RenameWidget implements IRenameWidget, IContentWidget, IDisposable {
	...
	private _updateStyles(theme: IColorTheme): cognidream {
		if (!this._domNode) {
			return;
		}
	...
	afterRender(position: ContentWidgetPositionPreference | null): cognidream {
	...
	private _currentAcceptInput?: (wantsPreview: boolean) => cognidream;
	private _currentCancelInput?: (focusEditor: boolean) => cognidream;
...
class RenameCandidateListView {
	...
	constructor(parent: HTMLElement, opts: { fontInfo: FontInfo; onFocusChange: (newSymbolName: string) => cognidream; onSelectionChange: () => cognidream }) {
	...
	public layout({ height, width }: { height: number; width: number }): cognidream {
		this._availableHeight = height;
		this._minimumWidth = width;
	}

	public setCandidates(candidates: NewSymbolName[]): cognidream {
	...
	public clearCandidates(): cognidream {
	...
	private static _createListWidget(container: HTMLElement, candidateViewHeight: number, fontInfo: FontInfo) {
		...
		const renderer = new class implements IListRenderer<NewSymbolName, RenameCandidateView> {
			...
			renderElement(candidate: NewSymbolName, index: number, templateData: RenameCandidateView): cognidream {
				templateData.populate(candidate);
			}

			disposeTemplate(templateData: RenameCandidateView): cognidream {
...
class InputWithButton implements IDisposable {
	...
	private readonly _onDidInputChange = new Emitter<cognidream>();
