...
export class BracketMatchingController extends Disposable implements IEditorContribution {
	...
	public jumpToBracket(): cognidream {
	...
	public selectToBracket(selectBrackets: boolean): cognidream {
		if (!this._editor.hasModel()) {
			return;
		}
	...
	public removeBrackets(editSource?: string): cognidream {
		if (!this._editor.hasModel()) {
			return;
		}
	...
	private _updateBrackets(): cognidream {
		if (this._matchBrackets === 'never') {
			return;
		}
	...
	private _recomputeBrackets(): cognidream {
