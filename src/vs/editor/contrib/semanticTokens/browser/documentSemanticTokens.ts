...
class ModelSemanticColoring extends Disposable {
	...
	private _fetchDocumentSemanticTokensNow(): cognidream {
		if (this._currentDocumentRequestCancellationTokenSource) {
	...
	private static _copy(src: Uint32Array, srcOffset: number, dest: Uint32Array, destOffset: number, length: number): cognidream {
	...
	private _setDocumentSemanticTokens(provider: DocumentSemanticTokensProvider | null, tokens: SemanticTokens | SemanticTokensEdits | null, styling: SemanticTokensProviderStyling | null, pendingChanges: IModelContentChangedEvent[]): cognidream {
