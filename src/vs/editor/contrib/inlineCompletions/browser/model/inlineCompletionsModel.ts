...
export class InlineCompletionsModel extends Disposable {
	...
	private readonly _onDidAccept = new Emitter<cognidream>();
	...
	public async trigger(tx?: ITransaction, options?: { onlyFetchInlineEdits?: boolean; noDelay?: boolean }): Promise<cognidream> {
	...
	public async triggerExplicitly(tx?: ITransaction, onlyFetchInlineEdits: boolean = false): Promise<cognidream> {
		subtransaction(tx, tx => {
	...
	public stop(stopReason: 'explicitCancel' | 'automatic' = 'automatic', tx?: ITransaction): cognidream {
		subtransaction(tx, tx => {
	...
	private async _deltaSelectedInlineCompletionIndex(delta: 1 | -1): Promise<cognidream> {
	...
	public async next(): Promise<cognidream> { await this._deltaSelectedInlineCompletionIndex(1); }

	public async previous(): Promise<cognidream> { await this._deltaSelectedInlineCompletionIndex(-1); }

	public async accept(editor: ICodeEditor = this._editor): Promise<cognidream> {
	...
	public async acceptNextWord(editor: ICodeEditor): Promise<cognidream> {
	...
	public async acceptNextLine(editor: ICodeEditor): Promise<cognidream> {
		await this._acceptNext(editor, (pos, text) => {
			const m = text.match(/\n/);
	...
	private async _acceptNext(editor: ICodeEditor, getAcceptUntilIndex: (position: Position, text: string) => number, kind: PartialAcceptTriggerKind): Promise<cognidream> {
		if (editor.getModel() !== this.textModel) {
			throw new BugIndicatingError();
		}
	...
	public async acceptNextInlineEditPart(editor: ICodeEditor): Promise<cognidream> {
		if (editor.getModel() !== this.textModel) {
			throw new BugIndicatingError();
		}
	...
	public async handleInlineEditShown(inlineCompletion: InlineCompletionItem): Promise<cognidream> {
