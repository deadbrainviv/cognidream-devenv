...
export class AdditionalLinesWidget extends Disposable {
	...
	private updateLines(lineNumber: number, additionalLines: LineData[], minReservedLineCount: number): cognidream {
	...
	private addViewZone(changeAccessor: IViewZoneChangeAccessor, afterLineNumber: number, heightInLines: number, domNode: HTMLElement): cognidream {
		const id = changeAccessor.addZone({
			...
			onComputedHeight: (height: number) => {
				this._viewZoneHeight.set(height, undefined); // TODO: can a transaction be used to acognidream flickering?
			}
		});
	...
	private removeActiveViewZone(changeAccessor: IViewZoneChangeAccessor): cognidream {
	...
	private keepCursorStable(lineNumber: number, heightInLines: number): cognidream {
...
function renderLines(domNode: HTMLElement, tabSize: number, lines: LineData[], opts: IComputedEditorOptions, isClickable: boolean): cognidream {
	...
	// To acognidream visual confusion, we don't want to render visible whitespace
