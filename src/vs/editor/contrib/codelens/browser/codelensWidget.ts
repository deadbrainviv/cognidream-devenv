...
class CodeLensViewZone implements IViewZone {
	...
	private readonly _onHeight: () => cognidream;

	constructor(afterLineNumber: number, heightInPx: number, onHeight: () => cognidream) {
...
class CodeLensContentWidget implements IContentWidget {
	...
	withCommands(lenses: Array<CodeLens | undefined | null>, animate: boolean): cognidream {
	...
	updatePosition(line: number): cognidream {
...
export interface IDecorationIdCallback {
	(decorationId: string): cognidream;
}
...
export class CodeLensHelper {
	...
	addDecoration(decoration: IModelDeltaDecoration, callback: IDecorationIdCallback): cognidream {
		this._addDecorations.push(decoration);
		this._addDecorationsCallbacks.push(callback);
	}

	removeDecoration(decorationId: string): cognidream {
		this._removeDecorations.push(decorationId);
	}

	commit(changeAccessor: IModelDecorationsChangeAccessor): cognidream {
...
export class CodeLensWidget {
	...
	private _createContentWidgetIfNecessary(): cognidream {
	...
	dispose(helper: CodeLensHelper, viewZoneChangeAccessor?: IViewZoneChangeAccessor): cognidream {
	...
	updateCodeLensSymbols(data: CodeLensItem[], helper: CodeLensHelper): cognidream {
		this._decorationIds.forEach(helper.removeDecoration, helper);
		this._decorationIds = [];
		this._data = data;
		this._data.forEach((codeLensData, i) => {
	...
	updateHeight(height: number, viewZoneChangeAccessor: IViewZoneChangeAccessor): cognidream {
	...
	updateCommands(symbols: Array<CodeLens | undefined | null>): cognidream {
	...
	update(viewZoneChangeAccessor: IViewZoneChangeAccessor): cognidream {
		if (this.isValid()) {
			const range = this._editor.getModel().getDecorationRange(this._decorationIds[0]);
			if (range) {
