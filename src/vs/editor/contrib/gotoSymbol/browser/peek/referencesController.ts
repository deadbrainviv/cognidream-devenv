...
export abstract class ReferencesController implements IEditorContribution {
	...
	toggleWidget(range: Range, modelPromise: CancelablePromise<ReferencesModel>, peekMode: boolean): cognidream {
	...
	closeWidget(focusEditor = true): cognidream {
		this._widget?.dispose();
		this._model?.dispose();
		this._referenceSearchVisible.reset();
	...
	openReference(ref: Location, sideBySide: boolean, pinned: boolean): cognidream {
		// clear stage
		if (!sideBySide) {
			this.closeWidget();
		}
...
function withController(accessor: ServicesAccessor, fn: (controller: ReferencesController) => cognidream): cognidream {
