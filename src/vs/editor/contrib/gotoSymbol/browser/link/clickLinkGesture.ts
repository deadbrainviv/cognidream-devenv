...
export class ClickLinkGesture extends Disposable {
	...
	private readonly _onCancel: Emitter<cognidream> = this._register(new Emitter<cognidream>());
	public readonly onCancel: Event<cognidream> = this._onCancel.event;
	...
	private _onDidChangeCursorSelection(e: ICursorSelectionChangedEvent): cognidream {
		if (e.selection && e.selection.startColumn !== e.selection.endColumn) {
			this._resetHandler(); // immediately stop this feature if the user starts to select (https://github.com/microsoft/vscode/issues/7827)
		}
	}

	private _onEditorMouseMove(mouseEvent: ClickLinkMouseEvent): cognidream {
		this._lastMouseMoveEvent = mouseEvent;

		this._onMouseMoveOrRelevantKeyDown.fire([mouseEvent, null]);
	}

	private _onEditorMouseDown(mouseEvent: ClickLinkMouseEvent): cognidream {
	...
	private _onEditorMouseUp(mouseEvent: ClickLinkMouseEvent): cognidream {
	...
	private _onEditorKeyDown(e: ClickLinkKeyboardEvent): cognidream {
		if (
	...
	private _onEditorKeyUp(e: ClickLinkKeyboardEvent): cognidream {
		if (e.keyCodeIsTriggerKey) {
			this._onCancel.fire();
		}
	}

	private _resetHandler(): cognidream {
		this._lastMouseMoveEvent = null;
		this._hasTriggerKeyOnMouseDown = false;
		this._onCancel.fire();
	}
