...
export class SuggestWidget implements IDisposable {
	...
	private _currentSuggestionDetails?: CancelablePromise<cognidream>;
	...
	private _onCursorSelectionChanged(): cognidream {
		if (this._state !== State.Hidden) {
			this._contentWidget.layout();
		}
	}

	private _onListMouseDownOrTap(e: IListMouseEvent<CompletionItem> | IListGestureEvent<CompletionItem>): cognidream {
	...
	private _onListSelection(e: IListEvent<CompletionItem>): cognidream {
		if (e.elements.length) {
			this._select(e.elements[0], e.indexes[0]);
		}
	}

	private _select(item: CompletionItem, index: number): cognidream {
	...
	private _onListFocus(e: IListEvent<CompletionItem>): cognidream {
	...
	private _setState(state: State): cognidream {
	...
	showSuggestions(completionModel: CompletionModel, selectionIndex: number, isFrozen: boolean, isAuto: boolean, noFocus: boolean): cognidream {
	...
	toggleDetails(focused: boolean = false): cognidream {
		if (this._isDetailsVisible()) {
			// hide details widget
	...
	private _showDetails(loading: boolean, focused: boolean): cognidream {
	...
	resetPersistedSize(): cognidream {
		this._persistedSize.reset();
	}

	hideWidget(): cognidream {
		this._pendingLayout.clear();
		this._pendingShowDetails.clear();
		this._loadingTimeout?.dispose();
	...
	private _layout(size: dom.Dimension | undefined): cognidream {
	...
	private _resize(width: number, height: number): cognidream {
	...
	private _positionDetails(): cognidream {
		if (this._isDetailsVisible()) {
...
export class SuggestContentWidget implements IContentWidget {
	...
	setPosition(position: IPosition | null): cognidream {
