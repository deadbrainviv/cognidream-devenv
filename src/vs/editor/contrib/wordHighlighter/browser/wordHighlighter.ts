...
class WordHighlighter {
	...
	private readonly runDelayer: Delayer<cognidream> = this.toUnhook.add(new Delayer<cognidream>(50));
	...
	public restore(delay: number): cognidream {
		if (this.occurrencesHighlightEnablement === 'off') {
			return;
		}
	...
	private _removeSingleDecorations(): cognidream {
		// return if no model
	...
	private _removeAllDecorations(preservedModel?: URI): cognidream {
	...
	private _stopSingular(): cognidream {
	...
	private _stopAll(preservedModel?: URI): cognidream {
	...
	private _onPositionChanged(e: ICursorPositionChangedEvent): cognidream {
	...
	private async _run(multiFileConfigChange?: boolean, delay?: number): Promise<cognidream> {
	...
	private _beginRenderDecorations(delay: number): cognidream {
	...
	private renderDecorations(): cognidream {
		this.renderDecorationsTimer = -1;
...
export class WordHighlighterContribution extends Disposable implements IEditorContribution {
	...
	public restoreViewState(state: boolean | undefined): cognidream {
