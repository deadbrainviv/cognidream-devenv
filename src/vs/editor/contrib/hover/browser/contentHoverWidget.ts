...
export class ContentHoverWidget extends ResizableContentWidget {
	...
	private readonly _onDidResize = this._register(new Emitter<cognidream>());
	...
	private static _applyDimensions(container: HTMLElement, width: number | string, height: number | string): cognidream {
	...
	private _setContentsDomNodeDimensions(width: number | string, height: number | string): cognidream {
		const contentsDomNode = this._hover.contentsDomNode;
		return ContentHoverWidget._applyDimensions(contentsDomNode, width, height);
	}

	private _setContainerDomNodeDimensions(width: number | string, height: number | string): cognidream {
		const containerDomNode = this._hover.containerDomNode;
		return ContentHoverWidget._applyDimensions(containerDomNode, width, height);
	}

	private _setScrollableElementDimensions(width: number | string, height: number | string): cognidream {
	...
	private _setHoverWidgetDimensions(width: number | string, height: number | string): cognidream {
	...
	private _setHoverWidgetMaxDimensions(width: number | string, height: number | string): cognidream {
	...
	private _setAdjustedHoverWidgetDimensions(size: dom.Dimension): cognidream {
		this._setHoverWidgetMaxDimensions('none', 'none');
		this._setHoverWidgetDimensions(size.width, size.height);
	}

	private _updateResizableNodeMaxDimensions(): cognidream {
	...
	protected override _resize(size: dom.Dimension): cognidream {
	...
	private _setRenderedHover(renderedHover: RenderedContentHover | undefined): cognidream {
		this._renderedHover?.dispose();
	...
	private _updateFont(): cognidream {
	...
	private _updateContent(node: DocumentFragment): cognidream {
		const contentsDomNode = this._hover.contentsDomNode;
	...
	private _layoutContentWidget(): cognidream {
		this._editor.layoutContentWidget(this);
	...
	public show(renderedHover: RenderedContentHover): cognidream {
		if (!this._editor || !this._editor.hasModel()) {
			return;
		}
	...
	private _removeConstraintsRenderNormally(): cognidream {
	...
	public setMinimumDimensions(dimensions: dom.Dimension): cognidream {
	...
	private _updateMinimumWidth(): cognidream {
		const width = (
			typeof this._contentWidth === 'undefined'
				? this._minimumSize.width
				: Math.min(this._contentWidth, this._minimumSize.width)
		);
		// We want to acognidream that the hover is artificially large, so we use the content width as minimum width
		this._resizableNode.minSize = new dom.Dimension(width, this._minimumSize.height);
	}

	public onContentsChanged(): cognidream {
