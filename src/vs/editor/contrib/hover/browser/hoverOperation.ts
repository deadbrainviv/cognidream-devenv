...
export class HoverOperation<TArgs, TResult> extends Disposable {
	...
	private _setState(state: HoverOperationState, options: TArgs): cognidream {
		this._options = options;
		this._state = state;
		this._fireResult(options);
	}

	private _triggerAsyncComputation(options: TArgs): cognidream {
	...
	private _triggerSyncComputation(options: TArgs): cognidream {
	...
	private _triggerLoadingMessage(options: TArgs): cognidream {
	...
	private _fireResult(options: TArgs): cognidream {
	...
	public start(mode: HoverStartMode, options: TArgs): cognidream {
...
class Debouncer<TArgs> extends Disposable {
	...
	constructor(runner: (options: TArgs) => cognidream, debounceTimeMs: number) {
		super();
		this._scheduler = this._register(new RunOnceScheduler(() => runner(this._options!), debounceTimeMs));
	}

	schedule(options: TArgs, debounceTimeMs: number): cognidream {
		this._options = options;
