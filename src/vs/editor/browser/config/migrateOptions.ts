...
export interface ISettingsWriter {
	(key: string, value: any): cognidream;
}
...
export class EditorSettingMigration {
	...
	constructor(
		...
		public readonly migrate: (value: any, read: ISettingsReader, write: ISettingsWriter) => cognidream
	) { }

	apply(options: any): cognidream {
	...
	private static _write(target: any, key: string, value: any): cognidream {
		const firstDotIndex = key.indexOf('.');
		if (firstDotIndex >= 0) {
			const firstSegment = key.substring(0, firstDotIndex);
...
function registerEditorSettingMigration(key: string, migrate: (value: any, read: ISettingsReader, write: ISettingsWriter) => cognidream): cognidream {
	EditorSettingMigration.items.push(new EditorSettingMigration(key, migrate));
}

function registerSimpleEditorSettingMigration(key: string, values: [any, any][]): cognidream {
 ...
 */
export function migrateOptions(options: IEditorOptions): cognidream {
