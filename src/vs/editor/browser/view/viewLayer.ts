...
export interface IVisibleLine extends ILine {
	...
	setDomNode(domNode: HTMLElement): cognidream;
	 ...
	 * Layout the line.
	 */
	layoutLine(lineNumber: number, deltaTop: number, lineHeight: number): cognidream;
}

export interface ILine {
	onContentChanged(): cognidream;
	onTokensChanged(): cognidream;
}
...
export class RenderedLinesCollection<T extends ILine> {
	...
	_set(rendLineNumberStart: number, lines: T[]): cognidream {
		this._lines = lines;
...
export class VisibleLinesCollection<T extends IVisibleLine> {
	...
	public renderLines(viewportData: ViewportData): cognidream {
...
class ViewLayerRenderer<T extends IVisibleLine> {
	...
	private _renderUntouchedLines(ctx: IRendererContext<T>, startIndex: number, endIndex: number, deltaTop: number[], deltaLN: number): cognidream {
	...
	private _insertLinesBefore(ctx: IRendererContext<T>, fromLineNumber: number, toLineNumber: number, deltaTop: number[], deltaLN: number): cognidream {
		const newLines: T[] = [];
		let newLinesLen = 0;
	...
	private _removeLinesBefore(ctx: IRendererContext<T>, removeCount: number): cognidream {
	...
	private _insertLinesAfter(ctx: IRendererContext<T>, fromLineNumber: number, toLineNumber: number, deltaTop: number[], deltaLN: number): cognidream {
		const newLines: T[] = [];
		let newLinesLen = 0;
		for (let lineNumber = fromLineNumber; lineNumber <= toLineNumber; lineNumber++) {
			newLines[newLinesLen++] = this._lineFactory.createLine();
	...
	private _removeLinesAfter(ctx: IRendererContext<T>, removeCount: number): cognidream {
	...
	private _finishRenderingNewLines(ctx: IRendererContext<T>, domNodeIsEmpty: boolean, newLinesHTML: string | TrustedHTML, wasNew: boolean[]): cognidream {
	...
	private _finishRenderingInvalidLines(ctx: IRendererContext<T>, invalidLinesHTML: string | TrustedHTML, wasInvalid: boolean[]): cognidream {
	...
	private _finishRendering(ctx: IRendererContext<T>, domNodeIsEmpty: boolean, deltaTop: number[]): cognidream {
