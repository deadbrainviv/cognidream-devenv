...
export class DecorationToRender {
	public readonly _decorationToRenderBrand: cognidream = undefined;
	...
	protected _render(visibleStartLineNumber: number, visibleEndLineNumber: number, decorations: DecorationToRender[]): VisibleLineDecorationsToRender[] {
		...
		for (let i = 0, len = decorations.length; i < len; i++) {
			...
			if (prevClassName === className) {
				// Here we acognidream rendering the same className multiple times on the same line
...
export class GlyphMarginWidgets extends ViewPart {
	...
	public addWidget(widget: IGlyphMarginWidget): cognidream {
	...
	public removeWidget(widget: IGlyphMarginWidget): cognidream {
	...
	private _collectDecorationBasedGlyphRenderRequest(ctx: RenderingContext, requests: GlyphRenderRequest[]): cognidream {
	...
	private _collectWidgetBasedGlyphRenderRequest(ctx: RenderingContext, requests: GlyphRenderRequest[]): cognidream {
		const visibleStartLineNumber = ctx.visibleRange.startLineNumber;
		const visibleEndLineNumber = ctx.visibleRange.endLineNumber;
