...
export namespace GPULifecycle {
	export async function requestDevice(fallback?: (message: string) => cognidream): Promise<IReference<GPUDevice>> {
		try {
			if (!navigator.gpu) {
...
function wrapDestroyableInDisposable<T extends { destroy(): cognidream }>(value: T): IReference<T> {
	return {
		object: value,
