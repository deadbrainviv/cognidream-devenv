...
class VisibleTextAreaData {
	_visibleTextAreaBrand: cognidream = undefined;
	 ...
	 * style if multiple tokens come into play. This acognidreams flickering.
	 */
	...
	prepareRender(visibleRangeProvider: IVisibleRangeProvider): cognidream {
	...
	definePresentation(tokenPresentation: ITokenPresentation | null): ITokenPresentation {
		if (!this._previousPresentation) {
			// To acognidream flickering, once set, always reuse a presentation throughout the entire IME session
...
export class TextAreaEditContext extends AbstractEditContext {
	...
	private _setAccessibilityOptions(options: IComputedEditorOptions): cognidream {
	...
	private _ensureReadOnlyAttribute(): cognidream {
		const options = this._context.configuration.options;
	...
	private _render(): cognidream {
		if (this._visibleTextArea) {
			...
			if (startPosition && endPosition && visibleStart && visibleEnd && visibleEnd.left >= this._scrollLeft && visibleStart.left <= this._scrollLeft + this._contentWidth) {
				...
				// Here we are adding +1 to acognidream flickering that might be caused by having a width that is too small.
	...
	private _renderAtTopLeft(): cognidream {
	...
	private _doRender(renderData: IRenderData): cognidream {
