...
export class MultiDiffEditorWidgetImpl extends Disposable {
	...
	public setScrollState(scrollState: { top?: number; left?: number }): cognidream {
		this._scrollableElement.setScrollPosition({ scrollLeft: scrollState.left, scrollTop: scrollState.top });
	}

	public reveal(resource: IMultiDiffResourceId, options?: RevealOptions): cognidream {
	...
	public setViewState(viewState: IMultiDiffEditorViewState): cognidream {
...
class VirtualizedViewItem extends Disposable {
	...
	constructor(
		...
		private readonly _deltaScrollVertical: (delta: number) => cognidream,
	) {
		super();
	...
	public setViewState(viewState: IMultiDiffDocState, tx: ITransaction): cognidream {
	...
	private _updateTemplateData(tx: ITransaction): cognidream {
		const ref = this._templateRef.get();
		if (!ref) { return; }
		this.viewModel.lastTemplateData.set({
	...
	public render(verticalSpace: OffsetRange, offset: number, width: number, viewPort: OffsetRange): cognidream {
