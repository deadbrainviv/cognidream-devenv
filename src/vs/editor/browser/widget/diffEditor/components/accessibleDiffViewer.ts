...
export interface IAccessibleDiffViewerModel {
	 ...
	 */
	originalReveal(range: Range): cognidream;
	 ...
	 * Should do: `setSelection`, `revealLine` and `focus`
	 */
	modifiedReveal(range?: Range): cognidream;
	modifiedSetSelection(range: Range): cognidream;
	modifiedFocus(): cognidream;
...
export class AccessibleDiffViewer extends Disposable {
	...
	constructor(
		...
		private readonly _setVisible: (visible: boolean, tx: ITransaction | undefined) => cognidream,
...
class ViewModel extends Disposable {
	...
	private _goToGroupDelta(delta: number, tx?: ITransaction): cognidream {
	...
	nextGroup(tx?: ITransaction): cognidream { this._goToGroupDelta(1, tx); }
	previousGroup(tx?: ITransaction): cognidream { this._goToGroupDelta(-1, tx); }

	private _goToLineDelta(delta: number): cognidream {
	...
	goToNextLine(): cognidream { this._goToLineDelta(1); }
	goToPreviousLine(): cognidream { this._goToLineDelta(-1); }

	goToLine(line: ViewElement): cognidream {
		const group = this.currentGroup.get();
		if (!group) { return; }
	...
	revealCurrentElementInEditor(): cognidream {
...
class View extends Disposable {
	...
	private _render(store: DisposableStore): cognidream {
...
export class AccessibleDiffViewerModelFromEditors implements IAccessibleDiffViewerModel {
	...
	modifiedReveal(range?: Range | undefined): cognidream {
		if (range) {
