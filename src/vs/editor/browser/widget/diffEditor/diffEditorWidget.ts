...
export class DiffEditorWidget extends DelegatingEditor implements IDiffEditor {
	...
	public override restoreViewState(s: IDiffEditorViewState): cognidream {
	...
	override setModel(model: IDiffEditorModel | null | IDiffEditorViewModel): cognidream {
		const vm = !model ? null
	...
	setDiffModel(viewModel: RefCounted<IDiffEditorViewModel> | null, tx?: ITransaction): cognidream {
	 ...
	 */
	override updateOptions(changedOptions: IDiffEditorOptions): cognidream {
	...
	readonly onDidUpdateDiff: Event<cognidream> = Event.fromObservableLight(this._diffValue);
	...
	revert(diff: LineRangeMapping): cognidream {
	...
	revertRangeMappings(diffs: RangeMapping[]): cognidream {
		const model = this._diffModel.get();
		if (!model || !model.isDiffUpToDate.get()) { return; }
	...
	private _goTo(diff: DiffMapping): cognidream {
	...
	goToDiff(target: 'previous' | 'next'): cognidream {
	...
	accessibleDiffViewerNext(): cognidream { this._accessibleDiffViewer.get().next(); }

	accessibleDiffViewerPrev(): cognidream { this._accessibleDiffViewer.get().prev(); }

	async waitForDiff(): Promise<cognidream> {
		const diffModel = this._diffModel.get();
		if (!diffModel) { return; }
	...
	exitCompareMove(): cognidream {
		const model = this._diffModel.get();
		if (!model) { return; }
		model.movedTextToCompare.set(undefined, undefined);
	}

	collapseAllUnchangedRegions(): cognidream {
	...
	showAllUnchangedRegions(): cognidream {
		const unchangedRegions = this._diffModel.get()?.unchangedRegions.get();
		if (!unchangedRegions) { return; }
		transaction(tx => {
			for (const region of unchangedRegions) {
	...
	private _handleCursorPositionChange(e: ICursorPositionChangedEvent | undefined, isModifiedEditor: boolean): cognidream {
