...
export class ContiguousTokensStore {
	...
	private _ensureLine(lineIndex: number): cognidream {
	...
	private _deleteLines(start: number, deleteCount: number): cognidream {
		if (deleteCount === 0) {
			return;
		}
	...
	private _insertLines(insertIndex: number, insertCount: number): cognidream {
		if (insertCount === 0) {
			return;
		}
	...
	public acceptEdit(range: IRange, eolCount: number, firstLineLength: number): cognidream {
	...
	private _acceptInsertText(position: Position, eolCount: number, firstLineLength: number): cognidream {
