...
class ModelData implements IDisposable {
	...
	constructor(
		...
		onWillDispose: (model: ITextModel) => cognidream,
		onDidChangeLanguage: (model: ITextModel, e: IModelLanguageChangedEvent) => cognidream
	) {
		this.model = model;
...
export class ModelService extends Disposable implements IModelService {
	...
	private _updateModelOptions(e: IConfigurationChangeEvent | undefined): cognidream {
	...
	private static _setModelOptionsForModel(model: ITextModel, newOptions: ITextModelCreationOptions, currentOptions: ITextModelCreationOptions): cognidream {
	...
	private _insertDisposedModel(disposedModelData: DisposedModelInfo): cognidream {
	...
	private _ensureDisposedModelsHeapSize(maxModelsHeapSize: number): cognidream {
	...
	public updateModel(model: ITextModel, value: string | ITextBufferFactory): cognidream {
	...
	public destroyModel(resource: URI): cognidream {
	...
	private _onWillDispose(model: ITextModel): cognidream {
		const modelId = MODEL_ID(model.uri);
		const modelData = this._models[modelId];
	...
	private _onDidChangeLanguage(model: ITextModel, e: IModelLanguageChangedEvent): cognidream {
