...
export class TokenStore implements IDisposable {
	 ...
	 */
	buildStore(tokens: TokenUpdate[], tokenQuality: TokenQuality): cognidream {
	 ...
	 * @returns
	 */
	private traverseInOrderInRange(startOffsetInclusive: number, endOffsetExclusive: number, visitor: (node: Node, offset: number) => boolean): cognidream {
		const stack: { node: Node; offset: number }[] = [{ node: this._root, offset: 0 }];
	...
	markForRefresh(startOffsetInclusive: number, endOffsetExclusive: number): cognidream {
