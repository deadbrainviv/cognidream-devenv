...
export class TokenizerWithStateStoreAndTextModel<TState extends IState = IState> extends TokenizerWithStateStore<TState> {
	...
	public updateTokensUntilLine(builder: ContiguousMultilineTokensBuilder, lineNumber: number): cognidream {
...
export class TrackingTokenizationStateStore<TState extends IState> {
	...
	public acceptChange(range: LineRange, newLineCount: number): cognidream {
	...
	public invalidateEndStateRange(range: LineRange): cognidream {
...
interface RangePriorityQueue {
	...
	addRange(range: OffsetRange): cognidream;

	addRangeAndResize(range: OffsetRange, newLength: number): cognidream;
}
...
export class RangePriorityQueueImpl implements RangePriorityQueue {
	...
	public delete(value: number): cognidream {
	...
	public addRange(range: OffsetRange): cognidream {
		OffsetRange.addRange(range, this._ranges);
	}

	public addRangeAndResize(range: OffsetRange, newLength: number): cognidream {
...
export class DefaultBackgroundTokenizer implements IBackgroundTokenizer {
	...
	public handleChanges(): cognidream {
		this._beginBackgroundTokenization();
	}

	private _isScheduled = false;
	private _beginBackgroundTokenization(): cognidream {
	 ...
	 */
	private _backgroundTokenizeWithDeadline(deadline: IdleDeadline): cognidream {
	 ...
	 */
	private _backgroundTokenizeForAtLeast1ms(): cognidream {
	...
	public checkFinished(): cognidream {
		if (this._isDisposed) {
			return;
		}
	...
	public requestTokens(startLineNumber: number, endLineNumberExclusive: number): cognidream {
