...
export class TokenizationTextModelPart extends TextModelPart implements ITokenizationTextModelPart {
	...
	private createTokens(useTreeSitter: boolean): cognidream {
	...
	public handleDidChangeContent(e: IModelContentChangedEvent): cognidream {
		if (e.isFlush) {
	...
	public handleDidChangeAttached(): cognidream {
	...
	private _emitModelTokensChangedEvent(e: IModelTokensChangedEvent): cognidream {
	...
	private validateLineNumber(lineNumber: number): cognidream {
	...
	public forceTokenization(lineNumber: number): cognidream {
		this.validateLineNumber(lineNumber);
	...
	public setSemanticTokens(tokens: SparseMultilineTokens[] | null, isComplete: boolean): cognidream {
	...
	public setPartialSemanticTokens(range: Range, tokens: SparseMultilineTokens[]): cognidream {
	...
	public setLanguageId(languageId: string, source: string = 'api'): cognidream {
...
class GrammarTokens extends AbstractTokens {
	...
	protected readonly _onDidChangeBackgroundTokenizationState: Emitter<cognidream> = this._register(new Emitter<cognidream>());
	public readonly onDidChangeBackgroundTokenizationState: Event<cognidream> = this._onDidChangeBackgroundTokenizationState.event;
	...
	public resetTokenization(fireTokenChangeEvent: boolean = true): cognidream {
		this._tokens.flush();
	...
	private refreshAllVisibleLineTokens(): cognidream {
		const ranges = LineRange.joinMany([...this._attachedViewStates].map(([_, s]) => s.lineRanges));
		this.refreshRanges(ranges);
	}

	private refreshRanges(ranges: readonly LineRange[]): cognidream {
		for (const range of ranges) {
			this.refreshRange(range.startLineNumber, range.endLineNumberExclusive - 1);
		}
	}

	private refreshRange(startLineNumber: number, endLineNumber: number): cognidream {
		if (!this._tokenizer) {
			return;
		}
