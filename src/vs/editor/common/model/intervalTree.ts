...
const enum Constants {
	...
	/**
	 * Due to how deletion works (in order to acognidream always walking the right subtree of the deleted node),
	 ...
	 *
	 * The integer overflow occurs when 53 bits are used in the numbers, but we will try to acognidream it as
...
function setNodeColor(node: IntervalNode, color: NodeColor): cognidream {
	node.metadata = (
...
function setNodeIsVisited(node: IntervalNode, value: boolean): cognidream {
	node.metadata = (
...
function setNodeIsForValidation(node: IntervalNode, value: boolean): cognidream {
	node.metadata = (
...
function setNodeIsInGlyphMargin(node: IntervalNode, value: boolean): cognidream {
	node.metadata = (
...
function _setNodeStickiness(node: IntervalNode, stickiness: TrackedRangeStickiness): cognidream {
	node.metadata = (
...
function setCollapseOnReplaceEdit(node: IntervalNode, value: boolean): cognidream {
	node.metadata = (
		(node.metadata & Constants.CollapseOnReplaceEditMaskInverse) | ((value ? 1 : 0) << Constants.CollapseOnReplaceEditOffset)
	);
}
export function setNodeStickiness(node: IntervalNode, stickiness: ActualTrackedRangeStickiness): cognidream {
...
export class IntervalNode {
	...
	public reset(versionId: number, start: number, end: number, range: Range): cognidream {
		this.start = start;
		this.end = end;
		this.maxEnd = end;
	...
	public setCachedOffsets(absoluteStart: number, absoluteEnd: number, cachedVersionId: number): cognidream {
...
export class IntervalTree {
	...
	public resolveNode(node: IntervalNode, cachedVersionId: number): cognidream {
	...
	private _normalizeDeltaIfNecessary(): cognidream {
...
function normalizeDelta(T: IntervalTree): cognidream {
	let node = T.root;
	let delta = 0;
	while (node !== SENTINEL) {
 ...
 */
export function nodeAcceptEdit(node: IntervalNode, start: number, end: number, textLength: number, forceMoveMarkers: boolean): cognidream {
...
function noOverlapReplace(T: IntervalTree, start: number, end: number, textLength: number): cognidream {
	// https://en.wikipedia.org/wiki/Interval_tree#Augmented_tree
	// Now, it is known that two intervals A and B overlap only when both
	// A.low <= B.high and A.high >= B.low. When searching the trees for
	// nodes overlapping with a given interval, you can immediately skip:
...
function treeInsert(T: IntervalTree, z: IntervalNode): cognidream {
	let delta: number = 0;
	let x = T.root;
...
//#region Deletion
function rbTreeDelete(T: IntervalTree, z: IntervalNode): cognidream {
...
function resetSentinel(): cognidream {
	SENTINEL.parent = SENTINEL;
...
//#region Rotations
function leftRotate(T: IntervalTree, x: IntervalNode): cognidream {
...
function rightRotate(T: IntervalTree, y: IntervalNode): cognidream {
	const x = y.left;
...
export function recomputeMaxEnd(node: IntervalNode): cognidream {
	node.maxEnd = computeMaxEnd(node);
}

function recomputeMaxEndWalkToRoot(node: IntervalNode): cognidream {
	while (node !== SENTINEL) {
