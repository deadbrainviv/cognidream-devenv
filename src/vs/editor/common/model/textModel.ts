...
interface ITextStream {
	on(event: 'data', callback: (data: string) => cognidream): cognidream;
	on(event: 'error', callback: (err: Error) => cognidream): cognidream;
	on(event: 'end', callback: () => cognidream): cognidream;
	on(event: string, callback: any): cognidream;
}
...
export class TextModel extends Disposable implements model.ITextModel, IDecorationsTreesHost {
	...
	//#region Events
	private readonly _onWillDispose: Emitter<cognidream> = this._register(new Emitter<cognidream>());
	public readonly onWillDispose: Event<cognidream> = this._onWillDispose.event;
	...
	private readonly _onDidChangeAttached: Emitter<cognidream> = this._register(new Emitter<cognidream>());
	public readonly onDidChangeAttached: Event<cognidream> = this._onDidChangeAttached.event;
	...
	public onDidChangeContent(listener: (e: IModelContentChangedEvent) => cognidream): IDisposable {
		return this._eventEmitter.slowEvent((e: InternalModelContentChangeEvent) => listener(e.contentChangedEvent));
	}
	public onDidChangeContentOrInjectedText(listener: (e: InternalModelContentChangeEvent | ModelInjectedTextChangedEvent) => cognidream): IDisposable {
	...
	public override dispose(): cognidream {
		...
		this.__isDisposing = false;
		// Manually release reference to previous text buffer to acognidream large leaks
	...
	private _assertNotDisposed(): cognidream {
		if (this._isDisposed) {
	...
	private _emitContentChangedEvent(rawChange: ModelRawContentChangedEvent, change: IModelContentChangedEvent): cognidream {
	...
	public setValue(value: string | model.ITextSnapshot): cognidream {
		this._assertNotDisposed();
	...
	private _setValueFromTextBuffer(textBuffer: model.ITextBuffer, textBufferDisposable: IDisposable): cognidream {
		this._assertNotDisposed();
	...
	public setEOL(eol: model.EndOfLineSequence): cognidream {
		this._assertNotDisposed();
	...
	private _onBeforeEOLChange(): cognidream {
		// Ensure all decorations get their `range` set.
		this._decorationsTree.ensureAllNodesHaveRanges(this);
	}

	private _onAfterEOLChange(): cognidream {
	...
	public onBeforeDetached(view: model.IAttachedView): cognidream {
		this._attachedEditorCount--;
	...
	public updateOptions(_newOpts: model.ITextModelUpdateOptions): cognidream {
		this._assertNotDisposed();
	...
	public detectIndentation(defaultInsertSpaces: boolean, defaultTabSize: number): cognidream {
		this._assertNotDisposed();
	...
	public removeUnusualLineTerminators(selections: Selection[] | null = null): cognidream {
	...
	private _increaseVersionId(): cognidream {
		this._versionId = this._versionId + 1;
		this._alternativeVersionId = this._versionId;
	}

	public _overwriteVersionId(versionId: number): cognidream {
		this._versionId = versionId;
	}

	public _overwriteAlternativeVersionId(newAlternativeVersionId: number): cognidream {
		this._alternativeVersionId = newAlternativeVersionId;
	}

	public _overwriteInitialUndoRedoSnapshot(newInitialUndoRedoSnapshot: ResourceEditStackSnapshot | null): cognidream {
	...
	public validatePosition(position: IPosition): Position {
		...
		// Acognidream object allocation and cover most likely case
	...
	public pushStackElement(): cognidream {
		this._commandManager.pushStackElement();
	}

	public popStackElement(): cognidream {
		this._commandManager.popStackElement();
	}

	public pushEOL(eol: model.EndOfLineSequence): cognidream {
	...
	_applyUndo(changes: TextChange[], eol: model.EndOfLineSequence, resultingAlternativeVersionId: number, resultingSelection: Selection[] | null): cognidream {
	...
	_applyRedo(changes: TextChange[], eol: model.EndOfLineSequence, resultingAlternativeVersionId: number, resultingSelection: Selection[] | null): cognidream {
		const edits = changes.map<ISingleEditOperation>((change) => {
	...
	private _applyUndoRedoEdits(edits: ISingleEditOperation[], eol: model.EndOfLineSequence, isUndoing: boolean, isRedoing: boolean, resultingAlternativeVersionId: number, resultingSelection: Selection[] | null): cognidream {
		try {
			this._onDidChangeDecorations.beginDeferredEmit();
			this._eventEmitter.beginDeferredEmit();
	...
	public applyEdits(operations: readonly model.IIdentifiedSingleEditOperation[]): cognidream;
	public applyEdits(operations: readonly model.IIdentifiedSingleEditOperation[], computeUndoEdits: false): cognidream;
	...
	public applyEdits(rawOperations: readonly model.IIdentifiedSingleEditOperation[], computeUndoEdits: boolean = false): cognidream | model.IValidEditOperation[] {
		try {
			this._onDidChangeDecorations.beginDeferredEmit();
			this._eventEmitter.beginDeferredEmit();
	...
	private _doApplyEdits(rawOperations: model.ValidAnnotatedEditOperation[], computeUndoEdits: boolean): cognidream | model.IValidEditOperation[] {
	...
	public undo(): cognidream | Promise<cognidream> {
	...
	public redo(): cognidream | Promise<cognidream> {
	...
	private handleBeforeFireDecorationsChangedEvent(affectedInjectedTextLines: Set<number> | null): cognidream {
	...
	private _changeDecorations<T>(ownerId: number, callback: (changeAccessor: model.IModelDecorationsChangeAccessor) => T): T | null {
		const changeAccessor: model.IModelDecorationsChangeAccessor = {
			...
			changeDecoration: (id: string, newRange: IRange): cognidream => {
				this._changeDecorationImpl(id, newRange);
			},
			changeDecorationOptions: (id: string, options: model.IModelDecorationOptions) => {
				this._changeDecorationOptionsImpl(id, _normalizeOptions(options));
			},
			removeDecoration: (id: string): cognidream => {
	...
	public removeAllDecorationsWithOwnerId(ownerId: number): cognidream {
		if (this._isDisposed) {
			return;
		}
	...
	private _changeDecorationImpl(decorationId: string, _range: IRange): cognidream {
		const node = this._decorations[decorationId];
		if (!node) {
			return;
		}
	...
	private _changeDecorationOptionsImpl(decorationId: string, options: ModelDecorationOptions): cognidream {
		const node = this._decorations[decorationId];
		if (!node) {
			return;
		}
	...
	public setLanguage(languageIdOrSelection: string | ILanguageSelection, source?: string): cognidream {
		if (typeof languageIdOrSelection === 'string') {
	...
	private _setLanguage(languageId: string, source?: string): cognidream {
...
class DecorationsTrees {
	...
	public ensureAllNodesHaveRanges(host: IDecorationsTreesHost): cognidream {
	...
	public insert(node: IntervalNode): cognidream {
	...
	public delete(node: IntervalNode): cognidream {
		if (isNodeInjectedText(node)) {
	...
	private _resolveNode(node: IntervalNode, cachedVersionId: number): cognidream {
		if (isNodeInjectedText(node)) {
	...
	public acceptReplace(offset: number, length: number, textLength: number, forceMoveMarkers: boolean): cognidream {
...
export class ModelDecorationOverviewRulerOptions extends DecorationOptions {
	...
	public invalidateCachedColor(): cognidream {
		this._resolvedColor = null;
	}
...
class DidChangeDecorationsEmitter extends Disposable {
	...
	constructor(private readonly handleBeforeFire: (affectedInjectedTextLines: Set<number> | null) => cognidream) {
		super();
		this._deferredCnt = 0;
	...
	public beginDeferredEmit(): cognidream {
		this._deferredCnt++;
	}

	public endDeferredEmit(): cognidream {
		this._deferredCnt--;
	...
	public recordLineAffectedByInjectedText(lineNumber: number): cognidream {
	...
	public checkAffectedAndFire(options: ModelDecorationOptions): cognidream {
...
class DidChangeContentEmitter extends Disposable {
	...
	public endDeferredEmit(resultingSelection: Selection[] | null = null): cognidream {
		this._deferredCnt--;
		if (this._deferredCnt === 0) {
	...
	public fire(e: InternalModelContentChangeEvent): cognidream {
		if (this._deferredCnt > 0) {
