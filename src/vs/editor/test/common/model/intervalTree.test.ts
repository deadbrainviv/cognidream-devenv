...
suite('IntervalTree 1', () => {
    ...
    class Interval {
        _intervalBrand: cognidream = undefined;
    ...
    class Oracle {
        ...
        public delete(interval: Interval): cognidream {
    ...
    class TestState {
        ...
        public acceptOp(op: IOperation): cognidream {
    ...
    function testIntervalTree(ops: IOperation[]): cognidream {
    ...
    class AutoTest {
        ...
        private _doRandomInsert(): cognidream {
        ...
        private _doRandomDelete(): cognidream {
        ...
        private _doRandomChange(): cognidream {
        ...
        private _run(op: IOperation): cognidream {
            this._ops.push(op);
            this._state.acceptOp(op);
        }

        public print(): cognidream {
    ...
    suite('searching', () => {
        ...
        function assertIntervalSearch(start: number, end: number, expected: [number, number][]): cognidream {
...
suite('IntervalTree 2', () => {
    ...
    function assertNodeAcceptEdit(msg: string, nodeStart: number, nodeEnd: number, nodeStickiness: TrackedRangeStickiness, start: number, end: number, textLength: number, forceMoveMarkers: boolean, expectedNodeStart: number, expectedNodeEnd: number): cognidream {
...
function printTree(T: IntervalTree): cognidream {
    if (T.root === SENTINEL) {
        console.log(`~~ empty`);
        return;
    }
...
function _printTree(T: IntervalTree, n: IntervalNode, indent: string, delta: number, out: string[]): cognidream {
...
function assertTreeInvariants(T: IntervalTree): cognidream {
...
function assertValidNode(n: IntervalNode, delta: number): cognidream {
    if (n === SENTINEL) {
        return;
    }
...
function assertValidTree(T: IntervalTree): cognidream {
    if (T.root === SENTINEL) {
        return;
    }
