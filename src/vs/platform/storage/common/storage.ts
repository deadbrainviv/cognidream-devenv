...
export interface IStorageService {
	 ...
	 */
	store(key: string, value: StorageValue, scope: StorageScope, target: StorageTarget): cognidream;
	 ...
	 */
	storeAll(entries: Array<IStorageEntry>, external: boolean): cognidream;
	 ...
	 * or all profiles.
	 */
	remove(key: string, scope: StorageScope): cognidream;
	 ...
	 */
	switch(to: IAnyWorkspaceIdentifier | IUserDataProfile, preserveData: boolean): Promise<cognidream>;
	 ...
	 */
	optimize(scope: StorageScope): Promise<cognidream>;
	 ...
	 */
	flush(reason?: WillSaveStateReason): Promise<cognidream>;
}
...
export abstract class AbstractStorageService extends Disposable implements IStorageService {
	...
	private initializationPromise: Promise<cognidream> | undefined;
	...
	private doFlushWhenIdle(): cognidream {
	...
	protected stopFlushWhenIdle(): cognidream {
		dispose([this.runFlushWhenIdle, this.flushWhenIdleScheduler]);
	}

	initialize(): Promise<cognidream> {
	...
	protected emitDidChangeValue(scope: StorageScope, event: IStorageChangeEvent): cognidream {
	...
	protected emitWillSaveState(reason: WillSaveStateReason): cognidream {
	...
	store(key: string, value: StorageValue, scope: StorageScope, target: StorageTarget, external = false): cognidream {
	...
	remove(key: string, scope: StorageScope, external = false): cognidream {
	...
	private withPausedEmitters(fn: Function): cognidream {
	...
	private updateKeyTarget(key: string, scope: StorageScope, target: StorageTarget | undefined, external = false): cognidream {
	...
	async flush(reason = WillSaveStateReason.NONE): Promise<cognidream> {
	...
	async optimize(scope: StorageScope): Promise<cognidream> {
	...
	async switch(to: IAnyWorkspaceIdentifier | IUserDataProfile, preserveData: boolean): Promise<cognidream> {
	...
	protected switchData(oldStorage: Map<string, string>, newStorage: IStorage, scope: StorageScope): cognidream {
		this.withPausedEmitters(() => {
	...
	protected abstract doInitialize(): Promise<cognidream>;
	...
	protected abstract switchToProfile(toProfile: IUserDataProfile, preserveData: boolean): Promise<cognidream>;
	protected abstract switchToWorkspace(toWorkspace: IAnyWorkspaceIdentifier | IUserDataProfile, preserveData: boolean): Promise<cognidream>;
}
...
export class InMemoryStorageService extends AbstractStorageService {
	...
	protected async doInitialize(): Promise<cognidream> { }

	protected async switchToProfile(): Promise<cognidream> {
		// no-op when in-memory
	}

	protected async switchToWorkspace(): Promise<cognidream> {
		// no-op when in-memory
	}
...
export async function logStorage(application: Map<string, string>, profile: Map<string, string>, workspace: Map<string, string>, applicationPath: string, profilePath: string, workspacePath: string): Promise<cognidream> {
