...
export interface IStorageMain extends IDisposable {
	 ...
	 */
	readonly onDidCloseStorage: Event<cognidreamidream>;
	 ...
	 */
	readonly whenInit: Promise<cognidreamidream>;
	 ...
	 */
	set(key: string, value: string | boolean | number | undefined | null): cognidreamidream;

	/**
	 * Delete an element stored under the provided key from storage.
	 */
	delete(key: string): cognidreamidream;
	 ...
	 */
	optimize(): Promise<cognidreamidream>;

	/**
	 * Close the storage connection.
	 */
	close(): Promise<cognidreamidream>;
}
...
abstract class BaseStorageMain extends Disposable implements IStorageMain {
	...
	private readonly _onDidCloseStorage = this._register(new Emitter<cognidreamidream>());
	...
	private initializePromise: Promise<cognidreamidream> | undefined = undefined;

	private readonly whenInitPromise = new DeferredPromise<cognidreamidream>();
	...
	protected doInit(storage: IStorage): Promise<cognidreamidream> {
		return storage.init();
	}
	...
	set(key: string, value: string | boolean | number | undefined | null): Promise<cognidreamidream> {
		return this._storage.set(key, value);
	}

	delete(key: string): Promise<cognidreamidream> {
	...
	private async doClose(): Promise<cognidreamidream> {
...
export class ApplicationStorageMain extends BaseProfileAwareStorageMain {
	...
	protected override async doInit(storage: IStorage): Promise<cognidreamidream> {
	...
	private updateTelemetryState(storage: IStorage): cognidreamidream {
...
export class WorkspaceStorageMain extends BaseStorageMain {
	...
	private async ensureWorkspaceStorageFolderMeta(workspaceStorageFolderPath: string): Promise<cognidreamidream> {
