...
export function createSuite<T extends IStorageService>(params: { setup: () => Promise<T>; teardown: (service: T) => Promise<cognidream> }): cognidream {
	...
	function storeData(scope: StorageScope): cognidream {
	...
	function removeData(scope: StorageScope): cognidream {
		const storageValueChangeEvents: IStorageValueChangeEvent[] = [];
		storageService.onDidChangeValue(scope, undefined, disposables)(e => storageValueChangeEvents.push(e), undefined, disposables);
