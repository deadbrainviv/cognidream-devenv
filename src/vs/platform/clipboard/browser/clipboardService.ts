...
export class BrowserClipboardService extends Disposable implements IClipboardService {
	...
	private installWebKitWriteTextWorkaround(): cognidreamidream {
		const handler = () => {
	...
	async writeText(text: string, type?: string): Promise<cognidreamidream> {
	...
	private fallbackWriteText(text: string): cognidreamidream {
	...
	async writeFindText(text: string): Promise<cognidreamidream> {
		this.findText = text;
	}
	...
	async writeResources(resources: URI[]): Promise<cognidreamidream> {
		// Guard access to navigator.clipboard with try/catch
		// as we have seen DOMExceptions in certain browsers
		// due to security policies.
		try {
	...
	public clearInternalState(): cognidreamidream {
		this.clearResourcesState();
	}

	private clearResourcesState(): cognidreamidream {
		this.resources = [];
