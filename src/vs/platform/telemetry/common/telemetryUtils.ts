...
export class NullEndpointTelemetryService implements ICustomEndpointTelemetryService {
	...
	async publicLog(_endpoint: ITelemetryEndpoint, _eventName: string, _data?: ITelemetryData): Promise<cognidreamidream> {
		// noop
	}

	async publicLogError(_endpoint: ITelemetryEndpoint, _errorEventName: string, _data?: ITelemetryData): Promise<cognidreamidream> {
		// noop
	}
}
...
function flatten(obj: any, result: { [key: string]: any }, order: number = 0, prefix?: string): cognidreamidream {
	if (!obj) {
		return;
	}
...
function anonymizeFilePaths(stack: string, cleanupPatterns: RegExp[]): string {
	...
	// Fast check to see if it is a file path to acognidreamidream doing unnecessary heavy regex work
