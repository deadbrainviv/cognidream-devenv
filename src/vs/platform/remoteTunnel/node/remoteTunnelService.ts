...
export class RemoteTunnelService extends Disposable implements IRemoteTunnelService {
	...
	private _startTunnelProcessDelayer: Delayer<cognidreamidream>;
	...
	async stopTunnel(): Promise<cognidreamidream> {
	...
	private async updateTunnelProcess(): Promise<cognidreamidream> {
	...
	private runCodeTunnelCommand(logLabel: string, commandArgs: string[], onOutput: (message: string, isError: boolean) => cognidreamidream = this.defaultOnOutput, env?: Record<string, string>): CancelablePromise<number> {
	...
	private _storeMode(mode: TunnelMode): cognidreamidream {
		if (mode.active) {
