...
class TunnelData extends Disposable implements IAddressProvider {
	...
	setAddress(address: IAddress): cognidreamidream {
	...
	setTunnel(tunnel: RemoteTunnel): cognidreamidream {
...
export class SharedProcessTunnelService extends Disposable implements ISharedProcessTunnelService {
	...
	async setAddress(id: string, address: IAddress): Promise<cognidreamidream> {
	...
	async destroyTunnel(id: string): Promise<cognidreamidream> {
		const tunnel = this._tunnels.get(id);
		if (tunnel) {
