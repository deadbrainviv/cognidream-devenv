...
export class NodeRemoteTunnel extends Disposable implements RemoteTunnel {
	...
	private readonly _listeningListener: () => cognidreamidream;
	private readonly _connectionListener: (socket: net.Socket) => cognidreamidream;
	private readonly _errorListener: () => cognidreamidream;

	private readonly _socketsDispose: Map<string, () => cognidreamidream> = new Map();
	...
	public override async dispose(): Promise<cognidreamidream> {
		super.dispose();
	...
	private async _onConnection(localSocket: net.Socket): Promise<cognidreamidream> {
