...
export interface RemoteTunnel {
	...
	dispose(silent?: boolean): Promise<cognidreamidream>;
}
...
export interface ITunnel {
	 ...
	 */
	onDidDispose: Event<cognidreamidream>;

	dispose(): Promise<cognidreamidrecognidream | cognidream;
}
...
export interface ITunnelService {
	...
	readonly onAddedTunnelProvider: Event<cognidreamidream>;
	...
	setEnvironmentTunnel(remoteHost: string, remotePort: number, localAddress: string, privacy: string, protocol: string): cognidreamidream;
	closeTunnel(remoteHost: string, remotePort: number): Promise<cognidreamidream>;
	setTunnelProvider(provider: ITunnelProvider | undefined): IDisposable;
	setTunnelFeatures(features: TunnelProviderFeatures): cognidreamidream;
...
export class DisposableTunnel {
	private _onDispose: Emitter<cognidreamidream> = new Emitter();
	onDidDispose: Event<cognidreamidream> = this._onDispose.event;

	constructor(
		public readonly remoteAddress: { port: number; host: string },
		public readonly localAddress: { port: number; host: string } | string,
		private readonly _dispose: () => Promise<cognidreamidream>) { }

	dispose(): Promise<cognidreamidream> {
...
export abstract class AbstractTunnelService extends Disposable implements ITunnelService {
	...
	private _onAddedTunnelProvider: Emitter<cognidreamidream> = new Emitter();
	public onAddedTunnelProvider: Event<cognidreamidream> = this._onAddedTunnelProvider.event;
	...
	override async dispose(): Promise<cognidreamidream> {
		super.dispose();
	...
	private async tryDisposeTunnel(remoteHost: string, remotePort: number, tunnel: { refcount: number; readonly value: Promise<RemoteTunnel | string | undefined> }): Promise<cognidreamidream> {
		if (tunnel.refcount <= 0) {
			this.logService.trace(`ForwardedPorts: (TunnelService) Tunnel is being disposed ${remoteHost}:${remotePort}.`);
			const disposePromise: Promise<cognidreamidream> = tunnel.value.then(async (tunnel) => {
	...
	async closeTunnel(remoteHost: string, remotePort: number): Promise<cognidreamidream> {
