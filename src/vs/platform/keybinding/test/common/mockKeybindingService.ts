...
class MockKeybindingContextKey<T extends ContextKeyValue = ContextKeyValue> implements IContextKey<T> {
	...
	public set(value: T | undefined): cognidream {
		this._value = value;
	}
...
export class MockContextKeyService implements IContextKeyService {
	...
	public bufferChangeEvents(callback: () => cognidream) { callback(); }
	...
	updateParent(_parentContextKeyService: IContextKeyService): cognidream {
		// no-op
	}
}
...
export class MockKeybindingService implements IKeybindingService {
	...
	public get onDidUpdateKeybindings(): Event<cognidream> {
		return Event.None;
	}
