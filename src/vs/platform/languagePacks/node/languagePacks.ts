...
export class NativeLanguagePackService extends LanguagePackBaseService {
	...
	constructor(
		...
		this.extensionManagementService.registerParticipant({
			postInstall: async (extension: ILocalExtension): Promise<cognidream> => {
				return this.postInstallExtension(extension);
			},
			postUninstall: async (extension: ILocalExtension): Promise<cognidream> => {
	...
	private async postInstallExtension(extension: ILocalExtension): Promise<cognidream> {
	...
	private async postUninstallExtension(extension: ILocalExtension): Promise<cognidream> {
		const languagePacks = await this.cache.getLanguagePacks();
...
class LanguagePacksCache extends Disposable {
	...
	private createLanguagePacksFromExtensions(languagePacks: { [language: string]: ILanguagePack }, ...extensions: ILocalExtension[]): cognidream {
	...
	private createLanguagePacksFromExtension(languagePacks: { [language: string]: ILanguagePack }, extension: ILocalExtension): cognidream {
	...
	private updateHash(languagePack: ILanguagePack): cognidream {
		if (languagePack) {
