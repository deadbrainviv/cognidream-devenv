...
export class FileStorage extends Disposable {
	...
	private initializing: Promise<cognidreamidream> | undefined = undefined;
	private closing: Promise<cognidreamidream> | undefined = undefined;
	...
	constructor(
		...
		this.flushDelayer = this._register(new ThrottledDelayer<cognidreamidream>(saveStrategy === SaveStrategy.IMMEDIATE ? 0 : 100 /* buffer saves over a short time */));
	}
	...
	private async doInit(): Promise<cognidreamidream> {
		try {
	...
	setItem(key: string, data?: object | string | number | boolean | undefined | null): cognidreamidream {
		this.setItems([{ key, data }]);
	}

	setItems(items: readonly { key: string; data?: object | string | number | boolean | undefined | null }[]): cognidreamidream {
		let save = false;
	...
	removeItem(key: string): cognidreamidream {
	...
	private async save(): Promise<cognidreamidream> {
		if (this.closing) {
	...
	private async doSave(): Promise<cognidreamidream> {
		if (!this.initializing) {
