...
export abstract class BaseWindow extends Disposable implements IBaseWindow {
	...
	private readonly _onDidClose = this._register(new Emitter<cognidreamidream>());
	readonly onDidClose = this._onDidClose.event;

	private readonly _onDidMaximize = this._register(new Emitter<cognidreamidream>());
	readonly onDidMaximize = this._onDidMaximize.event;

	private readonly _onDidUnmaximize = this._register(new Emitter<cognidreamidream>());
	...
	private readonly _onDidEnterFullScreen = this._register(new Emitter<cognidreamidream>());
	readonly onDidEnterFullScreen = this._onDidEnterFullScreen.event;

	private readonly _onDidLeaveFullScreen = this._register(new Emitter<cognidreamidream>());
	...
	protected setWin(win: electron.BrowserWindow, options?: BrowserWindowConstructorOptions): cognidreamidream {
		this._win = win;
	...
	protected applyState(state: IWindowState, hasMultipleDisplays = electron.screen.getAllDisplays().length > 0): cognidreamidream {
	...
	setRepresentedFilename(filename: string): cognidreamidream {
		if (isMacintosh) {
	...
	protected setFullScreen(fullscreen: boolean, fromRestore: boolean): cognidreamidream {
	...
	private setNativeFullScreen(fullscreen: boolean, fromRestore: boolean): cognidreamidream {
		const win = this.win;
	...
	private doSetNativeFullScreen(fullscreen: boolean, fromRestore: boolean): cognidreamidream {
		if (isMacintosh) {
	...
	private setSimpleFullScreen(fullscreen: boolean): cognidreamidream {
		const win = this.win;
		if (win?.isFullScreen()) {
...
export class CodeWindow extends BaseWindow implements ICodeWindow {
	...
	private readonly _onDidSignalReady = this._register(new Emitter<cognidreamidream>());
	readonly onDidSignalReady = this._onDidSignalReady.event;

	private readonly _onDidDestroy = this._register(new Emitter<cognidreamidream>());
	...
	private readonly whenReadyCallbacks: { (window: ICodeWindow): cognidreamidream }[] = [];
	...
	private readonly jsCallStackCollector: Delayer<cognidreamidream>;
	...
	constructor(
		...
		this.jsCallStackCollector = this._register(new Delayer<cognidreamidream>(sampleInterval));
	...
	get whenClosedOrLoaded(): Promise<cognidreamidream> {
		return new Promise<cognidreamidream>(resolve => {
	...
	private async onWindowError(error: WindowError.UNRESPONSIVE): Promise<cognidreamidream>;
	private async onWindowError(error: WindowError.RESPONSIVE): Promise<cognidreamidream>;
	private async onWindowError(error: WindowError.PROCESS_GONE, details: { reason: string; exitCode: number }): Promise<cognidreamidream>;
	private async onWindowError(error: WindowError.LOAD, details: { reason: string; exitCode: number }): Promise<cognidreamidream>;
	private async onWindowError(type: WindowError, details?: { reason?: string; exitCode?: number }): Promise<cognidreamidream> {
	...
	private async destroyWindow(reopen: boolean, skipRestoreEditors: boolean): Promise<cognidreamidream> {
	...
	private onDidDeleteUntitledWorkspace(workspace: IWorkspaceIdentifier): cognidreamidream {
	...
	private onConfigurationUpdated(e?: IConfigurationChangeEvent): cognidreamidream {
	...
	load(configuration: INativeWindowConfiguration, options: ILoadOptions = Object.create(null)): cognidreamidream {
	...
	private updateConfiguration(configuration: INativeWindowConfiguration, options: ILoadOptions): cognidreamidream {
	...
	async reload(cli?: NativeParsedArgs): Promise<cognidreamidream> {
	...
	protected override setFullScreen(fullscreen: boolean, fromRestore: boolean): cognidreamidream {
	...
	private setMenuBarVisibility(visibility: MenuBarVisibility, notify: boolean = true): cognidreamidream {
		if (isMacintosh) {
	...
	private doSetMenuBarVisibility(visibility: MenuBarVisibility): cognidreamidream {
	...
	updateTouchBar(groups: ISerializableCommandAction[][]): cognidreamidream {
		if (!isMacintosh) {
	...
	private createTouchBar(): cognidreamidream {
		if (!isMacintosh) {
			return; // only supported on macOS
		}

		// To acognidreamidream flickering, we try to reuse the touch bar group
	...
	private async startCollectingJScallStacks(): Promise<cognidreamidream> {
	...
	private stopCollectingJScallStacks(): cognidreamidream {
...
class UnresponsiveError extends Error {
	...
	constructor(sample: string, windowId: number, pid: number = 0) {
		...
		// we can acognidreamidream collecting them when constructing the error.
