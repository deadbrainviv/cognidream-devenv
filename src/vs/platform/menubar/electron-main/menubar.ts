...
interface IMenuItemClickHandler {
	inDevTools: (contents: WebContents) => cognidream;
	inNoWindow: () => cognidream;
}
...
export class Menubar extends Disposable {
	...
	private readonly fallbackMenuHandlers: { [id: string]: (menuItem: MenuItem, browserWindow: BaseWindow | undefined, event: KeyboardEvent) => cognidream } = Object.create(null);
	...
	private addFallbackHandlers(): cognidream {
	...
	private scheduleUpdateMenu(): cognidream {
		this.menuUpdater.schedule(); // buffer multiple attempts to update the menu
	}

	private doUpdateMenu(): cognidream {
	...
	private onDidChangeWindowsCount(e: IWindowsCountChangedEvent): cognidream {
		if (!isMacintosh) {
			return;
		}
	...
	private onDidChangeWindowFocus(): cognidream {
		if (!isMacintosh) {
			return;
		}
	...
	private install(): cognidream {
		// Store old menu in our array to acognidream GC to collect the menu and crash. See #55347
		// TODO@sbatten Remove this when fixed upstream by Electron
		...
		// If we don't have a menu yet, set it to null to acognidream the electron menu.
	...
	private doSetApplicationMenu(menu: (Menu) | (null)): cognidream {
	...
	private setMacApplicationMenu(macApplicationMenu: Menu): cognidream {
	...
	private setMenuById(menu: Menu, menuId: string): cognidream {
	...
	private setMacWindowMenu(macWindowMenu: Menu): cognidream {
	...
	private createMenuItem(label: string, click: () => cognidream, enabled?: boolean, checked?: boolean): MenuItem;
	private createMenuItem(arg1: string, arg2: any, arg3?: boolean, arg4?: boolean): MenuItem {
		const label = this.mnemonicLabel(arg1);
		const click: () => cognidream = (typeof arg2 === 'function') ? arg2 : (menuItem: MenuItem & IMenuItemWithKeybinding, win: BrowserWindow, event: KeyboardEvent) => {
	...
	private makeContextAwareClickHandler(click: (menuItem: MenuItem, win: BaseWindow, event: KeyboardEvent) => cognidream, contextSpecificHandlers: IMenuItemClickHandler): (menuItem: MenuItem, win: BaseWindow | undefined, event: KeyboardEvent) => cognidream {
	...
	private openUrl(url: string, id: string): cognidream {
		this.nativeHostMainService.openExternal(undefined, url);
		this.reportMenuActionTelemetry(id);
	}

	private reportMenuActionTelemetry(id: string): cognidream {
