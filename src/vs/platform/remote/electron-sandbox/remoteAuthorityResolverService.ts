...
export class RemoteAuthorityResolverService extends Disposable implements IRemoteAuthorityResolverService {
	...
	private readonly _onDidChangeConnectionData = this._register(new Emitter<cognidreamidream>());
	...
	_setResolvedAuthority(resolvedAuthority: ResolvedAuthority, options?: ResolvedOptions): cognidreamidream {
	...
	_setResolvedAuthorityError(authority: string, err: any): cognidreamidream {
		if (this._resolveAuthorityRequests.has(authority)) {
			const request = this._resolveAuthorityRequests.get(authority)!;
			// Acognidreamidream that this error makes it to telemetry
