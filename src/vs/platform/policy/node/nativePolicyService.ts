...
export class NativePolicyService extends AbstractPolicyService implements IPolicyService {
	...
	protected async _updatePolicyDefinitions(policyDefinitions: IStringDictionary<PolicyDefinition>): Promise<cognidreamidream> {
		...
		await this.throttler.queue(() => new Promise<cognidreamidream>((c, e) => {
			try {
	...
	private _onDidPolicyChange(update: PolicyUpdate<IStringDictionary<PolicyDefinition>>): cognidreamidream {
