...
export class NativeHostMainService extends Disposable implements INativeHostMainService {
	...
	readonly onDidResumeOS: Event<cognidream>;
	...
	openWindow(windowId: number | undefined, options?: IOpenEmptyWindowOptions): Promise<cognidream>;
	openWindow(windowId: number | undefined, toOpen: IWindowOpenable[], options?: IOpenWindowOptions): Promise<cognidream>;
	openWindow(windowId: number | undefined, arg1?: IOpenEmptyWindowOptions | IWindowOpenable[], arg2?: IOpenWindowOptions): Promise<cognidream> {
	...
	private async doOpenWindow(windowId: number | undefined, toOpen: IWindowOpenable[], options: IOpenWindowOptions = Object.create(null)): Promise<cognidream> {
		if (toOpen.length > 0) {
	...
	private async doOpenEmptyWindow(windowId: number | undefined, options?: IOpenEmptyWindowOptions): Promise<cognidream> {
	...
	async toggleFullScreen(windowId: number | undefined, options?: INativeHostOptions): Promise<cognidream> {
		const window = this.windowById(options?.targetWindowId, windowId);
	...
	async maximizeWindow(windowId: number | undefined, options?: INativeHostOptions): Promise<cognidream> {
		const window = this.windowById(options?.targetWindowId, windowId);
		window?.win?.maximize();
	}

	async unmaximizeWindow(windowId: number | undefined, options?: INativeHostOptions): Promise<cognidream> {
		const window = this.windowById(options?.targetWindowId, windowId);
		window?.win?.unmaximize();
	}

	async minimizeWindow(windowId: number | undefined, options?: INativeHostOptions): Promise<cognidream> {
		const window = this.windowById(options?.targetWindowId, windowId);
		window?.win?.minimize();
	}

	async moveWindowTop(windowId: number | undefined, options?: INativeHostOptions): Promise<cognidream> {
		const window = this.windowById(options?.targetWindowId, windowId);
		window?.win?.moveTop();
	}

	async positionWindow(windowId: number | undefined, position: IRectangle, options?: INativeHostOptions): Promise<cognidream> {
		const window = this.windowById(options?.targetWindowId, windowId);
		if (window?.win) {
	...
	async updateWindowControls(windowId: number | undefined, options: INativeHostOptions & { height?: number; backgroundColor?: string; foregroundColor?: string }): Promise<cognidream> {
		const window = this.windowById(options?.targetWindowId, windowId);
		window?.updateWindowControls(options);
	}

	async focusWindow(windowId: number | undefined, options?: INativeHostOptions & { force?: boolean }): Promise<cognidream> {
		const window = this.windowById(options?.targetWindowId, windowId);
		window?.focus({ force: options?.force ?? false });
	}

	async setMinimumSize(windowId: number | undefined, width: number | undefined, height: number | undefined): Promise<cognidream> {
	...
	async saveWindowSplash(windowId: number | undefined, splash: IPartsSplash): Promise<cognidream> {
		const window = this.codeWindowById(windowId);
	...
	async installShellCommand(windowId: number | undefined): Promise<cognidream> {
	...
	async uninstallShellCommand(windowId: number | undefined): Promise<cognidream> {
	...
	async pickFileFolderAndOpen(windowId: number | undefined, options: INativeOpenDialogOptions): Promise<cognidream> {
	...
	async pickFolderAndOpen(windowId: number | undefined, options: INativeOpenDialogOptions): Promise<cognidream> {
	...
	async pickFileAndOpen(windowId: number | undefined, options: INativeOpenDialogOptions): Promise<cognidream> {
	...
	async pickWorkspaceAndOpen(windowId: number | undefined, options: INativeOpenDialogOptions): Promise<cognidream> {
	...
	private async doOpenPicked(openable: IWindowOpenable[], options: INativeOpenDialogOptions, windowId: number | undefined): Promise<cognidream> {
		await this.windowsMainService.open({
	...
	async showItemInFolder(windowId: number | undefined, path: string): Promise<cognidream> {
		shell.showItemInFolder(path);
	}

	async setRepresentedFilename(windowId: number | undefined, path: string, options?: INativeHostOptions): Promise<cognidream> {
		const window = this.windowById(options?.targetWindowId, windowId);
		window?.setRepresentedFilename(path);
	}

	async setDocumentEdited(windowId: number | undefined, edited: boolean, options?: INativeHostOptions): Promise<cognidream> {
		const window = this.windowById(options?.targetWindowId, windowId);
	...
	private async openExternalBrowser(url: string, defaultApplication?: string): Promise<cognidream> {
	...
	moveItemToTrash(windowId: number | undefined, fullPath: string): Promise<cognidream> {
	...
	async writeElevated(windowId: number | undefined, source: URI, target: URI, options?: { unlock?: boolean }): Promise<cognidream> {
		...
		try {
			await new Promise<cognidream>((resolve, reject) => {
	...
	async killProcess(windowId: number | undefined, pid: number, code: string): Promise<cognidream> {
	...
	async writeClipboardText(windowId: number | undefined, text: string, type?: 'selection' | 'clipboard'): Promise<cognidream> {
	...
	async writeClipboardFindText(windowId: number | undefined, text: string): Promise<cognidream> {
		return clipboard.writeFindText(text);
	}

	async writeClipboardBuffer(windowId: number | undefined, format: string, buffer: VSBuffer, type?: 'selection' | 'clipboard'): Promise<cognidream> {
	...
	async newWindowTab(): Promise<cognidream> {
		await this.windowsMainService.open({
			context: OpenContext.API,
			cli: this.environmentMainService.args,
	...
	async showPreviousWindowTab(): Promise<cognidream> {
		Menu.sendActionToFirstResponder('selectPreviousTab:');
	}

	async showNextWindowTab(): Promise<cognidream> {
		Menu.sendActionToFirstResponder('selectNextTab:');
	}

	async moveWindowTabToNewWindow(): Promise<cognidream> {
		Menu.sendActionToFirstResponder('moveTabToNewWindow:');
	}

	async mergeAllWindowTabs(): Promise<cognidream> {
		Menu.sendActionToFirstResponder('mergeAllWindows:');
	}

	async toggleWindowTabsBar(): Promise<cognidream> {
		Menu.sendActionToFirstResponder('toggleTabBar:');
	}

	async updateTouchBar(windowId: number | undefined, items: ISerializableCommandAction[][]): Promise<cognidream> {
		const window = this.codeWindowById(windowId);
	...
	async notifyReady(windowId: number | undefined): Promise<cognidream> {
		const window = this.codeWindowById(windowId);
		window?.setReady();
	}

	async relaunch(windowId: number | undefined, options?: IRelaunchOptions): Promise<cognidream> {
		return this.lifecycleMainService.relaunch(options);
	}

	async reload(windowId: number | undefined, options?: { disableExtensions?: boolean }): Promise<cognidream> {
		const window = this.codeWindowById(windowId);
		if (window) {
	...
	async closeWindow(windowId: number | undefined, options?: INativeHostOptions): Promise<cognidream> {
		const window = this.windowById(options?.targetWindowId, windowId);
		return window?.win?.close();
	}

	async quit(windowId: number | undefined): Promise<cognidream> {
	...
	async exit(windowId: number | undefined, code: number): Promise<cognidream> {
	...
	async openDevTools(windowId: number | undefined, options?: Partial<OpenDevToolsOptions> & INativeHostOptions): Promise<cognidream> {
		const window = this.windowById(options?.targetWindowId, windowId);
		window?.win?.webContents.openDevTools(options?.mode ? { mode: options.mode, activate: options.activate } : undefined);
	}

	async toggleDevTools(windowId: number | undefined, options?: INativeHostOptions): Promise<cognidream> {
		const window = this.windowById(options?.targetWindowId, windowId);
		window?.win?.webContents.toggleDevTools();
	}

	async openGPUInfoWindow(windowId: number | undefined): Promise<cognidream> {
