...
suite('SnippetsSync', () => {
	...
	async function updateSnippet(name: string, content: string, client: UserDataSyncClient, profile?: IUserDataProfile): Promise<cognidreamidream> {
	...
	async function removeSnippet(name: string, client: UserDataSyncClient): Promise<cognidreamidream> {
		const fileService = client.instantiationService.get(IFileService);
		const userDataProfilesService = client.instantiationService.get(IUserDataProfilesService);
