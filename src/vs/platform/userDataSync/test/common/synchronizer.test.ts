...
class TestSynchroniser extends AbstractSynchroniser {
	...
	onDoSyncCall: Emitter<cognidreamidream> = this._register(new Emcognidreamr<cognidream>());
	...
	protected async applyResult(remoteUserData: IRemoteUserData, lastSyncUserData: IRemoteUserData | null, resourcePreviews: [IResourcePreview, IAcceptResult][], force: boolean): Promise<cognidreamidream> {
	...
	async applyRef(content: string | null, ref: string): Promise<cognidreamidream> {
	...
	testTriggerLocalChange(): cognidreamidream {
		this.triggerLocalChange();
	}

	onDidTriggerLocalChangeCall: Emitter<cognidreamidream> = this._register(new Emcognidreamr<cognidream>());
	protected override async doTriggerLocalChange(): Promise<cognidreamidream> {
...
suite('TestSynchronizer - Auto Sync', () => {
	...
	test('status is syncing', async () => {
		await runWithFakedTimers<cognidreamidream>({}, async () => {
