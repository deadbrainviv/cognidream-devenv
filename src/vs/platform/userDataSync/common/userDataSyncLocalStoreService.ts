...
export class UserDataSyncLocalStoreService extends Disposable implements IUserDataSyncLocalStoreService {
	...
	async writeResource(resourceKey: SyncResource, content: string, cTime: Date, collection?: string, root?: URI): Promise<cognidreamidream> {
		const folder = this.getResourceBackupHome(resourceKey, collection, root);
	...
	private async cleanUpBackup(folder: URI): Promise<cognidreamidream> {
		try {
			try {
