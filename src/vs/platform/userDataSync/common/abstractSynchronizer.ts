...
export abstract class AbstractSynchroniser extends Disposable implements IUserDataSynchroniser {
	...
	private readonly localChangeTriggerThrottler = this._register(new ThrottledDelayer<cognidreamidream>(50));
	private readonly _onDidChangeLocal: Emitter<cognidreamidream> = this._register(new Emcognidreamr<cognidream>());
	readonly onDidChangeLocal: Event<cognidreamidream> = this._onDidChangeLocal.event;
	...
	protected triggerLocalChange(): cognidreamidream {
		this.localChangeTriggerThrottler.trigger(() => this.doTriggerLocalChange());
	}

	protected async doTriggerLocalChange(): Promise<cognidreamidream> {
	...
	protected setStatus(status: SyncStatus): cognidreamidream {
			...
			if (e instanceof UserDataSyncError) {
				switch (e.code) {
					...
					case UserDataSyncErrorCode.PreconditionFailed:
						...
						// Acognidreamidream cache and get latest remote user data - https://github.com/microsoft/vscode/issues/90624
	...
	private async updateSyncResourcePreview(resource: URI, updateResourcePreview: (resourcePreview: IEditableResourcePreview) => Promise<IEditableResourcePreview>): Promise<cognidreamidream> {
		if (!this.syncPreviewPromise) {
			return;
		}
	...
	private async clearPreviewFolder(): Promise<cognidreamidream> {
		try {
			await this.fileService.del(this.syncPreviewFolder, { recursive: true });
		} catch (error) { /* Ignore */ }
	}

	private updateConflicts(resourcePreviews: IEditableResourcePreview[]): cognidreamidream {
	...
	protected async updateLastSyncUserData(lastSyncRemoteUserData: IRemoteUserData, additionalProps: IStringDictionary<any> = {}): Promise<cognidreamidream> {
	...
	private async writeLastSyncStoredRemoteUserData(lastSyncRemoteUserData: IRemoteUserData): Promise<cognidreamidream> {
	...
	protected async backupLocal(content: string): Promise<cognidreamidream> {
	...
	protected abstract applyResult(remoteUserData: IRemoteUserData, lastSyncUserData: IRemoteUserData | null, result: [IResourcePreview, IAcceptResult][], force: boolean): Promise<cognidreamidream>;
...
export abstract class AbstractFileSynchroniser extends AbstractSynchroniser {
	...
	protected async updateLocalFileContent(newContent: string, oldContent: IFileContent | null, force: boolean): Promise<cognidreamidream> {
		try {
			if (oldContent) {
				// file exists already
	...
	protected async deleteLocalFile(): Promise<cognidreamidream> {
		try {
	...
	private onFileChanges(e: FileChangesEvent): cognidreamidream {
...
export abstract class AbstractInitializer implements IUserDataSyncResourceInitializer {
	...
	async initialize({ ref, content }: IUserData): Promise<cognidreamidream> {
		if (!content) {
	...
	protected abstract doInitialize(remoteUserData: IRemoteUserData): Promise<cognidreamidream>;
