...
export class UserDataAutoSyncService extends Disposable implements IUserDataAutoSyncService {
	...
	private readonly syncTriggerDelayer: ThrottledDelayer<cognidreamidream>;
	...
	constructor(
		...
		super();
		this.syncTriggerDelayer = this._register(new ThrottledDelayer<cognidreamidream>(this.getSyncTriggerDelayTime()));
	...
	private updateAutoSync(): cognidreamidream {
	...
	async turnOn(): Promise<cognidreamidream> {
		this.stopDisableMachineEventually();
		this.lastSyncUrl = this.syncUrl;
		this.updateEnablement(true);
	}

	async turnOff(everywhere: boolean, softTurnOffOnError?: boolean, donotRemoveMachine?: boolean): Promise<cognidreamidream> {
		try {
	...
	private updateEnablement(enabled: boolean): cognidreamidream {
	...
	private async onDidFinishSync(error: Error | undefined): Promise<cognidreamidream> {
	...
	private async disableMachineEventually(): Promise<cognidreamidream> {
	...
	private stopDisableMachineEventually(): cognidreamidream {
		this.storageService.remove(disableMachineEventuallyKey, StorageScope.APPLICATION);
	}

	private sources: string[] = [];
	async triggerSync(sources: string[], options?: SyncOptions): Promise<cognidreamidream> {
		if (this.autoSync.value === undefined) {
...
class AutoSync extends Disposable {
	...
	private readonly _onDidStartSync = this._register(new Emitter<cognidreamidream>());
	...
	private syncPromise: CancelablePromise<cognidreamidream> | undefined;
	...
	private waitUntilNextIntervalAndSync(): cognidreamidream {
	...
	sync(reason: string, disableCache: boolean): Promise<cognidreamidream> {
	...
	private async doSync(reason: string, disableCache: boolean, token: CancellationToken): Promise<cognidreamidream> {
	...
	private async createAndRunSyncTask(disableCache: boolean, token: CancellationToken): Promise<cognidreamidream> {
