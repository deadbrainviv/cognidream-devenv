...
export class UserDataSyncService extends Disposable implements IUserDataSyncService {
	...
	private _onDidResetLocal = this._register(new Emitter<cognidreamidream>());
	readonly onDidResetLocal = this._onDidResetLocal.event;

	private _onDidResetRemote = this._register(new Emitter<cognidreamidream>());
	...
	async createSyncTask(manifest: IUserDataManifest | null, disableCache?: boolean): Promise<IUserDataSyncTask> {
		...
		const that = this;
		let cancellablePromise: CancelablePromise<cognidreamidream> | undefined;
		return {
			manifest,
	...
	async createManualSyncTask(): Promise<IUserDataManualSyncTask> {
		...
		return {
			id: executionId,
			async merge(): Promise<cognidreamidream> {
				return that.sync(manifest, true, executionId, cancellableToken.token);
			},
			async apply(): Promise<cognidreamidream> {
				try {
					try {
			...
			async stop(): Promise<cognidreamidream> {
	...
	private async sync(manifest: IUserDataManifest | null, preview: boolean, executionId: string, token: CancellationToken): Promise<cognidreamidream> {
		this._syncErrors = [];
		try {
	...
	private async syncRemoteProfiles(remoteProfiles: ISyncUserDataProfile[], manifest: IUserDataManifest | null, preview: boolean, executionId: string, token: CancellationToken): Promise<cognidreamidream> {
	...
	private async applyManualSync(manifest: IUserDataManifest | null, executionId: string, token: CancellationToken): Promise<cognidreamidream> {
		try {
			this.setStatus(SyncStatus.Syncing);
	...
	private async stop(): Promise<cognidreamidream> {
	...
	async replace(syncResourceHandle: ISyncResourceHandle): Promise<cognidreamidream> {
		this.checkEnablement();
	...
	async accept(syncResource: IUserDataSyncResource, resource: URI, content: string | null | undefined, apply: boolean | { force: boolean }): Promise<cognidreamidream> {
		this.checkEnablement();
	...
	async reset(): Promise<cognidreamidream> {
		this.checkEnablement();
		await this.resetRemote();
		await this.resetLocal();
	}

	async resetRemote(): Promise<cognidreamidream> {
		this.checkEnablement();
		try {
	...
	async resetLocal(): Promise<cognidreamidream> {
		this.checkEnablement();
	...
	private async cleanUpStaleStorageData(): Promise<cognidreamidream> {
	...
	async cleanUpRemoteData(): Promise<cognidreamidream> {
	...
	async saveRemoteActivityData(location: URI): Promise<cognidreamidream> {
		this.checkEnablement();
		const data = await this.userDataSyncStoreService.getActivityData();
		await this.fileService.writeFile(location, data);
	}

	async extractActivityData(activityDataResource: URI, location: URI): Promise<cognidreamidream> {
	...
	private setStatus(status: SyncStatus): cognidreamidream {
	...
	private updateConflicts(): cognidreamidream {
	...
	private updateLastSyncTime(): cognidreamidream {
		if (this.status === SyncStatus.Idle) {
...
class ProfileSynchronizer extends Disposable {
	...
	private onDidChangeResourceEnablement(syncResource: SyncResource, enabled: boolean): cognidreamidream {
		if (enabled) {
			this.registerSynchronizer(syncResource);
		} else {
	...
	protected registerSynchronizer(syncResource: SyncResource): cognidreamidream {
	...
	private deRegisterSynchronizer(syncResource: SyncResource): cognidreamidream {
	...
	async apply(executionId: string, token: CancellationToken): Promise<cognidreamidream> {
		const syncHeaders = createSyncHeaders(executionId);
...
function reportUserDataSyncError(userDataSyncError: UserDataSyncError, executionId: string, userDataSyncStoreManagementService: IUserDataSyncStoreManagementService, telemetryService: ITelemetryService): cognidreamidream {
