...
async function pollUntil(fn: () => boolean, millis = 1000): Promise<cognidream> {
	while (!fn()) {
...
export class Win32UpdateService extends AbstractUpdateService implements IRelaunchHandler {
	...
	protected override async doDownloadUpdate(state: AvailableForDownload): Promise<cognidream> {
		if (state.update.url) {
	...
	private async cleanup(exceptVersion: string | null = null): Promise<cognidream> {
	...
	protected override async doApplyUpdate(): Promise<cognidream> {
	...
	override async _applySpecificUpdate(packagePath: string): Promise<cognidream> {
