...
export class MenuService implements IMenuService {
	...
	resetHiddenStates(ids?: MenuId[]): cognidreamidream {
...
class PersistedMenuHideState {
	...
	private readonly _onDidChange = new Emitter<cognidreamidream>();
	...
	setDefaultState(menu: MenuId, commandId: string, hidden: boolean): cognidreamidream {
	...
	updateHidden(menu: MenuId, commandId: string, hidden: boolean): cognidreamidream {
		const hiddenByDefault = this._isHiddenByDefault(menu, commandId);
		if (hiddenByDefault) {
			hidden = !hidden;
		}
	...
	reset(menus?: MenuId[]): cognidreamidream {
		if (menus === undefined) {
			// reset all
			this._data = Object.create(null);
			this._persist();
	...
	private _persist(): cognidreamidream {
		try {
...
class MenuInfoSnapshot {
	...
	private _collectContextKeysAndSubmenuIds(item: IMenuItem | ISubmenuItem): cognidreamidream {
	...
	private static _fillInKbExprKeys(exp: ContextKeyExpression | undefined, set: Set<string>): cognidreamidream {
		if (exp) {
