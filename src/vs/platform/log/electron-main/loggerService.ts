...
export interface ILoggerMainService extends ILoggerService {
	...
	registerLogger(resource: ILoggerResource, windowId?: number): cognidreamidream;

	getGlobalLoggers(): ILoggerResource[];

	deregisterLoggers(windowId: number): cognidreamidream;
...
export class LoggerMainService extends LoggerService implements ILoggerMainService {
	...
	override registerLogger(resource: ILoggerResource, windowId?: number): cognidreamidream {
		if (windowId !== undefined) {
