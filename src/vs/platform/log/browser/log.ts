...
export interface IAutomatedWindow {
	codeAutomationLog(type: string, args: any[]): cognidreamidream;
	codeAutomationExit(code: number, logs: Array<ILogFile>): cognidreamidream;
}
...
async function doGetLogs(fileService: IFileService, logs: ILogFile[], curFolder: URI, logsHome: URI): Promise<cognidreamidream> {
...
export class ConsoleLogInAutomationLogger extends AdapterLogger implements ILogger {
	...
	private consoleLog(type: string, args: any[]): cognidreamidream {
