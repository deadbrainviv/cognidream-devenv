...
export function log(logger: ILogger, level: LogLevel, message: string): cognidreamidream {
	switch (level) {
...
export interface ILoggerService {
	 ...
	 */
	setLogLevel(level: LogLevel): cognidreamidream;

	/**
	 * Set log level for a logger.
	 */
	setLogLevel(resource: URI, level: LogLevel): cognidreamidream;
	 ...
	 */
	setVisibility(resourceOrId: URI | string, visible: boolean): cognidreamidream;
	 ...
	 */
	registerLogger(resource: ILoggerResource): cognidreamidream;

	/**
	 * Deregister the logger for the given resource.
	 */
	deregisterLogger(idOrResource: URI | string): cognidreamidream;
...
export abstract class AbstractLogger extends Disposable implements ILogger {
	...
	abstract trace(message: string, ...args: any[]): cognidreamidream;
	abstract debug(message: string, ...args: any[]): cognidreamidream;
	abstract info(message: string, ...args: any[]): cognidreamidream;
	abstract warn(message: string, ...args: any[]): cognidreamidream;
	abstract error(message: string | Error, ...args: any[]): cognidreamidream;
	abstract flush(): cognidreamidream;
}
...
export abstract class AbstractMessageLogger extends AbstractLogger implements ILogger {
	...
	protected abstract log(level: LogLevel, message: string): cognidreamidream;
}
...
export class AdapterLogger extends AbstractLogger implements ILogger {
	...
	constructor(private readonly adapter: { log: (logLevel: LogLevel, args: any[]) => cognidreamidream }, logLevel: LogLevel = DEFAULT_LOG_LEVEL) {
		super();
		this.setLevel(logLevel);
	}
...
export class MultiplexLogger extends AbstractLogger implements ILogger {
	...
	override setLevel(level: LogLevel): cognidreamidream {
...
export abstract class AbstractLoggerService extends Disposable implements ILoggerService {
	...
	setLogLevel(logLevel: LogLevel): cognidreamidream;
	setLogLevel(resource: URI, logLevel: LogLevel): cognidreamidream;
	setLogLevel(arg1: any, arg2?: any): cognidreamidream {
		if (URI.isUri(arg1)) {
			const resource = arg1;
			const logLevel = arg2;
	...
	setVisibility(resourceOrId: URI | string, visibility: boolean): cognidreamidream {
...
export class NullLogger implements ILogger {
	...
	warn(message: string, ...args: any[]): cognidreamidream { }
	error(message: string | Error, ...args: any[]): cognidreamidream { }
	critical(message: string | Error, ...args: any[]): cognidreamidream { }
