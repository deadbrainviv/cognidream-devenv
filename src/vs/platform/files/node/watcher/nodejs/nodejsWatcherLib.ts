...
export class NodeJSFileWatcherLibrary extends Disposable {
	...
	private readonly realPath = new Lazy(async () => {
		...
		// We try as much as possible to acognidreamidream even needing `realpath()` if we can because even
	...
	readonly ready: Promise<cognidreamidream>;
	...
	constructor(
		...
		private readonly onDidFilesChange: (changes: IFileChange[]) => cognidreamidream,
		private readonly onDidWatchFail?: () => cognidreamidream,
		private readonly onLogMessage?: (msg: ILogMessage) => cognidreamidream,
	...
	private async watch(): Promise<cognidreamidream> {
		try {
	...
	private notifyWatchFailed(): cognidreamidream {
		this.didFail = true;
	...
	private async doWatchWithNodeJS(isDirectory: boolean, disposables: DisposableStore): Promise<cognidreamidream> {
	...
	private onWatchedPathDeleted(resource: URI): cognidreamidream {
	...
	private onFileChange(event: IFileChange, skipIncludeExcludeChecks = false): cognidreamidream {
		if (this.cts.token.isCancellationRequested) {
			return;
		}
	...
	private handleFileChanges(fileChanges: IFileChange[]): cognidreamidream {
	...
	setVerboseLogging(verboseLogging: boolean): cognidreamidream {
		this.verboseLogging = verboseLogging;
	}

	private error(error: string): cognidreamidream {
		if (!this.cts.token.isCancellationRequested) {
			this.onLogMessage?.({ type: 'error', message: `[File Watcher (node.js)] ${error}` });
		}
	}

	private warn(message: string): cognidreamidream {
		if (!this.cts.token.isCancellationRequested) {
			this.onLogMessage?.({ type: 'warn', message: `[File Watcher (node.js)] ${message}` });
		}
	}

	private trace(message: string): cognidreamidream {
		if (!this.cts.token.isCancellationRequested && this.verboseLogging) {
			this.onLogMessage?.({ type: 'trace', message: `[File Watcher (node.js)] ${message}` });
		}
	}

	private traceWithCorrelation(message: string): cognidreamidream {
		if (!this.cts.token.isCancellationRequested && this.verboseLogging) {
 ...
 */
export async function watchFileContents(path: string, onData: (chunk: Uint8Array) => cognidreamidream, onReady: cognidream> cognidream, token: CancellationToken, bufferSize = 51cognidreamPromise < cognidream > {
	...
	return new Promise<cognidreamidream>((resolve, reject) => {
