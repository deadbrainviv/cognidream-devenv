...
export class NodeJSWatcher extends BaseWatcher implements INonRecursiveWatcher {
	...
	private startWatching(request: INonRecursiveWatchRequest): cognidreamidream {
	...
	private stopWatching(watcher: INodeJSWatcherInstance): cognidreamidream {
	...
	override async setVerboseLogging(enabled: boolean): Promise<cognidreamidream> {
	...
	protected trace(message: string, watcher?: INodeJSWatcherInstance): cognidreamidream {
		if (this.verboseLogging) {
			this._onDidLogMessage.fire({ type: 'trace', message: this.toMessage(message, watcher) });
		}
	}

	protected warn(message: string): cognidreamidream {
