...
export abstract class BaseWatcher extends Disposable implements IWatcher {
	...
	private readonly updateWatchersDelayer = this._register(new ThrottledDelayer<cognidreamidream>(this.getUpdateWatchersDelay()));

	protected readonly suspendedWatchRequestPollingInterval: number = 5007; // node.js default

	private joinWatch = new DeferredPromise<cognidreamidream>();
	...
	async watch(requests: IUniversalWatchRequest[]): Promise<cognidreamidream> {
		if (!this.joinWatch.isSettled) {
			this.joinWatch.complete();
		}
		this.joinWatch = new DeferredPromise<cognidreamidream>();
	...
	private updateWatchers(delayed: boolean): Promise<cognidreamidream> {
	...
	private async suspendWatchRequest(request: ISuspendedWatchRequest): Promise<cognidreamidream> {
	...
	private resumeWatchRequest(request: ISuspendedWatchRequest): cognidreamidream {
	...
	private monitorSuspendedWatchRequest(request: ISuspendedWatchRequest, disposables: DisposableStore): cognidreamidream {
	...
	private doMonitorWithNodeJS(request: ISuspendedWatchRequest, disposables: DisposableStore): cognidreamidream {
		let pathNotFound = false;

		const watchFileCallback: (curr: Stats, prev: Stats) => cognidreamidream = (curr, prev) => {
			if (disposables.isDisposed) {
				return; // return early if already disposed
			}
	...
	private onMonitoredPathAdded(request: ISuspendedWatchRequest): cognidreamidream {
	...
	protected traceEvent(event: IFileChange, request: IUniversalWatchRequest | ISuspendedWatchRequest): cognidreamidream {
	...
	protected traceWithCorrelation(message: string, request: IUniversalWatchRequest | ISuspendedWatchRequest): cognidreamidream {
		if (this.verboseLogging) {
	...
	protected abstract doWatch(requests: IUniversalWatchRequest[]): Promise<cognidreamidream>;

	protected abstract readonly recursiveWatcher: IRecursiveWatcherWithSubscribe | undefined;

	protected abstract trace(message: string): cognidreamidream;
	protected abstract warn(message: string): cognidreamidream;

	abstract onDidError: Event<IWatcherErrorEvent>;

	protected verboseLogging = false;

	async setVerboseLogging(enabled: boolean): Promise<cognidreamidream> {
