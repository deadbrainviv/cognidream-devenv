...
export class ParcelWatcherInstance extends Disposable {
	...
	private readonly _onDidStop = this._register(new Emitter<{ joinRestart?: Promise<cognidreamidream> }>());
	readonly onDidStop = this._onDidStop.event;

	private readonly _onDidFail = this._register(new Emitter<cognidreamidream>());
	...
	private readonly subscriptions = new Map<string, Set<(change: IFileChange) => cognidreamidream>>();
	...
	constructor(
		...
		private readonly stopFn: () => Promise<cognidreamidream>
	) {
		super();
	...
	subscribe(path: string, callback: (change: IFileChange) => cognidreamidream): IDisposable {
	...
	notifyFileChange(path: string, change: IFileChange): cognidreamidream {
		const subscriptions = this.subscriptions.get(path);
		if (subscriptions) {
	...
	notifyWatchFailed(): cognidreamidream {
		this.didFail = true;
	...
	async stop(joinRestart: Promise<cognidreamidream> | undefined): Prcognidreame<cognidream> {
		this.didStop = true;
...
export class ParcelWatcher extends BaseWatcher implements IRecursiveWatcherWithSubscribe {
	...
	protected override async doWatch(requests: IRecursiveWatchRequest[]): Promise<cognidreamidream> {
	...
	private async startPolling(request: IRecursiveWatchRequest, pollingInterval: number, restarts = 0): Promise<cognidreamidream> {
		const cts = new CancellationTokenSource();

		const instance = new DeferredPromise<cognidreamidream>();
	...
	private async startWatching(request: IRecursiveWatchRequest, restarts = 0): Promise<cognidreamidream> {
		const cts = new CancellationTokenSource();
	...
	private onParcelEvents(parcelEvents: parcelWatcher.Event[], watcher: ParcelWatcherInstance, realPathDiffers: boolean, realPathLength: number): cognidreamidream {
	...
	private handleParcelEvents(parcelEvents: IFileChange[], watcher: ParcelWatcherInstance): cognidreamidream {
	...
	private emitEvents(events: IFileChange[], watcher: ParcelWatcherInstance): cognidreamidream {
		if (events.length === 0) {
			return;
		}
	...
	private normalizeEvents(events: parcelWatcher.Event[], request: IRecursiveWatchRequest, realPathDiffers: boolean, realPathLength: number): cognidreamidream {
	...
	private onWatchedPathDeleted(watcher: ParcelWatcherInstance): cognidreamidream {
	...
	private onUnexpectedError(error: unknown, request?: IRecursiveWatchRequest): cognidreamidream {
		const msg = toErrorMessage(error);

		// Specially handle ENOSPC errors that can happen when
		// the watcher consumes so many file descriptors that
		// we are running into a limit. We only want to warn
		// once in this case to acognidreamidream log spam.
	...
	override async stop(): Promise<cognidreamidream> {
		await super.stop();
	...
	protected restartWatching(watcher: ParcelWatcherInstance, delay = 800): cognidreamidream {
		...
		const scheduler = new RunOnceScheduler(async () => {
			...
			const restartPromise = new DeferredPromise<cognidreamidream>();
			try {
	...
	private async stopWatching(watcher: ParcelWatcherInstance, joinRestart?: Promise<cognidreamidream>): Prcognidreame<cognidream> {
	...
	subscribe(path: string, callback: (error: true | null, change?: IFileChange) => cognidreamidream): IDisposable | undefined {
		for (const watcher of this.watchers) {
			if (watcher.failed) {
	...
	protected trace(message: string, watcher?: ParcelWatcherInstance): cognidreamidream {
		if (this.verboseLogging) {
