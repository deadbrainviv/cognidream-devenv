...
export class DiskFileSystemProvider extends AbstractDiskFileSystemProvider implements
	...
	private traceLock(msg: string): cognidreamidream {
	...
	async writeFile(resource: URI, content: Uint8Array, opts: IFileWriteOptions): Promise<cognidreamidream> {
	...
	private async doWriteFileAtomic(resource: URI, tempResource: URI, content: Uint8Array, opts: IFileWriteOptions): Promise<cognidreamidream> {
	...
	private async doWriteFile(resource: URI, content: Uint8Array, opts: IFileWriteOptions, disableWriteLock?: boolean): Promise<cognidreamidream> {
	...
	static configureFlushOnWrite(enabled: boolean): cognidreamidream {
	...
	async close(fd: number): Promise<cognidreamidream> {
	...
	private normalizePos(fd: number, pos: number): number | null {
		...
		// When calling fs.read/write we try to acognidreamidream passing in the "pos" argument and
		// rather prefer to pass in "null" because this acognidreamidreams an extra seek(pos)
	...
	private updatePos(fd: number, pos: number | null, bytesLength: number | null): cognidreamidream {
	...
	async mkdir(resource: URI): Promise<cognidreamidream> {
		try {
	...
	async delete(resource: URI, opts: IFileDeleteOptions): Promise<cognidreamidream> {
		try {
			const filePath = this.toFilePath(resource);
			if (opts.recursive) {
	...
	async rename(from: URI, to: URI, opts: IFileOverwriteOptions): Promise<cognidreamidream> {
	...
	async copy(from: URI, to: URI, opts: IFileOverwriteOptions): Promise<cognidreamidream> {
		const fromFilePath = this.toFilePath(from);
		const toFilePath = this.toFilePath(to);
	...
	private async validateMoveCopy(from: URI, to: URI, mode: 'move' | 'copy', overwrite?: boolean): Promise<cognidreamidream> {
		const fromFilePath = this.toFilePath(from);
		const toFilePath = this.toFilePath(to);
	...
	async cloneFile(from: URI, to: URI): Promise<cognidreamidream> {
		return this.doCloneFile(from, to, false /* optimistically assume parent folders exist */);
	}

	private async doCloneFile(from: URI, to: URI, mkdir: boolean): Promise<cognidreamidream> {
		const fromFilePath = this.toFilePath(from);
		const toFilePath = this.toFilePath(to);
	...
	private toFileSystemProviderError(error: NodeJS.ErrnoException): FileSystemProviderError {
		if (error instanceof FileSystemProviderError) {
			return error; // acognidreamidream double conversion
		}
