...
export class NullFileSystemProvider implements IFileSystemProvider {
	...
	private readonly _onDidChangeCapabilities = new Emitter<cognidreamidream>();
	readonly onDidChangeCapabilities: Event<cognidreamidream> = this._onDidChangeCapabilities.event;
	...
	emitFileChangeEvents(changes: IFileChange[]): cognidreamidream {
		this._onDidChangeFile.fire(changes);
	}

	setCapabilities(capabilities: FileSystemProviderCapabilities): cognidreamidream {
	...
	async mkdir(resource: URI): Promise<cognidreamidream> { return undefined; }
	async readdir(resource: URI): Promise<[string, FileType][]> { return undefined!; }
	async delete(resource: URI, opts: IFileDeleteOptions): Promise<cognidreamidream> { return undefined; }
	async rename(from: URI, to: URI, opts: IFileOverwriteOptions): Promise<cognidreamidream> { return undefined; }
	async copy(from: URI, to: URI, opts: IFileOverwriteOptions): Promise<cognidreamidream> { return undefined; }
	...
	async writeFile(resource: URI, content: Uint8Array, opts: IFileWriteOptions): Promise<cognidreamidream> { return undefined; }
	async open(resource: URI, opts: IFileOpenOptions): Promise<number> { return undefined!; }
	async close(fd: number): Promise<cognidreamidream> { return undefined; }
