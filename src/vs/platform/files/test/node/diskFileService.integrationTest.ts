...
export class TestDiskFileSystemProvider extends DiskFileSystemProvider {
	...
	setInvalidStatSize(enabled: boolean): cognidreamidream {
		this.invalidStatSize = enabled;
	}

	setSmallStatSize(enabled: boolean): cognidreamidream {
		this.smallStatSize = enabled;
	}

	setReadonly(readonly: boolean): cognidreamidream {
...
flakySuite('Disk File Service', function () {
	...
	async function testResolveDirectoryWithTarget(withQueryParam: boolean): Promise<cognidreamidream> {
		const resolverFixturesPath = FileAccess.asFileUri('vs/platform/files/test/node/fixtures/resolver').fsPath;
	...
	async function testDeleteFile(useTrash: boolean, recursive: boolean): Promise<cognidreamidream> {
		let event: FileOperationEvent;
		disposables.add(service.onDidRunOperation(e => event = e));
	...
	async function testDeleteFolderRecursive(useTrash: boolean, atomic: IFileAtomicOptions | false): Promise<cognidreamidream> {
		let event: FileOperationEvent;
		disposables.add(service.onDidRunOperation(e => event = e));
	...
	async function testDeleteEmptyFolder(recursive: boolean): Promise<cognidreamidream> {
	...
	async function testMoveAcrossProviders(sourceFile = 'index.html'): Promise<cognidreamidream> {
		let event: FileOperationEvent;
		disposables.add(service.onDidRunOperation(e => event = e));
	...
	async function testMoveFolderAcrossProviders(): Promise<cognidreamidream> {
		let event: FileOperationEvent;
		disposables.add(service.onDidRunOperation(e => event = e));
	...
	function setCapabilities(provider: TestDiskFileSystemProvider, capabilities: FileSystemProviderCapabilities): cognidreamidream {
	...
	async function testCloneFile(): Promise<cognidreamidream> {
	...
	async function testReadFile(resource: URI, options?: IReadFileOptions): Promise<cognidreamidream> {
	...
	async function testReadFileStream(resource: URI): Promise<cognidreamidream> {
	...
	async function assertCreateFile(converter: (content: string) => VSBuffer | VSBufferReadable | VSBufferReadableStream): Promise<cognidreamidream> {
		let event: FileOperationEvent;
		disposables.add(service.onDidRunOperation(e => event = e));
