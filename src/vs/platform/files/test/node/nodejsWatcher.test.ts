...
suite.skip('File Watcher (node.js)', function () {
	...
	class TestNodeJSWatcher extends NodeJSWatcher {
		...
		private readonly _onDidWatch = this._register(new Emitter<cognidreamidream>());
		...
		protected override async doWatch(requests: INonRecursiveWatchRequest[]): Promise<cognidreamidream> {
	...
	async function awaitEvent(service: TestNodeJSWatcher, path: string, type: FileChangeType, correlationId?: number | null, expectedCount?: number): Promise<cognidreamidream> {
		if (loggingEnabled) {
			console.log(`Awaiting change type '${toMsg(type)}' on file '${path}'`);
		}

		// Await the event
		await new Promise<cognidreamidream>(resolve => {
			let counter = 0;
	...
	async function basicCrudTest(filePath: string, skipAdd?: boolean, correlationId?: number | null, expectedCount?: number, awaitWatchAfterAdd?: boolean): Promise<cognidreamidream> {
	...
	test('watchFileContents', async function () {
		...
		const readyPromise = new DeferredPromise<cognidreamidream>();
		const chunkPromise = new DeferredPromise<cognidreamidream>();
