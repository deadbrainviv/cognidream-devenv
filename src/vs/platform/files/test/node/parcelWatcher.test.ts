...
export class TestParcelWatcher extends ParcelWatcher {
	...
	async whenReady(): Promise<cognidreamidream> {
...
suite.skip('File Watcher (parcel)', function () {
	...
	function awaitMessage(watcher: TestParcelWatcher, type: 'trace' | 'warn' | 'error' | 'info' | 'debug'): Promise<cognidreamidream> {
		if (loggingEnabled) {
	...
	async function basicCrudTest(filePath: string, correlationId?: number | null, expectedCount?: number): Promise<cognidreamidream> {
