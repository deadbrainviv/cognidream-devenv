...
class IndexedDBFileSystemNode {
	...
	delete(path: string): cognidreamidream {
	...
	private doDelete(pathParts: string[], originalPath: string): cognidreamidream {
...
export class IndexedDBFileSystemProvider extends Disposable implements IFileSystemProviderWithFileReadWriteCapability {
	...
	readonly onDidChangeCapabilities: Event<cognidreamidream> = Event.None;
	...
	private triggerChanges(changes: IFileChange[]): cognidreamidream {
		if (changes.length) {
	...
	private async bulkWrite(files: [URI, Uint8Array][]): Promise<cognidreamidream> {
	...
	private async deleteKeys(keys: string[]): Promise<cognidreamidream> {
		if (keys.length) {
