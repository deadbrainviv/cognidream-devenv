...
export interface IActionWidgetService {
	...
	show<T>(user: string, supportsPreview: boolean, items: readonly IActionListItem<T>[], delegate: IActionListDelegate<T>, anchor: IAnchor, container: HTMLElement | undefined, actionBarActions?: readonly IAction[]): cognidreamidream;
...
class ActionWidgetService extends Disposable implements IActionWidgetService {
	...
	private _onWidgetClosed(didCancel?: boolean): cognidreamidream {
		this._list.value?.hide(didCancel);
	}
}
