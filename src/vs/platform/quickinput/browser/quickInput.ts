...
export interface IQuickInputOptions {
	...
	setContextKey(id?: string): cognidream;
	linkOpenerDelegate(content: string): cognidream;
	returnFocus(): cognidream;
	/**
...
export interface QuickInputUI {
	...
	list: QuickInputTree;
	onDidAccept: Event<cognidream>;
	onDidCustom: Event<cognidream>;
	onDidTriggerButton: Event<IQuickInputButton>;
	ignoreFocusOut: boolean;
	keyMods: Writeable<IKeyMods>;
	show(controller: QuickInput): cognidream;
	setVisibilities(visibilities: Visibilities): cognidream;
	setEnabled(enabled: boolean): cognidream;
	setContextKey(contextKey?: string): cognidream;
	linkOpenerDelegate(content: string): cognidream;
	hide(): cognidream;
}
...
abstract class QuickInput extends Disposable implements IQuickInput {
	...
	private readonly onDisposeEmitter = this._register(new Emitter<cognidream>());
	...
	didHide(reason = QuickInputHideReason.Other): cognidream {
		this.visible = false;
	...
	willHide(reason = QuickInputHideReason.Other): cognidream {
...
export class QuickPick<T extends IQuickPickItem, O extends { useSeparators: boolean } = { useSeparators: false }> extends QuickInput implements IQuickPick<T, O> {
	...
	private doSetValue(value: string, skipUpdate?: boolean): cognidream {
	...
	private handleAccept(inBackground: boolean): cognidream {
	...
	accept(inBackground?: boolean | undefined): cognidream {
