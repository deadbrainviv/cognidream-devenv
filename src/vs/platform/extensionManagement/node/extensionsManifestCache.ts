...
export class ExtensionsManifestCache extends Disposable {
	...
	private onDidInstallExtensions(results: readonly InstallExtensionResult[]): cognidreamidream {
	...
	private onDidUnInstallExtension(e: DidUninstallExtensionEvent): cognidreamidream {
		if (!e.error) {
			this.invalidate(e.profileLocation);
		}
	}

	async invalidate(extensionsManifestLocation: URI | undefined): Promise<cognidreamidream> {
	...
	private async deleteUserCacheFile(profile: IUserDataProfile): Promise<cognidreamidream> {
		try {
