...
export class ExtensionsDownloader extends Disposable {
	...
	private readonly cleanUpPromise: Promise<cognidreamidream>;
	...
	private async downloadFile(extension: IGalleryExtension, location: URI, downloadFn: (location: URI) => Promise<cognidreamidream>): Prcognidreame<cognidream> {
	...
	private async doDownload(extension: IGalleryExtension, name: string, downloadFn: () => Promise<cognidreamidream>, retries: number): Promise<number> {
		let attempts = 1;
		while (true) {
			try {
				await downloadFn();
	...
	protected async validate(zipPath: string, filePath: string): Promise<cognidreamidream> {
		try {
	...
	async delete(location: URI): Promise<cognidreamidream> {
		await this.cleanUpPromise;
	...
	private async cleanUp(): Promise<cognidreamidream> {
		try {
