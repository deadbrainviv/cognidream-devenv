...
export interface INativeServerExtensionManagementService extends IExtensionManagementService {
	...
	deleteExtensions(...extensions: IExtension[]): Promise<cognidreamidream>;
}
...
export class ExtensionManagementService extends AbstractExtensionManagementService implements INativeServerExtensionManagementService {
	...
	protected removeExtension(extension: ILocalExtension): Promise<cognidreamidream> {
	...
	private async downloadVsix(vsix: URI): Promise<{ location: URI; cleanup: () => Promise<cognidreamidream> }> {
	...
	private async onDidChangeExtensionsFromAnotherSource({ added, removed }: DidChangeProfileExtensionsEvent): Promise<cognidreamidream> {
		if (removed) {
	...
	private async watchForExtensionsNotInstalledBySystem(): Promise<cognidreamidream> {
	...
	private async onDidFilesChange(e: FileChangesEvent): Promise<cognidreamidream> {
	...
	private async addExtensionsToProfile(extensions: [ILocalExtension, Metadata | undefined][], profileLocation: URI): Promise<cognidreamidream> {
...
export class ExtensionsScanner extends Disposable {
	...
	constructor(
		private readonly beforeRemovingExtension: (e: ILocalExtension) => Promise<cognidreamidream>,
		@IFileService private readonly fileService: IFileService,
		@IExtensionsScannerService private readonly extensionsScannerService: IExtensionsScannerService,
		@IExtensionsProfileScannerService private readonly extensionsProfileScannerService: IExtensionsProfileScannerService,
	...
	async setExtensionsForRemoval(...extensions: IExtension[]): Promise<cognidreamidream> {
	...
	async deleteExtension(extension: ILocalExtension | IScannedExtension, type: string): Promise<cognidreamidream> {
	...
	async copyExtensions(fromProfileLocation: URI, toProfileLocation: URI, productVersion: IProductVersion): Promise<cognidreamidream> {
	...
	private async deleteExtensionFromLocation(id: string, location: URI, type: string): Promise<cognidreamidream> {
	...
	private withRemovedExtensions(updateFn?: (removed: IStringDictionary<boolean>) => cognidreamidream): Promise<IStringDictionary<boolean>> {
	...
	private async rename(extractPath: string, renamePath: string): Promise<cognidreamidream> {
		try {
	...
	private async initializeExtensionSize(): Promise<cognidreamidream> {
	...
	private async deleteExtensionsMarkedForRemoval(): Promise<cognidreamidream> {
	...
	private async removeTemporarilyDeletedFolders(): Promise<cognidreamidream> {
...
class InstallExtensionInProfileTask extends AbstractExtensionTask<ILocalExtension> implements IInstallExtensionTask {
	...
	private async updateMetadata(extension: ILocalExtension, token: CancellationToken): Promise<cognidreamidream> {
		try {
...
class UninstallExtensionInProfileTask extends AbstractExtensionTask<cognidreamidream> implements IUninstallExtensionTask {
	...
	protected doRun(token: CancellationToken): Promise<cognidreamidream> {
