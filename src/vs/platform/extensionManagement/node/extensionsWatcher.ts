...
export class ExtensionsWatcher extends Disposable {
	...
	private async onDidChangeProfiles(added: readonly IUserDataProfile[]): Promise<cognidreamidream> {
		try {
			if (added.length) {
	...
	private async onAddExtensions(e: ProfileExtensionsEvent): Promise<cognidreamidream> {
	...
	private async onDidAddExtensions(e: DidAddProfileExtensionsEvent): Promise<cognidreamidream> {
		for (const extension of e.extensions) {
	...
	private async onRemoveExtensions(e: ProfileExtensionsEvent): Promise<cognidreamidream> {
		for (const extension of e.extensions) {
			this.removeExtensionWithKey(this.getKey(extension.identifier, extension.version), e.profileLocation);
		}
	}

	private async onDidRemoveExtensions(e: DidRemoveProfileExtensionsEvent): Promise<cognidreamidream> {
		const extensionsToDelete: IExtension[] = [];
		const promises: Promise<cognidreamidream>[] = [];
		for (const extension of e.extensions) {
			const key = this.getKey(extension.identifier, extension.version);
			if (e.error) {
				this.addExtensionWithKey(key, e.profileLocation);
	...
	private onDidFilesChange(e: FileChangesEvent): cognidreamidream {
	...
	private async onDidExtensionsProfileChange(profileLocation: URI): Promise<cognidreamidream> {
	...
	private async populateExtensionsFromProfile(extensionsProfileLocation: URI): Promise<cognidreamidream> {
	...
	private async deleteExtensionsNotInProfiles(toDelete?: IExtension[]): Promise<cognidreamidream> {
		if (!toDelete) {
	...
	private addExtensionWithKey(key: string, extensionsProfileLocation: URI): cognidreamidream {
	...
	private removeExtensionWithKey(key: string, profileLocation: URI): cognidreamidream {
