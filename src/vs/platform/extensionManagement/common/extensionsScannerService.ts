...
export interface IExtensionsScannerService {
	...
	updateManifestMetadata(extensionLocation: URI, metadata: ManifestMetadata): Promise<cognidreamidream>;
	initializeDefaultProfileExtensions(): Promise<cognidreamidream>;
}
...
export abstract class AbstractExtensionsScannerService extends Disposable implements IExtensionsScannerService {
	...
	async updateManifestMetadata(extensionLocation: URI, metaData: ManifestMetadata): Promise<cognidreamidream> {
	...
	async initializeDefaultProfileExtensions(): Promise<cognidreamidream> {
		try {
	...
	private initializeDefaultProfileExtensionsPromise: Promise<cognidreamidream> | undefined = undefined;
	private async doInitializeDefaultProfileExtensions(): Promise<cognidreamidream> {
...
class ExtensionsScanner extends Disposable {
	...
	private async getLocalizedMessages(extensionLocation: URI, extensionManifest: IExtensionManifest, nlsConfiguration: NlsConfiguration): Promise<LocalizedMessages | undefined> {
		...
		const reportErrors = (localized: URI | null, errors: ParseError[]): cognidreamidream => {
			errors.forEach((error) => {
		...
		const reportInvalidFormat = (localized: URI | null): cognidreamidream => {
	...
	private findMessageBundles(extensionLocation: URI, nlsConfiguration: NlsConfiguration): Promise<{ localized: URI; original: URI | null }> {
		return new Promise<{ localized: URI; original: URI | null }>((c, e) => {
			const loop = (locale: string): cognidreamidream => {
...
class CachedExtensionsScanner extends ExtensionsScanner {
	...
	private readonly cacheValidatorThrottler: ThrottledDelayer<cognidreamidream> = this._register(new ThrottledDelayer(3000));

	private readonly _onDidChangeCache = this._register(new Emitter<cognidreamidream>());
	readonly onDidChangeCache = this._onDidChangeCache.event;
	...
	private async writeExtensionCache(cacheFile: URI, cacheContents: IExtensionCacheData): Promise<cognidreamidream> {
		try {
	...
	private async validateCache(): Promise<cognidreamidream> {
		if (!this.input) {
