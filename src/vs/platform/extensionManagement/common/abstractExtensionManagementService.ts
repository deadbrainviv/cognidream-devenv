...
export interface IUninstallExtensionTask {
	...
	waitUntilTaskIsFinished(): Promise<cognidreamidream>;
	cancel(): cognidreamidream;
}
...
export abstract class CommontExtensionManagementService extends Disposable implements IExtensionManagementService {
	...
	abstract uninstall(extension: ILocalExtension, options?: UninstallOptions): Promise<cognidreamidream>;
	abstract uninstallExtensions(extensions: UninstallExtensionInfo[]): Promise<cognidreamidream>;
	...
	abstract resetPinnedStateForAllUserExtensions(pinned: boolean): Promise<cognidreamidream>;
	abstract registerParticipant(pariticipant: IExtensionManagementParticipant): cognidreamidream;
	...
	abstract copyExtensions(fromProfileLocation: URI, toProfileLocation: URI): Promise<cognidreamidream>;
	abstract download(extension: IGalleryExtension, operation: InstallOperation, donotVerifySignature: boolean): Promise<URI>;
	abstract cleanUp(): Promise<cognidreamidream>;
...
export abstract class AbstractExtensionManagementService extends CommontExtensionManagementService implements IExtensionManagementService {
	...
	async uninstall(extension: ILocalExtension, options?: UninstallOptions): Promise<cognidreamidream> {
	...
	registerParticipant(participant: IExtensionManagementParticipant): cognidreamidream {
		this.participants.push(participant);
	}

	async resetPinnedStateForAllUserExtensions(pinned: boolean): Promise<cognidreamidream> {
		try {
	...
	protected async installExtensions(extensions: InstallableExtension[]): Promise<InstallExtensionResult[]> {
		...
		const createInstallExtensionTask = (manifest: IExtensionManifest, extension: IGalleryExtension | URI, options: InstallExtensionTaskOptions, root: IInstallExtensionTask | undefined): cognidreamidream => {
	...
	private async getAllDepsAndPackExtensions(extensionIdentifier: IExtensionIdentifier, manifest: IExtensionManifest, installPreRelease: boolean, productVersion: IProductVersion): Promise<{ gallery: IGalleryExtension; manifest: IExtensionManifest }[]> {
		...
		const collectDependenciesAndPackExtensionsToInstall = async (extensionIdentifier: IExtensionIdentifier, manifest: IExtensionManifest): Promise<cognidreamidream> => {
	...
	async uninstallExtensions(extensions: UninstallExtensionInfo[]): Promise<cognidreamidream> {
		...
		const postUninstallExtension = (extension: ILocalExtension, uninstallOptions: UninstallExtensionTaskOptions, error?: ExtensionManagementError): cognidreamidream => {
			if (error) {
	...
	private checkForDependents(extensionsToUninstall: ILocalExtension[], installed: ILocalExtension[], extensionToUninstall: ILocalExtension): cognidreamidream {
	...
	protected abstract removeExtension(extension: ILocalExtension): Promise<cognidreamidream>;
}
