...
export interface IExtensionStorageService {
	...
	setExtensionState(extension: IExtension | IGalleryExtension | string, state: IStringDictionary<any> | undefined, global: boolean): cognidreamidream;

	readonly onDidChangeExtensionStorageToSync: Event<cognidreamidream>;
	setKeysForSync(extensionIdWithVersion: IExtensionIdWithVersion, keys: string[]): cognidreamidream;
	getKeysForSync(extensionIdWithVersion: IExtensionIdWithVersion): string[] | undefined;

	addToMigrationList(from: string, to: string): cognidreamidream;
...
export class ExtensionStorageService extends Disposable implements IExtensionStorageService {
	...
	static async removeOutdatedExtensionVersions(extensionManagementService: IExtensionManagementService, storageService: IStorageService): Promise<cognidreamidream> {
	...
	private readonly _onDidChangeExtensionStorageToSync = this._register(new Emitter<cognidreamidream>());
	...
	private onDidChangeStorageValue(e: IProfileStorageValueChangeEvent): cognidreamidream {
