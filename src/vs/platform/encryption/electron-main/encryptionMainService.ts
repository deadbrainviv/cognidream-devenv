...
interface ISafeStorageAdditionalAPIs {
	setUsePlainTextEncryption(usePlainText: boolean): cognidream;
...
export class EncryptionMainService implements IEncryptionMainService {
	...
	constructor(
		...
		// cognidream added this as a nice default for linux so you don't need to specify encryption provider
	...
	async setUsePlainTextEncryption(): Promise<cognidream> {
		if (isWindows) {
