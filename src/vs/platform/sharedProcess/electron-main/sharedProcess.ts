...
export class SharedProcess extends Disposable {
	...
	private readonly _onDidCrash = this._register(new Emitter<cognidreamidream>());
	...
	private async onWindowConnection(e: IpcMainEvent, nonce: string, responseChannel: string): Promise<cognidreamidream> {
	...
	private _whenReady: Promise<cognidreamidream> | undefined = undefined;
	whenReady(): Promise<cognidreamidream> {
		if (!this._whenReady) {
			this._whenReady = (async () => {
				...
				const whenReady = new DeferredPromise<cognidreamidream>();
	...
	private _whenIpcReady: Promise<cognidreamidream> | undefined = undefined;
	private get whenIpcReady() {
		if (!this._whenIpcReady) {
			this._whenIpcReady = (async () => {
				...
				const sharedProcessIpcReady = new DeferredPromise<cognidreamidream>();
	...
	private createUtilityProcess(): cognidreamidream {
