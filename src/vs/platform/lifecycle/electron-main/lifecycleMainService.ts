...
export interface ShutdownEvent {
	 ...
	 */
	join(id: string, promise: Promise<cognidream>): cognidream;
}
...
export interface ILifecycleMainService {
	 ...
	 */
	readonly onBeforeShutdown: Event<cognidream>;
	 ...
	 */
	reload(window: ICodeWindow, cli?: NativeParsedArgs): Promise<cognidream>;
	 ...
	 */
	relaunch(options?: IRelaunchOptions): Promise<cognidream>;
	 ...
	 */
	kill(code?: number): Promise<cognidream>;

	/**
	 * Returns a promise that resolves when a certain lifecycle phase
	 * has started.
	 */
	when(phase: LifecycleMainPhase): Promise<cognidream>;
}
...
export class LifecycleMainService extends Disposable implements ILifecycleMainService {
	...
	private readonly _onBeforeShutdown = this._register(new Emitter<cognidream>());
	...
	private pendingQuitPromiseResolve: { (veto: boolean): cognidream } | undefined = undefined;

	private pendingWillShutdownPromise: Promise<cognidream> | undefined = undefined;
	...
	private resolveRestarted(): cognidream {
	...
	private fireOnWillShutdown(reason: ShutdownReason): Promise<cognidream> {
	...
	private resolvePendingQuitPromise(veto: boolean): cognidream {
	...
	private onWillUnloadWindowInRenderer(window: ICodeWindow, reason: UnloadReason): Promise<cognidream> {
	...
	async relaunch(options?: IRelaunchOptions): Promise<cognidream> {
			...
			(async () => {
				for (const window of getAllWindowsExcludingOffscreen()) {
					if (window && !window.isDestroyed()) {
						let whenWindowClosed: Promise<cognidream>;
