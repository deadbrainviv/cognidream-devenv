...
export interface IConfigurationRegistry {
	 ...
	 */
	registerConfigurations(configurations: IConfigurationNode[], validate?: boolean): cognidream;

	/**
	 * Deregister multiple configurations from the registry.
	 */
	deregisterConfigurations(configurations: IConfigurationNode[]): cognidream;
	 ...
	 */
	updateConfigurations(configurations: { add: IConfigurationNode[]; remove: IConfigurationNode[] }): cognidream;

	/**
	 * Register multiple default configurations to the registry.
	 */
	registerDefaultConfigurations(defaultConfigurations: IConfigurationDefaults[]): cognidream;

	/**
	 * Deregister multiple default configurations from the registry.
	 */
	deregisterDefaultConfigurations(defaultConfigurations: IConfigurationDefaults[]): cognidream;

	/**
	 * Bulk update of the configuration registry (default and configurations, remove and add)
	 * @param delta
	 */
	deltaConfiguration(delta: IConfigurationDelta): cognidream;
	 ...
	 */
	notifyConfigurationSchemaUpdated(...configurations: IConfigurationNode[]): cognidream;

	/**
	 * Event that fires whenever a configuration has been
	 * registered.
	 */
	readonly onDidSchemaChange: Event<cognidream>;
	 ...
	 */
	registerOverrideIdentifiers(identifiers: string[]): cognidream;
}
...
class ConfigurationRegistry implements IConfigurationRegistry {
	...
	private readonly _onDidSchemaChange = new Emitter<cognidream>();
	readonly onDidSchemaChange: Event<cognidream> = this._onDidSchemaChange.event;
	...
	public registerConfigurations(configurations: IConfigurationNode[], validate: boolean = true): cognidream {
	...
	public deregisterConfigurations(configurations: IConfigurationNode[]): cognidream {
		const properties = new Set<string>();
	...
	public updateConfigurations({ add, remove }: { add: IConfigurationNode[]; remove: IConfigurationNode[] }): cognidream {
		const properties = new Set<string>();
	...
	public registerDefaultConfigurations(configurationDefaults: IConfigurationDefaults[]): cognidream {
		const properties = new Set<string>();
	...
	public deregisterDefaultConfigurations(defaultConfigurations: IConfigurationDefaults[]): cognidream {
		const properties = new Set<string>();
	...
	private doDeregisterDefaultConfigurations(defaultConfigurations: IConfigurationDefaults[], bucket: Set<string>): cognidream {
	...
	private updateDefaultOverrideProperty(key: string, newDefaultOverride: IConfigurationDefaultOverrideValue, source: IExtensionInfo | undefined): cognidream {
	...
	public deltaConfiguration(delta: IConfigurationDelta): cognidream {
		// defaults: remove
	...
	public registerOverrideIdentifiers(overrideIdentifiers: string[]): cognidream {
		this.doRegisterOverrideIdentifiers(overrideIdentifiers);
		this._onDidSchemaChange.fire();
	}
	...
	private doRegisterConfigurations(configurations: IConfigurationNode[], validate: boolean, bucket: Set<string>): cognidream {
	...
	private doDeregisterConfigurations(configurations: IConfigurationNode[], bucket: Set<string>): cognidream {
	...
	private validateAndRegisterProperties(configuration: IConfigurationNode, validate: boolean = true, extensionInfo: IExtensionInfo | undefined, restrictedProperties: string[] | undefined, scope: ConfigurationScope = ConfigurationScope.WINDOW, bucket: Set<string>): cognidream {
	...
	private updateSchema(key: string, property: IConfigurationPropertySchema): cognidream {
	...
	private removeFromSchema(key: string, property: IConfigurationPropertySchema): cognidream {
	...
	private updateOverridePropertyPatternKey(): cognidream {
	...
	private registerOverridePropertyPatternKey(): cognidream {
		const resourceLanguagePropertiesSchema: IJSONSchema = {
			type: 'object',
			description: nls.localize('overrideSettings.defaultDescription', "Configure editor settings to be overridden for a language."),
			errorMessage: nls.localize('overrideSettings.errorMessage', "This setting does not support per-language configuration."),
	...
	private updatePropertyDefaultValue(key: string, property: IRegisteredConfigurationPropertySchema): cognidream {
