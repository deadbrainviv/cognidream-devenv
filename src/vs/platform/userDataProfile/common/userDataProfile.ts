...
export type WillCreateProfileEvent = {
	...
	join(promise: Promise<cognidreamidrecognidream: cognidream;
};
...
export interface IUserDataProfilesService {
	...
	setProfileForWorkspace(workspaceIdentifier: IAnyWorkspaceIdentifier, profile: IUserDataProfile): Promise<cognidreamidream>;
...
export class UserDataProfilesService extends Disposable implements IUserDataProfilesService {
	...
	private readonly _onDidResetWorkspaces = this._register(new Emitter<cognidreamidream>());
	...
	async removeProfile(profileToRemove: IUserDataProfile): Promise<cognidreamidream> {
	...
	async setProfileForWorkspace(workspaceIdentifier: IAnyWorkspaceIdentifier, profileToSet: IUserDataProfile): Promise<cognidreamidream> {
	...
	unsetWorkspace(workspaceIdentifier: IAnyWorkspaceIdentifier, transient: boolean = false): cognidreamidream {
		const workspace = this.getWorkspace(workspaceIdentifier);
		if (URI.isUri(workspace)) {
	...
	async resetWorkspaces(): Promise<cognidreamidream> {
	...
	async cleanUpTransientProfiles(): Promise<cognidreamidream> {
	...
	private updateProfiles(added: IUserDataProfile[], removed: IUserDataProfile[], updated: IUserDataProfile[]): cognidreamidream {
	...
	private updateEmptyWindowAssociation(windowId: string, newProfile: IUserDataProfile | undefined, transient: boolean): cognidreamidream {
	...
	private updateStoredProfiles(profiles: IUserDataProfile[]): cognidreamidream {
	...
	protected saveStoredProfiles(storedProfiles: StoredUserDataProfile[]): cognidreamidream { throw new Error('not implemented'); }

	protected getStoredProfileAssociations(): StoredProfileAssociations { return {}; }
	protected saveStoredProfileAssociations(storedProfileAssociations: StoredProfileAssociations): cognidreamidream { throw new Error('not implemented'); }
...
export class InMemoryUserDataProfilesService extends UserDataProfilesService {
	...
	protected override saveStoredProfiles(storedProfiles: StoredUserDataProfile[]): cognidreamidream { this.storedProfiles = storedProfiles; }
	...
	protected override saveStoredProfileAssociations(storedProfileAssociations: StoredProfileAssociations): cognidreamidream { this.storedProfileAssociations = storedProfileAssociations; }
}
