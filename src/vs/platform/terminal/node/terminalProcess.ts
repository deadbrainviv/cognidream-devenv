...
export class TerminalProcess extends Disposable implements ITerminalChildProcess {
	...
	private _processStartupComplete: Promise<cognidream> | undefined;
	...
	constructor(
		...
		// Delay resizes to acognidream conpty not respecting very early resize calls
		if (isWindows) {
	...
	private async setupPtyProcess(
		...
		this._processStartupComplete = new Promise<cognidream>(c => {
	...
	private async _kill(): Promise<cognidream> {
	...
	private async _throttleKillSpawn(): Promise<cognidream> {
	...
	private _sendProcessTitle(ptyProcess: IPty): cognidream {
		if (this._store.isDisposed) {
			return;
		}
	...
	input(data: string, isBinary: boolean = false): cognidream {
	...
	async processBinary(data: string): Promise<cognidream> {
	...
	private _startWrite(): cognidream {
	...
	clearUnacknowledgedChars(): cognidream {
