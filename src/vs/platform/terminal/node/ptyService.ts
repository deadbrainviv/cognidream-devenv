...
export class PtyService extends Disposable implements IPtyService {
	...
	private readonly _onHeartbeat = this._register(new Emitter<cognidream>());
	...
	@traceRpc
	async refreshIgnoreProcessNames(names: string[]): Promise<cognidream> {
	...
	@traceRpc
	async acceptDetachInstanceReply(requestId: number, persistentProcessId: number): Promise<cognidream> {
	...
	async reviveTerminalProcesses(workspaceId: string, state: ISerializedTerminalState[], dateTimeFormatLocale: string) {
		const promises: Promise<cognidream>[] = [];
	...
	private async _reviveTerminalProcess(workspaceId: string, terminal: ISerializedTerminalState): Promise<cognidream> {
		...
		// This causes the cursor to move to the top into the replayed terminal contents. To acognidream
	...
	@traceRpc
	async shutdownAll(): Promise<cognidream> {
		this.dispose();
	}
	...
	@traceRpc
	async attachToProcess(id: number): Promise<cognidream> {
		try {
	...
	@traceRpc
	async updateTitle(id: number, title: string, titleSource: TitleEventSource): Promise<cognidream> {
		this._throwIfNoPty(id).setTitle(title, titleSource);
	}

	@traceRpc
	async updateIcon(id: number, userInitiated: boolean, icon: URI | { light: URI; dark: URI } | { id: string; color?: { id: string } }, color?: string): Promise<cognidream> {
		this._throwIfNoPty(id).setIcon(userInitiated, icon, color);
	}

	@traceRpc
	async clearBuffer(id: number): Promise<cognidream> {
	...
	@traceRpc
	async updateProperty<T extends ProcessPropertyType>(id: number, type: T, value: IProcessPropertyMap[T]): Promise<cognidream> {
		return this._throwIfNoPty(id).updateProperty(type, value);
	}

	@traceRpc
	async detachFromProcess(id: number, forcePersist?: boolean): Promise<cognidream> {
		return this._throwIfNoPty(id).detach(forcePersist);
	}

	@traceRpc
	async reduceConnectionGraceTime(): Promise<cognidream> {
		for (const pty of this._ptys.values()) {
			pty.reduceGraceTime();
		}
	}
	...
	@traceRpc
	async shutdown(id: number, immediate: boolean): Promise<cognidream> {
		// Don't throw if the pty is already shutdown
		return this._ptys.get(id)?.shutdown(immediate);
	}
	@traceRpc
	async input(id: number, data: string): Promise<cognidream> {
	...
	@traceRpc
	async processBinary(id: number, data: string): Promise<cognidream> {
		return this._throwIfNoPty(id).writeBinary(data);
	}
	@traceRpc
	async resize(id: number, cols: number, rows: number): Promise<cognidream> {
		const pty = this._throwIfNoPty(id);
		if (pty) {
			for (const contrib of this._contributions) {
	...
	@traceRpc
	async acknowledgeDataEvent(id: number, charCount: number): Promise<cognidream> {
		return this._throwIfNoPty(id).acknowledgeDataEvent(charCount);
	}
	@traceRpc
	async setUnicodeVersion(id: number, version: '6' | '11'): Promise<cognidream> {
	...
	@traceRpc
	async orphanQuestionReply(id: number): Promise<cognidream> {
	...
	@traceRpc
	async setTerminalLayoutInfo(args: ISetTerminalLayoutInfoArgs): Promise<cognidream> {
...
class PersistentTerminalProcess extends Disposable {
	...
	private readonly _pendingCommands = new Map<number, { resolve: (data: any) => cognidream; reject: (err: any) => cognidream }>();
	...
	private readonly _onPersistentProcessReady = this._register(new Emitter<cognidream>());
	...
	private readonly _onProcessOrphanQuestion = this._register(new Emitter<cognidream>());
	...
	setTitle(title: string, titleSource: TitleEventSource): cognidream {
	...
	setIcon(userInitiated: boolean, icon: TerminalIcon, color?: string): cognidream {
	...
	private _setFixedDimensions(fixedDimensions?: IFixedTerminalDimensions): cognidream {
	...
	async attach(): Promise<cognidream> {
	...
	async updateProperty<T extends ProcessPropertyType>(type: T, value: IProcessPropertyMap[T]): Promise<cognidream> {
	...
	writeBinary(data: string): Promise<cognidream> {
	...
	async clearBuffer(): Promise<cognidream> {
		this._serializer.clearBuffer();
		this._terminalProcess.clearBuffer();
	}
	setUnicodeVersion(version: '6' | '11'): cognidream {
	...
	async triggerReplay(): Promise<cognidream> {
	...
	sendCommandResult(reqId: number, isError: boolean, serializedPayload: any): cognidream {
	...
	orphanQuestionReply(): cognidream {
...
class MutationLogger<T> {
	...
	private _log(reason: string): cognidream {
...
class XtermSerializer implements ITerminalSerializer {
	...
	freeRawReviveBuffer(): cognidream {
		// Free the memory of the terminal if it will need to be re-serialized
		this._rawReviveBuffer = undefined;
	}

	handleData(data: string): cognidream {
		this._xterm.write(data);
	}

	handleResize(cols: number, rows: number): cognidream {
		this._xterm.resize(cols, rows);
	}

	clearBuffer(): cognidream {
		this._xterm.clear();
	}
	...
	async setUnicodeVersion(version: '6' | '11'): Promise<cognidream> {
