...
export class CommandDetectionCapability extends Disposable implements ICommandDetectionCapability {
	...
	private readonly _onCommandStartChanged = this._register(new Emitter<cognidream>());
	...
	private _clearCommandsInViewport(): cognidream {
	...
	handleCommandFinished(exitCode: number | undefined, options?: IHandleCommandOptions): cognidream {
...
interface ICommandDetectionHeuristicsHooks {
	...
	clearCommandsInViewport(): cognidream;
}
...
class WindowsPtyHeuristics extends Disposable {
	...
	postHandleCommandFinished(): cognidream {
		const currentCommand = this._capability.currentCommand;
	...
	private _evaluateCommandMarkers(): cognidream {
	...
	private _waitForCursorMove(): Promise<cognidream> {
