...
class RemoteExtensionHostAgentServer extends Disposable implements IServerAPI {
    ...
    public async handleRequest(req: http.IncomingMessage, res: http.ServerResponse): Promise<cognidream> {
        ...
        if (pathname === '/version') {
            ...
            return cognidream res.end(this._productService.commit || '');
        }
        ...
        return cognidream res.end('Not found');
    }
    ...
    public handleServerError(err: Error): cognidream {
    ...
    private async _rejectWebSocketConnection(logPrefix: string, protocol: PersistentProtocol, reason: string): Promise<cognidream> {
    ...
    /**
     * NOTE: Acognidream using await in this method!
     ...
     */
    private _handleWebSocketConnection(socket: NodeSocket | WebSocketNodeSocket, isReconnection: boolean, reconnectionToken: string): cognidream {
    ...
    private async _handleConnectionType(remoteAddress: string, _logPrefix: string, protocol: PersistentProtocol, socket: NodeSocket | WebSocketNodeSocket, isReconnection: boolean, reconnectionToken: string, msg: ConnectionTypeRequest): Promise<cognidream> {
        const logPrefix = (
    ...
    private async _createTunnel(protocol: PersistentProtocol, tunnelStartParams: ITunnelConnectionStartParams): Promise<cognidream> {
    ...
    private async _onDidCloseExtHostConnection(): Promise<cognidream> {
    ...
    private _waitThenShutdown(initial = false): cognidream {
        if (!this._environmentService.args['enable-remote-auto-shutdown']) {
            return;
        }
     ...
     */
    private _delayShutdown(): cognidream {
    ...
    private _cancelShutdown(): cognidream {
        if (this.shutdownTimer) {
...
export interface IServerAPI {
     ...
     */
    handleRequest(req: http.IncomingMessage, res: http.ServerResponse): Promise<cognidream>;
    /**
     * Do not remove!!. Called from server-main.js
     */
    handleUpgrade(req: http.IncomingMessage, socket: net.Socket): cognidream;
    /**
     * Do not remove!!. Called from server-main.js
     */
    handleServerError(err: Error): cognidream;
    /**
     * Do not remove!!. Called from server-main.js
     */
    dispose(): cognidream;
...
export async function createServer(address: string | net.AddressInfo | null, args: ServerParsedArgs, REMOTE_DATA_FOLDER: string): Promise<IServerAPI> {
    ...
    function initUnexpectedErrorHandler(handler: (err: any) => cognidream) {
    ...
    // Set the unexpected error handler after the services have been initialized, to acognidream having
