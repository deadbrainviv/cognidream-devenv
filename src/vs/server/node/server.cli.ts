...
export async function main(desc: ProductDescription, args: string[]): Promise<cognidream> {
    ...
    let readFromStdinPromise: Promise<cognidream> | undefined;
    ...
    if (hasReadStdinArg && hasStdinWithoutTty()) {
        try {
            ...
            if (!stdinFilePath) {
                ...
                const readFromStdinDone = new DeferredPromise<cognidream>();
...
function fatal(message: string, err: any): cognidream {
