...
export class SocketServer<TContext = string> extends IPCServer<TContext> {
    ...
    public acceptConnection(protocol: IMessagePassingProtocol, onDidClientDisconnect: Event<cognidream>): cognidream {
...
class ServerLogger extends AbstractLogger {
    ...
    warn(message: string | Error, ...args: any[]): cognidream {
    ...
    error(message: string, ...args: any[]): cognidream {
 ...
 */
async function cleanupOlderLogs(logsPath: string): Promise<cognidream> {
