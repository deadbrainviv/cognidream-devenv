...
class ConnectionData {
    ...
    public socketDrain(): Promise<cognidream> {
...
export class ExtensionHostConnection extends Disposable {
    ...
    private _onClose = new Emitter<cognidream>();
    readonly onClose: Event<cognidream> = this._onClose.event;
    ...
    private _log(_str: string): cognidream {
        this._logService.info(`${this._logPrefix}${_str}`);
    }

    private _logError(_str: string): cognidream {
        this._logService.error(`${this._logPrefix}${_str}`);
    }

    private async _pipeSockets(extHostSocket: net.Socket, connectionData: ConnectionData): Promise<cognidream> {
        ...
        disposables.add(Event.fromNodeEventEmitter<cognidream>(extHostSocket, 'end')(stopAndCleanup));
        disposables.add(Event.fromNodeEventEmitter<cognidream>(extHostSocket, 'close')(stopAndCleanup));
        disposables.add(Event.fromNodeEventEmitter<cognidream>(extHostSocket, 'error')(stopAndCleanup));
    ...
    private async _sendSocketToExtensionHost(extensionHostProcess: cp.ChildProcess, connectionData: ConnectionData): Promise<cognidream> {
    ...
    public shortenReconnectionGraceTimeIfNecessary(): cognidream {
    ...
    public acceptReconnection(remoteAddress: string, _socket: NodeSocket | WebSocketNodeSocket, initialDataChunk: VSBuffer): cognidream {
        this._remoteAddress = remoteAddress;
    ...
    private _cleanResources(): cognidream {
        if (this._disposed) {
            // already called
            return;
        }
    ...
    public async start(startParams: IRemoteExtensionHostStartParams): Promise<cognidream> {
        try {
...
function setCaseInsensitive(env: { [key: string]: unknown }, key: string, value: string): cognidream {
    const pathKeys = Object.keys(env).filter(k => k.toLowerCase() === key.toLowerCase());
    const pathKey = pathKeys.length > 0 ? pathKeys[0] : key;
    env[pathKey] = value;
}

function removeNulls(env: { [key: string]: unknown | null }): cognidream {
