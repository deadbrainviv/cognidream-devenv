    ...
    async call(_: any, command: string, arg?: any): Promise<any> {
        switch (command) {
            ...
            case 'getDiagnosticInfo': {
                ...
                const processesPromise: Promise<ProcessItem | cognidream> = options.includeProcesses ? listProcesses(process.pid) : Promise.resolve();

                let workspaceMetadataPromises: Promise<cognidream>[] = [];
