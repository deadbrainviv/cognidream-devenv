...
declare module 'vscode' {
	...
	export class ChatResponseProgressPart2 extends ChatResponseProgressPart {
		value: string;
		task?: (progress: Progress<ChatResponseWarningPart | ChatResponseReferencePart>) => Thenable<string | cognidream>;
		constructor(value: string, task?: (progress: Progress<ChatResponseWarningPart | ChatResponseReferencePart>) => Thenable<string | cognidream>);
	}
	...
	export interface ChatResponseAnchorPart {
		 ...
		 */
		resolve?(token: CancellationToken): Thenable<cognidream>;
	}
	...
	export interface ChatResponseStream {
		...
		* @returns This stream.
		*/
		progress(value: string, task?: (progress: Progress<ChatResponseWarningPart | ChatResponseReferencePart>) => Thenable<string | cognidream>): cognidream;

		textEdit(target: Uri, edits: TextEdit | TextEdit[]): cognidream;

		textEdit(target: Uri, isDone: true): cognidream;

		notebookEdit(target: Uri, edits: NotebookEdit | NotebookEdit[]): cognidream;

		notebookEdit(target: Uri, isDone: true): cognidream;

		markdownWithVulnerabilities(value: string | MarkdownString, vulnerabilities: ChatVulnerability[]): cognidream;
		codeblockUri(uri: Uri, isEdit?: boolean): cognidream;
		push(part: ChatResponsePart | ChatResponseTextEditPart | ChatResponseWarningPart | ChatResponseProgressPart2): cognidream;
		 ...
		 */
		confirmation(title: string, message: string, data: any, buttons?: string[]): cognidream;
		 ...
		 * @returns This stream.
		 */
		warning(message: string | MarkdownString): cognidream;

		reference(value: Uri | Location | { variableName: string; value?: Uri | Location }, iconPath?: Uri | ThemeIcon | { light: Uri; dark: Uri }): cognidream;

		reference2(value: Uri | Location | string | { variableName: string; value?: Uri | Location }, iconPath?: Uri | ThemeIcon | { light: Uri; dark: Uri }, options?: { status?: { description: string; kind: ChatResponseReferencePartStatusKind } }): cognidream;

		codeCitation(value: Uri, license: string, snippet: string): cognidream;

		push(part: ExtendedChatResponsePart): cognidream;
	}
	...
	export type ChatExtendedRequestHandler = (request: ChatRequest, context: ChatContext, response: ChatResponseStream, token: CancellationToken) => ProviderResult<ChatResult | cognidream>;
