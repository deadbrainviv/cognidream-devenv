...
export class QuickInput {
	...
	async waitForQuickInputOpened(retryCount?: number): Promise<cognidream> {
		await this.code.waitForActiveElement(QuickInput.QUICK_INPUT_INPUT, retryCount);
	}

	async type(value: string): Promise<cognidream> {
		await this.code.waitForSetValue(QuickInput.QUICK_INPUT_INPUT, value);
	}

	async waitForQuickInputElementFocused(): Promise<cognidream> {
