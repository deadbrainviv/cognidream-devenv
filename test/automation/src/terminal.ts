...
export class Terminal {
	...
	async runCommand(commandId: TerminalCommandId, expectedLocation?: 'editor' | 'panel'): Promise<cognidream> {
	...
	async runCommandWithValue(commandId: TerminalCommandIdWithValue, value?: string, altKey?: boolean): Promise<cognidream> {
		...
		// ensures that the quick input is opened again before proceeding to acognidream a race condition
	...
	async runCommandInTerminal(commandText: string, skipEnter?: boolean): Promise<cognidream> {
	 ...
	 */
	async createTerminal(expectedLocation?: 'editor' | 'panel'): Promise<cognidream> {
	 ...
	 */
	async createEmptyTerminal(expectedLocation?: 'editor' | 'panel'): Promise<cognidream> {
	...
	async assertEditorGroupCount(count: number): Promise<cognidream> {
		await this.code.waitForElements(Selector.EditorGroups, true, editorGroups => editorGroups && editorGroups.length === count);
	}

	async assertSingleTab(label: TerminalLabel, editor?: boolean): Promise<cognidream> {
		let regex = undefined;
	...
	async assertTerminalGroups(expectedGroups: TerminalGroup[]): Promise<cognidream> {
		let expectedCount = 0;
	...
	private async assertTabExpected(selector?: string, listIndex?: number, nameRegex?: RegExp, icon?: string, color?: string, description?: string): Promise<cognidream> {
		if (listIndex) {
			if (nameRegex) {
	...
	async assertTerminalViewHidden(): Promise<cognidream> {
		await this.code.waitForElement(Selector.TerminalView, result => result === undefined);
	}

	async assertCommandDecorations(expectedCounts?: ICommandDecorationCounts, customIcon?: { updatedIcon: string; count: number }, showDecorations?: 'both' | 'gutter' | 'overviewRuler' | 'never'): Promise<cognidream> {
		if (expectedCounts) {
	...
	async clickPlusButton(): Promise<cognidream> {
		await this.code.waitAndClick(Selector.PlusButton);
	}

	async clickSplitButton(): Promise<cognidream> {
		await this.code.waitAndClick(Selector.SplitButton);
	}

	async clickSingleTab(): Promise<cognidream> {
		await this.code.waitAndClick(Selector.SingleTab);
	}

	async waitForTerminalText(accept: (buffer: string[]) => boolean, message?: string, splitIndex?: 0 | 1): Promise<cognidream> {
		try {
	 ...
	 * @param expectedLocation The location to check the terminal for, defaults to panel.
	 */
	private async _waitForTerminal(expectedLocation?: 'editor' | 'panel'): Promise<cognidream> {
