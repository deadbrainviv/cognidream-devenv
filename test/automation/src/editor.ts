...
export class Editor {
	...
	async rename(filename: string, line: number, from: string, to: string): Promise<cognidream> {
	...
	async gotoDefinition(filename: string, term: string, line: number): Promise<cognidream> {
		await this.clickOnTerm(filename, term, line);
	...
	private async clickOnTerm(filename: string, term: string, line: number): Promise<cognidream> {
		const selector = await this.getSelector(filename, term, line);
		await this.code.waitAndClick(selector);
	}

	async waitForEditorFocus(filename: string, lineNumber: number, selectorPrefix = ''): Promise<cognidream> {
	...
	async waitForEditorSelection(filename: string, accept: (selection: { selectionStart: number; selectionEnd: number }) => boolean): Promise<cognidream> {
