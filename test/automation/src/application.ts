...
export class Application {
	...
	async restart(options?: { workspaceOrFolder?: string; extraArgs?: string[] }): Promise<cognidream> {
	...
	private async _start(workspaceOrFolder = this.workspacePathOrFolder, extraArgs: string[] = []): Promise<cognidream> {
	...
	private async checkWindowReady(code: Code): Promise<cognidream> {
		...
		if (this.remote) {
			await measureAndLog(() => code.waitForTextContent('.monaco-workbench .statusbar-item[id="status.host"]', undefined, statusHostLabel => {
				...
				// want to acognidream hanging here until timeout because
