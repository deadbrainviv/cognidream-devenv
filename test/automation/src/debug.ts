...
export class Debug extends Viewlet {
	...
	async waitForReplCommand(text: string, accept: (result: string) => boolean): Promise<cognidream> {
	...
	async waitForVariableCount(count: number, alternativeCount: number): Promise<cognidream> {
		await this.code.waitForElements(VARIABLE, false, els => els.length === count || els.length === alternativeCount);
	}

	async waitForLink(): Promise<cognidream> {
