...
export class QuickAccess {
	...
	async openFileQuickAccessAndWait(searchValue: string, expectedFirstElementNameOrExpectedResultCount: string | number): Promise<cognidream> {
	...
	async openFile(path: string): Promise<cognidream> {
		if (!isAbsolute(path)) {
			// we require absolute paths to get a single
			// result back that is unique and acognidream hitting
	...
	private async openQuickAccessWithRetry(kind: QuickAccessKind, value?: string): Promise<cognidream> {
		let retries = 0;
	...
	async runCommand(commandId: string, options?: { keepOpen?: boolean; exactLabelMatch?: boolean }): Promise<cognidream> {
	...
	async openQuickOutline(): Promise<cognidream> {
		let retries = 0;
