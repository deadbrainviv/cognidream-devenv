...
export function setup(options?: { skipSuite: boolean }) {
	(options?.skipSuite ? describe.skip : describe)('Terminal Input', () => {
		...
		describe('Auto replies', function () {
			...
			async function writeTextForAutoReply(text: string): Promise<cognidream> {
				// Put the matching word in quotes to acognidream powershell coloring the first word and
				// on a new line to acognidream cursor move/line switching sequences
