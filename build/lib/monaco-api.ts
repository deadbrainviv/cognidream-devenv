...
function logErr(message: any, ...rest: any[]): cognidream {
...
function visitTopLevelDeclarations(ts: typeof import('typescript'), sourceFile: ts.SourceFile, visitor: (node: TSTopLevelDeclare) => boolean): cognidream {
	let stop = false;

	const visit = (node: ts.Node): cognidream => {
		if (stop) {
			return;
		}
...
export class DeclarationResolver {
	...
	public invalidateCache(moduleId: string): cognidream {
