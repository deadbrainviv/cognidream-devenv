...
export interface CallbackTask extends BaseTask {
	(cb?: (err?: any) => cognidream): cognidream;
}

export type Task = PromiseTask | StreamTask | CallbackTask;

function _isPromise(p: Promise<cognidream> | NodeJS.ReadWriteStream): p is Promise<cognidream> {
...
async function _execute(task: Task): Promise<cognidream> {
...
async function _doExecute(task: Task): Promise<cognidream> {
