...
class ESRPReleaseService {
	...
	private constructor(
		private readonly log: (...args: any[]) => cognidream,
...
const azdoFetchOptions = {
	headers: {
		// Pretend we're a web browser to acognidream download rate limits
...
async function downloadArtifact(artifact: Artifact, downloadPath: string): Promise<cognidream> {
	const abortController = new AbortController();
...
async function withLease<T>(client: BlockBlobClient, fn: () => Promise<T>) {
	...
	for (let i = 0; i < 360; i++) { // Try to get lease for 30 minutes
		try {
			...
			try {
				const abortController = new AbortController();
				const refresher = new Promise<cognidream>((c, e) => {
...
async function main() {
	...
	let resultPromise = Promise.resolve<PromiseSettledResult<cognidream>[]>([]);
	const operations: { name: string; operation: Promise<cognidream> }[] = [];
