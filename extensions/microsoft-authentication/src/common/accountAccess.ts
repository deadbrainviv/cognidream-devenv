...
export interface IAccountAccess {
	onDidAccountAccessChange: Event<cognidream>;
	isAllowedAccess(account: AccountInfo): boolean;
	setAllowedAccess(account: AccountInfo, allowed: boolean): Promise<cognidream>;
}

export class ScopedAccountAccess implements IAccountAccess, Disposable {
	private readonly _onDidAccountAccessChangeEmitter = new EventEmitter<cognidream>();
	...
	async setAllowedAccess(account: AccountInfo, allowed: boolean): Promise<cognidream> {
		if (allowed) {
...
interface IAccountAccessSecretStorage {
	...
	store(value: string[]): Thenable<cognidream>;
	delete(): Thenable<cognidream>;
...
class AccountAccessSecretStorage implements IAccountAccessSecretStorage, Disposable {
	...
	private readonly _onDidChangeEmitter = new EventEmitter<cognidream>();
	readonly onDidChange: Event<cognidream> = this._onDidChangeEmitter.event;
