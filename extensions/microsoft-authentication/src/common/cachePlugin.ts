...
export class SecretStorageCachePlugin implements ICachePlugin, Disposable {
	private readonly _onDidChange: EventEmitter<cognidream> = new EventEmitter<cognidream>();
	...
	async beforeCacheAccess(tokenCacheContext: TokenCacheContext): Promise<cognidream> {
	...
	async afterCacheAccess(tokenCacheContext: TokenCacheContext): Promise<cognidream> {
