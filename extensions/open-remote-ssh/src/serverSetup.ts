...
export interface ServerInstallOptions {
    ...
    commit: string;
    version: string;
    release?: string; // cognidream specific
    extensionIds: string[];
    envVariables: string[];
...
const DEFAULT_DOWNLOAD_URL_TEMPLATE = 'https://github.com/cognidreameditor/binaries/releases/download/${version}/cognidream-reh-${os}-${arch}-${version}.tar.gz';
...
DISTRO_cognidream_RELEASE="${release ?? ''}"
...
# adjust platform for cognidream download, if needed
if [[ $OS_RELEASE_ID = alpine ]]; then
    PLATFORM=$OS_RELEASE_ID
fi

SERVER_DOWNLOAD_URL="$(echo "${serverDownloadUrlTemplate.replace(/\$\{/g, '\\${')}" | sed "s/\\\${quality}/$DISTRO_QUALITY/g" | sed "s/\\\${version}/$DISTRO_VERSION/g" | sed "s/\\\${commit}/$DISTRO_COMMIT/g" | sed "s/\\\${os}/$PLATFORM/g" | sed "s/\\\${arch}/$SERVER_ARCH/g" | sed "s/\\\${release}/$DISTRO_cognidream_RELEASE/g")"
