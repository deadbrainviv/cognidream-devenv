...
interface MarkdownPreviewDelegate {
	...
	getAdditionalState(): {};
	openPreviewLinkToMarkdownFile(markdownLink: vscode.Uri, fragment: string | undefined): cognidream;
}
...
class MarkdownPreview extends Disposable implements WebviewResourceProvider {
	...
	*/
	...
	private async _updatePreview(forceUpdate?: boolean): Promise<cognidream> {
		clearTimeout(this._throttleTimer);
	...
	private async _onDidClickPreview(line: number): Promise<cognidream> {
	...
	private _updateWebviewContent(html: string, reloadPage: boolean): cognidream {
		if (this._disposed) {
			return;
		}
...
export interface IManagedMarkdownPreview {
	...
	readonly onDispose: vscode.Event<cognidream>;
	readonly onDidChangeViewState: vscode.Event<vscode.WebviewPanelOnDidChangeViewStateEvent>;

	copyImage(id: string): cognidream;
	dispose(): cognidream;
	refresh(): cognidream;
	updateConfiguration(): cognidream;
...
export class StaticMarkdownPreview extends Disposable implements IManagedMarkdownPreview {
	...
	private readonly _onDispose = this._register(new vscode.EventEmitter<cognidream>());
...
export class DynamicMarkdownPreview extends Disposable implements IManagedMarkdownPreview {
	...
	private readonly _onDisposeEmitter = this._register(new vscode.EventEmitter<cognidream>());
