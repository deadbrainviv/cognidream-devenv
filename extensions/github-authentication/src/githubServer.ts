...
export interface IGitHubServer {
	...
	logout(session: vscode.AuthenticationSession): Promise<cognidream>;
	getUserInfo(token: string): Promise<{ id: string; accountName: string }>;
	sendAdditionalTelemetryInfo(session: vscode.AuthenticationSession): Promise<cognidream>;
	friendlyName: string;
}
...
export class GitHubServer implements IGitHubServer {
	...
	public async logout(session: vscode.AuthenticationSession): Promise<cognidream> {
	...
	public async sendAdditionalTelemetryInfo(session: vscode.AuthenticationSession): Promise<cognidream> {
	...
	private async checkUserDetails(session: vscode.AuthenticationSession): Promise<cognidream> {
	...
	private async checkEnterpriseVersion(token: string): Promise<cognidream> {
		try {
			let version: string;
			if (!isSupportedTarget(this._type, this._ghesUri)) {
