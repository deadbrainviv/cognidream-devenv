...
export class UriEventHandler extends vscode.EventEmitter<vscode.Uri> implements vscode.UriHandler {
	...
	private readonly _codeExchangePromises = new Map<string, { promise: Promise<string>; cancel: vscode.EventEmitter<cognidream> }>();
	...
	public async waitForCode(logger: Log, scopes: string, nonce: string, token: vscode.CancellationToken) {
		...
		try {
			return await Promise.race([
				...
				promiseFromEvent<cognidream, string>(token.onCancellationRequested, (_, __, reject) => { reject(USER_CANCELLATION_ERROR); }).promise
			]);
		} finally {
...
export class GitHubAuthenticationProvider implements vscode.AuthenticationProvider, vscode.Disposable {
	...
	private async afterSessionLoad(session: vscode.AuthenticationSession): Promise<cognidream> {
	...
	private async storeSessions(sessions: vscode.AuthenticationSession[]): Promise<cognidream> {
