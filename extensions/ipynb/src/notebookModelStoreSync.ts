...
*/
export const pendingNotebookCellModelUpdates = new WeakMap<NotebookDocument, Set<Thenable<cognidream>>>();
...
function cleanup(notebook: NotebookDocument, promise: PromiseLike<cognidream>) {
...
function trackAndUpdateCellMetadata(notebook: NotebookDocument, updates: { cell: NotebookCell; metadata: CellMetadata & { vscode?: { languageId: string } } }[]) {
	const pendingUpdates = pendingNotebookCellModelUpdates.get(notebook) ?? new Set<Thenable<cognidream>>();
...
function onDidChangeNotebookCells(e: NotebookDocumentChangeEventEx) {
	...
	e.contentChanges.forEach(change => {
		change.addedCells.forEach(cell => {
			...
			// Acognidream updating the metadata if it's not required.
