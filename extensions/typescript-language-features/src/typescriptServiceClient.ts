...
export default class TypeScriptServiceClient extends Disposable implements ITypeScriptServiceClient {
	...
	private readonly _onReady?: { promise: Promise<cognidream>; resolve: () => cognidream; reject: () => cognidream };
	...
	constructor(
		...
		let resolve: () => cognidream;
		let reject: () => cognidream;
		const p = new Promise<cognidream>((res, rej) => {
			resolve = res;
			reject = rej;
		});
	...
	private readonly _onDidChangeCapabilities = this._register(new vscode.EventEmitter<cognidream>());
	...
	private cancelInflightRequestsForResource(resource: vscode.Uri): cognidream {
		if (this.serverState.type !== ServerState.Type.Running) {
			return;
		}
	...
	public restartTsServer(fromUserAction = false): cognidream {
		if (this.serverState.type === ServerState.Type.Running) {
	...
	private readonly _onResendModelsRequested = this._register(new vscode.EventEmitter<cognidream>());
	...
	public onReady(f: () => cognidream): Promise<cognidream> {
	...
	public async showVersionPicker(): Promise<cognidream> {
	...
	private serviceStarted(resendModels: boolean): cognidream {
	...
	private setCompilerOptionsForInferredProjects(configuration: TypeScriptServiceConfiguration): cognidream {
	...
	private serviceExited(restart: boolean, tsVersion: API): cognidream {
		this.resetWatchers();
		this.loadingIndicator.reset();
	...
	public executeWithoutWaitingForResponse(command: keyof TypeScriptRequests, args: any): cognidream {
	...
	private fatalError(command: string, error: unknown): cognidream {
		/* __GDPR__
			"fatalError" : {
				"owner": "mjbvz",
				"${include}": [
	...
	private dispatchTelemetryEvent(telemetryData: Proto.TelemetryEventBody): cognidream {
...
class ServerInitializingIndicator extends Disposable {
	...
	private _task?: { project: string; resolve: () => cognidream };
	 ...
	 */
	public startedLoadingProject(projectName: string): cognidream {
		...
		}, () => new Promise<cognidream>(resolve => {
	...
	public startedLoadingFile(fileName: string, task: Promise<unknown>): cognidream {
		if (!this._task) {
			vscode.window.withProgress({
				location: vscode.ProgressLocation.Window,
	...
	public finishedLoadingProject(projectName: string): cognidream {
