...
export function withRandomFileEditor(
	...
	run: (editor: vscode.TextEditor, doc: vscode.TextDocument) => Thenable<cognidream>
): Thenable<boolean> {
...
export const wait = (ms: number) => new Promise<cognidream>(resolve => setTimeout(() => resolve(), ms));
...
export function assertEditorContents(editor: vscode.TextEditor, expectedDocContent: string, message?: string): cognidream {
...
export async function enumerateConfig(
	...
	f: (message: string) => Promise<cognidream>
): Promise<cognidream> {
