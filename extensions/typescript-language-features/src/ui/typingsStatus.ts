...
export default class TypingsStatus extends Disposable {
	...
	private onBeginInstallTypings(eventId: number): cognidream {
	...
	private onEndInstallTypings(eventId: number): cognidream {
...
export class AtaProgressReporter extends Disposable {
	...
	private _onBegin(eventId: number): cognidream {
		const handle = setTimeout(() => this._onEndOrTimeout(eventId), typingsInstallTimeout);
		const promise = new Promise<cognidream>(resolve => {
	...
	private _onEndOrTimeout(eventId: number): cognidream {
