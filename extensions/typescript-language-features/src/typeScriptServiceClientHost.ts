...
export default class TypeScriptServiceClientHost extends Disposable {
	...
	constructor(
		...
		onCompletionAccepted: (item: vscode.CompletionItem) => cognidream,
	) {
		super();
	...
	private registerExtensionLanguageProvider(description: LanguageDescription, onCompletionAccepted: (item: vscode.CompletionItem) => cognidream) {
		const manager = new LanguageProvider(this.client, description, this.commandManager, this.client.telemetryReporter, this.typingsStatus, this.fileConfigurationManager, onCompletionAccepted);
		this.languages.push(manager);
		this._register(manager);
		this.languagePerId.set(description.id, manager);
	...
	public reloadProjects(): cognidream {
	...
	private configurationChanged(): cognidream {
	...
	private populateService(): cognidream {
	...
	private configFileDiagnosticsReceived(event: Proto.ConfigFileDiagnosticEvent): cognidream {
