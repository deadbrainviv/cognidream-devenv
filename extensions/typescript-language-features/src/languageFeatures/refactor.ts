...
class DidApplyRefactoringCommand implements Command {
	...
	public async execute(args: DidApplyRefactoringCommand.Args): Promise<cognidream> {
		/* __GDPR__
			"refactor.execute" : {
				"owner": "mjbvz",
...
class SelectRefactorCommand implements Command {
	...
	public async execute(args: SelectRefactorCommand.Args): Promise<cognidream> {
...
class MoveToFileRefactorCommand implements Command {
	...
	public async execute(args: MoveToFileRefactorCommand.Args): Promise<cognidream> {
		const file = this.client.toOpenTsFilePath(args.document);
		if (!file) {
			return;
		}
