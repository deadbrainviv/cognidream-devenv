...
export class WebTypingsInstallerClient implements ts.server.ITypingsInstaller {
	 ...
	 */
	private async handleResponse(response: InstallerResponse): Promise<cognidream> {
	...
	enqueueInstallTypingsRequest(p: ts.server.Project, typeAcquisition: ts.TypeAcquisition, unresolvedImports: ts.SortedReadonlyArray<string>): cognidream {
	...
	attach(projectService: ts.server.ProjectService): cognidream {
		this.projectService = projectService;
	}

	onProjectClosed(_projectService: ts.server.Project): cognidream {
		// noop
	}
}
...
class WebTypingsInstallerServer extends ts.server.typingsInstaller.TypingsInstaller {
	...
	private constructor(
		...
		private readonly handleResponse: (response: InstallerResponse) => cognidream,
	...
	static async initialize(
		handleResponse: (response: InstallerResponse) => cognidream,
		fs: ts.server.ServerHost,
		globalTypingsCachePath: string,
	 ...
	 */
	protected override installWorker(requestId: number, packageNames: string[], cwd: string, onRequestCompleted: ts.server.typingsInstaller.RequestCompletedAction): cognidream {
	 ...
	 */
	protected override sendResponse(response: InstallerResponse): cognidream {
