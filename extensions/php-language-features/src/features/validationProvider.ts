...
export default class PHPValidationProvider {
	...
	private loadConfigP: Promise<cognidream>;

	private documentListener: vscode.Disposable | null = null;
	private diagnosticCollection?: vscode.DiagnosticCollection;
	private delayers?: { [key: string]: ThrottledDelayer<cognidream> };
	...
	private async loadConfiguration(): Promise<cognidream> {
	...
	private async triggerValidate(textDocument: vscode.TextDocument): Promise<cognidream> {
		...
		if (vscode.workspace.isTrusted) {
			...
			if (!delayer) {
				delayer = new ThrottledDelayer<cognidream>(this.config?.trigger === RunTrigger.onType ? 250 : 0);
	...
	private doValidate(textDocument: vscode.TextDocument): Promise<cognidream> {
	...
	private async showError(error: any, executable: string): Promise<cognidream> {
	...
	private async showErrorMessage(message: string): Promise<cognidream> {
