...
suite('vscode API - editors', () => {
	...
	function withRandomFileEditor(initialContents: string, run: (editor: TextEditor, doc: TextDocument) => Thenable<cognidream>): Thenable<boolean> {
	 ...
	 */
	test('insert snippet with replacement, acognidream adjusting indentation', () => {
		const snippetString = new SnippetString()
