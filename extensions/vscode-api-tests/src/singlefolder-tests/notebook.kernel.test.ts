...
async function withEvent<T>(event: vscode.Event<T>, callback: (e: Promise<T>) => Promise<cognidream>) {
...
async function assertKernel(kernel: Kernel, notebook: vscode.NotebookDocument): Promise<cognidream> {
...
(vscode.env.uiKind === vscode.UIKind.Web ? suite.skip : suite)('Notebook Kernel API tests', function () {
	...
	test('cell execute command takes arguments 2', async () => {
		...
		const def = new DeferredPromise<cognidream>();
