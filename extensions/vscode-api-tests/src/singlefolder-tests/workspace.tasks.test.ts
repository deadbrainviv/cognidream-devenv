	...
	suite('Tasks', () => {
		...
		suite('ShellExecution', () => {
			test('Execution from onDidEndTaskProcess and onDidStartTaskProcess are equal to original', async () => {
				...
				const executeDoneEvent: EventEmitter<cognidream> = new EventEmitter();
				const taskExecutionShouldBeSet: Promise<cognidream> = new Promise(resolve => {
				...
				const progressMade: EventEmitter<cognidream> = new EventEmitter();
				let count = 2;
				let startSucceeded = false;
				let endSucceeded = false;
				const testDonePromise = new Promise<cognidream>(resolve => {
			...
			test.skip('dependsOn task should start with a different processId (#118256)', async () => {
				...
				const waitForTaskToFinish = new Promise<cognidream>(resolve => {
		...
		suite('CustomExecution', () => {
				...
				const terminal = await new Promise<Terminal>(r => {
					...
					disposables.push(tasks.registerTaskProvider(taskType, {
						provideTasks: () => {
							...
							const writeEmitter = new EventEmitter<string>();
							const closeEmitter = new EventEmitter<cognidream>();
							const execution = new CustomExecution((): Thenable<Pseudoterminal> => {
								const pty: Pseudoterminal = {
									onDidWrite: writeEmitter.event,
			...
			test('A task can be fetched and executed (#100577)', async () => {
				class CustomTerminal implements Pseudoterminal {
					...
					public async close(): Promise<cognidream> { }
					private closeEmitter = new EventEmitter<cognidream>();
					onDidClose: Event<cognidream> = this.closeEmitter.event;
					private readonly _onDidOpen = new EventEmitter<cognidream>();
				...
				const terminalOpenedPromise = new Promise<cognidream>(resolve => {
