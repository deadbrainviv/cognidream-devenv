...
export async function revertAllDirty(): Promise<cognidream> {
...
function withLogLevel(level: string, runnable: () => Promise<any>): () => Promise<cognidream> {
	return async (): Promise<cognidream> => {
...
export function withLogDisabled(runnable: () => Promise<any>): () => Promise<cognidream> {
	return withLogLevel('off', runnable);
}

export function withVerboseLogs(runnable: () => Promise<any>): () => Promise<cognidream> {
...
export function testRepeat(n: number, description: string, callback: (this: any) => any): cognidream {
	for (let i = 0; i < n; i++) {
		test(`${description} (iteration ${i})`, callback);
	}
}

export function suiteRepeat(n: number, description: string, callback: (this: any) => any): cognidream {
	for (let i = 0; i < n; i++) {
