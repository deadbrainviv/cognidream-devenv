...
export class TreeViewPane extends ViewPane {
	...
	protected override layoutBody(height: number, width: number): cognidream {
	...
	protected renderTreeView(container: HTMLElement): cognidream {
		this.treeView.show(container);
	}

	protected layoutTreeView(height: number, width: number): cognidream {
		this.treeView.layout(height, width);
	}

	private updateTreeVisibility(): cognidream {
...
abstract class AbstractTreeView extends Disposable implements ITreeView {
	...
	private readonly _onDidChangeActions: Emitter<cognidream> = this._register(new Emitter<cognidream>());
	readonly onDidChangeActions: Event<cognidream> = this._onDidChangeActions.event;

	private readonly _onDidChangeWelcomeState: Emitter<cognidream> = this._register(new Emitter<cognidream>());
	readonly onDidChangeWelcomeState: Event<cognidream> = this._onDidChangeWelcomeState.event;
	...
	private readonly _onDidCompleteRefresh: Emitter<cognidream> = this._register(new Emitter<cognidream>());
	...
	set dataProvider(dataProvider: ITreeViewDataProvider | undefined) {
		if (dataProvider) {
			...
			this._dataProvider = new class implements ITreeViewDataProvider {
				...
				private _onDidChangeEmpty: Emitter<cognidream> = new Emitter();
				public onDidChangeEmpty: Event<cognidream> = this._onDidChangeEmpty.event;
				...
				private updateEmptyState(nodes: ITreeItem[], childrenGroups: ITreeItem[][]): cognidream {
	...
	setVisibility(isVisible: boolean): cognidream {
	...
	protected abstract activate(): cognidream;

	focus(reveal: boolean = true, revealItem?: ITreeItem): cognidream {
	...
	show(container: HTMLElement): cognidream {
		this._container = container;
	...
	private onContextMenu(treeMenus: TreeMenus, treeEvent: ITreeContextMenuEvent<ITreeItem>, actionRunner: MultipleSelectionActionRunner): cognidream {
	...
	protected updateMessage(): cognidream {
		if (this._message) {
	...
	private showMessage(message: string | IMarkdownString): cognidream {
	...
	private hideMessage(): cognidream {
		this.resetMessageElement();
		this.messageElement?.classList.add('hide');
		this.layout(this._height, this._width);
	}

	private resetMessageElement(): cognidream {
		if (this.messageElement) {
	...
	async refresh(elements?: readonly ITreeItem[], checkboxes?: readonly ITreeItem[]): Promise<cognidream> {
	...
	async expand(itemOrItems: ITreeItem | ITreeItem[]): Promise<cognidream> {
	...
	setSelection(items: ITreeItem[]): cognidream {
	...
	setFocus(item?: ITreeItem): cognidream {
		if (this.tree) {
			if (item) {
	...
	async reveal(item: ITreeItem): Promise<cognidream> {
		if (this.tree) {
	...
	private async doRefresh(elements: readonly ITreeItem[]): Promise<cognidream> {
		const tree = this.tree;
	...
	private updateContentAreas(): cognidream {
...
class TreeRenderer extends Disposable implements ITreeRenderer<ITreeItem, FuzzyScore, ITreeExplorerTemplateData> {
	...
	renderElement(element: ITreeNode<ITreeItem, FuzzyScore>, index: number, templateData: ITreeExplorerTemplateData): cognidream {
	...
	disposeElement(resource: ITreeNode<ITreeItem, FuzzyScore>, index: number, templateData: ITreeExplorerTemplateData): cognidream {
	...
	disposeTemplate(templateData: ITreeExplorerTemplateData): cognidream {
...
class MultipleSelectionActionRunner extends ActionRunner {
	...
	protected override async runAction(action: IAction, context: TreeViewItemHandleArg | TreeViewPaneHandleArg): Promise<cognidream> {
...
export class CustomTreeViewDragAndDrop implements ITreeDragAndDrop<ITreeItem> {
	...
	async drop(data: IDragAndDropData, targetNode: ITreeItem | undefined, targetIndex: number | undefined, targetSector: ListViewTargetSector | undefined, originalEvent: DragEvent): Promise<cognidream> {
		const dndController = this.dndController;
