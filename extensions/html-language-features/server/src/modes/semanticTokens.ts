...
function applyTypesMapping(tokens: SemanticTokenData[], typesMapping: number[] | undefined): cognidream {
	if (typesMapping) {
		for (const token of tokens) {
...
function applyModifiersMapping(tokens: SemanticTokenData[], modifiersMapping: number[] | undefined): cognidream {
	if (modifiersMapping) {
		for (const token of tokens) {
