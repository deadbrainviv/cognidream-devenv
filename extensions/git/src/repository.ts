...
export class Resource implements SourceControlResourceState {
	...
	async open(): Promise<cognidream> {
		const command = this.command;
		await commands.executeCommand<cognidream>(command.command, ...(command.arguments || []));
	}

	async openFile(): Promise<cognidream> {
		const command = this._commandResolver.resolveFileCommand(this);
		await commands.executeCommand<cognidream>(command.command, ...(command.arguments || []));
	}

	async openChange(): Promise<cognidream> {
...
class ProgressManager {
	...
	private updateEnablement(): cognidream {
...
class FileEventLogger {
	...
	private onDidChangeLogLevel(logLevel: LogLevel): cognidream {
...
export class Repository implements Disposable {
	...
	private _onDidChangeStatus = new EventEmitter<cognidream>();
	readonly onDidRunGitStatus: Event<cognidream> = this._onDidChangeStatus.event;
	...
	private _onDidChangeBranchProtection = new EventEmitter<cognidream>();
	readonly onDidChangeBranchProtection: Event<cognidream> = this._onDidChangeBranchProtection.event;

	@memoize
	get onDidChangeOperations(): Event<cognidream> {
	...
	@throttle
	async status(): Promise<cognidream> {
		await this.run(Operation.Status);
	}

	@throttle
	async refresh(): Promise<cognidream> {
	...
	async add(resources: Uri[], opts?: { update?: boolean }): Promise<cognidream> {
		await this.run(
	...
	async rm(resources: Uri[]): Promise<cognidream> {
		await this.run(Operation.Remove, () => this.repository.rm(resources.map(r => r.fsPath)));
	}

	async stage(resource: Uri, contents: string, encoding: string): Promise<cognidream> {
	...
	async revert(resources: Uri[]): Promise<cognidream> {
		await this.run(
	...
	async commit(message: string | undefined, opts: CommitOptions = Object.create(null)): Promise<cognidream> {
	...
	async clean(resources: Uri[]): Promise<cognidream> {
			...
			async () => {
				...
				if (toClean.length > 0) {
					if (discardUntrackedChangesToTrash) {
						const limiter = new Limiter<cognidream>(5);
	...
	closeDiffEditors(indexResources: string[] | undefined, workingTreeResources: string[] | undefined, ignoreSetting = false): cognidream {
		const config = workspace.getConfiguration('git', Uri.file(this.root));
	...
	async branch(name: string, _checkout: boolean, _ref?: string): Promise<cognidream> {
		await this.run(Operation.Branch, () => this.repository.branch(name, _checkout, _ref));
	}

	async deleteBranch(name: string, force?: boolean): Promise<cognidream> {
	...
	async renameBranch(name: string): Promise<cognidream> {
		await this.run(Operation.RenameBranch, () => this.repository.renameBranch(name));
	}

	@throttle
	async fastForwardBranch(name: string): Promise<cognidream> {
		// Get branch details
	...
	async cherryPick(commitHash: string): Promise<cognidream> {
		await this.run(Operation.CherryPick, () => this.repository.cherryPick(commitHash));
	}

	async cherryPickAbort(): Promise<cognidream> {
		await this.run(Operation.CherryPick, () => this.repository.cherryPickAbort());
	}

	async move(from: string, to: string): Promise<cognidream> {
	...
	async setBranchUpstream(name: string, upstream: string): Promise<cognidream> {
		await this.run(Operation.SetBranchUpstream, () => this.repository.setBranchUpstream(name, upstream));
	}

	async merge(ref: string): Promise<cognidream> {
		await this.run(Operation.Merge, () => this.repository.merge(ref));
	}

	async mergeAbort(): Promise<cognidream> {
		await this.run(Operation.MergeAbort, async () => await this.repository.mergeAbort());
	}

	async rebase(branch: string): Promise<cognidream> {
		await this.run(Operation.Rebase, () => this.repository.rebase(branch));
	}

	async tag(options: { name: string; message?: string; ref?: string }): Promise<cognidream> {
		await this.run(Operation.Tag, () => this.repository.tag(options));
	}

	async deleteTag(name: string): Promise<cognidream> {
		await this.run(Operation.DeleteTag, () => this.repository.deleteTag(name));
	}

	async deleteRemoteRef(remoteName: string, refName: string, options?: { force?: boolean }): Promise<cognidream> {
		await this.run(Operation.DeleteRemoteRef, () => this.repository.deleteRemoteRef(remoteName, refName, options));
	}

	async checkout(treeish: string, opts?: { detached?: boolean; pullBeforeCheckout?: boolean }): Promise<cognidream> {
	...
	async checkoutTracking(treeish: string, opts: { detached?: boolean } = {}): Promise<cognidream> {
		const refLabel = opts.detached ? getCommitShortHash(Uri.file(this.root), treeish) : treeish;
	...
	async reset(treeish: string, hard?: boolean): Promise<cognidream> {
		await this.run(Operation.Reset, () => this.repository.reset(treeish, hard));
	}

	async deleteRef(ref: string): Promise<cognidream> {
	...
	async addRemote(name: string, url: string): Promise<cognidream> {
		await this.run(Operation.Remote, () => this.repository.addRemote(name, url));
	}

	async removeRemote(name: string): Promise<cognidream> {
		await this.run(Operation.Remote, () => this.repository.removeRemote(name));
	}

	async renameRemote(name: string, newName: string): Promise<cognidream> {
		await this.run(Operation.Remote, () => this.repository.renameRemote(name, newName));
	}

	@throttle
	async fetchDefault(options: { silent?: boolean } = {}): Promise<cognidream> {
		await this._fetch({ silent: options.silent });
	}

	@throttle
	async fetchPrune(): Promise<cognidream> {
		await this._fetch({ prune: true });
	}

	@throttle
	async fetchAll(options: { silent?: boolean } = {}, cancellationToken?: CancellationToken): Promise<cognidream> {
		await this._fetch({ all: true, silent: options.silent, cancellationToken });
	}

	async fetch(options: FetchOptions): Promise<cognidream> {
		await this._fetch(options);
	}

	private async _fetch(options: { remote?: string; ref?: string; all?: boolean; prune?: boolean; depth?: number; silent?: boolean; cancellationToken?: CancellationToken } = {}): Promise<cognidream> {
		if (!options.prune) {
			const config = workspace.getConfiguration('git', Uri.file(this.root));
	...
	@throttle
	async pullWithRebase(head: Branch | undefined): Promise<cognidream> {
	...
	@throttle
	async pull(head?: Branch, unshallow?: boolean): Promise<cognidream> {
		let remote: string | undefined;
		let branch: string | undefined;
	...
	async pullFrom(rebase?: boolean, remote?: string, branch?: string, unshallow?: boolean): Promise<cognidream> {
	...
	private async _pullAndHandleTagConflict(rebase?: boolean, remote?: string, branch?: string, options: PullOptions = {}): Promise<cognidream> {
		try {
	...
	@throttle
	async push(head: Branch, forcePushMode?: ForcePushMode): Promise<cognidream> {
		let remote: string | undefined;
		let branch: string | undefined;
	...
	async pushTo(remote?: string, name?: string, setUpstream = false, forcePushMode?: ForcePushMode): Promise<cognidream> {
		await this.run(Operation.Push, () => this._push(remote, name, setUpstream, undefined, forcePushMode));
	}

	async pushFollowTags(remote?: string, forcePushMode?: ForcePushMode): Promise<cognidream> {
		await this.run(Operation.Push, () => this._push(remote, undefined, false, true, forcePushMode));
	}

	async pushTags(remote?: string, forcePushMode?: ForcePushMode): Promise<cognidream> {
	...
	@throttle
	sync(head: Branch, rebase: boolean): Promise<cognidream> {
		return this._sync(head, rebase);
	}

	private async _sync(head: Branch, rebase: boolean): Promise<cognidream> {
	...
	async apply(patch: string, reverse?: boolean): Promise<cognidream> {
	...
	async createStash(message?: string, includeUntracked?: boolean, staged?: boolean): Promise<cognidream> {
		const indexResources = [...this.indexGroup.resourceStates.map(r => r.resourceUri.fsPath)];
	...
	async popStash(index?: number): Promise<cognidream> {
		return await this.run(Operation.Stash, () => this.repository.popStash(index));
	}

	async dropStash(index?: number): Promise<cognidream> {
		return await this.run(Operation.Stash, () => this.repository.dropStash(index));
	}

	async applyStash(index?: number): Promise<cognidream> {
	...
	async ignore(files: Uri[]): Promise<cognidream> {
	...
	async rebaseAbort(): Promise<cognidream> {
	...
	private async _push(remote?: string, refspec?: string, setUpstream = false, followTags = false, forcePushMode?: ForcePushMode, tags = false): Promise<cognidream> {
		try {
	...
	private async _updateModelState(optimisticResourcesGroups?: GitResourceGroups, cancellationToken?: CancellationToken): Promise<cognidream> {
		try {
	...
	private _updateResourceGroupsState(resourcesGroups: GitResourceGroups): cognidream {
		// set resource groups
	...
	private setCountBadge(): cognidream {
		const config = workspace.getConfiguration('git', Uri.file(this.repository.root));
	...
	private onFileChange(_uri: Uri): cognidream {
		const config = workspace.getConfiguration('git');
	...
	@debounce(1000)
	private eventuallyUpdateWhenIdleAndWait(): cognidream {
		this.updateWhenIdleAndWait();
	}

	@throttle
	private async updateWhenIdleAndWait(): Promise<cognidream> {
		await this.whenIdleAndFocused();
		await this.status();
		await timeout(5000);
	}

	async whenIdleAndFocused(): Promise<cognidream> {
		while (true) {
			if (!this.operations.isIdle()) {
	...
	private updateInputBoxPlaceholder(): cognidream {
	...
	private updateBranchProtectionMatchers(root: Uri): cognidream {
