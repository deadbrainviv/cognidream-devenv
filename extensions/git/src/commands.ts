...
class CheckoutItem extends BranchItem {
	async run(repository: Repository, opts?: { detached?: boolean }): Promise<cognidream> {
		if (!this.ref.name) {
			return;
		}
...
class BranchDeleteItem extends BranchItem {
	...
	async run(repository: Repository, force?: boolean): Promise<cognidream> {
...
class TagDeleteItem extends RefItem {
	...
	async run(repository: Repository): Promise<cognidream> {
		if (this.ref.name) {
...
class RemoteTagDeleteItem extends RefItem {
	...
	async run(repository: Repository, remote: string): Promise<cognidream> {
		if (this.ref.name) {
...
class CommandErrorOutputTextDocumentContentProvider implements TextDocumentContentProvider {
	...
	set(uri: Uri, contents: string): cognidream {
		this.items.set(uri.path, contents);
	}

	delete(uri: Uri): cognidream {
...
export class CommandCenter {
	...
	showOutput(): cognidream {
		this.logger.show();
	}

	@command('git.refresh', { repository: true })
	async refresh(repository: Repository): Promise<cognidream> {
		await repository.refresh();
	}

	@command('git.openResource')
	async openResource(resource: Resource): Promise<cognidream> {
	...
	async openChanges(repository: Repository): Promise<cognidream> {
		for (const resource of [...repository.workingTreeGroup.resourceStates, ...repository.untrackedGroup.resourceStates]) {
			...
			cognidream commands.executeCommand(
				'vscode.open',
	...
	async cloneRepository(url?: string, parentPath?: string, options: { recursive?: boolean; ref?: string } = {}): Promise<cognidream> {
	...
	async continueInLocalClone(): Promise<Uri | cognidream> {
		...
		if (uri !== undefined) {
			...
			cognidream this.clone(uri, undefined, { ref: ref });
		}
	}

	@command('git.clone')
	async clone(url?: string, parentPath?: string, options?: { ref?: string }): Promise<cognidream> {
		await this.cloneRepository(url, parentPath, options);
	}

	@command('git.cloneRecursive')
	async cloneRecursive(url?: string, parentPath?: string): Promise<cognidream> {
		await this.cloneRepository(url, parentPath, { recursive: true });
	}

	@command('git.init')
	async init(skipFolderPrompt = false): Promise<cognidream> {
	...
	async openRepository(path?: string): Promise<cognidream> {
		if (!path) {
			const result = await window.showOpenDialog({
				canSelectFiles: false,
				canSelectFolders: true,
	...
	async reopenClosedRepositories(): Promise<cognidream> {
	...
	async close(repository: Repository, ...args: SourceControl[]): Promise<cognidream> {
	...
	async closeOtherRepositories(repository: Repository, ...args: SourceControl[]): Promise<cognidream> {
		const otherRepositories = args
			.map(sourceControl => this.model.getRepository(sourceControl))
			.filter(isDefined);
	...
	async openFile(arg?: Resource | Uri, ...resourceStates: SourceControlResourceState[]): Promise<cognidream> {
	...
	async openFile2(arg?: Resource | Uri, ...resourceStates: SourceControlResourceState[]): Promise<cognidream> {
		this.openFile(arg, ...resourceStates);
	}

	@command('git.openHEADFile')
	async openHEADFile(arg?: Resource | Uri): Promise<cognidream> {
		...
		return await commands.executeCommand<cognidream>('vscode.open', HEAD, opts, title);
	}

	@command('git.openChange')
	async openChange(arg?: Resource | Uri, ...resourceStates: SourceControlResourceState[]): Promise<cognidream> {
	...
	async rename(repository: Repository, fromUri: Uri | undefined): Promise<cognidream> {
	...
	@command('git.stage')
	async stage(...resourceStates: SourceControlResourceState[]): Promise<cognidream> {
	...
	async stageAll(repository: Repository): Promise<cognidream> {
	...
	private async _stageDeletionConflict(repository: Repository, uri: Uri): Promise<cognidream> {
	...
	async stageAllTracked(repository: Repository): Promise<cognidream> {
	...
	async stageAllUntracked(repository: Repository): Promise<cognidream> {
		const resources = [...repository.workingTreeGroup.resourceStates, ...repository.untrackedGroup.resourceStates]
			.filter(r => r.type === Status.UNTRACKED || r.type === Status.IGNORED);
		const uris = resources.map(r => r.resourceUri);
	...
	async stageAllMerge(repository: Repository): Promise<cognidream> {
	...
	async stageChange(uri: Uri, changes: LineChange[], index: number): Promise<cognidream> {
		if (!uri) {
			return;
		}
	...
	async diffStageHunk(changes: DiffEditorSelectionHunkToolbarContext | undefined): Promise<cognidream> {
		this.diffStageHunkOrSelection(changes);
	}

	@command('git.diff.stageSelection')
	async diffStageSelection(changes: DiffEditorSelectionHunkToolbarContext | undefined): Promise<cognidream> {
		this.diffStageHunkOrSelection(changes);
	}

	async diffStageHunkOrSelection(changes: DiffEditorSelectionHunkToolbarContext | undefined): Promise<cognidream> {
		if (!changes) {
			return;
		}
	...
	async stageSelectedChanges(): Promise<cognidream> {
		const textEditor = window.activeTextEditor;
	...
	async stageFile(uri: Uri): Promise<cognidream> {
		if (!uri) {
			return;
		}
	...
	async acceptMerge(_uri: Uri | unknown): Promise<cognidream> {
	...
	async runGitMergeNoDiff3(): Promise<cognidream> {
		await this.runGitMerge(false);
	}

	@command('git.runGitMergeDiff3')
	async runGitMergeDiff3(): Promise<cognidream> {
		await this.runGitMerge(true);
	}

	private async runGitMerge(diff3: boolean): Promise<cognidream> {
		const { activeTab } = window.tabGroups.activeTabGroup;
		if (!activeTab) {
			return;
		}
	...
	private async _stageChanges(textEditor: TextEditor, changes: LineChange[]): Promise<cognidream> {
		const modifiedDocument = textEditor.document;
	...
	async revertChange(uri: Uri, changes: LineChange[], index: number): Promise<cognidream> {
		if (!uri) {
			return;
		}
	...
	async revertSelectedRanges(): Promise<cognidream> {
		const textEditor = window.activeTextEditor;
	...
	private async _revertChanges(textEditor: TextEditor, changes: LineChange[]): Promise<cognidream> {
		const modifiedDocument = textEditor.document;
		const modifiedUri = modifiedDocument.uri;
	...
	@command('git.unstage')
	async unstage(...resourceStates: SourceControlResourceState[]): Promise<cognidream> {
		resourceStates = resourceStates.filter(s => !!s);
	...
	async unstageAll(repository: Repository): Promise<cognidream> {
		await repository.revert([]);
	}

	@command('git.unstageSelectedRanges')
	async unstageSelectedRanges(): Promise<cognidream> {
		const textEditor = window.activeTextEditor;
	...
	async unstageFile(uri: Uri): Promise<cognidream> {
		if (!uri) {
			return;
		}
	...
	@command('git.clean')
	async clean(...resourceStates: SourceControlResourceState[]): Promise<cognidream> {
	...
	async cleanAll(repository: Repository): Promise<cognidream> {
		await this._cleanAll(repository.workingTreeGroup.resourceStates);
	}

	@command('git.cleanAllTracked', { repository: true })
	async cleanAllTracked(repository: Repository): Promise<cognidream> {
		const resources = repository.workingTreeGroup.resourceStates
			.filter(r => r.type !== Status.UNTRACKED && r.type !== Status.IGNORED);
	...
	async cleanAllUntracked(repository: Repository): Promise<cognidream> {
		const resources = [...repository.workingTreeGroup.resourceStates, ...repository.untrackedGroup.resourceStates]
			.filter(r => r.type === Status.UNTRACKED || r.type === Status.IGNORED);
	...
	private async _cleanAll(resources: Resource[]): Promise<cognidream> {
		if (resources.length === 0) {
			return;
		}
	...
	private async _cleanTrackedChanges(resources: Resource[]): Promise<cognidream> {
	...
	private async _cleanUntrackedChanges(resources: Resource[]): Promise<cognidream> {
	...
	private async commitWithAnyInput(repository: Repository, opts: CommitOptions): Promise<cognidream> {
	...
	async commit(repository: Repository, postCommitCommand?: string | null): Promise<cognidream> {
		await this.commitWithAnyInput(repository, { postCommitCommand });
	}

	@command('git.commitAmend', { repository: true })
	async commitAmend(repository: Repository): Promise<cognidream> {
		await this.commitWithAnyInput(repository, { amend: true });
	}

	@command('git.commitSigned', { repository: true })
	async commitSigned(repository: Repository): Promise<cognidream> {
		await this.commitWithAnyInput(repository, { signoff: true });
	}

	@command('git.commitStaged', { repository: true })
	async commitStaged(repository: Repository): Promise<cognidream> {
		await this.commitWithAnyInput(repository, { all: false });
	}

	@command('git.commitStagedSigned', { repository: true })
	async commitStagedSigned(repository: Repository): Promise<cognidream> {
		await this.commitWithAnyInput(repository, { all: false, signoff: true });
	}

	@command('git.commitStagedAmend', { repository: true })
	async commitStagedAmend(repository: Repository): Promise<cognidream> {
		await this.commitWithAnyInput(repository, { all: false, amend: true });
	}

	@command('git.commitAll', { repository: true })
	async commitAll(repository: Repository): Promise<cognidream> {
		await this.commitWithAnyInput(repository, { all: true });
	}

	@command('git.commitAllSigned', { repository: true })
	async commitAllSigned(repository: Repository): Promise<cognidream> {
		await this.commitWithAnyInput(repository, { all: true, signoff: true });
	}

	@command('git.commitAllAmend', { repository: true })
	async commitAllAmend(repository: Repository): Promise<cognidream> {
		await this.commitWithAnyInput(repository, { all: true, amend: true });
	}

	@command('git.commitMessageAccept')
	async commitMessageAccept(arg?: Uri): Promise<cognidream> {
	...
	async commitMessageDiscard(arg?: Uri): Promise<cognidream> {
		if (!arg && !window.activeTextEditor) { return; }
		arg ??= window.activeTextEditor!.document.uri;
	...
	private _closeEditorTab(uri: Uri): cognidream {
	...
	private async _commitEmpty(repository: Repository, noVerify?: boolean): Promise<cognidream> {
		const root = Uri.file(repository.root);
		const config = workspace.getConfiguration('git', root);
	...
	async commitAllAmendNoVerify(repository: Repository): Promise<cognidream> {
	...
	async commitEmptyNoVerify(repository: Repository): Promise<cognidream> {
		await this._commitEmpty(repository, true);
	}

	@command('git.restoreCommitTemplate', { repository: true })
	async restoreCommitTemplate(repository: Repository): Promise<cognidream> {
		repository.inputBox.value = await repository.getCommitTemplate();
	}

	@command('git.undoCommit', { repository: true })
	async undoCommit(repository: Repository): Promise<cognidream> {
	...
	async checkout2(repository: Repository, historyItem?: SourceControlHistoryItem, historyItemRefId?: string): Promise<cognidream> {
	...
	async branch(repository: Repository, historyItem?: SourceControlHistoryItem): Promise<cognidream> {
		await this._branch(repository, undefined, false, historyItem?.id);
	}

	@command('git.branchFrom', { repository: true })
	async branchFrom(repository: Repository): Promise<cognidream> {
	...
	private async _branch(repository: Repository, defaultName?: string, from = false, target?: string): Promise<cognidream> {
		target = target ?? 'HEAD';
	...
	async deleteBranch(repository: Repository, name: string | undefined, force?: boolean): Promise<cognidream> {
		await this._deleteBranch(repository, undefined, name, { remote: false, force });
	}

	@command('git.graph.deleteBranch', { repository: true })
	async deleteBranch2(repository: Repository, historyItem?: SourceControlHistoryItem, historyItemRefId?: string): Promise<cognidream> {
		const historyItemRef = historyItem?.references?.find(r => r.id === historyItemRefId);
		if (!historyItemRef) {
			return;
		}
	...
	async deleteRemoteBranch(repository: Repository): Promise<cognidream> {
		await this._deleteBranch(repository, undefined, undefined, { remote: true });
	}

	private async _deleteBranch(repository: Repository, remote: string | undefined, name: string | undefined, options: { remote: boolean; force?: boolean }): Promise<cognidream> {
		let run: (force?: boolean) => Promise<cognidream>;
	...
	async renameBranch(repository: Repository): Promise<cognidream> {
	...
	async merge(repository: Repository): Promise<cognidream> {
		const config = workspace.getConfiguration('git');
		const showRefDetails = config.get<boolean>('showReferenceDetails') === true;
	...
	async abortMerge(repository: Repository): Promise<cognidream> {
		await repository.mergeAbort();
	}

	@command('git.rebase', { repository: true })
	async rebase(repository: Repository): Promise<cognidream> {
		const config = workspace.getConfiguration('git');
		const showRefDetails = config.get<boolean>('showReferenceDetails') === true;
	...
	async createTag(repository: Repository, historyItem?: SourceControlHistoryItem): Promise<cognidream> {
	...
	async deleteTag(repository: Repository): Promise<cognidream> {
		const config = workspace.getConfiguration('git');
		const showRefDetails = config.get<boolean>('showReferenceDetails') === true;
	...
	async deleteTag2(repository: Repository, historyItem?: SourceControlHistoryItem, historyItemRefId?: string): Promise<cognidream> {
		const historyItemRef = historyItem?.references?.find(r => r.id === historyItemRefId);
		if (!historyItemRef) {
			return;
		}
	...
	async deleteRemoteTag(repository: Repository): Promise<cognidream> {
	...
	async fetch(repository: Repository): Promise<cognidream> {
	...
	async fetchPrune(repository: Repository): Promise<cognidream> {
		if (repository.remotes.length === 0) {
			window.showWarningMessage(l10n.t('This repository has no remotes configured to fetch from.'));
			return;
		}
	...
	async fetchAll(repository: Repository): Promise<cognidream> {
		if (repository.remotes.length === 0) {
			window.showWarningMessage(l10n.t('This repository has no remotes configured to fetch from.'));
			return;
		}
	...
	async fetchRef(repository: Repository, ref?: string): Promise<cognidream> {
	...
	async pullFrom(repository: Repository): Promise<cognidream> {
	...
	async pull(repository: Repository): Promise<cognidream> {
		const remotes = repository.remotes;
	...
	async pullRebase(repository: Repository): Promise<cognidream> {
		const remotes = repository.remotes;
	...
	async pullRef(repository: Repository, ref?: string): Promise<cognidream> {
		ref = ref ?? repository?.historyProvider.currentHistoryItemRemoteRef?.id;
		if (!repository || !ref) {
			return;
		}
	...
	async cherryPick(repository: Repository): Promise<cognidream> {
	...
	async cherryPick2(repository: Repository, historyItem?: SourceControlHistoryItem): Promise<cognidream> {
		if (!historyItem) {
			return;
		}
	...
	async cherryPickAbort(repository: Repository): Promise<cognidream> {
		await repository.cherryPickAbort();
	}

	@command('git.pushTo', { repository: true })
	async pushTo(repository: Repository, remote?: string, refspec?: string, setUpstream?: boolean): Promise<cognidream> {
	...
	async pushToForce(repository: Repository, remote?: string, refspec?: string, setUpstream?: boolean): Promise<cognidream> {
	...
	async pushTags(repository: Repository): Promise<cognidream> {
	...
	async removeRemote(repository: Repository): Promise<cognidream> {
		const remotes = repository.remotes;
	...
	private async _sync(repository: Repository, rebase: boolean): Promise<cognidream> {
		const HEAD = repository.HEAD;
	...
	async sync(repository: Repository): Promise<cognidream> {
		const config = workspace.getConfiguration('git', Uri.file(repository.root));
	...
	async syncAll(): Promise<cognidream> {
	...
	async syncRebase(repository: Repository): Promise<cognidream> {
		try {
	...
	async publish(repository: Repository): Promise<cognidream> {
	...
	@command('git.ignore')
	async ignore(...resourceStates: SourceControlResourceState[]): Promise<cognidream> {
		resourceStates = resourceStates.filter(s => !!s);
	...
	async revealInExplorer(resourceState: SourceControlResourceState): Promise<cognidream> {
		if (!resourceState) {
			return;
		}
	...
	async revealFileInOS(resourceState: SourceControlResourceState): Promise<cognidream> {
		if (!resourceState) {
			return;
		}
	...
	async stashPop(repository: Repository): Promise<cognidream> {
	...
	async stashPopLatest(repository: Repository): Promise<cognidream> {
	...
	async stashPopEditor(uri: Uri): Promise<cognidream> {
	...
	async stashApply(repository: Repository): Promise<cognidream> {
	...
	async stashApplyLatest(repository: Repository): Promise<cognidream> {
		const stashes = await repository.getStashes();
	...
	async stashApplyEditor(uri: Uri): Promise<cognidream> {
		const result = await this.getStashFromUri(uri);
		if (!result) {
			return;
		}
	...
	async stashDrop(repository: Repository): Promise<cognidream> {
	...
	async stashDropAll(repository: Repository): Promise<cognidream> {
		const stashes = await repository.getStashes();
	...
	async stashDropEditor(uri: Uri): Promise<cognidream> {
		const result = await this.getStashFromUri(uri);
		if (!result) {
			return;
		}
	...
	async stashView(repository: Repository): Promise<cognidream> {
	...
	async rebaseAbort(repository: Repository): Promise<cognidream> {
	...
	async openRepositoriesInParentFolders(): Promise<cognidream> {
	...
	async manageUnsafeRepositories(): Promise<cognidream> {
	...
	async copyCommitId(repository: Repository, historyItem: SourceControlHistoryItem): Promise<cognidream> {
	...
	async copyCommitMessage(repository: Repository, historyItem: SourceControlHistoryItem): Promise<cognidream> {
		if (!repository || !historyItem) {
			return;
		}
	...
	async viewCommit(repository: Repository, historyItemId: string): Promise<cognidream> {
	...
	async copyContentToClipboard(content: string): Promise<cognidream> {
		...
		const result = (...args: any[]) => {
			...
			*/
			...
			return result.catch(err => {
				...
				const choices = new Map<string, () => cognidream>();
	...
	private async showErrorNotification(type: 'error' | 'warning' | 'information', message: string, options: MessageOptions, choices: Map<string, () => cognidream>): Promise<cognidream> {
