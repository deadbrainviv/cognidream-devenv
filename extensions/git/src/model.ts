...
class ClosedRepositoriesManager {
	...
	addRepository(repository: string): cognidream {
	...
	private onDidChangeRepositories(): cognidream {
...
class ParentRepositoriesManager {
	...
	openRepository(repository: string): cognidream {
...
class UnsafeRepositoriesManager {
	...
	addRepository(repository: string, path: string): cognidream {
...
export class Model implements IRepositoryResolver, IBranchProtectionProviderRegistry, IRemoteSourcePublisherRegistry, IPostCommitCommandsProviderRegistry, IPushErrorHandlerRegistry, ISourceControlHistoryItemDetailsProviderRegistry {
	...
	setState(state: State): cognidream {
		this._state = state;
	...
	@memoize
	get isInitialized(): Promise<cognidream> {
	...
	private _onDidChangePostCommitCommandsProviders = new EventEmitter<cognidream>();
	...
	private async doInitialScan(): Promise<cognidream> {
	 ...
	 */
	private async scanWorkspaceFolders(): Promise<cognidream> {
		try {
			const config = workspace.getConfiguration('git');
			const autoRepositoryDetection = config.get<boolean | 'subFolders' | 'openEditors'>('autoRepositoryDetection');
	...
	private onPossibleGitRepositoryChange(uri: Uri): cognidream {
		const config = workspace.getConfiguration('git');
		const autoRepositoryDetection = config.get<boolean | 'subFolders' | 'openEditors'>('autoRepositoryDetection');
	...
	@debounce(500)
	private eventuallyScanPossibleGitRepositories(): cognidream {
	...
	private async onDidChangeWorkspaceFolders({ added, removed }: WorkspaceFoldersChangeEvent): Promise<cognidream> {
		try {
	...
	private onDidChangeConfiguration(): cognidream {
	...
	private async onDidChangeVisibleTextEditors(editors: readonly TextEditor[]): Promise<cognidream> {
		try {
	...
	private onDidChangeActiveTextEditor(): cognidream {
	...
	@sequentialize
	async openRepository(repoPath: string, openIfClosed = false): Promise<cognidream> {
	...
	async openParentRepository(repoPath: string): Promise<cognidream> {
	...
	private open(repository: Repository): cognidream {
	...
	close(repository: Repository): cognidream {
	...
	private async showParentRepositoryNotification(): Promise<cognidream> {
	...
	private async showUnsafeRepositoryNotification(): Promise<cognidream> {
