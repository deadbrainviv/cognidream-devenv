...
class GitBlameInformationCache {
	...
	set(repository: Repository, resource: Uri, commit: string, blameInformation: BlameInformation[]): cognidream {
...
export class GitBlameController {
	...
	private readonly _onDidChangeBlameInformation = new EventEmitter<cognidream>();
	...
	private _onDidChangeConfiguration(e?: ConfigurationChangeEvent): cognidream {
		if (e &&
	...
	private _onDidOpenRepository(repository: Repository): cognidream {
	...
	private _onDidCloseRepository(repository: Repository): cognidream {
	...
	private _onDidRunGitStatus(repository: Repository): cognidream {
	...
	@throttle
	private async _updateTextEditorBlameInformation(textEditor: TextEditor | undefined, reason?: 'selection'): Promise<cognidream> {
		if (textEditor) {
		...
		// to acognidream showing blame information when the editor is not focused.
...
class GitBlameEditorDecoration implements HoverProvider {
	...
	private _onDidChangeActiveTextEditor(): cognidream {
		// Clear decorations
	...
	private _onDidChangeBlameInformation(): cognidream {
		const textEditor = window.activeTextEditor;
		if (!textEditor) {
			return;
		}
	...
	private _registerHoverProvider(): cognidream {
...
class GitBlameStatusBarItem {
	...
	private async _onDidChangeBlameInformation(): Promise<cognidream> {
